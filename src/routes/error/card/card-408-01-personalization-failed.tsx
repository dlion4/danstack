...
import { Printer, Info, Zap, ArrowUpRight, Home, ShieldAlert, History } from 'lucide-react';
...
import styles from './card-408-01-personalization-failed.module.css';
...
export const Route = createFileRoute('/error/card/card-408-01-personalization-failed')({
  component: Card40801,
});
...
function Card40801() {
  ...
  const reorderCard = () => {
    addToast('Re-order Card Free', 'CARD-408-01 logged', 'red');
  };

  const contactCardOps = () => {
    addToast('Contact Card Ops', 'Opening', 'emerald');
  };
    ...
    <ErrorLayout
      ...
      rightAction={
        <div className="flex gap-2">
          <span className="px-3 py-1.5 rounded-full bg-white border border-[#E8E2D9] font-bold text-[10px]">CARD-408-01</span>
          <button
    ...
    >
      <div className="bg-white border border-[#E8E2D9] rounded-[28px] shadow-xl overflow-hidden animate-rise">
        ...
        <div className="p-6.5 sm:p-7 text-center">
          ...
          <div className="w-24 h-24 mx-auto mb-3.5 rounded-[28px] bg-[#FEF2F2] border border-[#FECACA] flex items-center justify-center text-[44px] text-[#EF4444] relative">
            ...
            <Printer size={48} className="animate-shake" />
          </div>
          ...
          <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#FEF2F2] border border-[#FECACA] text-[#B91C1C] font-bold text-[11px] uppercase tracking-wider mb-3">
            <ShieldAlert size={14} />
            Cards • Print Error • Re-order
          </div>

          {/* Title */}
          <div className="font-['Space_Grotesk'] text-[22px] font-bold mb-2">
            Card Personalization Failed — Print Error
          </div>

          {/* Subtitle */}
          <div className="text-[14px] text-[#4B5563] leading-relaxed">
            Physical card printer error — chip encoding failed. Re-order with same details, no charge.
          </div>
          ...
          <div className="bg-[#FEF2F2] border border-[#FECACA] border-l-4 border-l-[#EF4444] rounded-xl p-3.5 flex gap-2.5 items-start text-left mt-4">
            ...
            <div>
              ...
              <span className="text-[12px] text-[#4B5563] leading-relaxed mt-1 block">
                Card printer HP-001 chip encoding failed — ATR mismatch. Card damaged, need re-order. Free re-order.
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2.5 mt-4 text-left">
            <div className="bg-[#FFFCF5] border border-[#E8E2D9] rounded-xl p-3">
              ...
              <span className="text-[12px] text-[#4B5563]">•••• 8888 • Physical</span>
            </div>
            <div className="bg-[#FFFCF5] border border-[#E8E2D9] rounded-xl p-3">
              <span className="font-bold text-[11px] uppercase tracking-wider block">Printer</span>
              <span className="text-[12px] text-[#4B5563]">HP-001 • Line 2</span>
            </div>
            <div className="bg-[#FFFCF5] border border-[#E8E2D9] rounded-xl p-3">
              <span className="font-bold text-[11px] uppercase tracking-wider block">Error</span>
              <span className="text-[12px] text-[#4B5563]">Chip ATR mismatch</span>
            </div>
            <div className="bg-[#FFFCF5] border border-[#E8E2D9] rounded-xl p-3">
              <span className="font-bold text-[11px] uppercase tracking-wider block">Cost</span>
              <span className="text-[12px] text-[#4B5563]">Free re-order</span>
            </div>
          </div>
      ...
      >
        ...
        <div className="p-3 rounded-2xl bg-[#FFFCF5] border border-[#E8E2D9]">
          ...
          <code className="bg-white border border-[#E8E2D9] px-1.5 py-0.5 rounded text-[11px]">card-408-01_xxx_KE</code> • Funds safe • No double charge
        </div>
      </ErrorModal>
