...
import { PauseCircle, Info, Zap, ArrowUpRight, Home, ShieldAlert, History } from 'lucide-react';
...
import styles from './card-403-05-program-inactive.module.css';
...
export const Route = createFileRoute('/error/card/card-403-05-program-inactive')({
  component: Card40305,
});
...
function Card40305() {
  ...
  const uploadAgreement = () => {
    addToast('Upload Cardholder Agreement', 'CARD-403-05 logged', 'red');
  };

  const contactAdmin = () => {
    addToast('Contact Admin', 'Opening', 'emerald');
  };
    ...
    <ErrorLayout
      ...
      rightAction={
        <div className="flex gap-2">
          <span className="px-3 py-1.5 rounded-full bg-white border border-[#E8E2D9] font-bold text-[10px]">CARD-403-05</span>
          <button
    ...
    >
      <div className="bg-white border border-[#E8E2D9] rounded-[28px] shadow-xl overflow-hidden animate-rise">
        ...
        <div className="p-6.5 sm:p-7 text-center">
          ...
          <div className="w-24 h-24 mx-auto mb-3.5 rounded-[28px] bg-[#FEF2F2] border border-[#FECACA] flex items-center justify-center text-[44px] text-[#EF4444] relative">
            ...
            <PauseCircle size={48} className="animate-shake" />
          </div>
          ...
          <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#FEF2F2] border border-[#FECACA] text-[#B91C1C] font-bold text-[11px] uppercase tracking-wider mb-3">
            <ShieldAlert size={14} />
            BAAS • Program Inactive • Config
          </div>

          {/* Title */}
          <div className="font-['Space_Grotesk'] text-[22px] font-bold mb-2">
            BAAS Card Program Inactive / Suspended
          </div>

          {/* Subtitle */}
          <div className="text-[14px] text-[#4B5563] leading-relaxed">
            Your card program not active — suspended for compliance review. Cannot issue cards.
          </div>
          ...
          <div className="bg-[#FEF2F2] border border-[#FECACA] border-l-4 border-l-[#EF4444] rounded-xl p-3.5 flex gap-2.5 items-start text-left mt-4">
            ...
            <div>
              ...
              <span className="text-[12px] text-[#4B5563] leading-relaxed mt-1 block">
                Card program Prog_001 status = Suspended, reason: Compliance review — missing cardholder agreement. Contact admin.
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2.5 mt-4 text-left">
            <div className="bg-[#FFFCF5] border border-[#E8E2D9] rounded-xl p-3">
              ...
              <span className="text-[12px] text-[#4B5563]">Prog_001 • DanStack Cards</span>
            </div>
            <div className="bg-[#FFFCF5] border border-[#E8E2D9] rounded-xl p-3">
              <span className="font-bold text-[11px] uppercase tracking-wider block">Status</span>
              <span className="text-[12px] text-[#4B5563]">Suspended</span>
            </div>
            <div className="bg-[#FFFCF5] border border-[#E8E2D9] rounded-xl p-3">
              <span className="font-bold text-[11px] uppercase tracking-wider block">Since</span>
              <span className="text-[12px] text-[#4B5563]">2026-08-01</span>
            </div>
            <div className="bg-[#FFFCF5] border border-[#E8E2D9] rounded-xl p-3">
              <span className="font-bold text-[11px] uppercase tracking-wider block">Reason</span>
              <span className="text-[12px] text-[#4B5563]">Missing cardholder agreement</span>
            </div>
          </div>
      ...
      >
        ...
        <div className="p-3 rounded-2xl bg-[#FFFCF5] border border-[#E8E2D9]">
          ...
          <code className="bg-white border border-[#E8E2D9] px-1.5 py-0.5 rounded text-[11px]">card-403-05_xxx_KE</code> • Funds safe • No double charge
        </div>
      </ErrorModal>
