...
import { Settings, Info, Zap, ArrowUpRight, Home, ShieldAlert, ShieldCheck, History } from 'lucide-react';
...
import styles from './card-403-07-settings-conflict.module.css';
...
export const Route = createFileRoute('/error/card/card-403-07-settings-conflict')({
  component: Card40307,
});
...
function Card40307() {
  ...
  const enableBoth = () => {
    addToast('Enable Both ON', 'CARD-403-07 logged', 'amber');
  };

  const fixSettings = () => {
    addToast('Fix Settings', 'Opening', 'emerald');
  };
    ...
    <ErrorLayout
      ...
      rightAction={
        <div className="flex gap-2">
          <span className="px-3 py-1.5 rounded-full bg-white border border-[#E8E2D9] font-bold text-[10px]">CARD-403-07</span>
          <button
    ...
    >
      <div className="bg-white border border-[#E8E2D9] rounded-[28px] shadow-xl overflow-hidden animate-rise">
        ...
        <div className="p-6.5 sm:p-7 text-center">
          ...
          <div className="w-24 h-24 mx-auto mb-3.5 rounded-[28px] bg-[#FFFBEB] border border-[#FDE68A] flex items-center justify-center text-[44px] text-[#F59E0B] relative">
            ...
            <Settings size={48} className="animate-float" />
          </div>
          ...
          <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#FFFBEB] border border-[#FDE68A] text-[#92400E] font-bold text-[11px] uppercase tracking-wider mb-3">
            <Zap size={14} />
            Cards • Settings Conflict • Policy
          </div>

          {/* Title */}
          <div className="font-['Space_Grotesk'] text-[22px] font-bold mb-2">
            Card Settings Conflict — Cannot Enable Both
          </div>

          {/* Subtitle */}
          <div className="text-[14px] text-[#4B5563] leading-relaxed">
            Can't enable ATM + Ecomm off — settings conflict. Choose compatible combo.
          </div>
          ...
          <div className="bg-[#FFFBEB] border border-[#FDE68A] border-l-4 border-l-[#F59E0B] rounded-xl p-3.5 flex gap-2.5 items-start text-left mt-4">
            ...
            <div>
              ...
              <span className="text-[12px] text-[#4B5563] leading-relaxed mt-1 block">
                Trying to enable ATM withdrawal but disable E-commerce — but ATM needs online PIN which needs e-comm on. Conflict.
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2.5 mt-4 text-left">
            <div className="bg-[#FFFCF5] border border-[#E8E2D9] rounded-xl p-3">
              <span className="font-bold text-[11px] uppercase tracking-wider block">Tried</span>
              <span className="text-[12px] text-[#4B5563]">ATM ON + Ecomm OFF</span>
            </div>
            <div className="bg-[#FFFCF5] border border-[#E8E2D9] rounded-xl p-3">
              <span className="font-bold text-[11px] uppercase tracking-wider block">Conflict</span>
              <span className="text-[12px] text-[#4B5563]">ATM requires Ecomm ON</span>
            </div>
            <div className="bg-[#FFFCF5] border border-[#E8E2D9] rounded-xl p-3 col-span-2">
              <span className="font-bold text-[11px] uppercase tracking-wider block">Allowed</span>
              <span className="text-[12px] text-[#4B5563]">Both ON or Both OFF</span>
            </div>
          </div>
      ...
      >
        ...
        <div className="p-3 rounded-2xl bg-[#FFFCF5] border border-[#E8E2D9]">
          ...
          <code className="bg-white border border-[#E8E2D9] px-1.5 py-0.5 rounded text-[11px]">card-403-07_xxx_KE</code> • Funds safe • No double charge
        </div>
      </ErrorModal>
