...
async function fetchTransferOverview(): Promise<TransferContent> {
	const response = await fetch("/api/transfer-overview", {
		headers: { Accept: "application/json" },
	});
	if (!response.ok)
		throw new Error(`Transfer overview API responded HTTP ${response.status}`);
		...
		<div className={s.pageRoot} style={{ position: "relative" }}>
			...
			{error && (
				...
				>
					...
					<div className="small mt-1">
						<code>/api/transfer-overview</code> — {error.message}. Using bundled
						sample data.
					</div>
					<button
						type="button"
			...
			<div className={s.pageBar}>
				<div>
					<div className={s.breadcrumb}>
						<a href="/app">Home</a> / <a href="/app/transfers">Transfers</a> /{" "}
					...
					<p className={s.pageCopy}>
						...
						beneficiaries, and monitor all money movement across M-Pesa, banks,
			...
			<div className="row g-3">
				<div className="col-lg-4">
					<div className={cx(s.card, s.cardAccent)} style={{ minHeight: 170 }}>
						...
						>
							Transfer center is live{" "}
							<span style={{ color: "#86efac" }}>●</span>
						</p>
						<div className={s.statValue} style={{ color: "#fff" }}>
							KES 2.84M transferred
						</div>
						<p
							style={{
								margin: 0,
				...
				<div className="col-lg-2 col-md-4 col-6">
					<div className={s.card} style={{ minHeight: 170 }}>
						<p className={s.statLabel} style={{ color: "var(--tr-accent)" }}>
							COMPLETED
						</p>
						...
						<div className={s.statSub}>
							Avg time: <strong>12 seconds</strong>
						</div>
					</div>
				</div>
				<div className="col-lg-3 col-md-4 col-6">
					<div className={s.card} style={{ minHeight: 170 }}>
						<p className={s.statLabel} style={{ color: "var(--tr-info)" }}>
							PENDING / SCHEDULED
						</p>
						...
						<div className={s.statSub}>
							Next execution: <strong>Today 3:00 PM</strong>
						</div>
					</div>
				</div>
				<div className="col-lg-3 col-md-4">
					<div
						className={cx(s.card, s.cardWarnEdge)}
						style={{ minHeight: 170 }}
					>
						<p className={s.statLabel} style={{ color: "var(--tr-warning)" }}>
							FAILED / REJECTED
						</p>
						...
						<div className={s.statSub}>
							Most common: <strong>Insufficient funds</strong>
						</div>
					</div>
				</div>
			</div>
								...
								<tbody>
									{content.recentTransfers.map((t) => (
										<tr key={t.ref}>
											...
											<td>{t.method}</td>
											<td>
												<span className={cx(s.badge, badgeTone[t.status])}>
													{t.statusLabel}
												</span>
											</td>
											<td>{t.ref}</td>
				...
				<div className={s.favGrid}>
					{content.favorites.map((fav) => (
						<div
							className={s.favCard}
							key={fav.name}
							onClick={() => openModal("favoritesQuick")}
							role="button"
							tabIndex={0}
						>
			...
			<div className={s.card}>
				<div className={s.sectionHead}>
					<div>
						<h3 className={s.sectionTitle}>
							<i className="bi bi-calendar-check" /> Scheduled & Recurring
							Transfers
						</h3>
						<p className={s.sectionSub}>
							Manage your automated payments and upcoming scheduled transfers.
						</p>
					</div>
					<button
						className={cx(s.btn, s.btnSm, s.btnPrimary)}
								...
								<tr key={row.label}>
									...
									<td>
										<span
											className={cx(
												s.badge,
												row.status === "active" ? s.badgeSuccess : s.badgeWarn,
											)}
										>
						...
						{content.successByChannel.map((r) => (
							<div className={s.rowItem} key={r.channel}>
								...
								<span
									className={cx(
										s.badge,
										r.tone === "success" ? s.badgeSuccess : s.badgeWarn,
									)}
								>
									{r.rate}
								</span>
...
function toneColor(tone: string): string {
	switch (tone) {
		case "primary":
			return "var(--tr-accent-2)";
		case "info":
			return "var(--tr-info)";
		case "success":
			return "var(--tr-accent)";
		case "warn":
			return "var(--tr-warning)";
		case "danger":
			return "var(--tr-danger)";
		case "purple":
			return "var(--tr-purple)";
		default:
			return "var(--tr-accent-2)";
	}
}
