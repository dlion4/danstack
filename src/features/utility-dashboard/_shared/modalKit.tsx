...
export function useModals(s: S, active: string | null, onClose: () => void) {
	...
	const PickedBox = ({
		...
		<div
			className={`p-3 border rounded text-center ${isPicked(k, v) ? s.selectableActive : ""}`}
	...
	const payMethodRadios = (
		name: string,
		opts: [string, string, string, string, boolean][] = [
			["M-Pesa", "bi-phone", "var(--pm-accent)", "0712***890", true],
			[
				"PayMo Wallet",
				"bi-wallet2",
				"var(--pm-primary)",
			...
			["Bank Transfer", "bi-bank", "var(--pm-info)", "Equity ***4521", false],
		],
	) => (
		<div className="mb-3">
			{opts.map(([label, icon, color, sub, def]) => (
				<label
					key={label}
					className={`p-3 border rounded mb-2 d-flex align-items-center gap-3 ${isPicked(name, label) ? s.selectableActive : ""}`}
					style={{ cursor: "pointer" }}
					onClick={() => setPicked(name, label)}
				>
					<input type="radio" name={name} defaultChecked={def} readOnly />{" "}
