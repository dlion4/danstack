...
import { fetchWaterDeepDive, initialMockData } from "../data/waterData";
...
export default function WaterDeepDive() {
	const { data } = useQuery({
		queryKey: ["water-deep-dive"],
		queryFn: fetchWaterDeepDive,
		staleTime: 60_000,
		retry: 1,
	});
	...
	const accounts =
		filter === "all"
			? config.waterAccounts
			: config.waterAccounts.filter((a) => a.status === filter);
	const counts = {
		all: config.waterAccounts.length,
		due: config.waterAccounts.filter((a) => a.status === "due").length,
		paid: config.waterAccounts.filter((a) => a.status === "paid").length,
	};
			...
			<div className={s.main}>
				<div className={s.pageBar}>
					...
					<div className="d-flex flex-wrap" style={{ gap: 8 }}>
						<button
							className={s.btnPm}
							onClick={() => open("waterHealthModal")}
						>
							<i className="bi bi-heart-pulse text-info" /> Health Check
						</button>
						<button
							className={s.btnPm}
							onClick={() => open("orderBowserModal")}
						>
							<i className="bi bi-truck text-primary" /> Order Bowser
						</button>
						<button
							className={`${s.btnPm} ${s.btnPmI}`}
							onClick={() => open("payWaterModal")}
						>
							<i className="bi bi-droplet" /> Pay Bill
						</button>
						<button
							className={`${s.btnPm} ${s.btnPmP}`}
							onClick={() => open("addWaterMeterModal")}
						>
					...
					<div className="row g-3">
						<div className="col-lg-4">
							...
							>
								...
								>
									{config.hero.live} <span style={{ color: "#67e8f9" }}>●</span>
								</p>
								<div
									className={s.sv}
						...
						<div className="col-lg-4">
							<div className={s.card}>
								<div className="d-flex justify-content-between align-items-center mb-3">
									<h3 className={s.sectionTitle}>Water Alerts</h3>
									<button
										className={`${s.btnPm} ${s.btnSm}`}
										onClick={() => open("waterNotificationsModal")}
									>
										View all
									</button>
								</div>
								{config.waterAlerts.map((it) => (
									<div key={it.title} className={s.feedItem}>
										...
										>
											{it.iconText ?? <i className={`bi ${it.icon}`} />}
										</div>
							...
							<div className={s.card}>
								<div className="mb-3">
									...
									<p className={s.sectionSub}>
										Frequent water management workflows
									</p>
								</div>
						...
						>
							<div>
								<h3 className={s.sectionTitle}>
									...
									/>{" "}
									3.3.1 — Water Bill Payments & Account Wallet (All Counties)
								</h3>
								<p className={s.sectionSub}>
									Manage 47-county water utility providers, pay bills, handle
									connection fees, and download integrated receipts.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
									className={`${s.btnPm} ${s.btnSm}`}
									onClick={() => open("exportWaterModal")}
								>
									<i className="bi bi-clock-history" /> History
								</button>
								<button
									className={`${s.btnPm} ${s.btnSm} ${s.btnPmI}`}
									onClick={() => open("payWaterModal")}
								>
							...
							<div className="col-lg-4">
								<div className={`${s.utilityBlock} mb-3`}>
									...
									<div className="d-flex mt-3" style={{ gap: 8 }}>
										<button
											className={`${s.btnPm} ${s.btnSm}`}
											onClick={() => open("waterBillBreakdownModal")}
										>
											View full statement
										</button>
									</div>
						...
						>
							<div>
								<h3 className={s.sectionTitle}>
									...
									/>{" "}
									3.3.2 — Consumption Analytics, Meter Tracking & Dispute
								</h3>
								<p className={s.sectionSub}>
									Track cubic meters (m³) over time, compare properties, detect
									abnormal surges (leaks), and resolve billing disputes.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
								...
								<button
									className={`${s.btnPm} ${s.btnSm}`}
									onClick={() => open("disputeWaterModal")}
								>
						...
						<div className="row g-3">
							<div className="col-lg-5">
								<div className={s.utilityBlock}>
									...
									>
										6-Month Portfolio Consumption (m³)
									</h4>
									<div className={s.chartBarGroup}>
										{config.consumptionTrend.map((t) => (
											<div
												key={t.month}
												className={s.chartBar}
												style={{ height: t.height, background: t.color }}
												onClick={() => open("waterAnalyticsModal")}
												role="button"
												tabIndex={0}
											>
							...
							<div className="col-lg-4">
								<div className={s.utilityBlock}>
									...
									{config.openDisputes.map((d) => (
										<div key={d.id} className={s.statusRow}>
											<div>
												<strong>{d.id}</strong>
												<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
													{d.sub}
						...
						>
							<div>
								<h3 className={s.sectionTitle}>
									...
									/>{" "}
									3.3.3 — Bulk Water, Bowsers & Emergency Supply Management
								</h3>
								<p className={s.sectionSub}>
									Supplement county water during rationing. Order from verified
									private boreholes, manage truck deliveries, and track supply
									schedules.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
									className={`${s.btnPm} ${s.btnSm}`}
									onClick={() => open("outageWaterModal")}
								>
									<i className="bi bi-cone-striped" /> Rationing Schedule
								</button>
								<button
									className={`${s.btnPm} ${s.btnSm} ${s.btnPmP}`}
									onClick={() => open("orderBowserModal")}
								>
									<i className="bi bi-plus-lg" /> Book Delivery
								</button>
							</div>
						</div>
						<div className="row g-3">
							<div className="col-lg-8">
								<div className={s.utilityBlock}>
									<div className="d-flex justify-content-between align-items-center mb-3">
										<h4 style={{ fontSize: 14, fontWeight: 700, margin: 0 }}>
											Verified Private Bowser Suppliers
										</h4>
										<button
											className={`${s.btnPm} ${s.btnSm}`}
											onClick={() => open("compareSuppliersModal")}
										>
											Compare Prices
										</button>
									</div>
							...
							<div className="col-lg-4">
								<div className={s.utilityBlock}>
									...
									>
										<i
											className="bi bi-truck d-block mb-2"
											style={{ fontSize: 24, color: "var(--pm-primary)" }}
										/>
										<strong>Delivery in Progress</strong>
										<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
											Nairobi Pure Water Ltd · 10,000L
										</div>
										<div className={`${s.progress} mt-2 mb-1`}>
											<div
												className={s.progressBar}
												style={{
													width: "75%",
										...
										<div style={{ fontSize: 11, fontWeight: 600 }}>
											Arriving in approx 45 mins
										</div>
									</div>
									<button
										className={`${s.btnPm} ${s.btnSm} w-100 justify-content-center`}
										onClick={() => open("waterHealthModal")}
									>
