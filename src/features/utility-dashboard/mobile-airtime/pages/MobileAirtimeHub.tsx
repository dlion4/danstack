...
import {
	fetchMobileAirtimeHub,
	initialMockData,
} from "../data/mobileAirtimeData";
...
export default function MobileAirtimeHub() {
	const { data } = useQuery({
		queryKey: ["mobile-airtime-hub"],
		queryFn: fetchMobileAirtimeHub,
		staleTime: 60_000,
		retry: 1,
	});
			...
			<div className={s.main}>
				<div className={s.pageBar}>
					...
					<div className="d-flex flex-wrap" style={{ gap: 8 }}>
						<button
							className={s.btnPm}
							onClick={() => open("agentLocatorModal")}
						>
							<i className="bi bi-geo-alt text-primary" /> Find Agent
						</button>
						<button
							className={s.btnPm}
							onClick={() => open("addPhoneNumberModal")}
						>
							<i className="bi bi-sim text-info" /> Link Line
						</button>
						<button
							className={`${s.btnPm} ${s.btnPmI}`}
							onClick={() => open("buyAirtimeModal")}
						>
							<i className="bi bi-phone" /> Buy Airtime
						</button>
						<button
							className={`${s.btnPm} ${s.btnPmP}`}
							onClick={() => open("sendMoneyCrossNetworkModal")}
						>
				...
				<div className={s.content}>
							...
							>
								...
								>
									<div>
										...
										<strong>{config.hero.received}</strong>
									</div>
									<div>
										Total Sent Today: <strong>{config.hero.sent}</strong>
									</div>
								</div>
							</div>
						</div>
					...
					<div className="row g-3">
						...
						<div className="col-lg-4">
							<div className={s.card}>
								<div className="mb-3">
									<h3 className={s.sectionTitle}>Mobile Money Actions</h3>
									<p className={s.sectionSub}>
										Frequent transfers and airtime operations
									</p>
								</div>
						...
						>
							<div>
								<h3 className={s.sectionTitle}>
									...
									/>{" "}
									3.5.1 — Airtime & Data Management
								</h3>
								<p className={s.sectionSub}>
									Unified dashboard for Safaricom, Airtel, and Telkom balances,
									bundle status, and auto-renewals.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
									className={`${s.btnPm} ${s.btnSm}`}
									onClick={() => open("airtimeAutoRenewModal")}
								>
									<i className="bi bi-arrow-repeat" /> Auto-Renew
								</button>
								<button
									className={`${s.btnPm} ${s.btnSm} ${s.btnPmP}`}
									onClick={() => open("addPhoneNumberModal")}
								>
									<i className="bi bi-plus-lg" /> Link Line
								</button>
							</div>
						</div>
									...
									{config.lines.map((ln) => (
										<tr key={ln.phone}>
											<td data-label="Phone Number">
												<strong>{ln.phone}</strong>{" "}
												{ln.isDefault && (
													<span
														className={`${s.badge} ms-1`}
											...
											<td data-label="Network">
												<span
													style={{ color: ln.networkColor, fontWeight: 600 }}
												>
													{ln.network}
												</span>
											</td>
											...
											<td data-label="Auto-Renew">
												<span className={`${s.badge} ${s[ln.autoRenewTone]}`}>
													{ln.autoRenew}
												</span>
											</td>
											<td data-label="Actions">
												<div className="d-flex" style={{ gap: 4 }}>
													<button
														className={`${s.btnPm} ${s.btnSm}`}
														onClick={() => open(ln.topupModal)}
													>
														Top-up
													</button>
													<button
														className={`${s.btnPm} ${s.btnSm}`}
														onClick={() => open("manageLinkedLineModal")}
													>
						...
						>
							<div>
								<h3 className={s.sectionTitle}>
									...
									/>{" "}
									3.5.2 — Mobile Money Interoperability & Agents
								</h3>
								<p className={s.sectionSub}>
									Cross-network transfers, transparent fee calculator, and
									nearby agent locator with GPS.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
									className={`${s.btnPm} ${s.btnSm}`}
									onClick={() => open("linkBankAccountModal")}
								>
						...
						<div className="row g-3">
							<div className="col-lg-6">
								<div className={s.utilityBlock}>
									...
									>
										Interoperability Transfer Network
									</h4>
								...
								<div className={s.utilityBlock}>
									...
									>
										...
										<div style={{ fontWeight: 600, marginTop: 8 }}>
											Located 3 agents near Kilimani
										</div>
										<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
											GPS active · Loita Plains, Nairobi
										</div>
									</div>
									{config.agents.map((a) => (
					...
					<div className={s.card}>
						...
						>
							<div>
								...
								<p className={s.sectionSub}>
									Manage your Fuliza limit, M-Shwari savings, Mali investments,
									Pochi la Biashara and Global transfers directly.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
									className={`${s.btnPm} ${s.btnSm}`}
									onClick={() => open("mobileMoneyAnalyticsModal")}
								>
									<i className="bi bi-graph-up" /> M-Pesa Analytics
								</button>
							</div>
						</div>
						<div className="row g-3">
							{config.suite.map((c) => (
								<div key={c.title} className="col-md-4">
									...
									>
										...
										<button
											className={`${s.btnPm} ${s.btnSm} w-100 mt-3 ${c.actionClass ? s[c.actionClass] : ""}`}
					...
					<div className={s.card}>
						<div className="d-flex justify-content-between align-items-center mb-3">
							<h3 className={s.sectionTitle}>
								...
								/>{" "}
								Recent Mobile Money & Airtime Transactions
							</h3>
							<button
								className={`${s.btnPm} ${s.btnSm}`}
									...
									{config.transactions.map((t, i) => (
										<tr key={i}>
											...
											<td data-label="Amount">
												<strong style={{ color: t.amountColor }}>
													{t.amount}
												</strong>
											</td>
