...
export default function BulkDisbursementsModals({
	active,
	onClose,
	onOpen,
}: ModalsProps) {
			...
			footer={
				<>
					...
					{ndStep < 4 && ndStep !== 2 && (
						...
						>
							{ndStep === 3 ? "Submit Batch" : "Next"}{" "}
							<i
								className={`bi ${ndStep === 3 ? "bi-check2" : "bi-arrow-right"}`}
							/>
						</button>
					)}
				</>
		...
		>
				...
				<div className={cx(s.fstep, s.fstepActive)}>
					...
					>
						...
						<p style={{ fontSize: 11, color: "var(--pm-muted)" }}>
							Name, Phone/Bank, Amount, Reference
						</p>
					</div>
					<div className="d-flex gap-2 mb-2">
						<button
							className={cx(s.btnPm, s.btnSm)}
							onClick={() => onOpen("templateLibraryModal")}
						>
							<i className="bi bi-file-earmark-text" /> Use Template
						</button>
						<button
							className={cx(s.btnPm, s.btnSm)}
							onClick={() => onOpen("recipientValidationModal")}
						>
							<i className="bi bi-people" /> Validate
						</button>
					</div>
					{showUpload && (
			...
			{ndStep === 2 && (
				<div className={cx(s.fstep, s.fstepActive)}>
					...
					>
						<strong>File: recipients.csv</strong> · 120 records detected
						<br />
						<span style={{ color: "var(--pm-accent)" }}>✓ 118 valid</span> ·{" "}
						<span style={{ color: "var(--pm-danger)" }}>
							✗ 2 invalid accounts
						</span>
					</div>
					<div className="mb-3">
						<label className={s.formLabel}>Column Mapping</label>
					...
					<button
						...
						onClick={() => nextFlow("disbursement", 4)}
					>
			...
			{ndStep === 3 && (
				<div className={cx(s.fstep, s.fstepActive)}>
					<div className="mb-3">
						<label className={s.formLabel}>Batch Name</label>
						<input
							type="text"
							className={s.formControl}
							defaultValue="June Payroll — Emergency"
						/>
					</div>
					<div className="mb-3">
					...
					<div className="form-check">
						...
						<label className="form-check-label">
							Require checker approval before disbursement
						</label>
					</div>
				</div>
			)}
				...
				renderActionBody(
					...
					<div className={s.receipt}>
						...
						<h5 style={{ fontWeight: 700, color: "var(--pm-accent)" }}>
							Batch validated and sent for approval!
						</h5>
						<p style={{ fontSize: 12, color: "var(--pm-muted)" }}>
							Ref: BTH-9924 · 118 valid recipients · KES 450,000
						</p>
					</div>,
				)}
		</MBox>
	...
	const renderScheduleBatch = () => (
		...
		>
			{renderActionBody(
				...
				<>
					<div className="mb-3">
						<label className={s.formLabel}>Batch Template</label>
						<select className={s.formControl}>
	...
	const renderRecipientValidation = () => (
		...
		>
			...
			>
				<strong>2 invalid accounts</strong>
				<br />
				<span style={{ fontSize: 11 }}>
					Grace T. — Bank account length mismatch
					<br />
					Ali O. — M-Pesa number format error
				</span>
			</div>
			<button
				className={cx(s.btnPm, s.btnSm)}
			...
			>
				<i className="bi bi-pencil" /> Fix Invalid
			</button>
		</MBox>
	);
			...
			footer={
				<>
					...
					<button
						className={cx(s.btnPm, s.btnPmP)}
						onClick={() =>
							doAction("fundWalletModal", "Top-up initiated!", "TOPUP-2025-001")
						}
					>
						Top-up
					</button>
				...
				<>
					...
					<div className="mb-3">
						...
						<select className={s.formControl}>
							...
							<option>Equity — Float Account</option>
						</select>
					</div>
					<div
					...
					>
						<i className="bi bi-info-circle" /> Current float: KES 14.2M · After
						top-up: KES 19.2M
					</div>
				</>,
			)}
	...
	const renderBatchDetail = () => (
		...
		>
			...
			>
				...
				<div className="d-flex justify-content-between">
					<span>Type</span>
					<strong>Salary Disbursement</strong>
				</div>
				<div className="d-flex justify-content-between">
			...
			{renderActionBody(
				...
				<>
					...
					>
						...
						<div className="d-flex justify-content-between">
							<span>Amount</span>
							<strong style={{ color: "var(--pm-accent)" }}>KES 450,000</strong>
						</div>
						<div className="d-flex justify-content-between">
							<span>Initiator</span>
							<strong>Sam O. (Operations Manager)</strong>
						</div>
					</div>
					<div
					...
					>
						<i className="bi bi-exclamation-triangle" /> Emergency batch —
			...
			footer={
				<>
					...
					<button
						...
						onClick={() =>
							doAction(
								"emergencyDisbursementModal",
								"Emergency batch created!",
								"BTH-EMERG",
							)
						}
					>
				...
				<>
					<div className="mb-3">
						...
						<textarea
							className={s.formControl}
							rows={2}
							defaultValue="Critical vendor payment — immediate disbursement required"
						/>
					</div>
					<div className="mb-3">
					...
					<div className="mb-3">
						<label className={s.formLabel}>Total Amount (KES)</label>
						<input
							type="number"
							className={s.formControl}
							defaultValue="250000"
					...
					>
						<i className="bi bi-exclamation-triangle" /> Emergency batches
						bypass scheduler and go directly to approval
					</div>
				</>,
			)}
		</MBox>
	...
	const renderAuditTrail = () => (
		...
		>
			<div className={s.feedItem}>
				...
				<div>
					<strong>BTH-9920 completed</strong>
					<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
						118/120 processed · 25 Jun · System
					</div>
				</div>
			</div>
			<div className={s.feedItem}>
				...
				<div>
					<strong>BTH-9919 approved</strong>
					<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
						CFO Peter K. · 22 Jun · 09:15
					</div>
				</div>
			</div>
			<div className={s.feedItem}>
				...
				<div>
					<strong>2 invalid accounts flagged</strong>
					<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
						BTH-9920 · Bank length mismatch · 25 Jun
					</div>
				</div>
			</div>
		</MBox>
	...
	const renderFloatManagement = () => (
		...
		>
			...
			>
				...
				<div className="d-flex justify-content-between">
					<span>Buffer</span>
					<strong style={{ color: "var(--pm-accent)" }}>KES 5.68M</strong>
				</div>
			</div>
			<div className={s.progress}>
				<div
					className={s.progressBar}
					style={{ width: "60%", background: "var(--pm-warning)" }}
				/>
			</div>
			<button
				className={cx(s.btnPm, s.btnSm, s.btnPmA, "mt-2 w-100")}
				onClick={() => onOpen("fundWalletModal")}
			>
				<i className="bi bi-wallet2" /> Top-up Float
			</button>
		</MBox>
	);
	...
	const renderSettlementReport = () => (
			...
			footer={
				<>
					...
					<button
						className={cx(s.btnPm, s.btnPmP)}
						onClick={() => {
							downloadFile("settlement.csv", "Batch,Amount,Status");
							onClose();
						}}
					>
						Download
		...
		>
			...
			>
				<div className="d-flex justify-content-between">
					<span>Total Disbursed (June)</span>
					<strong>KES 2.18M</strong>
				</div>
				<div className="d-flex justify-content-between">
					<span>Successful</span>
					<strong style={{ color: "var(--pm-accent)" }}>118/120 (97.8%)</strong>
				</div>
				<div className="d-flex justify-content-between">
					<span>Failed</span>
					<strong style={{ color: "var(--pm-danger)" }}>2</strong>
				</div>
				<div className="d-flex justify-content-between">
	...
	const renderAttentionAction = () => (
		...
		>
			<div className={s.feedItem}>
				...
				<div style={{ flex: 1 }}>
					<div style={{ fontWeight: 600 }}>Approval Needed</div>
					<div style={{ fontSize: 12, color: "var(--pm-muted)" }}>
						BTH-9922 · Emergency · KES 450K
					</div>
				</div>
				<button
					className={cx(s.btnPm, s.btnSm)}
					onClick={() => onOpen("approvalModal")}
				>
					Review
				</button>
			</div>
			<div className={s.feedItem}>
				...
				<div style={{ flex: 1 }}>
					<div style={{ fontWeight: 600 }}>Invalid Accounts</div>
					<div style={{ fontSize: 12, color: "var(--pm-muted)" }}>
						2 bank mismatches · BTH-9920
					</div>
				</div>
				<button
					className={cx(s.btnPm, s.btnSm)}
			...
			<div className={s.feedItem}>
				...
				<div style={{ flex: 1 }}>
					<div style={{ fontWeight: 600 }}>Low Float Warning</div>
					<div style={{ fontSize: 12, color: "var(--pm-muted)" }}>
						Batches exceed KES 14.2M float
					</div>
				</div>
				<button
					className={cx(s.btnPm, s.btnSm)}
		...
		>
			...
			>
				<div className="d-flex align-items-center gap-3">
					...
					<div>
						<strong>Safiri Operations Ltd</strong>
						<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
							PayBill: 400192
						</div>
					</div>
			...
			<div
				className="p-3 border rounded mb-3 d-flex align-items-center justify-content-between"
				style={{ cursor: "pointer" }}
			>
				<div className="d-flex align-items-center gap-3">
					<div
					...
					<div>
						<strong>Safiri Logistics (Subsidiary)</strong>
						<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
							PayBill: 400193
						</div>
					</div>
			...
			<div className="d-flex gap-2">
				<button className={cx(s.btnPm, s.btnSm)}>
					<i className="bi bi-plus" /> Add New Business
				</button>
				<button className={cx(s.btnPm, s.btnSm)}>
		...
		>
			...
			>
				...
				<div style={{ fontSize: 11, color: "var(--pm-muted)", marginTop: 4 }}>
					BTH-9920 successfully completed. 118 records processed.
				</div>
			</div>
			<div
				className="p-3 rounded mb-2"
			...
			>
				...
				<div style={{ fontSize: 11, color: "var(--pm-muted)", marginTop: 4 }}>
					BTH-9922 (Emergency Relief) is waiting for checker approval.
				</div>
			</div>
			<div
				className="p-3 rounded mb-2"
				style={{ background: "var(--pm-danger-soft)", fontSize: 13 }}
			>
				<strong>Float Alert</strong>
				<div style={{ fontSize: 11, color: "var(--pm-muted)", marginTop: 4 }}>
					Payments for next 48h exceed available float by KES 800K.
				</div>
			</div>
			<div
				className="p-3 rounded mb-2"
			...
			>
				...
				<div style={{ fontSize: 11, color: "var(--pm-muted)", marginTop: 4 }}>
					PesaLink routing optimized for lower fees.
				</div>
			</div>
		</MBox>
	);
	...
	const renderHealthCheck = () => (
		...
		>
			<div className={s.statusRow}>
				<div>
					<strong>M-Pesa B2C Gateway</strong>
					<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
						Response time: 120ms
					</div>
				</div>
				<span className={cx(s.badge, s.badgeS)}>Operational</span>
			</div>
			<div className={s.statusRow}>
				<div>
					<strong>PesaLink Core</strong>
					<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
						Response time: 450ms
					</div>
				</div>
			...
			<div className={s.statusRow}>
				...
				<span className={cx(s.badge, s.badgeW)}>Degraded</span>
			</div>
			<div className={s.statusRow}>
				<div>
					<strong>PayMo Webhooks</strong>
					<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
			...
			>
				Batches targeting Airtel Money will auto-retry up to 3x or fallback to
				Bank if configured.
			</div>
		</MBox>
	);
