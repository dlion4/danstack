			...
			<div className={s.content}>
					...
					{config.heroStats.map((hs) => (
						<div key={hs.key} className={hs.col}>
							<div
								className={cx(
									s.card,
									hs.key === "nextPayroll" ? s.cardAccent : "",
								)}
				...
				<div className={s.card}>
					...
					{config.complianceAlerts.map((ca) => (
						<div
							key={ca.title}
							className={s.complianceAlert}
							style={{ background: ca.iconBg }}
						>
							<div
								className={cx(s.iconCircle)}
								style={{
									background: ca.iconBg,
									color: ca.iconColor,
									width: 36,
									height: 36,
									minWidth: 36,
							...
							<span className={cx(s.badge, s[ca.tone])}>
								{ca.title.split(" ")[0]}
							</span>
						</div>
					))}
					...
					<div className="d-flex justify-content-between align-items-center mb-3">
						<div>
							<h3 className={cx(s.st, "text-success")}>
								...
								/>{" "}
								3.4.1 — Employee Database
							</h3>
							<p className={s.ss}>
								Manage employee records, departments, and payroll assignments.
							</p>
						</div>
				...
				<div className={s.card}>
					<div className="d-flex justify-content-between align-items-center mb-3">
						<div>
							...
							<p className={s.ss}>
								Run payroll batches, review, approve, and disburse salary
								payments.
							</p>
						</div>
						<button
								...
								{config.batches.map((b) => (
									<tr key={b.id}>
										...
										<td>
											...
											>
												{b.status === "Pending Approval" ? "Approve" : "View"}
											</button>
										</td>
									</tr>
								))}
					...
					>
						<strong>Funding Limits:</strong> Current float KES 14.2M · Monthly
						payroll KES 4.2M · Buffer: KES 10M
					</div>
				</div>
				...
				<div className={s.card}>
					<div className="d-flex justify-content-between align-items-center mb-3">
						<div>
							...
							<p className={s.ss}>
								Generate payslips, file statutory returns, and manage tax
								documents.
							</p>
						</div>
					</div>
						...
						{config.actionCards.map((ac) => (
							<div key={ac.key} className="col-lg-3 col-md-6">
								<div
									...
									onClick={() => setActiveModal(ac.modal)}
								>
									<i
										className={`bi ${ac.icon}`}
										style={{ fontSize: 24, color: ac.iconColor }}
									/>
				...
				<div className={s.card}>
					<div className="d-flex justify-content-between align-items-center mb-3">
						<div>
							...
							<p className={s.ss}>
								Leave requests, expense claims, and employee portal management.
							</p>
						</div>
						<button
							className={cx(s.btnPm, s.btnSm)}
							onClick={() => setActiveModal("employeeSelfServiceModal")}
						>
							<i className="bi bi-laptop" /> ESS Portal
						</button>
					</div>
					<div className="table-responsive">
						<table className={s.tbl}>
								...
								{config.requests.map((req) => (
									<tr key={req.name + req.type}>
										...
										<td>
											...
											>
												{req.status === "Pending" ? "Review" : "Approve"}
											</button>
										</td>
									</tr>
								))}
