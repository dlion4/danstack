			...
			<div className={s.content}>
					...
					{config.heroStats.map((hs) => (
						<div key={hs.key} className={hs.col}>
							<div
								className={cx(
									s.card,
									hs.key === "outstanding" ? s.cardAccent : "",
								)}
				...
				<div className={s.card}>
					<h3 className={cx(s.st, "text-warning")}>
						<i className="bi bi-stars" style={{ color: "var(--pm-warning)" }} />{" "}
						Smart Suggestions
					</h3>
					{config.suggestions.map((sg) => (
						<div key={sg.title} className={s.feedItem}>
							<div
								className={s.iconCircle}
								style={{ background: sg.iconBg, color: sg.iconColor }}
							>
								<i className={`bi ${sg.icon}`} />
							</div>
							<div style={{ flex: 1 }}>
							...
							<button
								...
								onClick={() => setActiveModal(sg.modal)}
							>
								{sg.btnLabel}
							</button>
						</div>
						...
						<div>
							<h3 className={cx(s.st, "text-primary")}>
								<i
									...
									style={{ color: "var(--pm-primary)" }}
								/>{" "}
								3.3.1 — Invoice Creation & Management
							</h3>
							<p className={s.ss}>
								Create, send, and track all invoices with payment status.
							</p>
						</div>
						<button
					...
					<div className="d-flex justify-content-between align-items-center mb-3">
						<div>
							<h3 className={cx(s.st, "text-info")}>
								...
								/>{" "}
								3.3.2 — Payment Links & Checkout
							</h3>
							<p className={s.ss}>
								Generate payment links, track views, conversions, and revenue.
							</p>
						</div>
						<button
							className={cx(s.btnPm, s.btnSm)}
					...
					<div className="row g-3">
						{config.linkCards.map((lc) => (
							<div key={lc.key} className="col-lg-4 col-md-6">
								<div
									...
									onClick={() => setActiveModal(lc.modal)}
								>
									<div className="d-flex justify-content-between align-items-center">
										<strong style={{ fontSize: 13 }}>{lc.name}</strong>
										<span className={cx(s.badge, s[lc.statusTone])}>
											{lc.status}
										</span>
									</div>
									<div className={s.sv} style={{ fontSize: 18 }}>
										{lc.amount}
									</div>
									<div className="d-flex gap-2 mt-2" style={{ fontSize: 12 }}>
										<span>
											<i className="bi bi-eye" /> {lc.views} views
										</span>
										<span>
											<i className="bi bi-check-circle" /> {lc.conversions} paid
										</span>
									</div>
								</div>
							</div>
						...
						<div className="col-lg-4 col-md-6">
							<div
								...
								style={{ cursor: "pointer" }}
								onClick={() => setActiveModal("linkAnalyticsModal")}
							>
								...
								<span className={cx(s.badge, s.badgeS)}>
									<i className="bi bi-arrow-up" /> +8% this month
								</span>
							</div>
						</div>
					</div>
				...
				<div className={s.card}>
					<div className="d-flex justify-content-between align-items-center mb-3">
						<div>
							<h3 className={cx(s.st, "text-warning")}>
								...
								/>{" "}
								3.3.3 — Collections Tracking & Aging
							</h3>
							<p className={s.ss}>
								Monitor aging buckets, escalation workflows, and collection
								rates.
							</p>
						</div>
						<button
							className={cx(s.btnPm, s.btnSm)}
							onClick={() => setActiveModal("agingReportModal")}
						>
					...
					<div className="row g-3">
						{config.agingBlocks.map((ab) => (
							<div key={ab.range} className="col-md-4">
								...
								>
									<div
										style={{ fontSize: 12, color: ab.color, fontWeight: 600 }}
									>
										{ab.range}
									</div>
									<div
					...
					<div className="mt-3 d-flex gap-2">
						<button
							className={cx(s.btnPm, s.btnSm)}
							onClick={() => setActiveModal("reminderSettingsModal")}
						>
							<i className="bi bi-bell" /> Send Auto-Reminders
						</button>
						<button
							className={cx(s.btnPm, s.btnSm)}
							onClick={() => setActiveModal("bulkRemindersModal")}
						>
					...
					<div className="d-flex justify-content-between align-items-center mb-3">
						<div>
							<h3 className={cx(s.st, "text-purple")}>
								...
								/>{" "}
								3.3.4 — Subscription & Recurring Billing
							</h3>
							<p className={s.ss}>
								Manage plans, subscriber lifecycle, and recurring revenue.
							</p>
						</div>
						<button
							className={cx(s.btnPm, s.btnSm)}
						...
						{config.planCards.map((pc) => (
							<div key={pc.key} className="col-lg-4 col-md-6">
								<div
									...
									style={{ cursor: "pointer" }}
									onClick={() => setActiveModal(pc.modal)}
								>
									<div className="d-flex justify-content-between align-items-center">
										<strong>{pc.name}</strong>
										<span className={cx(s.badge, s[pc.badge.tone])}>
											{pc.badge.text}
										</span>
									</div>
									<div className={s.sv} style={{ fontSize: 16 }}>
										{pc.price}
									</div>
