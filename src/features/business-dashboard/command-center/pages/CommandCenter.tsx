...
const initialMockData: CommandCenterConfig = {
	...
	statCards: [
		...
		},
		{
			...
			value: "KES 1.82M",
			valueColor: "var(--pm-ink)",
			badge: {
				icon: "bi-graph-up-arrow",
				text: "+12% vs last month",
				tone: "badgeS",
			},
			badgeExtra: "Target: 2.0M",
			modal: "revenueDetailsModal",
		},
		{
			key: "expenses",
			col: "col-lg-3 col-md-6",
			...
			progress: {
				percent: 65,
				color: "var(--pm-danger)",
				lines: ["Payroll: 450K", "Supplier: 320K"],
			},
			modal: "expenseDetailsModal",
		},
		{
			key: "pending",
			col: "col-lg-3 col-md-6",
	...
	attentionItems: [
		{
			...
			title: "Payroll Run: October 2025 Requires Approval",
			sub: "24 Employees · Total KES 450,500 · Maker: HR Dept",
		...
		},
		{
			...
			title: "KYB Update Required: Annual Returns",
			sub: "Upload 2024 CR12 to maintain Tier 3 limit (Overdue in 5 days)",
			btnLabel: "Upload Doc",
			btnClass: "btnPmD",
			modal: "kybUploadModal",
		},
		{
			icon: "bi-receipt",
			iconBg: "var(--pm-info-soft)",
			iconColor: "var(--pm-info)",
			title: "3 Invoices Aging > 60 Days",
			sub: "Total outstanding: KES 145,000 · Action recommended",
			btnLabel: "Send Reminders",
			btnClass: "",
			modal: "agingInvoicesModal",
		},
	],
	quickActions: [
		{
		...
		},
		{
			...
			iconColor: "var(--pm-info)",
			label: "Disburse",
			modal: "disburseFundsModal",
		},
		{
			icon: "bi-arrow-left-right",
			iconColor: "var(--pm-purple)",
			label: "Transfer",
			modal: "interCompanyTransferModal",
		},
		{
			icon: "bi-person-plus",
			iconColor: "var(--pm-warning)",
	...
	suggestion: {
		...
		text: "You have KES 1.2M idle cash. Consider moving 500K to the Money Market Fund to earn ~11% p.a.",
		btnLabel: "View Investment Options",
		modal: "interCompanyTransferModal",
	},
	chartBars: [
					...
					{config.statCards.map((sc) => (
						<div key={sc.key} className={sc.col}>
							...
							>
								<div
									className={s.sl}
									style={sc.labelColor ? { color: sc.labelColor } : {}}
								>
									{sc.label}
								</div>
								<div
									className={s.sv}
									style={sc.valueColor ? { color: sc.valueColor } : {}}
								>
									{sc.value}
								</div>
								{sc.badge && (
					...
					<div className="d-flex justify-content-between align-items-center mb-4">
						<div>
							<h3 className={cx(s.st, "text-primary")}>
								...
								/>{" "}
								3.1.1 — Business Overview Dashboard
							</h3>
							<p className={s.ss}>
								Collections vs Targets, Aging Invoices, and Disbursement Rates.
							</p>
						</div>
						<button
							className={cx(s.btnPm, s.btnSm)}
							onClick={() => setActiveModal("collectionTargetModal")}
						>
					...
					<div className="row g-4">
						<div className="col-lg-6">
							<div className={s.statusBlock}>
								...
								>
									Collections vs Target (Last 6 Months)
								</h4>
								<div className={s.chartContainer}>
									{config.chartBars.map((cb) => (
										<div
											...
											onClick={() => setActiveModal("revenueDetailsModal")}
										>
											<div
												style={{
													position: "relative",
							...
							<div className={s.statusBlock}>
								...
								<div className="mt-3">
									<button
										...
										onClick={() => setActiveModal("agingInvoicesModal")}
									>
										<i className="bi bi-envelope" /> Send Auto-Reminders to All
										Overdue
									</button>
								</div>
							</div>
							...
							<div className="d-flex justify-content-between align-items-center mb-3">
								<div>
									<h3 className={cx(s.st, "text-info")}>
										...
										/>{" "}
										3.1.2 — Business Profile & Settings
									</h3>
							...
							<div className="d-flex justify-content-between align-items-center mb-3">
								<div>
									<h3 className={s.st} style={{ color: "var(--pm-purple)" }}>
										...
										/>{" "}
										3.1.3 — Multi-Business Management
									</h3>
									<p className={s.ss}>
