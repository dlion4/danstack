...
export const UtilityShellContext =
	createContext<UtilityShellContextValue | null>(null);

export function useUtilityShell(): UtilityShellContextValue {
	const ctx = useContext(UtilityShellContext);
	if (!ctx) {
		throw new Error(
			"useUtilityShell() must be used inside <UtilityShell> (UtilityShellContext provider).",
		);
	}
	return ctx;
}
