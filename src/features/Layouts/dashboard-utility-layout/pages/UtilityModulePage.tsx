/* ============================================================================
 * UtilityModulePage.tsx — generic /utility/$module destination.
 * ----------------------------------------------------------------------------
 * Every sidebar entry (Electricity, Water, Auto-Pay, Saved Accounts …) and
 * every overview card links to /utility/$module. The overview is the layout
 * index (/utility). Unknown modules render a friendly empty state.
 * ========================================================================== */
import type { CSSProperties } from 'react';
import { Link } from '@tanstack/react-router';
import { useQuery } from '@tanstack/react-query';
import { cx, fetchUtilityLayoutContent, findModule, initialMockData } from '../data/utilityLayoutData';
...
interface UtilityModulePageProps {
  module: string;
}
...
export default function UtilityModulePage({ module }: UtilityModulePageProps) {
  const { showToast, openAside } = useUtilityShell();
        ...
        <section className={d.sectionCard}>
          <div className={d.emptyState}>
            ...
            <p style={{ color: 'var(--sh-ink-2)' }}>
              There is no utility module at <code>/{module}</code>.
            </p>
            <div className={d.actionRow} style={{ justifyContent: 'center' }}>
              <Link to="/utility" className={c.btnPrimary} style={{ textDecoration: 'none' }}>
  ...
  return (
    <div className={d.pageWrap}>
      ...
      <div className={d.actionRow}>
        <Link to="/utility" className={c.btnLink} style={{ textDecoration: 'none' }}>
          <i className="bi bi-arrow-left" /> Back to overview
        </Link>
      </div>
    </div>
