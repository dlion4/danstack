/* ============================================================================
 * UtilityShell.tsx — the reusable Paymo BAAS Utility Layout shell.
 * ----------------------------------------------------------------------------
 * MIGRATED FROM: Angular dashboard-utility-layout (typescript + html).
 ...
 *   and can call useUtilityShell() to fire toasts or open an aside panel.
 *
 * STACK ........: Vite + React + TypeScript + TanStack Router + TanStack Query
 * ARCHITECTURE .: Layout route renders <UtilityShell>, whose <Outlet /> hosts
 *                 the page route (UtilityHome, UtilityModulePage…).
 * ========================================================================== */
...
import {
  cx,
  fetchUtilityLayoutContent,
  initialMockData,
} from '../data/utilityLayoutData';
import type { AsideKind, ToastTone } from '../data/utilityLayoutData';
import { UtilityShellContext } from '../data/utilityLayoutContext';
import type { UtilityShellContextValue, ToastInput } from '../data/utilityLayoutContext';
import styles from '../styles/utilityLayout.module.css';

import UtilitySidebar from './UtilitySidebar';
import UtilityHeader from './UtilityHeader';
import type { DropdownName } from './UtilityHeader';
import UtilityAside from './UtilityAside';
import UtilityToasts from './UtilityToasts';
import type { UtilityToastRecord } from './UtilityToasts';
...
export default function UtilityShell() {
  ...
  /* ---------- toasts ---------- */
  const [toasts, setToasts] = useState<UtilityToastRecord[]>([]);
  ...
  const activeSection = useMemo(() => {
    ...
    // base path is /utility (the overview) which has no nav key -> '' (no highlight);
    // service routes put the module slug in segments[1].
  ...
  const showToast = useCallback((toast: ToastInput | string, tone?: ToastTone) => {
    ...
    const record: UtilityToastRecord = {
      id,
  ...
  /* ---------- context value ---------- */
  const ctxValue: UtilityShellContextValue = useMemo(
    () => ({ showToast, openAside }),
    [showToast, openAside],
  );
   ...
   * ==================================================================== */
  return (
    <UtilityShellContext.Provider value={ctxValue}>
      <div className={s.utilityRoot}>
        <UtilitySidebar
          content={content}
          isDesktop={isDesktop}
          expanded={expanded}
          ...
          {isLoading ? (
            <div className="d-flex justify-content-center align-items-center" style={{ padding: 80 }}>
              <div className="spinner-border text-primary" role="status">
                <span className="visually-hidden">Loading utility workspace…</span>
              </div>
            </div>
          ) : (
            <Outlet />
        ...
        <UtilityAside
          ...
          savedAccounts={content.savedAccounts}
          onClose={closeAside}
          onToast={showToast}
        />

        <UtilityToasts toasts={toasts} onDismiss={dismissToast} />
      </div>
    </UtilityShellContext.Provider>
  );
}
