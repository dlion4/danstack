 ...
 * ========================================================================== */
import type { AsideKind, ShellContent } from "../data/shellData";
...
export default function RightAside({
	...
	return (
		...
		>
			...
			<div className={cx(s.asidePanel, activePanel === "security" && s.active)}>
				<div className={s.asideHeader}>
					<span className={s.asideTitle}>
						<i
							className={cx("bi bi-shield-check", "me-1")}
				...
				<div className={s.asideBody}>
					<div className={s.asideCard}>
						...
						>
							TOTP via authenticator app is currently{" "}
							{twoFactorOn ? "active" : "disabled"}.
						</p>
					</div>
					...
					<div className={s.asideCard}>
						...
						{content.security.sessions.map((sess) => (
							<div className={s.statusRow} key={sess.device}>
								...
								<span className={s.statusPill}>
									<span className={cx(s.statusDot, s[sess.status])} />{" "}
									{sess.statusText}
								</span>
							</div>
						))}
						<button
							type="button"
							className={cx(s.btnDanger, "btn-sm w-100 mt-2")}
							onClick={onClose}
						>
			...
			{/* ============ DEVELOPERS ============ */}
			<div
				className={cx(s.asidePanel, activePanel === "developers" && s.active)}
			>
				<div className={s.asideHeader}>
					<span className={s.asideTitle}>
						<i
							className={cx("bi bi-code-slash", "me-1")}
							style={{ color: "var(--sh-accent-2)" }}
						/>{" "}
						Developer Tools
					</span>
					...
					<div className={s.asideCard}>
						...
						{content.developers.health.map((row) => (
							<div className={s.statusRow} key={row.service}>
								...
								<span className={s.statusPill}>
									<span className={cx(s.statusDot, s[row.status])} />{" "}
									{row.statusText}
								</span>
							</div>
						))}
