...
import { MODULES, NAV_GROUPS, NOTICES, PAY_METHODS, SCHEDULES, UTILITIES, kes, utilityOf } from "./data";
  ...
  return (
    <div className="canvas-wash min-h-screen">
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-30 flex items-stretch border-t border-line bg-white/95 backdrop-blur-xl lg:hidden">
        {[
          { key: "home", label: "Home", icon: "home" as const, target: "top" },
          { key: "utilities", label: "Utilities", icon: "grid" as const },
          { key: "__buy", label: "Buy", icon: "bolt" as const },
          { key: "history", label: "History", icon: "receipt" as const, target: "sec-history" },
          { key: "more", label: "More", icon: "more" as const },
        ].map((it) =>
          it.key === "__buy" ? (
            ...
            <button key={it.key} onClick={() => onNav(it.key, it.target)} className="focus-ring flex flex-1 flex-col items-center justify-center gap-1 py-2.5">
              <Icon name={it.icon} size={19} className={active === it.key ? "text-pmgreen" : "text-faint"} />
              <span className={cn("text-[10px] font-bold", active === it.key ? "text-ink" : "text-faint")}>{it.label}</span>
            </button>
          )
        )}
      </nav>
