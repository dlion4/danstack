...
const initialChannels: NotifChannel[] = [
	{
		...
		desc: "Every autopay run and manual payment receipt",
		icon: "check-circle",
		sms: true,
		email: true,
		push: true,
	...
	{
		id: "nc-2",
		label: "Low balance / units",
		desc: "Prepaid meter or data bundle running out",
		icon: "gauge",
		sms: true,
		email: false,
		push: true,
	...
	},
	{
		...
		desc: "3 days, 1 day and same-day reminders",
		icon: "calendar",
		sms: false,
		email: true,
		push: true,
	...
	},
	{
		...
		desc: "Failed, reversed or rejected transactions",
		icon: "alert",
		sms: true,
		email: true,
		push: true,
	...
	},
	{
		...
		desc: "A payment needs your sign-off",
		icon: "shield",
		sms: false,
		email: true,
		push: true,
	...
	{
		id: "nc-6",
		label: "Tariff & provider updates",
		desc: "Rate changes and service notices",
		icon: "info",
		sms: false,
		email: true,
		push: false,
...
const initialApprovers: Approver[] = [
	...
	},
	{
		...
		name: "Peter Otieno",
		role: "Site Manager",
		email: "peter@paymo.co.ke",
		limit: 25000,
		scope: "Electricity · Water",
		active: true,
	},
	{
		id: "ap-4",
...
const auditLog: AuditEntry[] = [
	{
		...
		target: "Home · Karen",
		detail: "KES 2,500 KPLC token · funded from wallet",
		tone: "success",
		icon: "repeat",
	},
	{
		...
		time: "09:15",
		actor: "Joseph Mwangi",
		action: "Cap increased",
		target: "Family TV",
		detail: "Monthly cap KES 10,000 → KES 12,000",
		tone: "info",
		icon: "sliders",
	},
	{
		...
		target: "Shop · Westlands",
		detail: "KES 8,400 KPLC postpaid · spike approval",
		tone: "success",
		icon: "shield",
	},
	{
		...
		actor: "Joseph Mwangi",
		action: "Rule paused",
		target: "Personal line",
		detail: "Airtime auto-renew disabled manually",
		tone: "warning",
		icon: "pause-circle",
	},
	{
		...
		target: "Cold Room",
		detail: "Bill 34% above 3-month average",
		tone: "warning",
		icon: "alert",
	},
	{
		...
		target: "Borehole permit",
		detail: "WRA account BH-001488 verified",
		tone: "info",
		icon: "plus",
	},
	{
		...
		action: "Rule failed",
		target: "Office fibre",
		detail: "Bank timeout · auto-retried from wallet",
		tone: "danger",
		icon: "alert",
	},
	{
		...
		target: "Mary Achieng",
		detail: "Accounts Clerk · KES 10,000 limit",
		tone: "info",
		icon: "users",
	},
];
...
const initialIntegrations: Integration[] = [
	{
		id: "in-1",
		name: "QuickBooks",
		desc: "Auto-post utility expenses to your ledger",
		icon: "file",
		category: "Accounting",
		connected: true,
		lastSync: "27 Jun · 15:00",
	},
	{
		id: "in-2",
		name: "Xero",
		desc: "Sync bills, receipts and reconciliations",
		icon: "file",
		category: "Accounting",
		connected: false,
	},
	{
		...
		desc: "Send receipts and alerts to tenants",
		icon: "phone",
		category: "Messaging",
		connected: true,
		lastSync: "27 Jun · 14:32",
	},
	{
		id: "in-4",
		name: "Google Drive",
		desc: "Archive every receipt PDF automatically",
		icon: "upload",
		category: "Storage",
		connected: true,
		lastSync: "27 Jun · 12:00",
	},
	{
		id: "in-5",
		name: "Slack",
		desc: "Post autopay events into a finance channel",
		icon: "send",
		category: "Messaging",
		connected: false,
	},
	{
		id: "in-6",
		name: "Analytics API",
		desc: "Pull consumption data into your BI tool",
		icon: "chart",
		category: "Data",
		connected: false,
	},
...
export function SettingsPage() {
	...
	const [ruleFilter, setRuleFilter] = useState<
		"all" | "active" | "paused" | "threshold" | "schedule"
	>("all");
	const [auditQuery, setAuditQuery] = useState("");
	const [auditFilter, setAuditFilter] = useState<"all" | "system" | "user">(
		"all",
	);
	...
	const shownRules = useMemo(() => {
		...
		if (ruleFilter === "threshold")
			return rules.filter((r) => r.type === "Threshold");
		if (ruleFilter === "schedule")
			return rules.filter((r) => r.type === "Schedule");
		return rules;
	}, [rules, ruleFilter]);
	...
	const shownAudit = useMemo(() => {
		...
		if (auditFilter === "system")
			rows = rows.filter((a) => a.actor.startsWith("System"));
		if (auditFilter === "user")
			rows = rows.filter((a) => !a.actor.startsWith("System"));
		if (auditQuery.trim()) {
	...
	const patchRule = (id: string, patch: Partial<AutoRule>) =>
	...
	return (
		<div className="mx-auto max-w-1320px">
				...
				<div className="position-relative d-grid gap-6 xl-grid-cols-1-12fr-0-88fr xl-gap-10">
					<div>
						<span className="d-inline-flex align-items-center gap-2 rounded-full border border-white-15 bg-white-10 px-3 py-15 fs-115 fw-semibold text-white-80 backdrop-blur">
							<span className="live-dot" /> {activeRules.length} automation
							rules running · {guardrailCount} guardrails enforced
						</span>
						...
						<p className="mt-3 max-w-56ch fs-135 leading-relaxed text-white-70 sm-fs-145">
							Build schedule or threshold rules, set caps and funding
							waterfalls, require approvals on spikes, and route every alert to
							the right channel — with a full audit trail behind it.
						</p>
						...
						<div className="mt-5 d-grid gap-2 sm-grid-cols-4">
							{[
								{
									k: "Active rules",
									v: `${activeRules.length}/${rules.length}`,
									s: "across 5 utilities",
									icon: "repeat" as IconName,
								},
								{
									k: "Automated (Jun)",
									v: kes(monthSpend),
									s: `${capUsed}% of monthly cap`,
									icon: "wallet" as IconName,
								},
								{
									k: "Approvers",
									v: `${approvers.filter((a) => a.active).length} people`,
									s: "maker–checker enabled",
									icon: "users" as IconName,
								},
								{
									k: "Integrations",
									v: `${integrations.filter((i) => i.connected).length} live`,
									s: "auto-sync receipts",
					...
					<div className="card-sheen position-relative overflow-hidden rounded-5 border border-white-12 bg-white-06 p-5 backdrop-blur">
						<div className="d-flex align-items-center justify-content-between gap-3">
							...
							<Badge
								tone={
									capUsed > 80 ? "danger" : capUsed > 60 ? "warning" : "success"
								}
							>
								{capUsed}% used
							</Badge>
						</div>
						<p className="num mt-2 font-display fs-26 fw-extrabold lh-1 text-white">
							{kes(monthSpend)}
						</p>
						<p className="mt-1 fs-11 text-white-50">
							of {kes(globalCap)} authorised for June
						</p>
						<div className="mt-3 h-8px w-100 overflow-hidden rounded-full bg-white-10">
							<div
								className={cn(
									"h-100 rounded-full transition-width duration-700",
									capUsed > 80
										? "bg-danger"
										: capUsed > 60
											? "bg-warn"
								...
								style={{ width: `${Math.min(capUsed, 100)}%` }}
							/>
						</div>
						<p className="mt-2 fs-11 text-white-45">
							Rules auto-pause at 100%. You'll be alerted at 80%.
						</p>
								...
								{[
									{
										n: "1",
										t: "PayMo wallet",
										d: `${kes(balance)} available · zero fee`,
									},
									...
									<div
										key={s.n}
										className="d-flex align-items-center gap-25 rounded-3 bg-white-05 p-25"
									>
			...
			<SectionHead
				...
				sub="Schedule-based or threshold-based rules with per-rule caps, funding waterfalls and spike protection."
			>
				<div className="d-flex flex-wrap gap-2">
					...
					<Chip
						on={ruleFilter === "paused"}
						onClick={() => setRuleFilter("paused")}
						count={rules.length - activeRules.length}
					>
						Paused
					</Chip>
					<Chip
						on={ruleFilter === "schedule"}
						onClick={() => setRuleFilter("schedule")}
						count={rules.filter((r) => r.type === "Schedule").length}
					>
						Schedule
					</Chip>
					<Chip
						on={ruleFilter === "threshold"}
						onClick={() => setRuleFilter("threshold")}
						count={rules.filter((r) => r.type === "Threshold").length}
					>
						Threshold
					</Chip>
				</div>
				...
				<div className="d-grid gap-3 sm-grid-cols-2 xl-grid-cols-3">
					{shownRules.map((r, i) => (
						<RuleCard
							...
							onDelete={() => {
								...
								toast({
									title: "Rule deleted",
									msg: `${r.nickname} will no longer auto-pay.`,
									tone: "warn",
								});
							}}
						/>
					...
					>
						...
						<p className="max-w-30ch fs-115 leading-relaxed text-muted">
							Pick a utility account, choose schedule or threshold, then set the
							cap and funding source.
						</p>
					</button>
				</div>
			)}
			...
			<SectionHead
				...
				sub="Workspace-wide protections that apply to every automation rule and manual payment."
			>
				<Badge tone="success" dot>
					{guardrailCount} enforced
				</Badge>
			</SectionHead>
			...
			<div className="d-grid gap-3 lg-grid-cols-3">
				...
				<Card className="lg-col-span-2">
					...
					<div className="mt-4 d-grid gap-4 sm-grid-cols-2">
						<Field
							label="Monthly automation cap (KES)"
							hint="All rules pause when this is reached."
						>
							<Input
								type="number"
								className="no-spin"
						...
						<Field
							label="Spike approval threshold (%)"
							hint="Require manual approval when a bill exceeds the 3-month average by this much."
						>
							<div className="d-flex align-items-center gap-3">
								<input
									...
									className="flex-1"
								/>
								<span className="num w-14 rounded-3 bg-canvas px-2 py-15 text-center fs-125 fw-bold text-ink">
									{spikeThreshold}%
								</span>
							</div>
						</Field>
					...
					<div className="mt-4 rounded-4 bg-paper-2 p-35">
						<Row
							k="Cap consumed"
							v={`${kes(monthSpend)} of ${kes(globalCap)}`}
							strong
						/>
						<Progress
							value={capUsed}
							tone={capUsed > 80 ? "red" : capUsed > 60 ? "amber" : "green"}
							className="mt-2"
						/>
						<div className="mt-2 d-flex align-items-center justify-content-between fs-11 text-muted">
							<span>{capUsed}% used</span>
							<span className="num">
								{kes(Math.max(globalCap - monthSpend, 0))} remaining
							</span>
						</div>
					</div>
					...
					<div className="mt-4 space-y-2">
						<GuardToggle
							...
							onChange={(v) => {
								setDualApproval(v);
								toast({
									title: v ? "Dual approval on" : "Dual approval off",
									msg: v
										? `Payments above ${kes(dualThreshold)} need two approvers.`
										: "Single approver is now sufficient.",
									tone: v ? "success" : "warn",
								});
							}}
							icon="users"
						...
						>
							{dualApproval && (
								<div className="mt-25">
									<Select
										value={dualThreshold}
										onChange={(e) => setDualThreshold(Number(e.target.value))}
									>
						...
						<GuardToggle
							on={pauseOnFail}
							onChange={setPauseOnFail}
							icon="pause-circle"
							title="Pause rule after 2 consecutive failures"
							desc="Stops repeated debits when a provider or funding source is down."
						/>
						<GuardToggle
							on={retryOnFail}
							onChange={setRetryOnFail}
							icon="refresh"
							title="Auto-retry from fallback source"
							desc="If the primary source fails, retry once from the fallback before pausing."
						/>
						<GuardToggle
							on={autoReverse}
							onChange={setAutoReverse}
							icon="repeat"
							title="Auto-reverse unconfirmed payments"
							desc="Reverse to source if the provider does not acknowledge within 24 hours."
						/>
						<GuardToggle
							on={weekendHold}
							onChange={setWeekendHold}
							icon="calendar"
							title="Hold executions on weekends"
							desc="Shift Saturday and Sunday runs to the next business day at 08:00."
						/>
						<GuardToggle
							on={receiptArchive}
							onChange={setReceiptArchive}
							icon="upload"
							title="Archive every receipt to Drive"
							desc="Store a signed PDF of each payment in your connected Google Drive folder."
						/>
					</div>
				</Card>
						...
						<label className="mt-3 d-flex align-items-start gap-3 rounded-4 border border-line bg-paper-2 p-3">
							...
							<span className="min-w-0 flex-1">
								<span className="d-block fs-125 fw-bold text-ink">
									Mute non-urgent alerts 22:00 – 06:00
								</span>
								<span className="mt-05 d-block fs-115 leading-relaxed text-muted">
									Failed payments and approval requests always break through.
								</span>
							</span>
						</label>
						{quietHours && (
			...
			<SectionHead
				...
				sub="Choose exactly which channel receives which event — per category, across SMS, email, push and WhatsApp."
			>
				<Button
					size="sm"
					variant="outline"
					...
					onClick={() => {
						...
						toast({
							...
							msg: "Every event now notifies all four channels.",
							tone: "success",
						});
					}}
				>
			...
			<Card className="p-0">
								...
								<tr key={c.id} className="transition hover-bg-paper-3">
									<td className="px-4 py-3">
										<div className="d-flex align-items-start gap-25">
											<span className="mt-05 d-grid h-8 w-8 flex-none place-items-center rounded-3 bg-canvas text-muted">
				...
				<div className="divide-y divide-line d-lg-none">
					{channels.map((c) => (
						<div key={c.id} className="p-35">
							<div className="d-flex align-items-start gap-25">
								<span className="d-grid h-9 w-9 flex-none place-items-center rounded-3 bg-canvas text-muted">
				...
				<div className="d-flex flex-wrap align-items-center justify-content-between gap-2 border-top border-line bg-paper-2 px-4 py-35">
					<p className="fs-115 text-muted">
						Alerts are sent to{" "}
						<span className="fw-bold text-ink-2">j@paymo.co.ke</span> and{" "}
						<span className="num fw-bold text-ink-2">0712 *** 890</span>.
					</p>
					<Button
						size="sm"
						variant="outline"
						...
						onClick={() =>
							toast({
								...
								msg: "Check your SMS, email and push notifications.",
								tone: "info",
							})
						}
					>
			...
			<SectionHead
				no="3.6C"
				id="sec-approvers"
				title="Approvers & spend limits"
				sub="Maker–checker controls: who can authorise what, and up to which amount."
			>
				<Button
					size="sm"
					variant="outline"
					...
					onClick={() =>
						toast({
							...
							msg: "Send an invite from Team & roles to add a new approver.",
							tone: "info",
						})
					}
				>
				...
				{approvers.map((a, i) => (
					...
					>
						<div className="d-flex align-items-start gap-3">
							...
							<Toggle
								on={a.active}
								label={`${a.name} approver`}
								onChange={(v) => {
									setApprovers((p) =>
										p.map((x) => (x.id === a.id ? { ...x, active: v } : x)),
									);
									toast({
										title: v ? "Approver enabled" : "Approver disabled",
										msg: `${a.name} · ${a.role}`,
										tone: v ? "success" : "warn",
									});
								}}
						...
						<div className="mt-3 rounded-4 bg-paper-2 p-3">
							...
							<Row
								k="Email"
								v={<span className="text-truncate fs-115">{a.email}</span>}
							/>
						</div>
						<div className="mt-3">
			...
			<SectionHead
				...
				id="sec-integrations"
				title="Integrations"
				sub="Push receipts and consumption data into the tools your finance team already uses."
			>
				<Badge tone="info">
								...
								onClick={() => {
									...
									toast({
										...
										msg: it.connected
											...
											: "Initial sync started — receipts will appear shortly.",
										tone: it.connected ? "warn" : "success",
									});
								}}
							>
			...
			<SectionHead
				...
				sub="Every rule change, approval and automated execution, attributed and timestamped."
			>
				<Button
					size="sm"
					variant="outline"
			...
			<Card className="p-0">
				<div className="space-y-3 border-bottom border-line p-4">
					<div className="d-flex flex-wrap gap-2">
						<div className="min-w-220px flex-1">
							<Input
								icon="search"
								placeholder="Search actor, action or account…"
								value={auditQuery}
						...
						<ol className="position-relative space-y-4 border-start border-dashed border-line ps-6">
							{shownAudit.map((a) => (
								<li key={a.id} className="position-relative">
									<span
										className={cn(
											"position-absolute left-n31px d-grid h-6 w-6 place-items-center rounded-full ring-4 ring-white",
											toneBg(a.tone),
										)}
									>
									...
									<div className="rounded-4 border border-line bg-paper-2 p-3">
										<div className="d-flex flex-wrap align-items-center gap-2">
											...
											<span className="num ms-auto text-nowrap fs-11 fw-semibold text-faint">
												{a.date} · {a.time}
											</span>
										</div>
										...
										<p className="mt-15 d-flex align-items-center gap-15 fs-11 fw-semibold text-ink-2">
											<Icon
												name={a.actor.startsWith("System") ? "repeat" : "user"}
												size={12}
				...
				<div className="d-flex flex-wrap align-items-center justify-content-between gap-2 border-top border-line bg-paper-2 px-4 py-35">
					<p className="fs-115 text-muted">
						Showing {shownAudit.length} of {auditLog.length} entries · retained
						7 years
					</p>
					<Button
						size="sm"
						...
						onClick={() =>
							toast({
								title: "Compliance pack queued",
								msg: "Full audit export will be emailed within 5 minutes.",
								tone: "success",
							})
						}
					>
			...
			<section className="mt-6 d-grid gap-3 lg-grid-cols-3" data-reveal>
					...
					<div className="d-flex flex-wrap align-items-start justify-content-between gap-4">
						<div className="max-w-52ch">
							...
							<p className="mt-2 fs-13 leading-relaxed text-white-65">
								Pause every automation instantly if a provider outage or funding
								issue occurs. Nothing is deleted — rules resume exactly where
								they left off.
							</p>
							<div className="mt-4 d-flex flex-wrap gap-2">
								<Button
									...
									onClick={() => {
										...
										toast({
											title: "All automation paused",
											msg: `${rules.length} rules stopped. Resume any time.`,
											tone: "warn",
										});
									}}
								>
								...
								<Button
									...
									onClick={() => {
										...
										toast({
											title: "All automation resumed",
											msg: `${rules.length} rules are running again.`,
											tone: "success",
										});
									}}
								>
				...
				<Card hover>
					...
					<div className="mt-3 space-y-25">
						{[
							{
								t: "Fund from the wallet first",
								d: "Zero fees vs KES 25 per bank debit — saves KES 300/yr per rule.",
							},
							{
								t: "Set caps 20% above average",
								d: "Catches billing errors without blocking legitimate seasonal spikes.",
							},
							{
								t: "Use threshold rules for prepaid",
								d: "Meters and SIMs run dry unpredictably — schedules miss them.",
							},
							{
								t: "Keep two active approvers",
								d: "Avoids a single point of failure when someone is on leave.",
							},
						].map((x) => (
							<div
								key={x.t}
				...
				onSave={(rule) => {
					if (editRule) {
						...
						toast({
							...
							msg: `${rule.nickname} will auto-pay ${kes(rule.amount)}.`,
							tone: "success",
						});
					}
					setBuilderOpen(false);
...
function GuardToggle({
	...
	return (
		<div
			className={cn(
				"rounded-4 border p-35 transition",
				on ? "border-pmgreen-30 bg-pmgreen-soft-25" : "border-line bg-paper-2",
			)}
		>
			<div className="d-flex align-items-start gap-3">
				<span
					className={cn(
						...
						on ? "bg-white text-pmgreen-ink shadow-sm" : "bg-canvas text-muted",
					)}
				>
...
function RuleCard({
	...
	return (
		...
		>
			<div className="d-flex align-items-start gap-3">
				...
				<div className="min-w-0 flex-1">
					<div className="d-flex flex-wrap align-items-center gap-15">
						...
						<Badge tone={rule.active ? "success" : "muted"} dot>
							{rule.active ? "Active" : "Paused"}
						</Badge>
					</div>
					<p className="num mt-05 fs-115 text-muted">
						{rule.provider} · {rule.accountRef}
					</p>
				</div>
				<Menu
					trigger={() => (
					...
					items={[
						...
						},
						{
							...
							icon: rule.active ? "pause-circle" : "play",
							onClick: () => {
								onPatch({ active: !rule.active });
								toast({
									title: rule.active ? "Rule paused" : "Rule resumed",
									msg: rule.nickname,
									tone: rule.active ? "warn" : "success",
								});
							},
						},
						{
			...
			<div className="mt-3 d-flex flex-wrap align-items-center gap-15">
				<Badge
					tone={rule.type === "Threshold" ? "violet" : "info"}
					icon={rule.type === "Threshold" ? "gauge" : "calendar"}
				>
					{rule.type}
				</Badge>
				{rule.approvalOnSpike && (
					<Badge tone="warning" icon="shield">
						Spike guard
					</Badge>
				)}
				{rule.notifyOnRun && (
					<Badge tone="muted" icon="bell">
						Notify
					</Badge>
				)}
			</div>
			...
			<div className="mt-3 rounded-4 bg-paper-2 p-3">
				<Row
					k="Trigger"
					v={<span className="text-end fs-115">{rule.timing}</span>}
				/>
				...
				<Progress
					value={capPct}
					tone={capPct > 85 ? "red" : capPct > 60 ? "amber" : "green"}
					className="mt-2"
				/>
				<div className="mt-15 d-flex align-items-center justify-content-between fs-11 text-muted">
					...
					<span>
						{rule.runsThisMonth} run{rule.runsThisMonth === 1 ? "" : "s"} this
						month
					</span>
				</div>
			</div>
...
function RuleBuilder({
	...
	const scheduleOptions = [
		"Monthly on the 1st · 08:00",
		"Monthly on the 5th · 08:00",
		"Monthly on the 30th · 08:00",
		"Weekly · Monday 08:00",
		"3 days before due date",
	];
	const thresholdOptions = [
		"When units fall below 10 kWh",
		"When units fall below 25 kWh",
		"When data falls below 1 GB",
		"When data falls below 500 MB",
		"When balance falls below KES 500",
	];
	...
	return (
		...
		>
			<Stepper
				steps={["Utility & type", "Amount & timing", "Guardrails & funding"]}
				current={step}
			/>
			...
			{step === 0 && (
				<div className="mt-4 space-y-4">
					<Field
						...
						required
						hint="Only verified saved accounts can be automated."
					>
						<Select
							value={accountId}
					...
					<div>
						...
						<div className="d-grid gap-2 sm-grid-cols-2">
							{[
								{
									v: "Schedule" as RuleType,
									icon: "calendar" as IconName,
									t: "Schedule based",
									d: "Pay on fixed dates or due dates",
								},
								{
									v: "Threshold" as RuleType,
									icon: "gauge" as IconName,
									t: "Threshold based",
									d: "Top up when balance drops",
								},
							].map((o) => (
								<button
									key={o.v}
								...
								>
									...
									<span
										className={cn(
											"mt-1 h-4 w-4 flex-none rounded-full border-2",
											type === o.v
					...
					{acc && (
						<div className="d-flex align-items-start gap-25 rounded-4 bg-pmblue-soft-70 p-3">
							...
							<p className="fs-12 leading-relaxed text-pmblue-ink">
								{utilityOf(acc.utility).name} · {acc.provider}.{" "}
								{type === "Threshold"
									? "Threshold rules check the balance every 30 minutes."
									: "Schedule rules execute at the chosen time in EAT."}
							</p>
						</div>
					)}
				</div>
			...
			{step === 1 && (
				<div className="mt-4 space-y-4">
					<Field
						label="Amount rule"
						hint="How PayMo decides what to pay each run."
					>
						<Select
							value={amountRule}
							onChange={(e) => setAmountRule(e.target.value as AmountRule)}
						>
							{(
								[
									"Fixed amount",
					...
					<Field label="Execution timing" required>
						<Select value={timing} onChange={(e) => setTiming(e.target.value)}>
							{(type === "Schedule" ? scheduleOptions : thresholdOptions).map(
								(t) => (
					...
					<div className="rounded-4 bg-paper-2 p-35">
						...
						<Row
							k="Max runs / month"
							v={`${Math.max(Math.floor(cap / Math.max(amount, 1)), 0)} runs`}
						/>
			...
			{step === 2 && (
				<div className="mt-4 space-y-4">
					<div className="d-grid gap-3 sm-grid-cols-2">
						...
						<Field
							label="Fallback funding source"
							hint="Used only if the primary fails."
						>
							<Select
								value={fallback}
					...
					<div className="space-y-2">
						<label className="d-flex align-items-start gap-3 rounded-4 border border-line bg-paper-2 p-35">
							<Toggle
								on={approvalOnSpike}
								onChange={setApprovalOnSpike}
								label="Spike approval"
							/>
							<span className="min-w-0 flex-1">
								<span className="d-block fs-125 fw-bold text-ink">
									Require manual approval on spikes
								</span>
								<span className="mt-05 d-block fs-115 leading-relaxed text-muted">
									Hold the payment if the bill is 20% higher than the 3-month
									average.
								</span>
							</span>
						</label>
						<label className="d-flex align-items-start gap-3 rounded-4 border border-line bg-paper-2 p-35">
							...
							<span className="min-w-0 flex-1">
								...
								<span className="mt-05 d-block fs-115 leading-relaxed text-muted">
									SMS and push the moment the rule runs, with the receipt
									attached.
								</span>
							</span>
						</label>
					...
					<div className="rounded-5 border border-line bg-white p-4">
						...
						<Row
							k="Trigger"
							v={<span className="text-end fs-115">{timing}</span>}
						/>
						<Row k="Amount" v={`${kes(amount)} · ${amountRule}`} />
						<Row k="Monthly cap" v={kes(cap)} />
						<Row
							k="Funding"
							v={`${methodName(primary)} → ${methodName(fallback)}`}
						/>
						<div className="my-1 h-px bg-line" />
						<Row
							k="Protections"
							v={`${[approvalOnSpike && "Spike guard", notifyOnRun && "Notify"].filter(Boolean).join(" · ") || "None"}`}
						/>
					</div>
					...
					<div className="d-flex align-items-start gap-25 rounded-4 bg-pmgreen-soft-60 p-3">
						...
						<p className="fs-12 leading-relaxed text-pmgreen-ink">
							Every automated run still requires your PIN once per day. Rules
							pause automatically at the monthly cap and never silently
							overspend.
						</p>
					</div>
				</div>
