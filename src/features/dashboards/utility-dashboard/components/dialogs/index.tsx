...
export function TxnDrawer() {
	...
	const timeline = [
		...
		{
			t: `${txn.method} debited`,
			d: `${kes(txn.amount + txn.fee, 2)} · authorised by J. Mwangi`,
			done: true,
		},
		{
			t: `${txn.provider} notified`,
			d:
				txn.status === "Pending"
					? "Awaiting provider confirmation"
					: `Confirmed in 6s · ref ${txn.ref}`,
		...
		},
		{
			...
			d:
				txn.status === "Failed"
					? (txn.note ?? "Rejected by provider")
					: txn.token
						? `SMS sent to 0712 *** 890`
						: (txn.note ?? "Receipt emailed"),
			done: txn.status === "Success",
		},
	];
	...
	return (
		<Drawer open onClose={close} width="max-w-480px">
			<DrawerHead
				title={`${u.name} receipt`}
				subtitle={`${txn.ref} · ${txn.date} ${txn.time}`}
				icon={u.icon}
				onClose={close}
			/>
			...
			<div className="thin-scroll flex-1 overflow-y-auto p-4">
				...
				<div
					className={cn(
						...
						txn.status === "Success" &&
							"bg-gradient-to-br from-pmgreen-soft to-white border border-pmgreen-25",
						txn.status === "Pending" &&
							"bg-gradient-to-br from-warn-soft to-white border border-warn-30",
						txn.status === "Failed" &&
							"bg-gradient-to-br from-danger-soft to-white border border-danger-25",
					)}
				>
					<div className="d-flex align-items-start gap-3">
						...
						<div className="min-w-0 flex-1">
							<div className="d-flex flex-wrap align-items-center gap-2">
								<p className="font-display fs-17 fw-extrabold text-ink">
									{kes(txn.amount, 2)}
								</p>
							...
							<Button
								...
								onClick={() =>
									toast({
										...
										msg: "Equity confirmed settlement — receipt updated.",
										tone: "success",
									})
								}
							>
				...
				{txn.token && (
					<div className="mt-4 rounded-5 border border-pmgreen-30 bg-pmgreen-soft-40 p-4">
						...
						<p className="num mt-15 font-display fs-18 fw-extrabold tracking-0-05em text-ink sm-fs-20">
							{txn.token}
						</p>
						<div className="mt-25">
				...
				<div className="mt-4 rounded-5 border border-line bg-white p-4">
					<p className="mb-15 fs-11 fw-bold text-uppercase tracking-0-14em text-faint">
						Transaction detail
					</p>
					<Row
						k="Reference"
					...
					<Row
						k="Convenience fee"
						v={txn.fee === 0 ? "Free" : kes(txn.fee, 2)}
					/>
				...
				{/* timeline */}
				<div className="mt-4 rounded-5 border border-line bg-white p-4">
					<p className="mb-3 fs-11 fw-bold text-uppercase tracking-0-14em text-faint">
						Settlement timeline
					</p>
					...
					<Button
						...
						onClick={() =>
							toast({
								...
								msg: `${txn.ref}.pdf saved to your device.`,
								tone: "success",
							})
						}
					>
					...
					<Button
						...
						onClick={() =>
							toast({
								...
								msg: "Link copied — valid for 7 days.",
								tone: "info",
							})
						}
					>
					...
					<Button
						...
						onClick={() =>
							toast({
								...
								msg: "A5 receipt format selected.",
								tone: "info",
							})
						}
					>
					...
					<Button
						...
						onClick={() =>
							toast({
								...
								msg: "Sent to j@paymo.co.ke and the finance alias.",
								tone: "success",
							})
						}
					>
				...
				<Button
					...
					onClick={() => {
						...
						open({
							kind: "buy",
							utility: txn.utility,
							accountId: accounts.find((a) => a.ref === txn.account)?.id,
							amount: txn.amount,
						});
					}}
				>
...
export function HistoryDrawer() {
	...
	const rows = useMemo(() => {
		...
		if (q.trim()) {
			...
			r = r.filter((t) =>
				`${t.ref} ${t.provider} ${t.account} ${t.nickname} ${t.method} ${kes(t.amount)}`
					.toLowerCase()
					.includes(s),
			);
		}
		r.sort((a, b) =>
			sort === "amount"
				? b.amount - a.amount
				: b.iso.localeCompare(a.iso) || b.time.localeCompare(a.time),
		);
		return r;
			...
			{selected ? (
				<div className="thin-scroll flex-1 overflow-y-auto p-4">
					<button
						...
						className="focus-ring mb-3 d-inline-flex align-items-center gap-15 fs-125 fw-bold text-pmgreen-ink"
					>
						<Icon name="chevron-left" size={14} /> Back to history
					</button>
					<ReceiptInline
						txn={selected}
						onOpenFull={() => open({ kind: "txn", txn: selected })}
					/>
				</div>
			) : (
					...
					<div className="space-y-3 border-bottom border-line p-4">
						<div className="d-flex flex-wrap gap-2">
							<div className="min-w-200px flex-1">
								<Input
									icon="search"
									placeholder="Search reference, provider, account…"
									value={q}
											...
											{rows.slice(0, limit).map((t) => (
												...
												>
													<td className="text-nowrap px-3 py-25 fs-12 fw-semibold text-ink-2">
														{t.date}
														<span className="ms-1 fs-11 fw-normal text-faint">
															{t.time}
														</span>
													</td>
													<td className="px-3 py-25">
														<span className="d-flex align-items-center gap-2">
															<span className="d-grid h-7 w-7 flex-none place-items-center rounded-3 bg-canvas text-muted">
																<Icon
																	name={utilityOf(t.utility).icon}
																	size={14}
																/>
															</span>
															<span className="fs-12 fw-semibold text-ink">
																{t.provider}
															</span>
														</span>
													</td>
													<td className="num px-3 py-25 fs-12 text-muted">
														{t.account}
													</td>
													<td className="num px-3 py-25 text-end fs-125 fw-bold text-ink">
														{kes(t.amount)}
													</td>
													<td className="px-3 py-25 fs-12 text-muted">
														{t.method}
													</td>
													<td className="num px-3 py-25 fs-115 fw-semibold text-muted">
														{t.ref}
													</td>
													<td className="px-3 py-25">
														<Badge
								...
								<div className="space-y-2 d-md-none">
									{rows.slice(0, limit).map((t) => (
										<button
											key={t.id}
											onClick={() => setSelected(t)}
											className="d-flex w-100 align-items-center gap-3 rounded-4 border border-line bg-white p-3 text-start transition active-scale-099"
										>
											...
											<span className="min-w-0 flex-1">
												<span className="d-flex align-items-center justify-content-between gap-2">
													<span className="text-truncate fs-125 fw-bold text-ink">
														{t.provider}
													</span>
													<span className="num fs-125 fw-bold text-ink">
														{kes(t.amount)}
													</span>
												</span>
							...
							<Button
								...
								onClick={() =>
									toast({
										...
										msg: "Monthly PDF statement will be emailed.",
										tone: "info",
									})
								}
							>
...
export function ExportModal() {
	...
	const run = () => {
		if (fmt === "csv") {
					...
					rows.push(
						keys.map((k) =>
							k === "date"
								? `${t.date} ${t.time}`
								: k === "utility"
									? `${utilityOf(t.utility).name} · ${t.provider}`
									: k === "account"
										? t.account
										: k === "amount"
											? t.amount
			...
			downloadCSV(
				`paymo-utilities-${range}d-${new Date().toISOString().slice(0, 10)}.csv`,
				rows,
			);
			toast({
				title: "Export ready",
				msg: `${rows.length - 1} transactions downloaded as CSV.`,
				tone: "success",
			});
		} else {
			toast({
				title: `${fmt.toUpperCase()} queued`,
				msg: "Your export will land in your inbox in ~2 minutes.",
				tone: "info",
			});
		}
		close();
			...
			<div className="space-y-4">
				<Field label="File format">
					<Segmented
						...
						options={[
							...
							{ value: "xls", label: "Excel", icon: "file" },
						]}
					/>
				</Field>
				<Field label="Period">
					...
					<div className="d-grid grid-cols-2 gap-2 sm-grid-cols-3">
						{Object.keys(cols).map((k) => (
							<button
								...
								className={cn(
									"d-flex align-items-center gap-2 rounded-4 border px-3 py-2 text-start fs-12 fw-semibold transition",
									cols[k]
										? "border-pmgreen bg-pmgreen-soft-40 text-ink"
										: "border-line bg-white text-muted",
								)}
							>
								<span
									className={cn(
										"d-grid h-4 w-4 place-items-center rounded border",
										cols[k]
											? "border-pmgreen bg-pmgreen text-white"
											: "border-gray-300",
									)}
								>
				...
				<label className="d-flex align-items-center gap-3 rounded-4 border border-line bg-paper-2 p-3">
					...
					<span className="fs-125 fw-medium text-ink-2">
						Include failed &amp; reversed payments
					</span>
				</label>
				<div className="rounded-4 bg-canvas p-3">
					<Row
						...
						v={
							failed
								? txns.length
								: txns.filter((t) => t.status !== "Failed").length
						}
					/>
					<Row
						k="Gross value"
						v={kes(txns.reduce((s, t) => s + t.amount, 0))}
					/>
...
export function AutopayDrawer() {
	...
	return (
		<Drawer open onClose={close} width="max-w-500px">
			<DrawerHead
				title="Autopay rules"
				subtitle={`${activeCount} of ${rules.length} rules active · KES 12,800 saved in fees this year`}
				icon="repeat"
				onClose={close}
			/>
			...
			<div className="thin-scroll flex-1 overflow-y-auto p-4">
				<div className="mb-4 d-grid grid-cols-3 gap-2">
					{[
						...
						>
							...
							<p className="num mt-15 font-display fs-15 fw-extrabold text-ink">
								{s.v}
							</p>
				...
				<div className="space-y-2">
					{rules.map((r) => (
						<div
							key={r.id}
							className="rounded-5 border border-line bg-white p-35"
						>
							<div className="d-flex align-items-start gap-3">
								<span className="d-grid h-10 w-10 flex-none place-items-center rounded-4 bg-canvas text-muted">
								...
								<div className="min-w-0 flex-1">
									<div className="d-flex flex-wrap align-items-center gap-15">
										<p className="fs-13 fw-bold text-ink">
											{r.account.nickname}
										</p>
										{r.on ? (
									...
									<p className="mt-05 fs-115 text-muted">
										{r.account.provider} ·{" "}
										<span className="num">{r.account.ref}</span>
									</p>
								...
								<Toggle
									...
									onChange={(v) => {
										setRules((prev) =>
											prev.map((x) => (x.id === r.id ? { ...x, on: v } : x)),
										);
										updateAccount(r.account.id, { autopay: v });
										toast({
											title: v ? "Autopay enabled" : "Autopay paused",
											msg: `${r.account.nickname} · ${r.trigger}`,
											tone: v ? "success" : "info",
										});
									}}
								/>
							...
							<div className="mt-3 d-grid grid-cols-2 gap-2 border-top border-line pt-3">
								...
								<div>
									...
									<Select
										value={
											PAY_METHODS.find((m) => m.name === r.method)?.id ??
											"mpesa"
										}
										onChange={(e) => {
											const val =
												PAY_METHODS.find((m) => m.id === e.target.value)
													?.name ?? "M-Pesa";
											setRules((prev) =>
												prev.map((x) =>
													x.id === r.id ? { ...x, method: val } : x,
												),
											);
										}}
									>
							...
							<div className="mt-25 d-flex flex-wrap gap-2">
								...
								<Button
									...
									onClick={() => {
										...
										toast({
											title: "Rule deleted",
											msg: `${r.account.nickname} will no longer auto-pay.`,
											tone: "warn",
										});
									}}
								>
				...
				{adding ? (
					<div className="mt-4 rounded-5 border border-pmgreen-30 bg-pmgreen-soft-25 p-4">
						<p className="mb-3 font-display fs-14 fw-bold text-ink">
							New autopay rule
						</p>
							...
							<div className="d-grid grid-cols-2 gap-3">
								...
								<Field label="Monthly cap (KES)">
									<Input
										...
										value={form.cap}
										onChange={(e) =>
											setForm({ ...form, cap: Number(e.target.value) })
										}
									/>
								</Field>
							</div>
								...
								<Button
									...
									onClick={() => {
										...
										if (acc) {
												...
												...prev,
												{
													...
													trigger:
														form.trigger === "low"
															...
															: form.trigger === "weekly"
																? "Weekly · Monday 8am"
																: form.trigger === "monthly"
																	? "Monthly on the 5th"
																	: "3 days before due date",
													amount: form.amount,
													method: PAY_METHODS.find((m) => m.id === form.method)!
														.name,
													on: true,
												},
											]);
											...
											toast({
												title: "Autopay rule created",
												msg: `${acc.nickname} will auto-pay ${kes(form.amount)}.`,
												tone: "success",
											});
										}
										setAdding(false);
				...
				<div className="mt-4 d-flex align-items-start gap-25 rounded-4 bg-pmblue-soft-70 p-3">
					...
					<p className="fs-12 leading-relaxed text-pmblue-ink">
						Every autopay run needs your PIN once per day. We pause a rule and
						alert you when 80% of its monthly cap is reached — no silent
						charges, ever.
					</p>
				</div>
			</div>
...
export function RenameModal() {
	...
	return (
		...
		>
			<Field
				label="Nickname"
				hint="Shown on the dashboard, receipts and reminders"
			>
				<Input
					value={name}
	...
	return (
						...
						onClick={() => {
							if (acc) {
								...
								toast({
									...
									msg: `${acc.nickname} deleted. History is retained.`,
									tone: "warn",
								});
							}
							close();
		...
		>
			<p className="fs-13 leading-relaxed text-ink-2">
				This only removes the saved reference —{" "}
				<strong>past receipts stay in your history</strong> for 7 years. Any
				active autopay rule on this account will be paused.
			</p>
				...
				<>
					...
					<Button
						...
						onClick={() => {
							toast({
								title: "Request sent",
								msg: `${m.title} access will be enabled for your workspace.`,
								tone: "success",
							});
							close();
						}}
		...
		>
			...
			<div className="mt-4 d-flex align-items-center gap-3 rounded-4 bg-ink p-4 text-white">
				...
				<p className="fs-125 leading-relaxed">
					You are currently on the <strong>Utilities 3.1</strong> page. All
					modules share one balance, one audit trail and one approval flow.
				</p>
			</div>
		</Modal>
	);
...
export function HelpModal() {
	...
	const channels = [
		{
			...
			sub: "Median reply 47 seconds",
			cta: "Start chat",
		},
		{
			icon: "mail" as const,
			name: "Email finance desk",
			sub: "utilities@paymo.co.ke",
			cta: "Send email",
		},
		{
	...
	return (
		<Modal
			...
			icon="lifebuoy"
			title="Help centre"
			subtitle="Answers, tariff explainers and 24/7 human support"
		>
			<div className="d-grid gap-2 sm-grid-cols-3">
				{channels.map((c) => (
					<div
						key={c.name}
						className="d-flex flex-column rounded-4 border border-line bg-paper-2 p-35"
					>
						<Icon name={c.icon} size={18} className="text-pmgreen" />
						<p className="mt-2 fs-125 fw-bold text-ink">{c.name}</p>
						<p className="mt-05 flex-1 fs-115 leading-relaxed text-muted">
							{c.sub}
						</p>
						<Button
							size="sm"
							...
							className="mt-25"
							onClick={() =>
								toast({ title: c.cta, msg: `${c.name} opening…`, tone: "info" })
							}
						>
							{c.cta}
						</Button>
			...
			<p className="mb-2 mt-5 fs-11 fw-bold text-uppercase tracking-0-14em text-faint">
				Frequent questions
			</p>
				...
				{FAQ.map((f, i) => (
					<div key={f.q}>
						<button
							...
							className="d-flex w-100 align-items-center gap-3 px-35 py-3 text-start transition hover-bg-paper-2"
						>
							<span className="d-grid h-6 w-6 flex-none place-items-center rounded-2 bg-canvas fs-11 fw-bold text-muted">
								{i + 1}
							</span>
						...
						{openIdx === i && (
							<p className="px-35 pb-35 fs-125 leading-relaxed text-muted">
								{f.a}
							</p>
						)}
					</div>
...
export function TariffModal() {
	...
	return (
		<Modal
			...
			icon="gauge"
			title="Tariff & fees"
			subtitle="ERC pass-through tariff · effective 01 Jul 2025"
		>
			<div className="rounded-5 border border-warn-30 bg-warn-soft-40 p-4">
				<p className="fs-11 fw-bold text-uppercase tracking-0-14em text-warn-ink">
					Domestic prepaid rate
				</p>
				<p className="num mt-1 font-display fs-26 fw-extrabold text-ink">
					KES {TARIFF.toFixed(2)}{" "}
					<span className="fs-14 fw-bold text-muted">/ kWh</span>
				</p>
				<p className="mt-1 fs-115 text-warn-ink">
					Units are indicative — KPLC applies fixed charges, ERC levy & VAT on
					the final token.
				</p>
			</div>
					...
					<tbody className="divide-y divide-line fs-125">
						{[
							...
							<tr key={r[0]}>
								...
								<td className="num px-3 py-25 text-end fw-bold text-ink">
									{r[2]}
								</td>
							</tr>
						))}
			...
			<div className="space-y-2">
				{PAY_METHODS.map((m) => (
					<div
						key={m.id}
						className="d-flex align-items-center gap-3 rounded-4 border border-line bg-white p-3"
					>
						...
						<Badge tone={m.fee === 0 ? "success" : "muted"}>
	...
	return (
				...
				<>
					...
					<Button
						...
						onClick={() => {
							toast({
								title: "Ticket opened",
								msg: "A specialist replies within 30 minutes on WhatsApp.",
								tone: "warn",
							});
							close();
						}}
		...
		>
			<div className="space-y-3">
				...
				<Field
					label="Describe it"
					hint="Include any SMS or reference you received — it speeds up the trace."
				>
					<textarea
						value={note}
