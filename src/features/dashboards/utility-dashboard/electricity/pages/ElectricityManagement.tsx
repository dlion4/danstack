...
export default function ElectricityManagement() {
	const { data } = useQuery({
		queryKey: ["electricity-deep-dive"],
		queryFn: fetchElectricityDeepDive,
		staleTime: 60_000,
		retry: 1,
	});
	...
	const rows =
		filter === "all"
			? config.postpaidRows
			: config.postpaidRows.filter((r) => r.status === filter);
	const counts = {
		all: config.postpaidRows.length,
		due: config.postpaidRows.filter((r) => r.status === "due").length,
		paid: config.postpaidRows.filter((r) => r.status === "paid").length,
		dispute: config.postpaidRows.filter((r) => r.status === "dispute").length,
	};
	const statusTone = (st: string) =>
		st === "due" ? "badgeD" : st === "paid" ? "badgeS" : "badgeW";
	const statusLabel = (st: string) =>
		st === "due" ? "Due" : st === "paid" ? "Paid/Upcoming" : "In dispute";
			...
			<div className={s.main}>
				<div className={s.pageBar}>
					...
					<div className="d-flex flex-wrap" style={{ gap: 8 }}>
						<button
							className={s.btnPm}
							onClick={() => open("portfolioHealthModal")}
						>
							<i className="bi bi-heart-pulse text-info" /> Health Check
						</button>
						<button
							className={s.btnPm}
							onClick={() => open("tokenHistoryModal")}
						>
							<i className="bi bi-safe text-warning" /> Token Vault
						</button>
						<button
							className={`${s.btnPm} ${s.btnPmI}`}
							onClick={() => open("payPostpaidModal")}
						>
							<i className="bi bi-receipt-cutoff" /> Pay Bill
						</button>
						<button
							className={`${s.btnPm} ${s.btnPmP}`}
							onClick={() => open("addMeterModal")}
						>
							<i className="bi bi-plus-lg" /> Add Meter
						</button>
					</div>
				</div>
								...
								>
									...
									{st.progress && (
										<div className="mt-2">
											...
											>
												<span>{st.progress.label}</span>
												<span>{st.progress.left}</span>
											</div>
											<div className={`${s.progress} mt-1`}>
												<div
													className={s.progressBar}
													style={{
														width: st.progress.pct,
														background: st.progress.color,
													}}
												/>
											</div>
										</div>
							...
							<div className={s.card}>
								...
								{config.attention.map((it) => (
									<div key={it.title} className={s.feedItem}>
										...
										<button
											className={`${s.btnPm} ${s.btnSm} ${it.actionClass ? s[it.actionClass] : ""}`}
											onClick={() => open(it.modal)}
										>
											{it.actionLabel}
										</button>
									</div>
						...
						<div className="col-lg-4">
							<div className={s.card}>
								<div className="mb-3">
									...
									<p className={s.sectionSub}>Frequent electricity workflows</p>
								</div>
						...
						>
							<div>
								<h3 className={s.sectionTitle}>
									...
									/>{" "}
									Prepaid Token Purchase & Meter Wallet
								</h3>
								<p className={s.sectionSub}>
									Purchase prepaid electricity tokens, manage low-balance
									alerts, share tokens, and run automated top-ups across
									multiple meters.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								...
								<button
									...
									onClick={() => open("buyTokenModal")}
								>
									<i className="bi bi-plus-lg" /> Buy Token
								</button>
							</div>
						</div>
						<div className="row g-3">
										...
										{config.prepaidMeters.map((mt) => (
											...
											>
												...
												<div className="mt-2 d-flex justify-content-between align-items-center">
													<span
														className={`${s.badge} ${s[mt.status === "safe" ? "badgeS" : mt.status === "watch" ? "badgeI" : mt.status === "low" ? "badgeW" : "badgeD"]}`}
													>
														{mt.units}
													</span>
									...
									{config.alertFeed.map((a) => (
										<div key={a.name} className={s.statusRow}>
											...
											<div className="text-end">
												<span className={`${s.badge} ${s[a.tone]}`}>
													{a.badge}
												</span>
												<div>
													<button
														className={`${s.btnPm} ${s.btnSm} mt-1 ${a.actionClass ? s[a.actionClass] : ""}`}
														onClick={() => open(a.modal)}
													>
														{a.actionLabel}
													</button>
								...
								<div className={s.utilityBlock}>
									...
									>
										...
										<div style={{ fontSize: 15, fontWeight: 700 }}>
											{config.tokenSummary.lastToken}
										</div>
										<div className="d-flex mt-2" style={{ gap: 8 }}>
											<button
												className={`${s.btnPm} ${s.btnSm}`}
												onClick={() => open("shareTokenModal")}
											>
												Share
											</button>
											<button
												className={`${s.btnPm} ${s.btnSm}`}
												onClick={() => open("redemptionCheckModal")}
											>
												Redeem
											</button>
										</div>
									...
									>
										...
										<div className="d-flex" style={{ gap: 8 }}>
											<button
												...
												onClick={() => open("shareTokenModal")}
											>
												<i className="bi bi-send" /> Share Token
											</button>
											<button
												className={`${s.btnPm} ${s.btnSm}`}
												onClick={() => open("statementExportModal")}
											>
						...
						>
							<div>
								<h3 className={s.sectionTitle}>
									...
									/>{" "}
									Postpaid Bill Management & Settlement Desk
								</h3>
								<p className={s.sectionSub}>
									View active postpaid accounts, fetch current bills, inspect
									tariff breakdowns, pay partially or fully, and raise billing
									disputes.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
									className={`${s.btnPm} ${s.btnSm}`}
									onClick={() => open("billBreakdownModal")}
								>
									<i className="bi bi-list-columns" /> Breakdown
								</button>
								<button
									className={`${s.btnPm} ${s.btnSm} ${s.btnPmP}`}
									onClick={() => open("payPostpaidModal")}
								>
									<i className="bi bi-credit-card" /> Pay Bill
								</button>
							</div>
						</div>
						<div className="row g-3">
									...
									>
										...
										<div className={s.tabPills}>
											{(
												...
												<button
													key={k}
													className={`${s.tabPill} ${filter === k ? s.tabPillActive : ""}`}
													onClick={() => setFilter(k)}
												>
													{l} ({counts[k]})
												</button>
											...
											<tbody>
												{rows.map((r) => (
													<tr key={r.account}>
														...
														<td data-label="Current Bill">
															<strong>{r.bill}</strong>
														</td>
														<td data-label="Due Date">{r.due}</td>
														<td data-label="Status">
															<span
																className={`${s.badge} ${s[statusTone(r.status)]}`}
															>
							...
							<div className="col-lg-4">
								<div className={`${s.utilityBlock} mb-3`}>
									...
									{config.billingSnapshot.map((b) => (
										<div key={b.label} className={s.statusRow}>
											...
											<strong
												style={
													b.danger ? { color: "var(--pm-danger)" } : undefined
												}
											>
												{b.amount}
											</strong>
								...
								<div className={s.utilityBlock}>
									...
									>
										...
										>
											{config.forecastRisk.highTitle}
										</div>
										<div style={{ fontSize: 12, color: "#7F1D1D" }}>
											{config.forecastRisk.highSub}
										</div>
									</div>
									<div
										className="p-3 rounded"
									...
									>
										...
										>
											{config.forecastRisk.bestTitle}
										</div>
										<div style={{ fontSize: 12, color: "#065F46" }}>
											{config.forecastRisk.bestSub}
										</div>
									</div>
								</div>
							</div>
						...
						>
							<div>
								<h3 className={s.sectionTitle}>
									...
									/>{" "}
									Meter Portfolio, Usage Analytics & Consumption Intelligence
								</h3>
								<p className={s.sectionSub}>
									Track kWh trends, compare meter performance, forecast future
									bills, and identify waste across your electricity portfolio.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
									className={`${s.btnPm} ${s.btnSm}`}
									onClick={() => open("usageAnalyticsModal")}
								>
									<i className="bi bi-graph-up-arrow" /> Analytics
								</button>
								<button
									className={`${s.btnPm} ${s.btnSm}`}
									onClick={() => open("meterCompareModal")}
								>
									<i className="bi bi-layout-split" /> Compare
								</button>
							</div>
						</div>
						<div className="row g-3">
									...
									<div className={s.chartBarGroup}>
										{config.consumptionTrend.map((b) => (
											<div
												...
												onClick={() => open("usageAnalyticsModal")}
											>
												<span className={s.barLabel}>{b.month}</span>
											</div>
										))}
									</div>
									<div
							...
							<div className="col-lg-4">
								<div className={s.utilityBlock}>
									...
									>
										Meter Performance Board
									</h4>
									{config.perfBoard.map((p) => (
										<div key={p.name} className={s.statusRow}>
											...
											<span className={`${s.badge} ${s[p.tone]}`}>
												{p.badge}
											</span>
										</div>
									))}
									<div className="d-flex mt-3" style={{ gap: 8 }}>
										...
										<button
											className={`${s.btnPm} ${s.btnSm}`}
											onClick={() => open("billForecastModal")}
										>
											Forecast
										</button>
									</div>
							...
							<div className="col-12">
								<div className={s.utilityBlock}>
									...
									>
										<h4 style={{ fontSize: 14, fontWeight: 700, margin: 0 }}>
											Meter Portfolio Directory
										</h4>
										<button
											className={`${s.btnPm} ${s.btnSm}`}
											onClick={() => open("manageMeterModal")}
										>
											<i className="bi bi-gear" /> Manage selected meter
										</button>
									</div>
									<div className="table-responsive">
										<table className={s.table}>
						...
						>
							<div>
								<h3 className={s.sectionTitle}>
									...
									/>{" "}
									Automation, Outages, Service Requests & Household Access
								</h3>
								<p className={s.sectionSub}>
									Configure auto-top-up rules, monitor outages and maintenance
									notices, submit service requests, upload evidence, and control
							...
							<div className="d-flex" style={{ gap: 8 }}>
								<button
									className={`${s.btnPm} ${s.btnSm}`}
									onClick={() => open("outageTrackerModal")}
								>
									<i className="bi bi-broadcast-pin" /> Outages
								</button>
								<button
									className={`${s.btnPm} ${s.btnSm} ${s.btnPmP}`}
									onClick={() => open("serviceRequestModal")}
								>
									<i className="bi bi-headset" /> New Request
								</button>
							</div>
						</div>
						<div className="row g-3">
								...
								<div className={s.utilityBlock}>
									...
									{config.autoRules.map((r) => (
										<div key={r.name} className={s.statusRow}>
											...
											<span
												className={`${s.badge} ${s[r.state === "Active" ? "badgeS" : r.state === "Paused" ? "badgeW" : "badgeI"]}`}
											>
												{r.state}
											</span>
										</div>
							...
							<div className="col-lg-4">
								<div className={s.utilityBlock}>
									<div className="d-flex justify-content-between align-items-center mb-3">
										<h4 style={{ fontSize: 14, fontWeight: 700, margin: 0 }}>
											Outage & Maintenance Feed
										</h4>
										<button
											className={`${s.btnPm} ${s.btnSm}`}
											onClick={() => open("reportOutageModal")}
										>
											Report
										</button>
									</div>
									{config.outageFeed.map((o) => (
										<div key={o.title} className={s.statusRow}>
											...
											<span className={`${s.badge} ${s[o.tone]}`}>
												{o.badge}
											</span>
										</div>
									))}
							...
							<div className="col-lg-4">
								<div className={s.utilityBlock}>
									<div className="d-flex justify-content-between align-items-center mb-3">
										...
										<button
											className={`${s.btnPm} ${s.btnSm}`}
											onClick={() => open("householdAccessModal")}
										>
											Manage
										</button>
									</div>
									...
									>
										...
										<div className="d-flex" style={{ gap: 8 }}>
											<button
												...
												onClick={() => open("disputeBillModal")}
											>
												<i className="bi bi-exclamation-circle" /> Dispute
											</button>
											<button
												className={`${s.btnPm} ${s.btnSm}`}
												onClick={() => open("uploadReadingModal")}
											>
											...
											<button
												...
												onClick={() => open("serviceRequestModal")}
											>
												<i className="bi bi-tools" /> Service Request
											</button>
										</div>
									</div>
									<div className="table-responsive">
											...
											<tbody>
												{config.cases.map((c) => (
													<tr key={c.id}>
														...
														<td data-label="Status">
															<span className={`${s.badge} ${s[c.tone]}`}>
																{c.status}
															</span>
														</td>
