	...
	return (
		<>
			...
			<div className={`${s.note} ${s.noteSuccess} text-center`}>
				...
				<div
					className={s.sv}
					style={{ color: "var(--pm-accent)", marginTop: 4 }}
				>
					KES {monthly.toLocaleString(undefined, { maximumFractionDigits: 0 })}
				</div>
		...
		<>
						...
						{applyStep >= 3 && (
							<>
								<div className={`${s.note} ${s.noteInfo} mb-3`}>
									<i className="bi bi-info-circle me-1" /> Review your
									application before submitting. Our partner team responds
								...
								/>
								<Chk
									label="I accept the PayMo Partner Program Terms."
									defaultChecked
								/>
							</>
						)}
				...
				footer={m.footer(
					...
					m.cancelAnd(
						<button
							...
							className={`${s.btnPm} ${s.btnPmP}`}
							onClick={() =>
								m.doAction("certExamModal", "Redirecting to Exam Portal...")
							}
						>
							Start Exam
						</button>,
				...
				{m.body(
					...
					<>
						...
						<div className={`${s.note} ${s.noteInfo} mt-3`}>
							<i className="bi bi-info-circle me-1" /> Recertification is
							required every 24 months to retain Certified Partner benefits.
						</div>
					</>,
				)}
			</MBox>
			...
			>
				{m.body(
					...
					<>
						...
						<Chk
							label="We perform annual penetration testing."
							defaultChecked
						/>
						<Chk
							label="We encrypt all data at rest and in transit."
							defaultChecked
						/>
					</>,
				)}
			...
			>
				...
				<div className={`${s.note} ${s.noteMuted} mt-3`}>
					Revenue share is calculated on PayMo's net processing fee, not on
					...
					m.cancelAnd(
						<button
							...
							onClick={() =>
								m.doAction(
									"coMarketingModal",
									"Marketing request submitted to partner team.",
								)
							}
						>
							Submit Request
							...
							<>
								...
								<div className="mb-3">
									...
									<Fld
										s={s}
										type="email"
										defaultValue={data.header.user.email}
									/>
								</div>
							</>
						)}
						{appStep >= 3 && (
							<>
								...
								<div className={`${s.note} ${s.noteInfo}`}>
									<i className="bi bi-info-circle me-1" /> Marketplace review
									takes up to 5 business days. You'll be notified by email.
								</div>
							</>
						)}
					</>,
				...
				footer={m.footer(
					...
					<>
						...
						<button
							...
							className={`${s.btnPm} ${s.btnPmP}`}
							onClick={() =>
								m.doAction("editAppModal", "App listing updated successfully.")
							}
						>
							Save Changes
						</button>
					...
					<>
						<div className="mb-3">
							...
							<Fld
								s={s}
								as="select"
								options={data.marketplaceApps.map((a) => a.name)}
							/>
						</div>
						<div className="mb-3">
							<Lbl s={s}>Status</Lbl>
							<Fld
								s={s}
								as="select"
								options={["Published", "Unlisted", "Draft"]}
							/>
						</div>
						<div className="mb-3">
								...
								<div className="d-flex justify-content-between align-items-center flex-wrap gap-2">
									...
									{r.replied ? (
										...
										<button
											...
											className={`${s.btnPm} ${s.btnSm}`}
											onClick={() =>
												m.doAction("appReviewsModal", "Reply posted.")
											}
										>
											Reply
										</button>
					...
					m.cancelAnd(
						<button
							...
							onClick={() =>
								m.doAction(
									"betaEnrollModal",
									"Enrolled in Beta! Check your email for API keys.",
								)
							}
						>
							Enroll
			...
			>
				{m.body(
					...
					<>
						...
						<Chk
							label="I accept that beta APIs may change without notice."
							defaultChecked
						/>
					</>,
				)}
					...
					m.cancelAnd(
						<button
							...
							onClick={() =>
								m.doAction(
									"hackathonModal",
									"Team registered! Check your email for event details.",
								)
							}
						>
							Register Team
				...
				footer={m.footer(
					...
					m.cancelAnd(
						<button
							...
							className={`${s.btnPm} ${s.btnPmP}`}
							onClick={() =>
								m.doAction("newsletterSubscribeModal", "Preferences updated.")
							}
						>
							Save Preferences
						</button>,
					...
					m.cancelAnd(
						<button
							...
							onClick={() =>
								m.doAction(
									"officeHoursModal",
									"Session booked! Calendar invite sent.",
								)
							}
						>
							Book Session
			...
			>
				{m.body(
					...
					<>
						<div
							className={`${s.note} ${s.notePurpleFallback ?? s.noteMuted} mb-3`}
						>
							<strong>Authentication & Security Deep-Dive</strong>
							<div style={{ fontSize: 12 }}>
								15 Nov 2026 · 14:00–16:00 EAT · Online
							</div>
						</div>
						<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
							Covers OAuth 2.0 flows, HMAC signature verification, idempotency
							keys and certificate pinning, with live code walkthroughs.
						</p>
					...
					m.cancelAnd(
						<button
							...
							onClick={() =>
								m.doAction(
									"partnerSummitModal",
									"Ticket confirmed! See you in Nairobi.",
								)
							}
						>
							Confirm Ticket
			...
			>
				<div className={`${s.note} ${s.noteSuccess} text-center mb-3`}>
					<i className="bi bi-check-circle-fill me-1" /> All Partner Systems
					Operational
				</div>
