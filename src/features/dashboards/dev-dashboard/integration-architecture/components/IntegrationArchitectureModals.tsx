			...
			<div className="mb-3">
				...
				<div className="d-flex gap-2">
					...
					<Fld
						s={s}
						mono
						defaultValue="https://sandbox.paymo.com/v1/payments"
					/>
				</div>
			</div>
			<div className="mb-3">
					...
					m.cancelAnd(
						<button
							...
							onClick={() =>
								m.doAction(
									"archGuideModal",
									"Guide generated successfully! Downloading...",
								)
							}
						>
							Download Package
				...
				{m.body(
					...
					<>
						<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
							Select the architecture topology document tailored for your
							enterprise.
						</p>
						<div className="mb-3">
							<Lbl s={s}>Format</Lbl>
							...
							<>
								...
								<div className="mb-3 mt-3">
									...
									<Fld
										s={s}
										as="select"
										options={data.boilerplateOptions.deployment}
									/>
								</div>
								<div className="mb-3">
									...
									<Fld
										s={s}
										as="select"
										options={data.boilerplateOptions.database}
									/>
								</div>
							</>
						)}
						...
						{boilerStep >= 4 && (
							<div className="text-center py-4">
								...
								<p style={{ color: "var(--pm-muted)" }}>
									Your microservice boilerplate with Webhooks and HMAC auth is
									ready.
								</p>
							</div>
						)}
						...
						<div className="col-12">
							...
							<div className="d-flex gap-2">
								<Fld
									...
									type="password"
									mono
									defaultValue="whsec_8849201283894"
									readOnly
								/>
								<button
									type="button"
					...
					<>
						...
						<button
							...
							onClick={() =>
								m.doAction(
									"embeddedCheckoutModal",
									"Checkout settings saved globally.",
								)
							}
						>
							Save Checkout
				...
				{m.body(
					...
					<>
						...
						<p style={{ fontSize: 12, color: "var(--pm-muted)", margin: 0 }}>
							Ideal for POS devices in low-connectivity areas. SDK handles
							indexing and state reconciliation automatically.
						</p>
					</>,
				)}
			</MBox>
					...
					m.cancelAnd(
						<button
							...
							onClick={() =>
								m.doAction(
									"meshConfigModal",
									"Service mesh manifest downloaded.",
								)
							}
						>
							Download Manifest
					...
					<>
						<div className="mb-3">
							...
							<Fld
								s={s}
								as="select"
								options={["Istio", "Envoy Native", "Linkerd"]}
							/>
						</div>
						<div className="mb-3">
						...
						<p style={{ fontSize: 12, color: "var(--pm-muted)" }}>
							Download the sidecar injection templates for your Kubernetes
							cluster to map PayMo endpoints internally.
						</p>
						<CodeBox s={s}>
				...
				footer={m.footer(
					...
					<>
						...
						<button
							...
							className={`${s.btnPm} ${s.btnPmP}`}
							onClick={() =>
								m.doAction("grpcSetupModal", "Protobuf specs downloaded.")
							}
						>
			...
			>
				{m.body(
					...
					<>
						...
						>
							For ultra-low latency internal microservice communication, use our
							Protobuf specs instead of JSON REST.
						</p>
						<div className="mb-3">
  ...
  rpc CreateCharge (ChargeReq) returns (ChargeRes);
					...
					m.cancelAnd(
						<button
							...
							onClick={() =>
								m.doAction(
									"circuitBreakerModal",
									"Circuit breaker thresholds updated.",
								)
							}
						>
							Save Thresholds
			...
			>
				{m.body(
					...
					<>
						...
						<p style={{ fontSize: 12, color: "var(--pm-muted)", margin: 0 }}>
							If failures to downstream banks exceed threshold, requests will
							fail fast for the reset timeout to prevent cascading failures.
						</p>
					</>,
				)}
			</MBox>
					...
					<>
						...
						{soapStep >= 3 && (
							<>
								...
								<div className="mt-3">
									<Chk
										label="Wrap with basic XML Security signatures"
										defaultChecked
									/>
									<Chk
										label="Translate ISO-8601 to legacy YYYYMMDD"
										defaultChecked
									/>
								</div>
							</>
				...
				footer={m.footer(
					...
					m.cancelAnd(
						<button
							...
							className={`${s.btnPm} ${s.btnPmP}`}
							onClick={() =>
								m.doAction("legacyDbSyncModal", "CDC stream initialized.")
							}
						>
			...
			>
				{m.body(
					...
					<>
						<p style={{ fontSize: 13, color: "var(--pm-muted)" }}>
							Configure CDC (Change Data Capture) or batch sync into your
						...
						<div className="mb-3">
							...
							<Fld
								s={s}
								as="select"
								options={["Debezium CDC Stream", "Nightly SQL Dump"]}
							/>
						</div>
						<div className="mb-3">
						...
						<Chk
							label="Anonymize PII Data (Mask Names/Phones)"
							defaultChecked
						/>
					</>,
				)}
					...
					<>
						...
						<div className="p-3 border rounded mt-3">
							...
							<p style={{ color: "var(--pm-muted)", margin: 0, fontSize: 12 }}>
								Aggregated reporting across tenants requires specific Master
								Admin IAM roles.
							</p>
						</div>
					</>,
					...
					m.cancelAnd(
						<button
							...
							onClick={() =>
								m.doAction(
									"cacheStrategyModal",
									"Redis cache rules pushed to edge.",
								)
							}
						>
							Update Caching
				...
				footer={m.footer(
					...
					<>
						...
						<button
							...
							className={`${s.btnPm} ${s.btnPmP}`}
							onClick={() =>
								m.doAction("cdnSetupModal", "CDN purge and remap initiated.")
							}
						>
							Purge & Map CDN
						</button>
					...
					<>
						<div className="mb-3">
							...
							<Fld
								...
								type="url"
								mono
								defaultValue="https://assets.yourdomain.com"
							/>
						</div>
						<p style={{ fontSize: 12, color: "var(--pm-muted)" }}>
							Route your checkout iFrame scripts and CSS through PayMo's
							globally distributed CDN for sub-50ms latency across Africa.
						</p>
				...
				footer={m.footer(
					...
					m.cancelAnd(
						<button
							...
							className={`${s.btnPm} ${s.btnPmP}`}
							onClick={() =>
								m.doAction("rateLimitModal", "Rate limits adjusted.")
							}
						>
							Save Limits
						</button>,
			...
			>
				{m.body(
					...
					<div className="row g-3">
						...
						<div className="col-12">
							<p style={{ color: "var(--pm-muted)", margin: 0, fontSize: 12 }}>
								Current utilization is at 12% of threshold. No throttling events
								logged in last 24h.
							</p>
						</div>
					</div>,
			...
			>
				{m.body(
					...
					<>
						...
						<p style={{ fontSize: 12, color: "var(--pm-muted)", margin: 0 }}>
							This will redirect sandbox traffic entirely to the surviving nodes
							to test your application's connection retry logic and latency
							tolerance.
						</p>
					</>,
				)}
					...
					<>
						...
						<button
							...
							onClick={() =>
								m.doAction(
									"lbIntegrationModal",
									"LB rules updated across target groups.",
								)
							}
						>
							Apply Routing
					...
					<>
						...
						<button
							...
							onClick={() =>
								m.doAction(
									"drMonitorModal",
									"DR drill scheduled for next maintenance window.",
								)
							}
						>
							Schedule Drill
					...
					<div className="row g-3">
						...
						<div className="col-12 mt-3">
							...
							<p style={{ color: "var(--pm-muted)", margin: 0, fontSize: 13 }}>
								Passed on 12 Jun 2025. Data loss zero. Fallback switch completed
								within target RTO.
							</p>
						</div>
					</div>,
			...
			>
				{m.body(
					...
					<>
						<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
							Send the same idempotency key twice to confirm PayMo replays the
					...
					m.cancelAnd(
						<button
							...
							onClick={() =>
								m.doAction(
									"certPinningModal",
									"Certificate pins saved and distributed.",
								)
							}
						>
							Save Pins
					...
					<>
						...
						<div className="mb-3">
							...
							<Fld
								s={s}
								mono
								defaultValue="sha256/AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA="
							/>
						</div>
						<div className="mb-3">
							...
							<Fld
								s={s}
								mono
								defaultValue="sha256/BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB="
							/>
						</div>
						<div className={`${s.note} ${s.noteWarn}`}>
							<i className="bi bi-exclamation-triangle me-1" /> Always ship a
							backup pin. Pinning only one certificate will brick clients on
							rotation.
						</div>
					</>,
				)}
