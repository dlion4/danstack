...
const initialMockData: CardProgramAdministrationConfig = {
	...
	statCards: [
		...
		},
		{
			...
			value: "1,420",
			badge: { icon: "bi-graph-up-arrow", text: "+12% MoM", tone: "badgeS" },
			lines: [],
			warnBorder: false,
		},
		{
			...
			lines: ["Velocity breaches: 12", "Geo-mismatch: 11", "AML flags: 5"],
			warnBorder: false,
		},
		{
			key: "revenue",
			...
			lines: [
				"Interchange fee: KES 8.4M",
				"Issuance & FX: KES 4.2M",
				"ATM & Penalty: KES 1.6M",
			],
			warnBorder: false,
		},
	],
	alerts: [
		{
			...
			title: "5 High-Risk AML Alerts",
			sub: "Requires manual review & unblock",
			btnLabel: "Review",
			btnTone: "badgeD",
		...
		},
		{
			...
			title: "Batch Issuance #8892 Pending",
			sub: "250 Corporate virtual cards",
			btnLabel: "Approve",
			btnTone: "badgeS",
		...
		{
			initials: "DS",
			color: "var(--pm-purple-soft)",
			iconColor: "var(--pm-purple)",
			title: "14 New Chargeback Disputes",
			sub: "Deadline approaching for 3 cases",
			btnLabel: "Resolve",
			btnTone: "badgeI",
			modal: "disputeResolutionModal",
		},
		{
			initials: "ST",
			color: "var(--pm-info-soft)",
			iconColor: "var(--pm-info)",
			title: "Low Settlement Balance",
			sub: "BIN 441011 settlement account < 10%",
			btnLabel: "Top-up",
			btnTone: "badgeS",
			modal: "settlementDetailsModal",
		},
	],
	suggestions: [
		{
			...
			title: "Update Velocity Limits",
			sub: "Increase daily limits for corporate BINs based on usage",
			btnLabel: "Update",
			modal: "velocityLimitsModal",
		},
		{
			initials: "MC",
			color: "var(--pm-info-soft)",
			iconColor: "var(--pm-info)",
			title: "Review MCC Blocklist",
			sub: "High failure rate on MCC 5411 (Grocery)",
			btnLabel: "Review",
			modal: "mccBlocklistModal",
		},
		{
			initials: "FX",
			color: "var(--pm-warning-soft)",
			iconColor: "var(--pm-warning)",
			title: "Optimize FX Fee Schedule",
			sub: "Consider lowering cross-border fees by 0.5%",
			btnLabel: "Model",
			modal: "feeScheduleModal",
		},
		{
			initials: "WH",
			...
			sub: "3 endpoint timeouts in the last 24h",
			btnLabel: "Check",
	...
	supportActions: [
		{
			name: "James Kamau (Corporate)",
			sub: "Card *4421 · PIN reset requested",
			btnLabel: "Action",
			btnTone: "badgeS",
			modal: "resetPinModal",
		},
		{
			name: "Amina Hassan (Retail)",
			sub: "Card *8812 · Reported stolen",
			btnLabel: "Freeze",
			btnTone: "badgeD",
			modal: "forceFreezeModal",
		},
		{
			name: "TechCorp Ltd (Expense)",
			sub: "Card *9923 · Replacement damaged",
			btnLabel: "Initiate",
			btnTone: "badgeS",
	...
	batchQueue: [
		{
			...
			statusTone: "badgeW",
			sub: "250 Virtual · Corp Expense BIN · by John D.",
			btnLabel: "Review",
			modal: "approveBatchModal",
		},
		{
			...
			statusTone: "badgeI",
			sub: "150 Physical · Premium BIN · 40% printed",
			btnLabel: "Track",
			modal: "batchStatusModal",
		},
		{
			id: "#8890",
			status: "Completed",
			statusTone: "badgeS",
			sub: "500 Virtual · Standard BIN · 12 Jun 2025",
			btnLabel: "Details",
			modal: "batchStatusModal",
		},
	],
...
async function fetchCardProgramAdministration(): Promise<CardProgramAdministrationConfig> {
	...
	try {
		const res = await fetch("/api/card-program-administration", {
...
export default function CardProgramAdministration() {
	...
	return (
		<div className={styles.cardProgramAdministration}>
			<div className={styles.main}>
				...
				<div className={styles.pageBar}>
					...
					<div className="d-flex flex-wrap" style={{ gap: 8 }}>
						...
						>
							<i className="bi bi-shield-check" /> Compliance Check
						</button>
						<button
							className={styles.btnPm}
						...
						<button
							className={styles.btnPm}
							onClick={() => openModal("cardholderSearchModal")}
						>
							<i className="bi bi-search" /> Find Cardholder
						</button>
						<button
							className={`${styles.btnPm} ${styles.btnPmP}`}
							onClick={() => openModal("issueBatchModal")}
						>
							<i className="bi bi-upload" /> Issue Cards Batch
						</button>
					</div>
				</div>
				...
				<div className={styles.content}>
					...
					<div className="row g-3">
						<div className="col-lg-4">
							...
							>
								...
								>
									Operating across 3 BIN programs (Corporate, Standard Debit,
									Premium Credit).
								</p>
								<div className="d-flex flex-wrap mt-3" style={{ gap: 8 }}>
									...
									<button
										...
										onClick={() => openModal("binConfigModal")}
									>
										BIN config
									</button>
								</div>
							...
							<div className={styles.card}>
								...
								{config.alerts.map((a) => (
									<div key={a.title} className={styles.feedItem}>
										...
										<button
											className={`${styles.btnPm} ${styles.btnSm} ${a.btnTone === "badgeD" ? styles.btnPmD : a.btnTone === "badgeS" ? styles.btnPmA : styles.btnPmP}`}
							...
							<div className={styles.card}>
								<div className="d-flex justify-content-between align-items-center mb-3">
									...
									<span className={styles.badge + " " + styles.badgeP}>
										<i className="bi bi-stars"></i> System AI
									</span>
								</div>
					...
					{/* Program Overview & BIN Management */}
					<div className={styles.card}>
						<div
						...
						>
							<div>
								...
								<p className={styles.sectionSub}>
									Manage active card programs, view BIN performance, and
							...
							<div className="d-flex" style={{ gap: 8 }}>
								<button
									className={styles.btnPm + " " + styles.btnSm}
									onClick={() => openModal("addBinModal")}
								>
									<i className="bi bi-plus"></i> Request New BIN
								</button>
							</div>
						</div>
										...
										<tr key={b.bin}>
											...
											<td data-label="Network">
												<span
													...
													style={{ background: b.networkColor, color: "white" }}
												>
													{b.network}
												</span>
											</td>
											...
											<td data-label="Status">
												<span
													className={`${styles.badge} ${styles[b.statusTone]}`}
												>
													{b.status}
												</span>
											</td>
					...
					<div className={styles.card}>
						...
						>
							<div>
								...
								<p className={styles.sectionSub}>
									Support end-users, execute forced actions, track card
									deliveries, and manage batch requests.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
									...
									onClick={() => openModal("cardholderSearchModal")}
								>
									<i className="bi bi-search"></i> Search User
								</button>
								<button
									className={`${styles.btnPm} ${styles.btnSm} ${styles.btnPmP}`}
									onClick={() => openModal("issueBatchModal")}
								>
									<i className="bi bi-upload"></i> Issue Batch
								</button>
							</div>
						</div>
						<div className="row g-3">
								...
								<div className={styles.utilityBlock}>
									<div className="d-flex justify-content-between align-items-center mb-2">
										<h4 style={{ fontSize: 14, fontWeight: 700, margin: 0 }}>
											Recent Cardholder Support Actions
										</h4>
									</div>
									{config.supportActions.map((s) => (
										<div key={s.name} className={styles.statusRow}>
											...
											<button
												className={`${styles.btnPm} ${styles.btnSm} ${s.btnTone === "badgeD" ? styles.btnPmD : styles.btnPmP}`}
												onClick={() => openModal(s.modal)}
											>
												{s.btnLabel}
											</button>
					...
					{/* Compliance, Limits & Fee Configuration */}
					<div className={styles.card}>
						<div
							className="d-flex justify-content-between align-items-center mb-3 flex-wrap"
							style={{ gap: 8 }}
						>
							<div>
								...
								<p className={styles.sectionSub}>
									Define global velocity limits, construct fee schedules, and
									apply fraud and AML constraints.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
									className={styles.btnPm + " " + styles.btnSm}
									onClick={() => openModal("geoFencingModal")}
								>
									<i className="bi bi-globe"></i> Geo-Rules
								</button>
							</div>
						</div>
						<div className="row g-3">
							<div className="col-lg-4">
								<div
									className={styles.utilityBlock + " text-center"}
									style={{ cursor: "pointer" }}
									onClick={() => openModal("velocityLimitsModal")}
								>
									<div
									...
									>
										Set daily, weekly, monthly transaction caps and frequency
							...
							<div className="col-lg-4">
								<div
									className={styles.utilityBlock + " text-center"}
									style={{ cursor: "pointer" }}
									onClick={() => openModal("feeScheduleModal")}
								>
									<div
										className={styles.iconCircle + " mx-auto mb-2"}
										style={{
									...
									>
										Configure issuance, ATM, cross-border, and maintenance fees
										per BIN.
									</p>
								</div>
							</div>
							<div className="col-lg-4">
								<div
									className={styles.utilityBlock + " text-center"}
									style={{ cursor: "pointer" }}
									onClick={() => openModal("mccBlocklistModal")}
								>
									<div
										className={styles.iconCircle + " mx-auto mb-2"}
										style={{
									...
									>
										Block restricted categories (e.g. gambling, crypto) and
										setup 3DS enforcement.
									</p>
								</div>
							</div>
						</div>
			...
			{/* MODALS */}
			<CardProgramAdministrationModals
				active={activeModal}
				onClose={closeModal}
				onOpen={openModal}
			/>
