...
interface StatCard {
	...
	rawCard?: { bg: DebitCardBg; number: string; name: string };
}
...
//		"Order, activate, control limits, set geo-fencing, manage PINs, and handle replacements for physical debit cards.",
	...
	statCards: [
		...
		},
		{
			...
			lines: [
				"Geo-fencing: Active (Kenya Only)",
				"Contactless: Capped at KES 5k",
			],
			warnBorder: true,
		},
	],
	attention: [
		{
			...
			iconText: "ND",
			title: "Premium Card Dispatched",
			sub: "Arriving tomorrow via Fargo Courier",
			actionLabel: "Track",
			actionClass: "btnPmP",
		...
		},
		{
			...
			iconText: "PA",
			title: "Business Card Needs PIN",
			sub: "Card **** 1102 activated, set PIN to use",
			actionLabel: "Set PIN",
		...
		},
		{
			...
			iconText: "TX",
			title: "Declined POS Transaction",
			sub: "Naivas Supermarket · Exceeded daily tap limit",
			actionLabel: "Fix",
	...
	portfolio: [
		...
		},
		{
			...
			sub: "Needs PIN · Linked to Biz Wallet",
		...
		},
		{
			...
			sub: "Frozen · Reported compromised",
			subTone: "text-danger",
			actionLabel: "Replace",
	...
	replacements: [
		{
			...
			impact: "Blocks new authorizations instantly. Can be unfrozen anytime.",
		...
		},
		{
			...
			impact: "Permanent block. Reissues a new PAN. Stops fraud.",
...
async function fetchPhysicalDebitCards(): Promise<PhysicalDebitCardsConfig> {
	...
	try {
		const res = await fetch("/api/physical-debit-cards", {
		...
		<div className={styles.physicalDebitCards}>
			<div className={styles.main}>
				...
				<div className={styles.pageBar}>
					...
					<div className="d-flex flex-wrap" style={{ gap: 8 }}>
						<button
							...
							onClick={() => setActiveModal("cardDeliveryModal")}
						>
							<i className="bi bi-truck" /> Track Delivery
						</button>
						<button
							className={styles.btnPm}
							onClick={() => setActiveModal("activateCardModal")}
						>
							<i className="bi bi-check-circle" /> Activate Card
						</button>
						<button
							className={styles.btnPm}
							onClick={() => setActiveModal("freezeCardModal")}
						>
							<i className="bi bi-snow" /> Freeze All
						</button>
						<button
							className={`${styles.btnPm} ${styles.btnPmP}`}
							onClick={() => setActiveModal("orderCardModal")}
						>
							<i className="bi bi-plus-lg" /> Order New Card
						</button>
					</div>
				</div>
				...
				<div className={styles.content}>
					...
					<div className="row g-3">
						{(config.statCards ?? []).map((s) => (
							<div key={s.key} className={s.col}>
								<div
									className={`${styles.card} ${s.key === "primary" ? styles.cardAccent : ""}`}
									style={{
										minHeight: 170,
										...(s.warnBorder
								...
								>
									{s.key === "primary" ? (
											...
											<div className="d-flex justify-content-between">
												...
												{s.rawCard && (
													<div className="text-end">
														<div
															className={`${styles.debitCard} ${styles[s.rawCard.bg]}`}
															style={{
																width: 160,
																padding: 12,
																borderRadius: 8,
														...
														>
															...
															<div
																...
																style={{ fontSize: 10, margin: "8px 0 4px" }}
															>
																**** **** **** {s.rawCard.number}
															</div>
															<div
															...
															<div
																className={styles.cardVisa}
																style={{ fontSize: 12, bottom: 10, right: 10 }}
															>
																VISA
															</div>
														</div>
										...
										<>
											...
											{s.badge && (
												<span
													className={`${styles.badge} ${styles[s.badge.tone]}`}
												>
											...
											{s.key === "limit" && (
												<button
													className={`${styles.btnPm} ${styles.btnSm} mt-3 w-100`}
													onClick={() => setActiveModal("cardLimitsModal")}
												>
													Adjust Limits
												</button>
											...
											{s.key === "security" && (
												<button
													...
													onClick={() => setActiveModal("cardHealthModal")}
												>
													Security Audit
												</button>
											)}
						...
						<div className="col-lg-4">
							<div className={styles.card}>
								<div className="d-flex justify-content-between align-items-center mb-3">
									...
									<button
										className={`${styles.btnPm} ${styles.btnSm}`}
										onClick={() => setActiveModal("portfolioModal")}
									>
										Manage
									</button>
								</div>
								{(config.portfolio ?? []).map((item) => (
									<div key={item.number} className={styles.feedItem}>
										<div
											className={`${styles.debitCard} ${styles[item.bg]} ${styles.debitCardMini} ${item.subTone === "text-danger" ? styles.cardFrozenFilter : ""}`}
										>
										...
										<div style={{ flex: 1 }}>
											...
											<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
												<span className={item.subTone}>{item.sub}</span>
											</div>
										</div>
										<button
											className={`${styles.btnPm} ${styles.btnSm}`}
							...
							<div className={styles.card}>
								<div className="mb-3">
									...
									<p className={styles.sectionSub}>
										Section 5.2.2 & 5.2.3 operations
									</p>
								</div>
					...
					{/* Card Issuance, Ordering & Tracking */}
					<div className={styles.card}>
						<div
						...
						>
							<div>
								...
								<p className={styles.sectionSub}>
									Order new personal or corporate debit cards, customize
									designs, and track courier delivery in real-time.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
									className={`${styles.btnPm} ${styles.btnSm}`}
									onClick={() => setActiveModal("bulkOrderModal")}
								>
									<i className="bi bi-layers" /> Bulk Order (Corporate)
								</button>
								<button
							...
							<div className="col-lg-7">
								<div className={styles.utilityBlock}>
									...
									>
										<div className="d-flex justify-content-between align-items-center mb-2">
											<div>
												<span style={{ fontWeight: 600, fontSize: 14 }}>
													{config.issuance.delivery.title}
												</span>{" "}
												<span
													className={`${styles.badge} ${styles.badgeW} ms-2`}
												>
													{config.issuance.delivery.badge}
												</span>
											</div>
											<button
												className={`${styles.btnPm} ${styles.btnSm}`}
										...
										<div
											className={`${styles.stepper} mb-2 mt-3`}
										...
										<div style={{ fontSize: 12, color: "var(--pm-ink-soft)" }}>
											<i className="bi bi-truck me-1" /> Fargo Courier (
											{config.issuance.delivery.tracker}). Expected delivery:{" "}
											<strong>Tomorrow, 2:00 PM - 5:00 PM</strong> at Kilimani
											Office.
										</div>
									</div>
								</div>
								...
								<div className={styles.utilityBlock}>
									...
									<div className="d-flex flex-wrap gap-2">
										{(config.issuance.designs ?? []).map((d) => (
											...
											>
												<div
													className={`${styles.debitCard} ${styles[d.bg]} mb-2`}
					...
					{/* 5.2.2 & Physical Card Controls & PIN Management */}
					<div className={styles.card}>
						<div
							className="d-flex justify-content-between align-items-center mb-3 flex-wrap"
							style={{ gap: 8 }}
						>
							<div>
								...
								<p className={styles.sectionSub}>
									Configure daily limits, geographic scopes, MCC blocking,
									contactless caps, and manage card PINs.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
									className={`${styles.btnPm} ${styles.btnSm}`}
									onClick={() => setActiveModal("merchantControlsModal")}
								>
									<i className="bi bi-shop" /> Merchant Blocks
								</button>
								<button
									className={`${styles.btnPm} ${styles.btnSm}`}
									onClick={() => setActiveModal("cardLimitsModal")}
								>
									<i className="bi bi-speedometer2" /> Setup Limits
								</button>
							</div>
						</div>
						<div className="row g-3">
										...
										<div key={t.label} className={styles.statusRow}>
											...
											<div className="form-check form-switch">
												<input
													...
													type="checkbox"
													defaultChecked={t.on}
													onChange={() => setActiveModal("toggleConfirmModal")}
												/>
											</div>
										</div>
									))}
								...
								<div className={styles.utilityBlock}>
									...
									<div className="mb-3">
										<div
											...
											style={{ fontSize: 12 }}
										>
											<span>Contactless Cap</span>
											<span>Max KES 5,000/tap</span>
										</div>
									</div>
									<button
					...
					{/* Card Replacement, Freeze & Cancellation */}
					<div className={styles.card}>
						<div
							className="d-flex justify-content-between align-items-center mb-3 flex-wrap"
							style={{ gap: 8 }}
						>
							<div>
								...
								<p className={styles.sectionSub}>
									Handle lost/stolen cards, temporary freezes, damaged card
									replacements, and permanent account closures securely.
								</p>
							</div>
						</div>
									...
									{(config.replacements ?? []).map((r) => (
										<tr key={r.type}>
											...
											<td data-label="Action">
												<button
													className={`${styles.btnPm} ${styles.btnSm} ${r.actionClass ? styles[r.actionClass] : ""}`}
													style={
