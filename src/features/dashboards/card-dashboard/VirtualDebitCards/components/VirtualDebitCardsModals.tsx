...
export default function VirtualDebitCardsModals({
	...
	const renderCreateCard = () => (
			...
			footer={
				<button
					...
					onClick={() =>
						doAction(
							"createCardModal",
							"Virtual Card Created! New card is ready for immediate use!",
		...
		>
			<p
				style={{ fontSize: 13, color: "var(--pm-ink-soft)", marginBottom: 16 }}
			>
				Choose your card type, add a nickname, and customize security settings
				before issuing.
			</p>
			<div className="mb-3">
					...
					<div className="col-md-4">
						<div
							...
							style={{
								cursor: "pointer",
								borderColor:
									selectedCardType === "Global Shopping"
										? "var(--pm-primary)"
										: "var(--pm-border)",
								background:
									selectedCardType === "Global Shopping"
							...
							onClick={() => selectBox("Global Shopping")}
						>
							<i
								className="bi bi-globe d-block mb-2"
					...
					<div className="col-md-4">
						<div
							...
							style={{
								cursor: "pointer",
								borderColor:
									selectedCardType === "Subscription Manager"
										? "var(--pm-primary)"
										: "var(--pm-border)",
								background:
									selectedCardType === "Subscription Manager"
							...
							onClick={() => selectBox("Subscription Manager")}
						>
							<i
					...
					<div className="col-md-4">
						<div
							...
							style={{
								cursor: "pointer",
								borderColor:
									selectedCardType === "Single-Use Burner"
										? "var(--pm-primary)"
										: "var(--pm-border)",
								background:
									selectedCardType === "Single-Use Burner"
							...
							onClick={() => selectBox("Single-Use Burner")}
						>
							<i
								className="bi bi-lightning-charge d-block mb-2"
	...
	const renderViewCardDetails = () => (
		...
		>
			...
			<div
				...
				style={{ maxWidth: 340, margin: "0 auto 20px" }}
			>
				...
				<div className={styles.cwRow}>
					<div>
						<div className={styles.cwLabel}>Cardholder</div>JAMES KAMAU
					</div>
					<div>
						<div className={styles.cwLabel}>Expires</div>12/28
					</div>
					<div>
						<div className={styles.cwLabel}>CVV</div>882
					</div>
				</div>
			</div>
			<p
				style={{ fontSize: 12, color: "var(--pm-danger)", textAlign: "center" }}
			>
	...
	const renderSingleUseCard = () => (
		...
		>
			...
			>
				<i className="bi bi-exclamation-triangle me-1"></i> Single-use cards
				automatically destroy themselves after one approved transaction. Perfect
				for sketchy websites or free trials.
			</div>
			<div className="mb-3">
				<label className={styles.formLabel}>Exact Purchase Amount (KES)</label>
				<input type="number" className={styles.formControl} value="1500" />
			</div>
			<div className="mb-3">
	...
	const renderTopUpCard = () => (
		...
		>
			...
			>
				<i className="bi bi-exclamation-triangle me-1"></i> AWS is trying to
			...
			<div className="mb-3">
				...
				<div className="pm-amount-chips mb-2">
					<span className="pm-amount-chip" onClick={() => pickChip("1000")}>
						1,000
					</span>
					<span className="pm-amount-chip" onClick={() => pickChip("3000")}>
						3,000
					</span>
					<span
						className="pm-amount-chip active"
						onClick={() => pickChip("5000")}
					>
						5,000
					</span>
					<span className="pm-amount-chip" onClick={() => pickChip("10000")}>
						10,000
					</span>
				</div>
				<input
	...
	const renderFreezeCard = () => (
		<MBox
			...
			footer={
				<button
					className={`${styles.btnPm} ${styles.btnPmI}`}
					onClick={() =>
						doAction(
							"freezeCardModal",
		...
		>
			...
			>
				Freezing is temporary. All new transactions will be declined. Existing
				pending authorizations may still clear. You can unfreeze at any time.
			</div>
		</MBox>
	);
	...
	const renderDeleteCard = () => (
		...
		>
			<div className="text-center" style={{ marginBottom: 20 }}>
				<div
					className="pm-icon-circle mx-auto mb-3"
					style={{
						width: 64,
						height: 64,
						fontSize: 24,
				...
				<div className="mt-3 text-start">
					<label className="pm-form-label">Type "TERMINATE" to confirm</label>
					<input
						type="text"
	...
	const renderEditLimits = () => (
		...
		>
			...
			<div className="mb-3">
				<label className={styles.formLabel}>Monthly Spending Cap (KES)</label>
	...
	const renderMerchantLock = () => (
		<MBox
			...
			title={
				<>
					<i className="bi bi-lock text-primary me-2" />
					Merchant Lock
				</>
			}
			footer={
		...
		>
			...
			<div
				...
				style={{ background: "var(--pm-warning-soft)", fontSize: 12 }}
			>
				If the merchant uses multiple payment gateways with different billing
				names, locking may cause legitimate renewals to fail.
			</div>
		</MBox>
	);
	...
	const renderRotateCvv = () => (
		<MBox
			...
			title={
				<>
					<i className="bi bi-arrow-clockwise text-primary me-2" />
					Dynamic CVV Setup
				</>
			}
			footer={
	...
	const renderDisputeTx = () => (
				...
				<>
					...
					<button
						...
						onClick={() =>
							doAction(
								"disputeTxModal",
								"Transaction dispute filed successfully!",
		...
		>
			<div className="mb-3">
				...
				<select className={styles.formControl}>
					<option>
						AWS Web Services — KES 4,500 — Declined (Insufficient Funds)
					</option>
	...
	const renderDeleteSubscription = () => (
			...
			footer={
				<button
					...
					onClick={() =>
						doAction(
							"deleteSubModal",
							"Subscription cancelled successfully! Please check for refunds.",
		...
		>
			<div
				...
				style={{ background: "var(--pm-warning-soft)", fontSize: 12 }}
			>
				<i className="bi bi-exclamation-triangle me-1"></i> Cancelling this
	...
	const renderAddSubscription = () => (
		<MBox
			...
			title={
				<>
					<i className="bi bi-plus-lg text-success me-2" />
					Add Subscription
				</>
			}
			footer={
				<button
					...
					onClick={() =>
						doAction(
							...
							"Subscription added successfully! Your card will be charged on next renewal.",
							"SUBS-" + Math.random().toString(36).slice(2, 8).toUpperCase(),
						)
					}
				>
					Add Subscription
		...
		>
			<div className="mb-3">
				<label className={styles.formLabel}>Subscription Name</label>
				<input
					type="text"
					className={styles.formControl}
			...
			<div className="mb-3">
				<label className={styles.formLabel}>Expected Cost</label>
	...
	const renderManageSingleSub = () => (
		<MBox
			...
			title={
				<>
					<i className="bi bi-gear text-info me-2" />
					Manage Subscription
				</>
			}
			footer={
		...
		>
			...
			<div className="mb-3">
				<label className={styles.formLabel}>Auto-renew</label>
				<div className="form-check form-switch">
					<input className="form-check-input" type="checkbox" defaultChecked />
