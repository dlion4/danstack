...
interface CardItem {
	...
	logo: string;
	frozen?: boolean;
	actions: { label: string; icon: string; modal: string; btnClass?: string }[];
}
interface FeedItem {
	id: string;
	icon: string;
...
//		"Full visibility and control over all active cards, spending analytics, budget limits, security, and transactions.",
	quickActions: [
		...
		},
		{
			...
			label: "Set Limit",
			modal: "tempLimitModal",
		},
		{
			icon: "bi-wifi",
			iconColor: "var(--pm-primary)",
			label: "Contactless",
			modal: "contactlessLimitModal",
		},
		{
			icon: "bi-globe",
			iconColor: "var(--pm-purple)",
			label: "Online Txn",
			modal: "onlineTransactionsModal",
		},
		{
			icon: "bi-credit-card-2-front",
			iconColor: "var(--pm-ink-soft)",
			label: "Pay Bill",
			modal: "payCardBillModal",
		},
		{
			icon: "bi-arrow-left-right",
			iconColor: "var(--pm-ink-soft)",
			label: "Transfer",
			modal: "transferBetweenCardsModal",
		},
	],
	statCards: [
		{
			...
			badge: {
				icon: "bi-credit-card",
				text: "2 Virtual / 1 Physical / 1 Credit",
				tone: "badgeI",
			},
			lines: [],
		},
		{
			...
			labelColor: "var(--pm-primary)",
			value: "KES 145,000",
			badge: { icon: "bi-wallet2", text: "Limit: KES 250,000", tone: "badgeI" },
			lines: ["Used: KES 105,000"],
		},
		{
			key: "prepaid",
			col: "col-lg-3 col-md-6",
			...
			value: "KES 32,450",
			badge: { icon: "bi-arrow-up", text: "Top-up Ready", tone: "badgeS" },
			lines: ["Across 2 prepaid virtual cards"],
		},
		{
			key: "spend",
			col: "col-lg-3 col-md-6",
	...
	cards: [
		...
		},
		{
			...
			name: "JAMES KAMAU",
			expiry: "09/26",
			balance: "Linked to Main Acct",
			gradient: "bankCardBg3",
			logo: "VISA",
			frozen: true,
			actions: [
				{
	...
	attention: [
		...
		},
		{
			...
			actionLabel: "Update",
			actionModal: "manageBeneficiariesModal",
		},
		{
			id: "at3",
			icon: "bi-shield-exclamation",
			...
			actionLabel: "Approve",
			actionModal: "initiateTransferModal",
		},
	],
	portfolio: [
		{
			id: "p1",
			type: "Personal Standard Debit",
			number: "8422",
			name: "JAMES KAMAU",
			expiry: "",
			balance: "KES 145.2k",
			...
			frozen: false,
			actions: [
				{ label: "View", icon: "bi-eye", modal: "viewCardDetailsModal" },
			],
		},
		{
			id: "p2",
			type: "SME Business Debit",
			number: "1102",
			name: "JAMES KAMAU",
			expiry: "",
			balance: "Linked to Biz Wallet",
			gradient: "bankCardBg4",
			logo: "PayMo Biz",
			frozen: false,
			actions: [
				{ label: "PIN", icon: "bi-asterisk", modal: "pinManagementModal" },
			],
		},
		{
			id: "p3",
			type: "Premium Travel Debit",
			number: "5591",
			name: "JAMES KAMAU",
			expiry: "",
			balance: "In transit",
			...
			frozen: false,
			actions: [
				{ label: "Track", icon: "bi-truck", modal: "cardDeliveryModal" },
			],
		},
		{
			id: "p4",
			type: "Legacy Debit Card",
			number: "9421",
			name: "JAMES KAMAU",
			expiry: "",
			balance: "Frozen",
		...
		items: [
			{
				...
				sub: "Card ****3105 expiring in 72 days",
				actionLabel: "Review",
				actionModal: "autoRenewalModal",
			},
			{
				title: "3D Secure Setup",
				sub: "All active cards enrolled",
				icon: "bi-check-circle-fill",
			},
			{
				title: "International Txn",
				sub: "Allowed on Virtual Debit ****8810",
				actionLabel: "Manage",
				actionModal: "internationalTransactionsModal",
			},
		],
	},
	analytics: {
		...
		budgets: [
			...
			{
				label: "Online Purchases (Virtual)",
				used: "KES 42,000",
				total: "KES 50,000",
				percent: 84,
	...
	transactions: [
		{
			...
			category: "Groceries",
			amount: "KES 12,450.00",
			status: "Approved",
			statusTone: "badgeS",
			actionModal: "transactionDetailsModal",
		},
		{
			date: "Today, 09:15",
			card: "****8810",
...
async function fetchCardCommandCenter(): Promise<CardCommandCenterConfig> {
	...
	try {
		const res = await fetch("/api/card-command-center", {
			...
			<div className={styles.main}>
				...
				<div className={styles.pageBar}>
					...
					<div className="d-flex flex-wrap" style={{ gap: 8 }}>
						<button
							...
							onClick={() => setActiveModal("securityScoreModal")}
						>
				...
				<div className={styles.content}>
					...
					<div className="row g-3">
						{config.statCards.map((s) => (
							<div key={s.key} className={s.col}>
								...
								>
									<p className={styles.sl} style={{ color: s.labelColor }}>
										{s.label}
									</p>
							...
							<div className={styles.card} style={{ height: "100%" }}>
								<div className="d-flex justify-content-between align-items-center mb-3">
									...
									<button
										...
										onClick={() => setActiveModal("addCardModal")}
									>
										Add Card
									</button>
								</div>
								<div className="row g-3">
									{config.cards.map((c) => (
										<div key={c.id} className="col-md-4">
											<div
												className={`${styles.bankCard} ${styles[c.gradient]} ${c.frozen ? styles.bankCardFrozen : ""}`}
												onClick={() => setActiveModal("cardDetailsModal")}
											>
												{c.frozen && (
													<div className={styles.frozenOverlay}>
														<i className="bi bi-snow me-2"></i>FROZEN
													</div>
												)}
												...
												<div className={styles.bankCardNumber}>
													...
													<span>****</span>
													<span>****</span>
													<span>{c.number.replace("**** ", "")}</span>
												</div>
												<div className={styles.bankCardBottom}>
													<div>
														<div className={styles.bankCardName}>{c.name}</div>
											...
											<div className="d-flex justify-content-between mt-2 px-1">
												{c.actions.map((a) => (
													<button
														key={a.label}
														className={`${styles.btnPm} ${styles.btnSm} ${a.btnClass || ""}`}
					...
					<div className="row g-3">
						<div className="col-lg-4">
							<div className={styles.card} style={{ height: "100%" }}>
								<div className="d-flex justify-content-between align-items-center mb-3">
									<h3 className={styles.st}>
										<i className="bi bi-shield-check text-success" /> Health &
										Security
									</h3>
									<button
										className={`${styles.btnPm} ${styles.btnSm}`}
						...
						<div className="col-lg-8">
							<div className={styles.card} style={{ height: "100%" }}>
								<div className="d-flex justify-content-between align-items-center mb-3">
									<h3 className={styles.st}>
										<i className="bi bi-pie-chart text-warning" /> Spending
										Analytics & Budgets
									</h3>
									<div className="d-flex gap-2">
										<button
											className={`${styles.btnPm} ${styles.btnSm}`}
											onClick={() => setActiveModal("budgetSettingsModal")}
										>
											Budgets
										</button>
										<button
										...
										<div className={styles.chartBars}>
											{config.analytics.bars.map((b) => (
												<div
													...
													onClick={() =>
														setActiveModal("merchantAnalyticsModal")
													}
												>
					...
					<div className={styles.card}>
						<div className="d-flex justify-content-between align-items-center mb-3">
							<h3 className={styles.st}>
								<i className="bi bi-list-ul text-muted" /> Recent Card Activity
							</h3>
							<button
								className={`${styles.btnPm} ${styles.btnSm}`}
								onClick={() => setActiveModal("configureAlertsModal")}
							>
								<i className="bi bi-bell" /> Alerts Setup
							</button>
						</div>
									...
									{config.transactions.map((t) => (
										<tr key={t.date + t.merchant}>
											...
											<td data-label="Card">
												...
												>
													<i className={`bi ${t.cardIcon} me-1`}></i> {t.card}
												</span>
											</td>
