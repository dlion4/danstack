...
//		"Manage employee expense cards, setup policies, enforce approvals, automate reconciliation, and control corporate billing.",
	hero: {
		...
		activeSub:
			"38 employee cards, 4 department cards linked to the centralized Acme Corp billing account.",
		actions: [
	...
	stats: {
		pending: {
			value: "KES 485K",
			badge: "8 transactions",
			modal: "approvalQueueModal",
		},
		spend: {
			value: "KES 2.4M",
			badge: "12% under budget",
			progressWidth: "86%",
		},
		missing: {
			value: "14 items",
			badge: "KES 312,400 unverified",
			oldest: "12 days ago",
			modal: "missingReceiptsModal",
		},
	},
	attention: [
		{
			...
			sub: "S. Kamau · AWS EMEA · KES 145,000",
			actionLabel: "Review",
			actionClass: "btnPmP",
		...
		},
		{
			...
			sub: "P. Ochieng · Emirates Air · KES 85,200",
			actionLabel: "Chase",
			modal: "missingReceiptsModal",
		},
		{
			...
			sub: "J. Njoroge · Weekend spend (blocked)",
			actionLabel: "Inspect",
			modal: "violationDetailsModal",
		},
		{
			initials: "BL",
			...
			sub: "KES 490K / 500K used · Reset in 4 days",
			actionLabel: "Top-up",
			modal: "editLimitModal",
		},
	],
	recommendations: [
		{
			...
			sub: "Save 4 hours/month on SaaS approvals",
		...
		},
		{
			...
			sub: "4 employees using personal cards for AWS",
		...
		},
		{
			...
			sub: "Uber & Bolt used interchangeably",
			actionLabel: "Report",
			modal: "reportsModal",
		},
		{
			...
			sub: "3 issued cards not used in 90+ days",
			actionLabel: "Review",
			modal: "cardRosterModal",
		},
	],
	quickActions: [
		{
	...
	recentlyIssued: [
		{
			...
			statusTone: "badgeS",
			actionLabel: "Manage",
			modal: "manageEmployeeCardModal",
		},
		{
			initials: "M",
			avatarBg: "var(--pm-danger)",
			name: "Marketing Dept",
			dept: "Shared Procurement",
			type: "Virtual",
			limit: "500,000 /mo",
			status: "Active",
			statusTone: "badgeS",
	...
	programConfig: [
		{
			...
			sub: "Corporate liability, centralized",
			actionLabel: "Edit",
			modal: "billingSetupModal",
		},
		{
			label: "Card Branding",
			sub: "Acme Logo, Blue background",
			actionLabel: "Edit",
			modal: "brandingModal",
		},
		{
			label: "Issuance Scope",
			sub: "Domestic & Regional allowed",
			actionLabel: "Edit",
			modal: "policyRulesModal",
		},
	],
	policyGroups: [
		{
			...
			desc: "KES 500K limit, T&E allowed, No auto-blocks",
			members: "6 employees",
		},
		{
			name: "Standard Employee",
			desc: "KES 50K limit, strict MCC blocks, weekend block",
			members: "24 employees",
			active: true,
		},
		{
			name: "Department Head",
			desc: "KES 200K limit, Software & Procure allowed",
			members: "8 employees",
		},
	],
	approvalQueue: [
		{
			...
			merchant: "AWS EMEA",
			amount: "KES 145,000",
			flag: "Exceeds KES 100K auto-limit",
			flagTone: "badgeW",
		},
		{
			initials: "JW",
	...
	transactions: [
		{
			...
			receipt: "Matched",
			receiptTone: "badgeS",
			receiptIcon: "bi-check",
			actionLabel: "View",
			modal: "expenseDetailModal",
		},
		{
			date: "27 Jun",
			employee: "P. Ochieng",
...
async function fetchCorporateBusinessCards(): Promise<CorporateBusinessCardsConfig> {
	...
	try {
		const res = await fetch("/api/corporate-business-cards", {
	...
	return (
		<div className={styles.corporateBusinessCards}>
			...
			<div className={styles.pageBar}>
				...
				<div className="d-flex flex-wrap" style={{ gap: 8 }}>
					<button
						...
						onClick={() => setActiveModal("policyRulesModal")}
					>
						<i className="bi bi-shield-lock" /> Policies
					</button>
					<button
						className={styles.btnPm}
						onClick={() => setActiveModal("reconciliationModal")}
					>
						<i className="bi bi-journal-check" /> Recon
					</button>
					<button
						className={`${styles.btnPm} ${styles.btnPmP}`}
						onClick={() => setActiveModal("issueCardModal")}
					>
			...
			<div className={styles.content}>
				...
				<div className="row g-3">
					<div className="col-lg-4">
						...
						>
							...
							>
								{config.hero.activeSub}
							</p>
							<div className="d-flex flex-wrap mt-3" style={{ gap: 8 }}>
								{config.hero.actions.map((a) => (
									<button
										key={a.label}
										className={`${styles.btnPm} ${styles.btnSm} ${styles.btnGhostLight}`}
					...
					<div className="col-lg-2 col-md-4 col-6">
						<div className={styles.card} style={{ minHeight: 170 }}>
							...
							<div className={styles.statValue} style={{ margin: "6px 0" }}>
								{config.stats.pending.value}
							</div>
							<span className={`${styles.badge} ${styles.badgeW}`}>
								<i className="bi bi-clock" /> {config.stats.pending.badge}
							</span>
							<div className="mt-3">
								<button
									className={`${styles.btnPm} ${styles.btnSm} w-100`}
									onClick={() => setActiveModal(config.stats.pending.modal)}
								>
									Review Queue
								</button>
							</div>
					...
					<div className="col-lg-3 col-md-4 col-6">
						<div className={styles.card} style={{ minHeight: 170 }}>
							...
							<div className="mt-2">
								...
								>
									<span>Company budget limit</span>
									<span>86%</span>
								</div>
								<div className={`${styles.progress} mt-1`}>
									<div
										...
										style={{
											width: config.stats.spend.progressWidth,
					...
					<div className="col-lg-3 col-md-4">
						...
						>
							...
							<div className={styles.statValue} style={{ margin: "6px 0" }}>
								{config.stats.missing.value}
							</div>
							<span className={`${styles.badge} ${styles.badgeD}`}>
								<i className="bi bi-exclamation-triangle" />{" "}
								{config.stats.missing.badge}
							</span>
							<div
								className="mt-2"
							...
							>
								<div>
									Oldest missing: <strong>{config.stats.missing.oldest}</strong>
								</div>
								<div className="mt-1">
									<button
										className={`${styles.btnPm} ${styles.btnSm} ${styles.btnPmDSoft}`}
										onClick={() => setActiveModal(config.stats.missing.modal)}
									>
										Chase Employees
									</button>
								</div>
						...
						<div className={styles.card}>
							<div className="d-flex justify-content-between align-items-center mb-3">
								...
								<button
									...
									onClick={() => setActiveModal("attentionCenterModal")}
								>
									View all
								</button>
							</div>
							{config.attention.map((item) => (
								<div key={item.title} className={styles.feedItem}>
									...
									<button
										className={`${styles.btnPm} ${styles.btnSm} ${item.actionClass ? styles[item.actionClass] : ""}`}
				...
				<div className={styles.card}>
					...
					>
						<div>
							...
							<p className={styles.sectionSub}>
								Manage program configuration, bulk employee issuance, department
								cards, and shared project virtual cards.
							</p>
						</div>
						<div className="d-flex" style={{ gap: 8 }}>
							<button
								className={`${styles.btnPm} ${styles.btnSm}`}
								onClick={() => setActiveModal("cardRosterModal")}
							>
								<i className="bi bi-list-ul" /> Full Directory
							</button>
							<button
					...
					<div className="row g-3">
						<div className="col-lg-7">
							<div className={styles.utilityBlock}>
								<h4
									style={{ fontSize: 14, fontWeight: 700, margin: "0 0 12px" }}
								>
							...
							<div className={styles.utilityBlock}>
								...
								{config.programConfig.map((p) => (
									<div key={p.label} className={styles.statusRow}>
										...
										<button
											className={`${styles.btnPm} ${styles.btnSm}`}
											onClick={() => setActiveModal(p.modal)}
										>
											{p.actionLabel}
										</button>
									</div>
								...
								>
									...
									>
										<i className="bi bi-building" /> ACME CORP PROGRAM
									</div>
									<div
										style={{
											fontSize: 12,
				...
				{/* Corporate Controls, Policies & Approvals */}
				<div className={styles.card}>
					<div
						className="d-flex justify-content-between align-items-center mb-3 flex-wrap"
						style={{ gap: 8 }}
					>
						<div>
							...
							<p className={styles.sectionSub}>
								Define strict spending policies, configure multi-tier approval
								workflows, and monitor real-time policy compliance.
							</p>
						</div>
						<div className="d-flex" style={{ gap: 8 }}>
							<button
								...
								onClick={() => setActiveModal("policyRulesModal")}
							>
								<i className="bi bi-sliders" /> Edit Global Rules
							</button>
						</div>
					</div>
					<div className="row g-3">
						<div className="col-lg-4">
							<div className={styles.utilityBlock}>
								<div className="d-flex justify-content-between align-items-center mb-3">
									...
									<button
										className={`${styles.btnPm} ${styles.btnSm}`}
										onClick={() => setActiveModal("createPolicyModal")}
									>
										New
									</button>
								</div>
								{config.policyGroups.map((g) => (
									<div
										key={g.name}
										className={`${styles.policyCard} ${g.active ? styles.policyCardActive : ""}`}
										onClick={() => setActiveModal("policyRulesModal")}
									>
										...
										<tbody>
											{config.approvalQueue.map((r) => (
												<tr key={r.employee}>
													...
													<td data-label="Policy Flag">
														<span
															className={`${styles.badge} ${styles[r.flagTone]}`}
														>
															{r.flag}
														</span>
													</td>
													<td data-label="Action">
														<button
															className={`${styles.btnPm} ${styles.btnSm}`}
															onClick={() =>
																setActiveModal("reviewTransactionModal")
															}
														>
															Review
														</button>
				...
				<div className={styles.card}>
					...
					>
						<div>
							...
							<p className={styles.sectionSub}>
								Automate receipt matching, categorize spend by MCC and cost
								centers, and push reconciled data to accounting systems.
							</p>
						</div>
						<div className="d-flex" style={{ gap: 8 }}>
							<button
								...
								onClick={() => setActiveModal("reconciliationModal")}
							>
								<i className="bi bi-journal-arrow-up" /> Export GL
							</button>
							<button
								className={`${styles.btnPm} ${styles.btnSm}`}
								onClick={() => setActiveModal("missingReceiptsModal")}
							>
								<i className="bi bi-search" /> Find Missing
							</button>
						</div>
					</div>
					<div className="row g-3">
						<div className="col-lg-3">
							<div className={styles.utilityBlock}>
								...
								<div className={`${styles.donut} mx-auto mb-3`}>
									<svg viewBox="0 0 36 36" width="100%" height="100%">
										...
										<circle
											...
											strokeWidth="4"
											strokeDasharray={`${config.receiptStatus.pct} 100`}
											strokeLinecap="round"
										...
										<circle
											...
											r="16"
											fill="none"
											stroke="var(--pm-danger)"
											strokeWidth="4"
											strokeDasharray={`${100 - config.receiptStatus.pct} 100`}
											strokeDashoffset={-config.receiptStatus.pct}
											transform="rotate(-90 18 18)"
										/>
									</svg>
									<div className={styles.donutLabel}>
										{config.receiptStatus.pct}%
									</div>
								</div>
								<div
								...
								>
									...
									<strong>{config.receiptStatus.grace}</strong>
								</div>
							</div>
						</div>
										...
										<tbody>
											{config.transactions.map((t, i) => (
												<tr key={`${t.merchant}-${i}`}>
													...
													<td data-label="Category / GL Code">
														<span
															className={`${styles.badge} ${styles.badgeNeutral}`}
														>
															{t.category}
													...
													<td data-label="Receipt">
														<span
															className={`${styles.badge} ${styles[t.receiptTone]}`}
														>
															<i className={`bi ${t.receiptIcon}`} />{" "}
															{t.receipt}
														</span>
													</td>
													<td data-label="Action">
				...
				{/* Corporate Card Billing & Settlement */}
				<div className={styles.card}>
					<div
						className="d-flex justify-content-between align-items-center mb-3 flex-wrap"
						style={{ gap: 8 }}
					>
						<div>
							<h3 className={styles.sectionTitle}>
								<i className="bi bi-bank" style={{ color: "var(--pm-info)" }} />{" "}
								Corporate Card Billing & Settlement
							</h3>
							<p className={styles.sectionSub}>
								Manage consolidated statements, centralized payment accounts,
								and early settlement options.
							</p>
						</div>
						<div className="d-flex" style={{ gap: 8 }}>
							<button
							...
							>
								<i className="bi bi-file-earmark-pdf" /> Current Statement
							</button>
							<button
								className={`${styles.btnPm} ${styles.btnSm} ${styles.btnPmP}`}
								onClick={() => setActiveModal("settlementModal")}
							>
								<i className="bi bi-cash" /> Settle Balance
							</button>
						</div>
					</div>
					<div className="row g-3">
						<div className="col-lg-4">
							<div className={styles.utilityBlock}>
								...
								<div className="mt-3">
									<button
										className={`${styles.btnPm} ${styles.btnSm} w-100`}
										onClick={() => setActiveModal(config.billing.modal)}
									>
										Pay Now Manually
									</button>
								</div>
