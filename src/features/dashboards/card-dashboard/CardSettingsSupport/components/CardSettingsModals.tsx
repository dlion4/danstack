...
export default function CardSettingsModals({
	active,
	onClose,
	onOpen,
	config,
	...
	return (
		<>
			...
			<MBox
				...
				title={
					<>
						<i className="bi bi-key text-primary me-2" />
						Change Card PIN
					</>
				}
				footer={
					<div className="d-flex justify-content-end" style={{ gap: 8 }}>
						...
						{flows.pin < 3 && (
							<button
								...
								onClick={() => nextFlow("pin", 3)}
							>
								Continue <i className="bi bi-arrow-right" />
							</button>
						)}
						{flows.pin === 3 && results.changePinModal && (
							<button
								className={`${styles.btnPm} ${styles.btnPmP}`}
								onClick={onClose}
							>
			...
			>
				...
				<div style={flows.pin !== 1 ? { display: "none" } : {}}>
					...
					<div className="mb-3">
						<label className={styles.formLabel}>Card</label>
						<select className={styles.fc}>
							<option>Visa Debit ****4521</option>
							<option>Mastercard ****3392</option>
							<option>Corporate ****6677</option>
						</select>
					</div>
					<div className="mb-3">
						<label className={styles.formLabel}>Enter Current PIN</label>
				...
				<div style={flows.pin !== 2 ? { display: "none" } : {}}>
					...
					<div className={styles.pinRow} style={{ marginBottom: 12 }}>
						{[0, 1, 2, 3].map((i) => (
							<input
								key={i}
								type="password"
								...
								onInput={(e) => {
									...
									if (t.value.length === 1 && t.nextElementSibling)
										(t.nextElementSibling as HTMLInputElement).focus();
								}}
							/>
						))}
					</div>
					...
					>
						<i className="bi bi-info-circle me-1" /> PIN must not contain
						sequential (1234) or repeated (1111) digits.
					</div>
				</div>
				<div style={flows.pin !== 3 ? { display: "none" } : {}}>
					{results.changePinModal
						? renderReceipt(results.changePinModal)
						: null}
				</div>
			</MBox>
			...
			<MBox
				...
				title={
					<>
						<i className="bi bi-snow2 text-info me-2" />
					...
					<div className="d-flex justify-content-end" style={{ gap: 8 }}>
						...
						<button
							...
							onClick={() =>
								doAction(
									"freezeCardModal",
									"Card status updated successfully. Change is effective immediately.",
									"",
								)
							}
						>
				...
				{renderActionBody(
					...
					<>
						<div className="mb-3">
							...
							<select className={styles.fc}>
								<option>Visa Debit ****4521 — Active</option>
								<option>Mastercard ****3392 — Active</option>
								<option>Prepaid ****8890 — Frozen</option>
								<option>Virtual Credit ****1190 — Active</option>
							</select>
						</div>
						<div className="mb-3">
							<label className={styles.formLabel}>Action</label>
							<select className={styles.fc}>
								<option value="freeze">Freeze (temporarily block)</option>
								<option value="unfreeze">Unfreeze (reactivate)</option>
							</select>
						</div>
						<div className="mb-3">
							...
							<select className={styles.fc}>
								...
								<option>Lost — searching</option>
								<option>Suspected fraud</option>
								<option>Security precaution</option>
						...
						>
							<i className="bi bi-info-circle me-1" /> Frozen cards reject all
							transactions instantly. Unfreeze to resume normal use.
						</div>
					</>,
				)}
			</MBox>
					...
					<div className="d-flex justify-content-end" style={{ gap: 8 }}>
						...
						{flows.lost < 4 && (
							<button
								...
								onClick={() => nextFlow("lost", 4)}
							>
								{flows.lost === 3 ? (
									<>
										Block Card <i className="bi bi-shield-x" />
									</>
								) : (
									<>
										Continue <i className="bi bi-arrow-right" />
						...
						{flows.lost === 4 && results.reportLostModal && (
							<button
								className={`${styles.btnPm} ${styles.btnPmP}`}
								onClick={onClose}
							>
			...
			>
				...
				<div style={flows.lost !== 1 ? { display: "none" } : {}}>
					...
					<div className="mb-3">
						...
						<select className={styles.fc}>
							<option>Visa Debit ****4521</option>
							<option>Mastercard ****3392</option>
							<option>Prepaid Travel ****8890</option>
						</select>
					</div>
					<div className="mb-3">
					...
					>
						...
						<strong>Immediate block:</strong> Reporting a card as lost or stolen
						will permanently block it. This action cannot be undone.
					</div>
				</div>
				<div style={flows.lost !== 2 ? { display: "none" } : {}}>
					...
					<div className="row g-3">
						...
						<div className="col-md-6">
							<label className={styles.formLabel}>Last known location</label>
							<input className={styles.fc} defaultValue="Nairobi CBD" />
						</div>
						<div className="col-12">
							...
							<textarea
								className={styles.fc}
								rows={3}
								defaultValue="Card was in my wallet when I noticed it missing this morning."
							/>
						</div>
						<div className="col-md-6">
							<label className={styles.formLabel}>Need replacement?</label>
							<select className={styles.fc}>
								<option>Yes — express courier (4 hours)</option>
								<option>Yes — standard mail (5–7 days)</option>
								<option>Yes — branch pickup (same day)</option>
								<option>No replacement needed</option>
							</select>
						</div>
						<div className="col-md-6">
				...
				<div style={flows.lost !== 3 ? { display: "none" } : {}}>
					...
					<div className="mb-3">
						<label className={styles.formLabel}>Verify via OTP</label>
						<div className={styles.pinRow}>
							{[0, 1, 2, 3, 4, 5].map((i) => (
								<input
									key={i}
					...
					>
						OTP sent to +254 712***890
					</p>
				</div>
				<div style={flows.lost !== 4 ? { display: "none" } : {}}>
					{results.reportLostModal
						? renderReceipt(results.reportLostModal)
						: null}
				</div>
			</MBox>
					...
					<div className="d-flex justify-content-end" style={{ gap: 8 }}>
						...
						<button
							...
							onClick={() =>
								doAction(
									"cardControlsModal",
									"Card controls updated. Changes take effect immediately.",
									"",
								)
							}
						>
			...
			>
				{renderActionBody(
					...
					<>
						...
						<div
							style={{
								display: (tabs.ctrl ?? "txn") !== "txn" ? "none" : undefined,
							}}
							id="ctrl-txn"
						>
							<div className="mb-3">
							...
							<div className="row g-3">
								<div className="col-md-6">
									...
									<input className={styles.fc} defaultValue="50,000" />
								</div>
								<div className="col-md-6">
									<label className={styles.formLabel}>
										Per-Transaction Limit
									</label>
									<input className={styles.fc} defaultValue="25,000" />
								</div>
								<div className="col-md-6">
									<label className={styles.formLabel}>Contactless Limit</label>
									<input className={styles.fc} defaultValue="5,000" />
								</div>
								<div className="col-md-6">
									<label className={styles.formLabel}>
										ATM Withdrawal Limit
									</label>
									<input className={styles.fc} defaultValue="40,000" />
								</div>
							</div>
							...
							<div className="form-check form-switch mb-2">
								<input
									...
									defaultChecked
								/>{" "}
								<label className="form-check-label">Online transactions</label>
							</div>
							<div className="form-check form-switch mb-2">
								<input
									className="form-check-input"
							...
							<div className="form-check form-switch mb-2">
								<input
									...
									defaultChecked
								/>{" "}
								<label className="form-check-label">ATM withdrawals</label>
							</div>
							<div className="form-check form-switch mb-2">
								<input className="form-check-input" type="checkbox" />{" "}
								<label className="form-check-label">Recurring payments</label>
							</div>
							<div className="form-check form-switch">
								<input
									...
									defaultChecked
								/>{" "}
								<label className="form-check-label">Contactless / NFC</label>
							</div>
						</div>
						<div
							style={{
								display: (tabs.ctrl ?? "txn") !== "geo" ? "none" : undefined,
							}}
							id="ctrl-geo"
						>
							<div className="mb-3">
								<label className={styles.formLabel}>Geographic Mode</label>
								<select className={styles.fc}>
									<option>Global (all countries)</option>
									<option>East Africa only</option>
									<option>Kenya only</option>
									<option>Custom country list</option>
								</select>
							</div>
							<div className="form-check form-switch mb-2">
								<input className="form-check-input" type="checkbox" />{" "}
								<label className="form-check-label">
									Enable geo-fencing (block outside 50km radius)
								</label>
							</div>
							<div className="mb-3">
								<label className={styles.formLabel}>Travel Mode</label>
								<div className="row g-3">
									<div className="col-6">
										<label
							...
							<div className="mb-3">
								...
								<input
									className={styles.fc}
									defaultValue="United Kingdom, Germany, France"
								/>
							</div>
						</div>
						<div
							style={{
								display: (tabs.ctrl ?? "txn") !== "time" ? "none" : undefined,
							}}
							id="ctrl-time"
						>
							<div className="row g-3">
								...
								<div className="col-md-6">
									<label className={styles.formLabel}>Allowed Hours End</label>
									<select className={styles.fc}>
							...
							<div className="form-check form-switch mt-3 mb-2">
								...
								/>{" "}
								<label className="form-check-label">
									Allow weekend transactions
								</label>
							</div>
							<div className="form-check form-switch">
								<input className="form-check-input" type="checkbox" />{" "}
								<label className="form-check-label">
									Allow holiday transactions
								</label>
							</div>
						</div>
						<div
							style={{
								display:
									(tabs.ctrl ?? "txn") !== "merchant" ? "none" : undefined,
							}}
							id="ctrl-merchant"
						>
							<div className="mb-3">
								<label className={styles.formLabel}>
									Block by Category (MCC)
								</label>
							...
							<div className="mb-3">
								<label className={styles.formLabel}>
									Allowlist Specific Merchants
								</label>
								<input
									className={styles.fc}
									placeholder="Only these merchants allowed"
								/>
							</div>
						</div>
					</>,
			...
			>
				{renderActionBody(
					...
					<>
						...
						<div className="mb-3">
							<label className={styles.formLabel}>Default for ATM</label>
							<select className={styles.fc}>
								<option>Visa Debit ****4521</option>
								<option>Mastercard ****3392</option>
							</select>
						...
						<div className="mb-3">
							<label className={styles.formLabel}>
								Default Funding Source for Virtual Cards
							</label>
							<select className={styles.fc}>
						...
						<div className="mb-3">
							<label className={styles.formLabel}>
								Preferred International Currency
							</label>
							<select className={styles.fc}>
			...
			>
				{renderActionBody(
					...
					<>
						...
						<div
							style={{
								display: (tabs.org ?? "names") !== "names" ? "none" : undefined,
							}}
							id="org-names"
						>
							<div className="table-responsive">
								<table className={styles.tbl}>
									...
									<tbody>
										...
										<tr>
											<td>Prepaid ****8890</td>
											<td>Travel Prepaid</td>
											<td>
												<input
													className={styles.fc}
													style={{ padding: "6px 10px" }}
													defaultValue="Travel Fund"
												/>
											</td>
											<td>
												<input
						...
						<div
							style={{
								display:
									(tabs.org ?? "names") !== "groups" ? "none" : undefined,
							}}
							id="org-groups"
						>
							<div className="mb-3">
								<label className={styles.formLabel}>Personal Cards</label>
								<div className="form-check mb-1">
									<input
										className="form-check-input"
										type="checkbox"
										defaultChecked
									/>{" "}
									<label className="form-check-label">Visa ****4521</label>
								</div>
								<div className="form-check mb-1">
									<input
										className="form-check-input"
										...
										defaultChecked
									/>{" "}
									<label className="form-check-label">MC ****3392</label>
								</div>
							</div>
							<div className="mb-3">
								<label className={styles.formLabel}>
							...
							<div className="mb-3">
								<label className={styles.formLabel}>Travel</label>
								<div className="form-check">
									<input
										className="form-check-input"
										type="checkbox"
										defaultChecked
									/>{" "}
									<label className="form-check-label">Prepaid ****8890</label>
								</div>
							</div>
							<div className="mb-3">
								...
								<div className="form-check">
									<input
										...
										defaultChecked
									/>{" "}
									<label className="form-check-label">Corporate ****6677</label>
								</div>
							</div>
						</div>
						<div
							style={{
								display: (tabs.org ?? "names") !== "order" ? "none" : undefined,
							}}
							id="org-order"
						>
							<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
								Drag cards to set your preferred dashboard order:
							</p>
							<div className="p-3 border rounded mb-2 d-flex align-items-center gap-3">
								<i className="bi bi-grip-vertical text-muted" />
								<span className={`${styles.badge} ${styles.badgeP}`}>1</span>{" "}
								Visa ****4521 — Daily Driver
							</div>
							<div className="p-3 border rounded mb-2 d-flex align-items-center gap-3">
								<i className="bi bi-grip-vertical text-muted" />
								<span className={`${styles.badge} ${styles.badgeI}`}>2</span> MC
								****3392 — Tap-Pay Card
							</div>
							<div className="p-3 border rounded mb-2 d-flex align-items-center gap-3">
								<i className="bi bi-grip-vertical text-muted" />
								<span className={`${styles.badge} ${styles.badgeS}`}>3</span>{" "}
								Virtual ****1190 — Online Shopping
							</div>
							<div className="p-3 border rounded mb-2 d-flex align-items-center gap-3">
								<i className="bi bi-grip-vertical text-muted" />
								<span className={`${styles.badge} ${styles.badgeW}`}>4</span>{" "}
								Prepaid ****8890 — Travel Fund
							</div>
						</div>
					</>,
				)}
					...
					<div className="table-responsive">
						<table className={styles.tbl}>
							...
							<tbody>
								...
								<tr>
									<td>Large transactions (&gt;KES 10K)</td>
									<td>
										<input type="checkbox" defaultChecked />
									</td>
									<td>
								...
								<tr>
									<td>Security changes (PIN/freeze)</td>
									<td>
										<input type="checkbox" defaultChecked />
									</td>
									<td>
							...
							<div className="row g-3">
								<div className="col-md-6">
									...
									<select className={styles.fc}>
										...
										<option>Virtual ****1190</option>
									</select>
								</div>
								<div className="col-md-6">
									<label className={styles.formLabel}>Transaction Date</label>
									<input
										type="date"
										className={styles.fc}
								...
								<div className="col-md-6">
									<label className={styles.formLabel}>Merchant</label>
									<input
										className={styles.fc}
								...
								<div className="col-md-6">
									...
									<input className={styles.fc} defaultValue="4,500" />
								</div>
							</div>
						...
						<div style={flows.disp !== 2 ? { display: "none" } : {}}>
							...
							<div className="mb-3">
								...
								<textarea
									className={styles.fc}
									rows={4}
									defaultValue="I did not authorise this transaction. The charge appeared without my knowledge. I did not share card details with anyone."
								/>
							</div>
							<div className="row g-3">
								<div className="col-md-6">
							...
							<div className="mb-3 mt-3">
								...
								<textarea
									className={styles.fc}
									rows={2}
									defaultValue="Attempted to contact merchant on 22 Jun via email. No response received."
								/>
							</div>
						</div>
						<div style={flows.disp !== 3 ? { display: "none" } : {}}>
							{results.disputeModal
								? renderReceipt(results.disputeModal)
								: null}
						</div>
					</>,
				)}
			...
			>
				...
				<div
					style={{
						display: (tabs.faq ?? "general") !== "general" ? "none" : undefined,
					}}
					id="faq-general"
				>
					<div className={styles.sr}>
						<div>
							<strong>How do I activate my new card?</strong>
							<div
								style={{
									fontSize: 12,
							...
							>
								In-app: Go to Card Center → select card → tap Activate. Enter
								last 4 digits and the OTP sent to your phone. Your card is ready
								to use immediately.
							</div>
						</div>
					</div>
					<div className={styles.sr}>
						<div>
							<strong>What is the daily spending limit?</strong>
							<div
								style={{
									fontSize: 12,
									color: "var(--pm-ink-soft)",
							...
							>
								Default limit is KES 50,000/day for debit cards. You can adjust
								this in Card Controls → Transaction tab up to KES 200,000/day.
							</div>
						</div>
					</div>
					<div className={styles.sr}>
						<div>
							<strong>How do I change my card PIN?</strong>
							<div
								style={{
									fontSize: 12,
									color: "var(--pm-ink-soft)",
							...
							>
								Go to Card Settings → Change PIN. Enter your current PIN, then
								set a new 4-digit PIN. Changes take effect at the next ATM/POS
								use.
							</div>
						</div>
					</div>
				...
				<div
					style={{
						display:
							(tabs.faq ?? "general") !== "security" ? "none" : undefined,
					}}
					id="faq-security"
				>
					<div className={styles.sr}>
						<div>
							...
							>
								3D Secure adds an extra authentication step for online payments.
								You'll receive an OTP or biometric prompt to confirm each online
								transaction.
							</div>
						</div>
					</div>
					<div className={styles.sr}>
						<div>
							<strong>How do I report a fraudulent transaction?</strong>
							<div
								style={{
									fontSize: 12,
									color: "var(--pm-ink-soft)",
							...
							>
								Immediately freeze the card, then file a dispute from Support →
								Dispute. You can also call our 24/7 fraud hotline at +254 800
								999 001.
							</div>
						</div>
					</div>
				...
				<div
					style={{
						display: (tabs.faq ?? "general") !== "virtual" ? "none" : undefined,
					}}
					id="faq-virtual"
				>
					<div className={styles.sr}>
						<div>
							<strong>How do single-use virtual cards work?</strong>
							<div
								style={{
									fontSize: 12,
									color: "var(--pm-ink-soft)",
							...
							>
								A single-use card is created for one transaction. After the
								first successful charge, the card auto-destroys. Unused balance
								is refunded to your funding source.
							</div>
						</div>
					</div>
					<div className={styles.sr}>
						<div>
							<strong>Can I lock a virtual card to one merchant?</strong>
							<div
								style={{
									fontSize: 12,
									color: "var(--pm-ink-soft)",
							...
							>
								Yes. Merchant-locked cards only accept charges from the
								specified merchant MCC or name. This prevents misuse if card
				...
				<div
					style={{
						display: (tabs.faq ?? "general") !== "credit" ? "none" : undefined,
					}}
					id="faq-credit"
				>
					<div className={styles.sr}>
						<div>
							<strong>When is my credit card payment due?</strong>
							<div
								style={{
									fontSize: 12,
									color: "var(--pm-ink-soft)",
							...
							>
								Payment is due on the statement date shown in your billing
								cycle. You have a 30-day grace period. Minimum payment is KES
								2,000 or 5% of balance.
							</div>
						</div>
					</div>
					<div className={styles.sr}>
						<div>
							<strong>How do I increase my credit limit?</strong>
							<div
								style={{
									fontSize: 12,
									color: "var(--pm-ink-soft)",
							...
							>
								Go to Credit Card → Request Limit Increase. Upload recent income
								proof. Decisions are typically made within 24 hours.
							</div>
						</div>
					</div>
				</div>
					...
					<>
						<div style={flows.tshoot !== 1 ? { display: "none" } : {}}>
							...
							<div className="row g-2">
								<div className="col-md-6">
									...
									>
										<i
											className="bi bi-x-circle d-block mb-1"
											style={{ fontSize: 22, color: "var(--pm-danger)" }}
										/>
										<strong style={{ fontSize: 12 }}>Card Declined</strong>
									</div>
								</div>
								<div className="col-md-6">
									<div
									...
									>
										<i
											className="bi bi-wifi-off d-block mb-1"
											style={{ fontSize: 22, color: "var(--pm-warning)" }}
										/>
								...
								<div className="col-md-6">
									...
									>
										<i
											className="bi bi-cash-stack d-block mb-1"
											style={{ fontSize: 22, color: "var(--pm-info)" }}
										/>
										<strong style={{ fontSize: 12 }}>ATM Issue</strong>
									</div>
								</div>
								<div className="col-md-6">
									<div
									...
									>
										<i
											className="bi bi-globe d-block mb-1"
											style={{ fontSize: 22, color: "var(--pm-purple)" }}
										/>
										<strong style={{ fontSize: 12 }}>
						...
						<div style={flows.tshoot !== 2 ? { display: "none" } : {}}>
							...
							>
								...
								>
									<li>
										Daily spending limit may have been reached (KES 50,000)
									</li>
									<li>Online transactions may be toggled off</li>
									<li>
										International transactions disabled (if merchant is
										overseas)
									</li>
							...
							<div className={styles.sr}>
								<div>
									<strong>Check & increase daily limit</strong>
								</div>
								<button
									className={`${styles.btnPm} ${styles.btnSm}`}
									onClick={() => onOpen("cardControlsModal")}
								>
									Open Controls
								</button>
							</div>
							<div className={styles.sr}>
								<div>
									<strong>Enable online transactions</strong>
								</div>
								<button
									className={`${styles.btnPm} ${styles.btnSm}`}
									onClick={() => onOpen("cardControlsModal")}
							...
							<div className={styles.sr}>
								<div>
									<strong>Verify 3D Secure enrollment</strong>
								</div>
								<button
									className={`${styles.btnPm} ${styles.btnSm}`}
								...
								>
									<button
										...
										onClick={() =>
											doAction(
												"troubleshootModal",
												"Great! Glad the issue is resolved. Your card is ready to use.",
												"",
											)
										}
									>
									...
									<button
										...
										onClick={() => onOpen("contactSupportModal")}
									>
										No, contact support
									</button>
								</div>
			...
			>
				<div className="mb-3">
					...
					<select className={styles.fc}>
						<option>Visa Debit ****4521</option>
						<option>Virtual Credit ****1190</option>
						<option>Netflix Card ****5501</option>
					</select>
				</div>
				...
				<div id="revealedDetails">
					...
					>
						<div className="d-flex justify-content-between mb-2">
							...
							<strong>4521 6788 9012 4521</strong>
						</div>
						<div className="d-flex justify-content-between mb-2">
					...
					>
						<i className="bi bi-clock" /> Details auto-hide in 30 seconds
					</p>
				</div>
			</MBox>
			...
			>
				...
				<div
					...
					style={{ background: "var(--pm-info-soft)", fontSize: 12 }}
				>
					<i className="bi bi-lightbulb me-1" /> Improve score: Update PIN on
					Corporate card (91 days old) and enable biometric binding.
				</div>
			</MBox>
			...
			>
				{renderActionBody(
					...
					<>
						...
						<div className="mb-3">
							<label className={styles.formLabel}>Card (if relevant)</label>
							<select className={styles.fc}>
								<option>Visa ****4521</option>
								<option>MC ****3392</option>
						...
						<div className="mb-3">
							...
							<textarea
								className={styles.fc}
								rows={4}
								defaultValue="I need assistance with my card. Please advise on next steps."
							/>
						</div>
						<div className="mb-3">
							<label className={styles.formLabel}>Preferred Channel</label>
							<select className={styles.fc}>
			...
			>
				{renderActionBody(
					...
					<>
						...
						>
							<h6
								style={{ fontWeight: 700, color: "#991B1B", margin: "0 0 4px" }}
							>
								Lost/Stolen Card?
							</h6>
							<p style={{ fontSize: 13, color: "#7F1D1D", margin: "0" }}>
								Call <strong>+254 800 999 001</strong> (24/7, toll-free) or tap
								the button below to instantly block your card.
							</p>
							<button
								className={`${styles.btnPm} ${styles.btnSm} ${styles.btnPmD} mt-2`}
								onClick={() => onOpen("reportLostModal")}
							>
								Block Card Now
							</button>
						</div>
						...
						>
							<h6
								style={{ fontWeight: 700, color: "#B45309", margin: "0 0 4px" }}
							>
								Emergency Replacement
							</h6>
							<p style={{ fontSize: 13, color: "#92400E", margin: "0" }}>
								Express courier delivery within <strong>4 hours</strong> in
								Nairobi. Nationwide within <strong>24 hours</strong>.
							</p>
							<button
						...
						>
							<h6
								style={{ fontWeight: 700, color: "#1D4ED8", margin: "0 0 4px" }}
							>
								Emergency Cash Advance (Credit Cardholders)
							</h6>
							<p style={{ fontSize: 13, color: "#1E40AF", margin: 0 }}>
								Access emergency cash up to{" "}
								<strong>30% of your credit limit</strong> via M-Pesa or partner
								ATMs worldwide.
							</p>
							<button
								className={`${styles.btnPm} ${styles.btnSm} mt-2`}
								onClick={() =>
									doAction(
										"emergencyModal",
										"Emergency cash advance of KES 15,000 sent to M-Pesa 0712***890.",
										"ECA-20250627-0001",
									)
								}
							>
						...
						>
							<h6
								style={{ fontWeight: 700, color: "#6D28D9", margin: "0 0 4px" }}
							>
								Travel Assistance (Premium Cardholders)
							</h6>
							<p style={{ fontSize: 13, color: "#4C1D95", margin: 0 }}>
								Concierge, hotel booking, medical referral and embassy
							...
							<button
								...
								onClick={() =>
									doAction(
										"emergencyModal",
										"Travel concierge request submitted. An agent will call you within 5 minutes.",
										"TRV-20250627-0001",
									)
								}
							>
				...
				{renderActionBody(
					...
					<>
						<div className="mb-3">
							<label className={styles.formLabel}>Card</label>
							<select className={styles.fc}>
								<option>Visa Debit ****4521 (expiring 07/25)</option>
								<option>Prepaid ****8890</option>
								<option>Corporate ****6677</option>
							</select>
						</div>
						<div className="mb-3">
							...
							<select className={styles.fc}>
								...
								<option>Damaged (chip/stripe)</option>
						...
						<div className="mb-3">
							...
							<select className={styles.fc}>
								<option>Express courier — 4 hours (KES 500)</option>
								<option>Standard mail — 5-7 days (free)</option>
								<option>Branch pickup — same day (free)</option>
							</select>
						</div>
						<div className="mb-3">
							<label className={styles.formLabel}>Delivery Address</label>
						...
						<div className="form-check">
							...
							/>{" "}
							<label className="form-check-label" style={{ fontSize: 13 }}>
								Migrate linked recurring payments
							</label>
						</div>
					</>,
				)}
			...
			>
				...
				>
					...
					<div style={{ fontSize: 11, color: "#7F1D1D", marginTop: 2 }}>
						Renew or request replacement to maintain service.
					</div>
				</div>
				<div
					className="p-3 rounded mb-2"
				...
				>
					<strong>PIN reset PR-11228 processing</strong>
					<div style={{ fontSize: 11, color: "#1E40AF", marginTop: 2 }}>
						OTP verification required to complete reset.
					</div>
				</div>
				<div
					className="p-3 rounded mb-2"
				...
				>
					<strong>Credit statement ready</strong>
					<div style={{ fontSize: 11, color: "#065F46", marginTop: 2 }}>
						Virtual Credit ****1190 — June statement available.
					</div>
				</div>
				<div
					className="p-3 rounded mb-2"
				...
				>
					<strong>Replacement card RPL-22019 dispatched</strong>
					<div style={{ fontSize: 11, color: "var(--pm-muted)", marginTop: 2 }}>
						Courier tracking available. Expected today by 5 PM.
					</div>
				</div>
				<div
					className="p-3 rounded mb-2"
				...
				>
					<strong>Transaction declined at Amazon</strong>
					<div style={{ fontSize: 11, color: "var(--pm-muted)", marginTop: 2 }}>
						KES 3,200 · Reason: 3D Secure timeout.
					</div>
				</div>
			</MBox>
			...
			>
				...
				>
					...
					<div className="d-flex justify-content-between mb-2">
						...
						<strong>
							07/2025{" "}
							<span style={{ color: "var(--pm-danger)" }}>(28 days)</span>
						</strong>
					</div>
					<div className="d-flex justify-content-between mb-2">
						<span className="text-muted">3D Secure</span>
						<span className={`${styles.badge} ${styles.badgeS}`}>Enrolled</span>
					</div>
					<div className="d-flex justify-content-between mb-2">
						<span className="text-muted">Contactless</span>
			...
			>
				<div className="mb-3">
					<label className={styles.formLabel}>
						Enter first 6–8 digits of card number
					</label>
					<input
						className={styles.fc}
						defaultValue="452167"
						placeholder="e.g. 452167"
					/>
				</div>
				<div
					className="p-3 rounded"
				...
				>
					...
					<div className="d-flex justify-content-between mb-2">
						...
						<strong>Kenya (KE)</strong>
					</div>
					<div className="d-flex justify-content-between">
						<span className="text-muted">Level</span>
						<strong>Classic / Standard</strong>
					</div>
				</div>
			</MBox>
			...
			>
				...
				>
					...
					<div className="d-flex justify-content-between mb-2">
						<span className="text-muted">ATM Fee</span>
					...
					<div className="d-flex justify-content-between">
						...
						<strong style={{ color: "var(--pm-primary)" }}>KES 5,035</strong>
					</div>
				</div>
			</MBox>
			...
			>
				<div className="mb-3">
					<label className={styles.formLabel}>Search Location</label>
					<input
						className={styles.fc}
				...
				<div className="table-responsive">
					<table className={styles.tbl}>
						...
						<tbody>
							<tr>
								<td>
									<strong>PayMo Westlands</strong>
								</td>
								<td>Sarit Centre, Level 2</td>
								<td>Card issue, PIN reset, Pickup</td>
								<td>8AM – 5PM</td>
								<td>1.2 km</td>
							</tr>
							<tr>
								<td>
									<strong>PayMo CBD Kenyatta Ave</strong>
								</td>
								<td>Koinange / Kenyatta corner</td>
								<td>Card issue, PIN reset, Disputes</td>
								<td>8AM – 6PM</td>
								<td>3.4 km</td>
							</tr>
							<tr>
								<td>
									<strong>ATM — Galleria Mall</strong>
								</td>
								<td>Langata Road</td>
								<td>Cash, Balance inquiry</td>
								<td>24/7</td>
								<td>5.1 km</td>
							</tr>
							<tr>
								<td>
									<strong>ATM — JKIA Terminal 1</strong>
								</td>
			...
			>
				{renderActionBody(
					...
					<>
						...
						<div className={styles.sr}>
							<div>
								<strong>Dispute CDP-44892 evidence needed</strong>
								<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
									Upload before 30 Jun deadline
								</div>
							</div>
							<button
								className={`${styles.btnPm} ${styles.btnSm}`}
						...
						<div className={styles.sr}>
							<div>
								<strong>Credit card payment due in 3 days</strong>
								<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
									Virtual Credit ****1190 · KES 8,200
								</div>
							</div>
							<button
								...
								onClick={() =>
									doAction(
										"attentionFullModal",
										"Credit card payment of KES 8,200 scheduled for 30 Jun.",
										"",
									)
								}
							>
			...
			>
				<div className="text-center">
					<div
						className="pm-avatar mx-auto mb-3"
					...
					<p style={{ fontSize: 13, color: "var(--pm-muted)" }}>
						amina.k@email.com · +254 712 345 890
					</p>
				</div>
