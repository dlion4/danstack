...
const initialMockData: SettingsConfig = {
	...
	statCards: [
		...
		},
		{
			...
			lines: ["3D Secure on all cards", "PIN updated 14 days ago"],
		},
		{
			key: "cases",
			col: "col-lg-3 col-md-6",
			...
			value: "2",
			badge: { icon: "bi-clock", text: "1 awaiting response", tone: "badgeW" },
			lines: ["Dispute #CDP-44892", "PIN reset request #PR-11228"],
		},
		{
			key: "actions",
			col: "col-lg-3 col-md-6",
			...
			lines: ["1 expiring, 1 frozen, 1 PIN change due"],
		},
	],
	sections: {
		preferences: {
			title: "Card Preferences & Defaults",
			...
			alerts: [
				{
					...
					sub: "Notify on every card transaction",
					actionLabel: "Enable",
					actionModal: "notifSettingsModal",
					checked: true,
				},
				{
					type: "Large transactions only",
					icon: "bi-graph-up-arrow",
					iconBg: "var(--pm-info-soft)",
					iconColor: "var(--pm-info)",
					title: "Large transactions only",
					sub: "Notify if amount exceeds threshold",
				...
				},
				{
					...
					sub: "Alert for non-KES payments",
					actionLabel: "Enable",
					actionModal: "notifSettingsModal",
					checked: true,
				},
				{
					type: "Declined transactions",
					icon: "bi-x-circle",
					iconBg: "var(--pm-warning-soft)",
					iconColor: "var(--pm-warning)",
					title: "Declined transactions",
					sub: "With decline reason code",
					actionLabel: "Enable",
					actionModal: "notifSettingsModal",
					checked: true,
				},
				{
					type: "Contactless limit reached",
					icon: "bi-wifi",
					iconBg: "var(--pm-info-soft)",
					iconColor: "var(--pm-info)",
					title: "Contactless limit reached",
					sub: "When tap-to-pay limit hit",
					actionLabel: "Enable",
					actionModal: "notifSettingsModal",
					checked: true,
				},
		...
		support: {
			title: "Card Support & Dispute Help",
			...
			items: [
				...
				},
				{
					...
					sub: "Step-by-step decline diagnosis",
					link: "#",
					linkLabel: "Start",
				},
				{
...
async function fetchSettingsConfig(): Promise<SettingsConfig> {
	...
	try {
		const res = await fetch("/api/card-settings", {
					...
					<div className={styles.pageBar}>
						...
						<div className="d-flex flex-wrap" style={{ gap: 8 }}>
							<button
								...
								onClick={() => setActiveModal("changePinModal")}
							>
								<i className="bi bi-key" /> Change PIN
							</button>
							<button
								className={styles.btnPm}
								onClick={() => setActiveModal("freezeCardModal")}
							>
								<i className="bi bi-snow" /> Freeze Card
							</button>
							<button
								className={styles.btnPm}
								onClick={() => setActiveModal("troubleshootModal")}
							>
								<i className="bi bi-wrench" /> Troubleshoot
							</button>
							<button
								className={`${styles.btnPm} ${styles.btnPmP}`}
								onClick={() => setActiveModal("contactSupportModal")}
							>
								<i className="bi bi-headset" /> Emergency
							</button>
						</div>
					</div>
					...
					<div className="row g-3">
						<div className="col-lg-4">
							...
							>
								...
								>
									2 physical debit, 3 virtual debit, 1 virtual credit and 1
									prepaid card — all preferences and alerts centralised here.
								</p>
								<div className="mt-3">
									<button
										...
										onClick={() => setActiveModal("defaultCardsModal")}
									>
										Defaults
									</button>
									<button
										className={`${styles.btnPm} ${styles.btnSm} ms-2`}
										style={{
											background: "rgba(255,255,255,.12)",
											borderColor: "rgba(255,255,255,.22)",
											color: "#fff",
										}}
										onClick={() => setActiveModal("cardNamingModal")}
									>
										Organise
									</button>
									<button
										...
										onClick={() => setActiveModal("notifSettingsModal")}
									>
										Alerts
									</button>
								</div>
						...
						<div className="col-lg-3 col-md-4 col-6">
							<div className={styles.card} style={{ minHeight: 170 }}>
								<p className={styles.sl} style={{ color: "var(--pm-accent)" }}>
									SECURITY SCORE
								</p>
								...
								>
									3D Secure on all cards
									<br />
									PIN updated 14 days ago
								</div>
							</div>
						</div>
						<div className="col-lg-3 col-md-4 col-6">
							<div className={styles.card} style={{ minHeight: 170 }}>
								<p className={styles.sl} style={{ color: "var(--pm-info)" }}>
									OPEN SUPPORT CASES
								</p>
								<div className={styles.sv} style={{ margin: "6px 0" }}>
									2
								...
								<span className={`${styles.badge} ${styles.badgeW}`}>
									<i className="bi bi-clock" /> 1 awaiting response
								</span>
								<div
									className="mt-2"
									style={{ fontSize: 12, color: "var(--pm-ink-soft)" }}
						...
						<div className="col-lg-2 col-md-4">
							...
							>
								<p className={styles.sl} style={{ color: "var(--pm-warning)" }}>
								...
								<span className={`${styles.badge} ${styles.badgeD}`}>
									<i className="bi bi-exclamation-triangle" /> Review below
								</span>
								<div
									className="mt-2"
									style={{ fontSize: 12, color: "var(--pm-ink-soft)" }}
								>
									1 expiring, 1 frozen, 1 PIN change due
								</div>
							</div>
						</div>
					</div>
							...
							<div className={styles.card}>
									...
									<div className="d-flex align-items-center gap-3">
										...
										<div>
											<div style={{ fontWeight: 600, fontSize: 13 }}>
												Visa Debit expires in 28 days
											</div>
											<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
												****4521 · renew or replace
											</div>
										</div>
									</div>
									<button
										className={`${styles.btnPm} ${styles.btnSm}`}
										onClick={() => setActiveModal("renewCardModal")}
									>
										Renew
									</button>
								</div>
								<div className={styles.sr}>
									<div className="d-flex align-items-center gap-3">
										...
										<div>
											<div style={{ fontWeight: 600, fontSize: 13 }}>
												Travel prepaid card frozen
											</div>
											<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
												****8890 · unfreeze to use
											</div>
										</div>
									</div>
									<button
							...
							<div className={styles.card}>
								...
								<div className={styles.sr}>
									<div className="d-flex align-items-center gap-3">
										...
										<div>
											<div style={{ fontWeight: 600, fontSize: 13 }}>
												Enable transaction alerts on 2 cards
											</div>
											<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
												Increase fraud detection coverage
											</div>
										</div>
									</div>
									<button
								...
								<div className={styles.sr}>
									<div className="d-flex align-items-center gap-3">
										...
										<div>
											<div style={{ fontWeight: 600, fontSize: 13 }}>
												Lock international on daily-use card
											</div>
											<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
												Only enable when travelling
											</div>
										</div>
									</div>
									<button
										className={`${styles.btnPm} ${styles.btnSm}`}
										onClick={() => setActiveModal("cardControlsModal")}
									>
										Configure
									</button>
								</div>
								<div className={styles.sr}>
									<div className="d-flex align-items-center gap-3">
										...
										<div>
											<div style={{ fontWeight: 600, fontSize: 13 }}>
												Set default contactless card
											</div>
											<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
												No default contactless card selected
											</div>
										</div>
									</div>
									<button
							...
							<div className={styles.card}>
								...
								<div className="quick-grid">
									<button
										...
										onClick={() => setActiveModal("changePinModal")}
									>
										<i className="bi bi-key text-primary me-1" /> Change PIN
									</button>
									<button
										className="quick-btn"
										onClick={() => setActiveModal("freezeCardModal")}
									>
										<i className="bi bi-snow2 text-info me-1" /> Freeze/Unfreeze
									</button>
									<button
										className="quick-btn"
										onClick={() => setActiveModal("reportLostModal")}
									>
										<i className="bi bi-exclamation-diamond text-danger me-1" />{" "}
										Report Lost
									</button>
									<button
										className="quick-btn"
									...
									<button
										className="quick-btn"
										onClick={() => setActiveModal("disputeModal")}
									>
										<i className="bi bi-shield-exclamation text-warning me-1" />{" "}
										Dispute
									</button>
									<button
										className="quick-btn"
										onClick={() => setActiveModal("faqModal")}
									>
										<i
									...
									<button
										className="quick-btn"
										onClick={() => setActiveModal("feeCalcModal")}
									>
										<i className="bi bi-calculator text-muted me-1" /> Fee
										Calculator
									</button>
									<button
										className="quick-btn"
										onClick={() => setActiveModal("contactSupportModal")}
									>
										<i className="bi bi-headset text-primary me-1" /> Live
										Support
									</button>
								</div>
							</div>
								...
								>
									<div>
										<h3 className={styles.st}>
											...
											/>{" "}
											{config.sections.preferences.title}
										</h3>
										<p className={styles.ss}>
											{config.sections.preferences.sub}
										</p>
									</div>
									<div className="d-flex" style={{ gap: 8 }}>
										<button
											...
											onClick={() => setActiveModal("defaultCardsModal")}
										>
											<i className="bi bi-check2-circle" /> Defaults
										</button>
										<button
											className={`${styles.btnPm} ${styles.btnSm} ${styles.btnPmP}`}
											onClick={() => setActiveModal("cardNamingModal")}
										>
											<i className="bi bi-pencil" /> Organise
										</button>
									</div>
								</div>
								<div className="row g-3">
										...
										<div className={styles.ub}>
											...
											{config.sections.preferences.cards.map((c) => (
												<div key={c.last4} className={styles.sr}>
													<div>
														<strong>
															{c.type === "Virtual Credit"
																? "Online Payments"
																: c.type === "Prepaid"
																	? "ATM Withdrawals"
																	: "Contactless / Tap" === c.type
																		? "Contactless / Tap"
																		: c.type}
														</strong>
														<div
									...
									<div className="col-lg-3">
										<div className={styles.ub}>
											...
											>
												...
												<div className={styles.cwNumber}>
													•••• •••• •••• 4521
												</div>
												<div className={styles.cwRow}>
													<div>
														<div className={styles.cwLabel}>Cardholder</div>
														AMINA KAMAU
													</div>
													<div>
														<div className={styles.cwLabel}>Expires</div>07/25
													</div>
													<div>
														<div className={styles.cwLabel}>Type</div>Debit
													</div>
												</div>
											</div>
											<div className="d-flex flex-wrap" style={{ gap: 8 }}>
												<button
													className={`${styles.btnPm} ${styles.btnSm}`}
													onClick={() => setActiveModal("revealPanModal")}
												>
													<i className="bi bi-eye" /> Reveal
												</button>
												<button
													className={`${styles.btnPm} ${styles.btnSm}`}
													onClick={() => setActiveModal("cardControlsModal")}
												>
													<i className="bi bi-sliders" /> Controls
												</button>
											</div>
										</div>
									</div>
							...
							<div className={styles.card}>
								...
								>
									...
									<button
										...
										onClick={() => setActiveModal("notifSettingsModal")}
									>
										<i className="bi bi-sliders" /> Configure All
									</button>
								</div>
								<div className="row g-3">
									...
									<div className="col-lg-6">
										<div className={styles.ub}>
											...
											<div className={styles.sr}>
												<div>
													<strong>PIN / password change</strong>
												</div>
												<div className="form-check form-switch">
													<input
														className="form-check-input"
								...
								<div className="row g-3">
									<div className="col-lg-4">
										<div className={styles.ub}>
											...
											>
												<i className="bi bi-question-circle text-primary me-1"></i>{" "}
												Self-Service Help
											</h4>
											{config.sections.support.items.map((item) => (
												<div
													...
													onClick={() =>
														setActiveModal(
															item.title === "FAQ by card type"
																? "faqModal"
																: item.title === "Troubleshoot wizard"
																	? "troubleshootModal"
																	: item.title === "Card status checker"
																		? "cardStatusModal"
																		: item.title === "BIN lookup tool"
																			? "binLookupModal"
																			: item.title === "Fee calculator"
																				? "feeCalcModal"
																				: "contactSupportModal",
														)
													}
									...
									<div className="col-lg-4">
										<div className={styles.ub}>
											...
											>
												<i className="bi bi-chat-dots text-info me-1"></i>{" "}
												Support Channels
											</h4>
											<div
												className={styles.sr}
											...
											>
												<div className="d-flex align-items-center gap-2">
													...
													<div>
														<strong>Card hotline</strong>
														<div
															style={{ fontSize: 11, color: "var(--pm-muted)" }}
														>
															+254 800 723 001 · 24/7
														</div>
													</div>
												</div>
											</div>
											...
											<div
												className={styles.sr}
												style={{ cursor: "pointer" }}
												onClick={() => setActiveModal("branchLocatorModal")}
											>
												<div className="d-flex align-items-center gap-2">
													<div
														style={{
									...
									<div className="col-lg-4">
										<div className={styles.ub}>
											...
											>
												...
												>
													Express courier within 4 hours
												</div>
												<button
											...
											>
												...
												>
													Cash advance & concierge
												</div>
												<button
													className={`${styles.btnPm} ${styles.btnSm} mt-2`}
													onClick={() => setActiveModal("emergencyModal")}
												>
													Get Help Abroad
												</button>
											</div>
						...
						>
							<div>
								...
								<div className="d-flex" style={{ gap: 8 }}>
									...
									<button
										className={`${styles.btnPm} ${styles.btnSm}`}
										onClick={() => setActiveModal("caseExportModal")}
									>
									...
									{config.openCases.map((c) => (
										<tr key={c.caseId}>
											...
											<td data-label="Status">
												<span
													className={`${styles.badge} ${styles[c.statusTone]}`}
												>
													{c.status}
												</span>
											</td>
											<td data-label="Opened">
												{c.nextStep === "Upload merchant receipt"
													? "24 Jun 2025"
													: c.nextStep === "OTP verification pending"
														? "26 Jun 2025"
														: c.nextStep === "Card re-frozen by cardholder"
															? "20 Jun 2025"
															: "22 Jun 2025"}
											</td>
