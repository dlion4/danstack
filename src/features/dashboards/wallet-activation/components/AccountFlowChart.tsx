			...
			{viewMode === "flow" ? (
				<>
					{narrow ? (
						...
						>
							...
							<div className={styles.flowWalletNodeTop}>
								<div className={styles.flowWalletTopBrand}>
									...
									<div>
										<div className={styles.flowWalletLabel}>
											Primary PayMo Wallet
										</div>
							...
							<div className={styles.flowLanesStrip}>
								{links.map((link) => {
									...
									const meta =
										PERM_META[link.permission] || PERM_META["View Only"];
									...
									return (
										...
										>
											<div className={styles.flowLaneStripHead}>
												...
												<div className={styles.flowLaneBody}>
													...
													<div className={styles.flowLaneOrigin}>
														<i
															className={link.icon}
															style={{ color: link.color, fontSize: 10 }}
														></i>
														{link.origin}
													</div>
												</div>
											...
											<div className={styles.flowLaneBalance}>
												{link.balance}
											</div>
											<div className={styles.flowLaneStripChip}>
												{status === "active" || status === "revoking" ? (
													status === "revoking" ? (
														...
														<>
															<i className="bi bi-link-45deg"></i> Linked —{" "}
															{meta.chip}
														</>
													) : (
														<>
													...
													<>
														<i className="bi bi-x-circle"></i> {meta.chip}{" "}
														paused
													</>
												)}
											</div>
						...
						>
							...
							<svg
								...
								role="img"
								aria-label="Account flow chart between primary wallet and linked dashboards"
							>
								<defs>
									{FLOW_COLORS.map((hex) => (
										<filter
									...
									return (
										<g key={p.id}>
											...
											<path
												...
												strokeDasharray={
													active ? "6 7" : status === "revoking" ? "4 5" : "3 6"
												}
												strokeLinecap="round"
												opacity={
													active ? 0.95 : status === "revoking" ? 0.8 : 0.35
												}
											>
												{active && (
													<animate
							...
							{/* Wallet node */}
							<div
								className={styles.flowWalletNode}
								style={{ left: WALLET_LEFT, top: walletCY - 30 }}
							>
								<div className={styles.flowWalletIcon}>
									<i className="bi bi-wallet2"></i>
								</div>
											...
											{status === "active" && (
												<div className={styles.flowLaneFeatures}>
													{meta.features.slice(0, 2).map((feature) => (
														<span
															key={feature}
															className={styles.flowFeatureBadge}
															style={{
					...
					<div className={styles.flowLaneLegend}>
						<span>
							<span
								className={styles.flowLegendDot}
				...
				<div className={styles.flowTableContainer}>
					<div className={styles.flowTableHeader}>
						<div className={styles.flowTableTitle}>
								...
								{links.map((link) => {
									...
									return (
										<tr
											key={link.id}
											className={
												status === "revoked" ? styles.flowTableRowDisabled : ""
											}
										>
											<td>
					...
					<div className={styles.flowTableFooter}>
						<div className={styles.flowTableStats}>
							<span className={styles.flowTableStat}>
								<i className="bi bi-link-45deg"></i> {links.length} linked
								accounts
							</span>
							<span className={styles.flowTableStat}>
								<i className="bi bi-check-circle"></i>{" "}
								{Object.values(statuses).filter((s) => s === "active").length}{" "}
								active
							</span>
						</div>
					</div>
						...
						return (
							<>
								<div className={styles.flowHoverCardHeader}>
									<div
										className={styles.flowHoverCardIcon}
										style={{ background: link.bg, color: link.color }}
									>
										<i className={link.icon}></i>
									</div>
									<div className={styles.flowHoverCardTitle}>
										...
										<div className={styles.flowHoverCardOrigin}>
											{link.origin}
										</div>
									</div>
								</div>
								...
								<div className={styles.flowHoverCardBody}>
									<div className={styles.flowHoverCardSection}>
										...
										<div
											className={styles.flowHoverCardValue}
									...
									<div className={styles.flowHoverCardSection}>
										...
										<div className={styles.flowHoverCardValue}>
											{link.balance}
										</div>
									</div>
									<div className={styles.flowHoverCardSection}>
										...
										>
											{status === "active" ? (
												...
												<>
													...
													{status === "revoking" ? "Breaking..." : "Revoked"}
												</>
											)}
										</div>
									</div>
									{status === "active" && (
										<>
											<div className={styles.flowHoverCardDivider}></div>
											<div className={styles.flowHoverCardSection}>
												<div className={styles.flowHoverCardLabel}>
													Features Enabled
												</div>
												<div className={styles.flowHoverCardFeatures}>
													{meta.features.map((feature) => (
														<span
															key={feature}
															className={styles.flowHoverFeatureItem}
														>
															<i
											...
											{link.permission === "Full Control" && (
												<div className={styles.flowHoverCardFullAccess}>
													...
													<span>
														Full Access: Complete bidirectional wallet control
													</span>
												</div>
											)}
										</>
