...
export function WalletActivationModals({
	...
	/* ---- shared local state ---- */
	const activeDash = "Business Portal";
	...
	const [selectedLinkAccount, setSelectedLinkAccount] = useState<string | null>(
		null,
	);
	const [linkSourceWallet, setLinkSourceWallet] = useState<string | null>(null);
	const [linkDestinationWallet, setLinkDestinationWallet] = useState<
		string | null
	>(null);
			...
			{(step) => {
							...
							>
								...
								<div>
									<div style={{ fontWeight: 700, fontSize: 14 }}>
										Activating {activeDash}
									</div>
									<div style={{ fontSize: 12, color: "#0a6b3e" }}>
										Review and accept the consent items below to unlock this
										dashboard.
									</div>
								</div>
							</div>
									...
									>
										...
										<div style={{ flex: 1, minWidth: 0 }}>
											...
											{expandConsent === i && (
												...
												>
													Clause 3.1 — You agree to keep account credentials
													confidential, only transact with legitimate funds, and
													report any suspicious activity to PayMo compliance
													within 24 hours. This consent is captured with a
													timestamp and IP address for the audit trail.
												</div>
											)}
										</div>
									</div>
				...
				if (step === 2) {
					return (
						...
						>
							...
							>
								Enter your 4-digit PayMo PIN to confirm this activation. This
								action is logged to your security audit trail with timestamp and
								IP.
							</p>
							<div
							...
							>
								3 attempts before a 15-minute lockout. SMS OTP fallback
				...
				return (
					<div style={{ textAlign: "center", padding: "8px 0" }}>
						...
						>
							Your activation is complete. An activation certificate was
							generated and a welcome notification sent to your preferred
							channel.
						</p>
						<div
							style={{
	...
	/* ================= W6. Dashboard Tour Guide Launcher ================= */ const tourSteps =
		[
			{
				id: "tour-0",
				icon: "bi bi-grid-1x2",
				title: "Dashboard Overview",
				desc: "A snapshot of your balances, active links, recent activity and quick actions.",
			},
			{
				id: "tour-1",
				icon: "bi bi-wallet2",
				title: "Linked Accounts",
				desc: "All accounts linked to this dashboard, their balances and permission levels.",
			},
			{
				id: "tour-2",
				icon: "bi bi-arrow-left-right",
				title: "Transfers",
				desc: "Send and receive money between this dashboard and any linked account.",
			},
			{
				id: "tour-3",
				icon: "bi bi-sliders",
				title: "Permissions",
				desc: "Control visibility, inbound/outbound transfers and auto-sweep rules.",
			},
			{
				id: "tour-4",
				icon: "bi bi-bell",
				title: "Notifications",
				desc: "Configure alerts for balance, transactions and link changes.",
			},
			{
				id: "tour-5",
				icon: "bi bi-shield-check",
				title: "Security",
				desc: "Manage sessions, PIN and biometric requirements for this dashboard.",
			},
			{
				id: "tour-6",
				icon: "bi bi-clock-history",
				title: "Activity & Logs",
				desc: "Review every event on this dashboard with full audit detail.",
			},
			{
				id: "tour-7",
				icon: "bi bi-gear-wide-connected",
				title: "Settings",
				desc: "Customize naming, limits and automation rules for this dashboard.",
			},
		];
		...
		>
			<div className={s.stepper}>
				{tourSteps.map((step, i) => (
					<span
						key={step.id}
						className={`${s.step} ${i === tourStep ? s.stepActive : i < tourStep ? s.stepDone : ""}`}
					>
			...
			<InfoBox variant="info">
				<i className="bi bi-gift" /> Completing the tour earns you 200 reward
				points. You can replay it anytime from dashboard settings.
			</InfoBox>
		</ModalShell>
	);
			...
			{(step) => {
				if (step === 1) {
					return (
						<div>
							<InfoBox variant="info">
								<i className="bi bi-wallet2" /> Select the source wallet you
								want to link from
							</InfoBox>
								...
								{[
									...
									>
										...
										<div style={{ textAlign: "right" }}>
											...
											<i
												className={`bi ${linkSourceWallet === acc.name ? "bi-check-circle-fill" : "bi-circle"}`}
												style={{
													color:
														linkSourceWallet === acc.name
															? "var(--pri)"
				...
				if (step === 2) {
					return (
						<div>
							<InfoBox variant="info">
								<i className="bi bi-arrow-right" /> Select the destination
								wallet to link {linkSourceWallet || "the source wallet"} to
							</InfoBox>
							<div style={{ marginTop: 16 }}>
								<div
									style={{
								...
								{[
									...
									<button
										...
										style={{
											...
											borderRadius: 12,
											border:
												linkDestinationWallet === acc.name
													? "2px solid var(--pri)"
													: "1px solid var(--border)",
											background:
												linkDestinationWallet === acc.name
									...
									>
										...
										<div style={{ textAlign: "right" }}>
											...
											<i
												className={`bi ${linkDestinationWallet === acc.name ? "bi-check-circle-fill" : "bi-circle"}`}
												style={{
													color:
														linkDestinationWallet === acc.name
															? "var(--pri)"
					...
					return (
						<div>
							<InfoBox variant="warning">
								<i className="bi bi-diagram-3" /> Review how money will flow
							...
							>
								...
								>
									...
									<div style={{ flex: 1, textAlign: "center" }}>
										...
										>
											{linkDestinationWallet || "Destination"}
										</div>
										<div style={{ fontSize: 11, color: "var(--ink-500)" }}>
											Destination Wallet
										</div>
								...
								>
									<i className="bi bi-info-circle" /> With the default Full
									Control permission, money can flow freely in both directions.
				...
				if (step === 4) {
					return (
						<div>
							<InfoBox variant="info">
								<i className="bi bi-sliders" /> Configure permissions and
							...
							<div style={{ marginTop: 16 }}>
								...
								{[
									{
										name: "Full Control",
										desc: "Bidirectional flow + all features",
										active: true,
									},
									{
									...
									>
										<i
											className={`bi ${p.active ? "bi-check-circle-fill" : "bi-circle"}`}
											style={{
												color: p.active ? "var(--pri)" : "var(--ink-300)",
												fontSize: 17,
											}}
										/>
										<div style={{ flex: 1 }}>
								...
								>
									...
									<Toggle
										...
										description="Allow both wallets to see each other's balance"
									/>
									<Toggle
										checked={true}
										onChange={() => {}}
										label="Transaction History"
										description="Share full transaction history between wallets"
									/>
									<Toggle
										checked={false}
										onChange={() => {}}
									...
									<Toggle
										...
										description="Receive alerts about transactions on linked wallet"
									/>
								</div>
							</div>
						</div>
					...
					>
						<div style={{ textAlign: "center", maxWidth: 400 }}>
							<p style={{ fontSize: 13, color: "var(--ink-700)", margin: 0 }}>
								Confirm with your PIN to link{" "}
								<strong>{linkSourceWallet || "source wallet"}</strong> to{" "}
								<strong>{linkDestinationWallet || "destination wallet"}</strong>{" "}
								with <strong>Full Control</strong> permissions.
							</p>
						</div>
						...
						<InfoBox variant="success">
							<i className="bi bi-shield-check" /> Linking is instant and
							reversible. You can modify permissions or unlink at any time from
							the links panel.
						</InfoBox>
					</div>
				);
			...
			tabs={[
				{
					...
					render: () => (
						<div>
							<InfoBox variant="info">
								<i className="bi bi-info-circle" /> Quick permission presets for{" "}
								{selectedLinkAccount || "this account"}
							</InfoBox>
							{[
								{
									name: "Full Inter-Dashboard Access",
									desc: "Enables all toggles below - bidirectional flow",
									active: true,
								},
								{
									name: "View Only",
									desc: "Balance visible, no transfers allowed",
									active: false,
								},
								{
									name: "One-Way In",
									desc: "Can receive funds only - no outbound transfers",
									active: false,
								},
								{
									name: "One-Way Out",
									desc: "Can send funds only - no inbound transfers",
									active: false,
								},
								{
									name: "Custom",
									desc: "User-defined combination of permissions",
									active: false,
								},
							].map((p) => (
								<div
					...
					render: () => (
						<div>
							...
							>
								...
								<div style={{ fontSize: 11, color: "var(--ink-500)" }}>
									Control how money can flow between accounts
								</div>
							</div>
							<Toggle
								checked
								onChange={() => {}}
								label="Balance Visibility"
								description="Allow this dashboard to see balance from {selectedLinkAccount}"
							/>
							<Toggle
								checked
								onChange={() => {}}
								label="Inbound Transfers"
								description="Allow money to flow INTO this dashboard from {selectedLinkAccount}"
							/>
							<Toggle
								checked
								onChange={() => {}}
								label="Outbound Transfers"
								description="Allow money to flow OUT to {selectedLinkAccount}"
							/>
							<Toggle
								checked={false}
								onChange={() => {}}
							...
							<Toggle
								...
								onChange={() => {}}
								label="Auto-Top-Up"
								description="Refill this dashboard when balance drops below threshold"
							/>
							...
							>
								...
								<div style={{ fontSize: 11, color: "var(--ink-500)" }}>
									Control information sharing and alerts
								</div>
							</div>
							<Toggle
								checked
								onChange={() => {}}
								label="Notification Sharing"
								description="Receive alerts about transactions on {selectedLinkAccount}"
							/>
							<Toggle
								checked
								onChange={() => {}}
							...
							<Toggle
								...
								onChange={() => {}}
								label="Transaction History"
								description="View full transaction history from {selectedLinkAccount}"
							/>
						</div>
					),
				},
			...
			footer={
				<>
					<button
						type="button"
						className={s.button}
						onClick={() => close("linkPermissionsModal")}
					>
						Cancel
					</button>
					<button
		...
		>
			<div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
				<Toggle
					...
					description="Alert me when Business Float receives money"
				/>
				<Toggle
					checked
					onChange={() => {}}
					label="Balance drop alert"
					description="Notify when balance drops below a set amount"
				/>
				<Toggle
					checked
					onChange={() => {}}
					label="Failed transaction alerts"
					description="Alert me about failed transactions"
				/>
				<Toggle
					checked={false}
					onChange={() => {}}
					label="Daily balance summary"
					description="Send daily summary to my preferred channel"
				/>
				<Toggle
					checked
					onChange={() => {}}
					label="Link & permission changes"
					description="Alert me when this link is unlinked or permissions change"
				/>
				<div className="mt-2">
					<SelectField
					...
					<div style={fieldGrid}>
						<div>
							<label className={s.formLabel} htmlFor="quiet-hours-from">
								Quiet hours from
							</label>
							<input
								id="quiet-hours-from"
						...
						<div>
							<label className={s.formLabel} htmlFor="quiet-hours-to">
								Quiet hours to
							</label>
							<input
								id="quiet-hours-to"
			...
			tabs={[
				{
					...
					render: () => (
						<div>
							<InfoBox variant="warning">
								<i className="bi bi-arrow-left-right" /> Control the direction
								of money flow between your Primary Wallet and{" "}
								{selectedLinkAccount || "this account"}
							</InfoBox>
							<div style={{ marginTop: 16 }}>
								<div
								...
								{[
									{
										name: "Bidirectional",
										desc: "Money can flow both ways - full access",
										icon: "bi-arrow-left-right",
										active: true,
									},
									{
										name: "Inbound Only",
										desc: "Money can only flow INTO this account",
										icon: "bi-arrow-right",
										active: false,
									},
									{
										name: "Outbound Only",
										desc: "Money can only flow OUT from this account",
										icon: "bi-arrow-left",
										active: false,
									},
									{
										name: "No Flow",
										desc: "No transfers allowed - view only",
										icon: "bi-x-circle",
										active: false,
									},
								].map((p) => (
									...
									>
										<i
											className={`bi ${p.icon}`}
											style={{
												color: p.active ? "var(--pri)" : "var(--ink-400)",
												fontSize: 18,
												width: 24,
											}}
										/>
				...
				},
				{
					...
					render: () => (
						<div>
							<InfoBox variant="info">
								<i className="bi bi-shield-check" /> Set transfer limits to
								protect your funds
							</InfoBox>
							<div style={{ marginTop: 16 }}>
								<div style={fieldGrid}>
									...
									<div>
										<label
											className={s.formLabel}
											htmlFor="monthly-transfer-limit"
										>
											Monthly Transfer Limit
										</label>
										<input
											id="monthly-transfer-limit"
											type="number"
											className={s.formControl}
									...
									<div>
										<label
											className={s.formLabel}
											htmlFor="minimum-transfer-amount"
										>
											Minimum Transfer Amount
										</label>
										<input
											id="minimum-transfer-amount"
											type="number"
											className={s.formControl}
											defaultValue="100"
											placeholder="KES"
								...
								<div style={{ marginTop: 12 }}>
									<Toggle
										...
										description="Block transfers that exceed limits"
									/>
									<Toggle
										checked={false}
										onChange={() => {}}
										label="Allow limit override with PIN"
										description="Require PIN to exceed limits"
									/>
								</div>
							</div>
						</div>
				...
				},
				{
					...
					render: () => (
						<div>
							<InfoBox variant="info">
								<i className="bi bi-clock" /> Set time-based restrictions on
								transfers
							</InfoBox>
							<div style={{ marginTop: 16 }}>
								<Toggle
									...
									description="Only allow transfers during specific hours"
								/>
								<div style={{ ...fieldGrid, marginTop: 12 }}>
									<div>
										<label className={s.formLabel} htmlFor="allowed-from">
											Allowed from
										</label>
										<input
											id="allowed-from"
									...
									<div>
										<label className={s.formLabel} htmlFor="allowed-until">
											Allowed until
										</label>
										<input
											id="allowed-until"
			...
			footer={
				<>
					<button
						type="button"
						className={s.button}
						onClick={() => close("linkFlowControlModal")}
					>
						Cancel
					</button>
					<button
			...
			footer={
				<>
					<button
						type="button"
						className={s.button}
						onClick={() => close("activeLinksModal")}
					>
						Close
					</button>
					<button
		...
		>
			...
			>
				<span className={`${s.badge} ${s.badgeSuccess}`}>
					<i className="bi bi-link-45deg" /> 5 linked accounts
				</span>
				<span className={`${s.badge} ${s.badgeInfo}`}>
					Priority ordering applies to transfers
				</span>
			</div>
					...
					<div className="col-md-6 col-lg-4" key={acc.name}>
						...
						>
							...
							>
								...
								<span
									className={`${s.badge} ${acc.status === "Paused" ? s.badgeWarning : s.badgeSuccess}`}
								>
									{acc.status}
								</span>
							</div>
							...
							<span
								className={`${s.badge} ${acc.permission === "Full Control" ? s.badgeSuccess : s.badgeInfo}`}
							...
							>
								<button
									...
									onClick={() => {
										...
										openModal("linkPermissionsModal");
									}}
								>
									Permissions
								</button>
								...
								<button
									...
									onClick={() => {
										setSelectedLinkAccount(acc.name);
										openModal("linkFlowControlModal");
									}}
								>
									Flow Control
								</button>
								{acc.status === "Paused" && (
									<button
										...
										onClick={() => openModal("relinkAccountModal")}
									>
										Relink
									</button>
								)}
								<button
									...
									onClick={() => openModal("unlinkAccountModal")}
								>
									Unlink
								</button>
							</div>
			...
			{(step) => {
				if (step === 1) {
					return (
						<div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
							<InfoBox variant="warning">
								<i className="bi bi-exclamation-triangle" /> Business Float has
								a balance of <strong>KES 6,150,000</strong>. Funds must be
								relocated before unlinking.
							</InfoBox>
							<div
								style={{
									padding: "14px 16px",
							...
							>
								<i className="bi bi-arrow-left-right" /> Move funds with Money
								Relocation Wizard
							</button>
						</div>
					);
				...
				if (step === 2) {
					return (
						<div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
							<div>
								<label className={s.formLabel} htmlFor="unlink-reason">
							...
							<InfoBox variant="info">
								<i className="bi bi-shield-lock" /> Enter your PIN to authorize
								the unlink request. This is recorded to the audit log with
				...
				return (
					...
					>
						...
						>
							Your unlink request for <strong>Business Float</strong> is
							confirmed. A <strong>24-hour grace period</strong> applies — you
							can cancel or relink within 24 hours with one tap.
						</p>
						<InfoBox variant="success">
							<i className="bi bi-check-circle" /> You can relink instantly
							within 30 days of unlink without re-entering consent.
						</InfoBox>
					</div>
				);
			}}
	...
	const relinkAccountModal = (
		...
		>
			<div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
				<InfoBox variant="info">
					<i className="bi bi-arrow-counterclockwise" /> Unlinked{" "}
					<strong>20 Jun 2025</strong> — within the 30-day instant relink
					window. Previous permissions are preserved.
				</InfoBox>
				{[
					{
						name: "Business Float",
					...
					>
						...
						<i
							className={`bi ${acc.name === "Business Float" ? "bi-check-circle-fill" : "bi-circle"}`}
							style={{
								color:
									acc.name === "Business Float"
										? "var(--pri)"
			...
			{(step) => {
				if (step === 1) {
					return (
						<div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
							...
							>
								<i className="bi bi-exclamation-triangle-fill" /> This is the
								emergency panic button. All inter-dashboard transfers are
							...
							<Toggle
								...
								description="Send an SMS to your registered emergency number"
							/>
						</div>
					);
				}
				...
				return (
					<div style={{ textAlign: "center", padding: "8px 0" }}>
						...
						>
							All inter-dashboard transfers are suspended. A 72-hour cooldown is
							now active before you can reactivate. Your emergency contact has
							been notified.
						</p>
					</div>
				);
			...
			{(step) => {
					...
					return (
						<div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
							<InfoBox variant="warning">
								...
								<strong>KES 6,150,000 in Business Float</strong>. Where should
								we send it?
							</InfoBox>
							<div
								style={{
							...
							<InfoBox variant="danger">
								<i className="bi bi-exclamation-octagon" /> This action is
								irreversible after confirmation. Please review carefully before
								proceeding.
							</InfoBox>
						</div>
					);
				...
				if (step === 2) {
					return (
						<div>
							...
							/>
							<InfoBox variant="info">
								<i className="bi bi-info-circle" /> {relocationDestination}.
								Eligibility, limits and KYC tier requirements are validated
								automatically.
							</InfoBox>
						</div>
					);
				...
				if (step === 3) {
					return (
						<div>
							<div style={fieldGrid}>
								<div>
									<label className={s.formLabel} htmlFor="amount-to-move-kes">
										Amount to move (KES)
									</label>
									<input
										id="amount-to-move-kes"
										className={s.formControl}
										defaultValue="6,150,000"
									/>
								</div>
								<div>
									<label className={s.formLabel} htmlFor="move-percentage">
										Or percentage
									</label>
									<select
										id="move-percentage"
							...
							<div style={{ marginTop: 14, textAlign: "center" }}>
								<div
									style={{
										...
										borderRadius: "50%",
										background:
											"conic-gradient(var(--pri) 0 85%, var(--border) 85% 100%)",
										display: "flex",
										alignItems: "center",
										justifyContent: "center",
										margin: "0 auto",
								...
								>
									Allocatable across multiple destinations • reserved amounts
									excluded
								</p>
							</div>
						</div>
						...
						<div>
							...
							<div className="mt-2">
								<InfoBox variant="info">
									...
									<strong>REL-20250627-8841</strong> • Estimated completion time
									per destination shown during execution.
								</InfoBox>
							</div>
						</div>
					);
				...
				if (step === 5) {
					return (
						...
						>
							...
							>
								Multi-factor confirmation required to move{" "}
								<strong>KES 6,150,000</strong>.
							</p>
							<div style={{ width: "100%" }}>
								<div
									style={{
							...
							<InfoBox variant="success">
								<i className="bi bi-fingerprint" /> Biometric confirmation is
								enabled on this device — place your finger or use Face ID when
								prompted.
							</InfoBox>
							<div
								style={{
							...
							>
								Type to confirm:{" "}
								<em>
									"I confirm I want to move KES 6,150,000 to Primary PayMo
									Wallet"
								</em>
								<input
									className={s.formControl}
							...
							<InfoBox variant="warning">
								<i className="bi bi-robot" /> Fraud check: velocity limits,
								unusual pattern detection and device trust score are evaluated
								before execution.
							</InfoBox>
						</div>
					);
				...
				if (step === 6) {
					return (
						<div>
							<InfoBox variant="info">
								<i className="bi bi-person-check" /> Destination is your own
								Primary PayMo Wallet — name match confirmed automatically. No
						...
						<div>
							...
							>
								{[
									{
										name: "Queued",
										desc: "Relocation request accepted",
										done: true,
									},
									{
										name: "Validating",
										desc: "Checks, limits and fraud screening",
										done: true,
									},
									{
										name: "Processing",
										desc: "Moving KES 6,150,000 to Primary Wallet",
										done: false,
										active: true,
									},
							...
							<div className="mt-3">
								<InfoBox variant="info">
									<i className="bi bi-clock" /> Estimated time remaining:{" "}
									<strong>2 seconds</strong> • Auto-retry enabled (up to 3
				...
				return (
					<div style={{ textAlign: "center", padding: "8px 0" }}>
						...
						>
							KES 6,150,000 moved to Primary PayMo Wallet • KES 50 fee. A
							receipt with QR code and reference has been generated.
						</p>
						<div
							style={{
								display: "flex",
						...
						>
							...
							<button
								type="button"
								className={s.button}
								onClick={() => close("moneyRelocationModal")}
							>
								Return to Dashboard
							</button>
						</div>
	...
	const relocationReceiptModal = (
		...
		>
			<div className={s.receipt}>
				...
				<div style={{ fontWeight: 700, fontSize: 22, color: "var(--pri-700)" }}>
					KES 6,150,000
				</div>
				<div style={{ fontSize: 12, color: "var(--ink-500)" }}>
					Business Float → Primary PayMo Wallet
				</div>
				...
				>
					<i
						...
						style={{ fontSize: 44, color: "var(--ink-900)" }}
					/>
				</div>
			</div>
		</SimpleModal>
	...
	const supportHelpModal = (
			...
			footer={
				<>
					<button
						type="button"
						className={s.button}
						onClick={() => close("supportHelpModal")}
					>
						Close
					</button>
					<button type="button" className={`${s.button} ${s.buttonPrimary}`}>
		...
		>
				...
				<div className="col-md-6">
					...
					>
						...
						{[
							...
							{
								icon: "bi bi-play-btn",
								title: "Money Relocation Wizard — Video",
								desc: "8-step tutorial (4 min)",
							},
						].map((g) => (
					...
					>
						...
						<div className={s.summaryRow}>
							<span style={{ fontSize: 13 }}>
								Live chat (cross-dashboard specialist)
							</span>
							<button type="button" className={`${s.button} ${s.buttonSmall}`}>
								Start
							</button>
						</div>
						<div className={s.summaryRow}>
			...
			<div style={{ marginTop: 16 }}>
				...
				>
					<i className="bi bi-question-lg" /> Frequently asked questions
				</div>
				<div className={s.softBox}>
					{[
						{
							q: "How do I link an account from another dashboard?",
							a: "Open the Link Account wizard from the Overview quick actions, choose the account you want (primary wallet, virtual account, or any account from a previously activated dashboard), pick a permission level, confirm your PIN, and the live flow chart will draw a new money path to it.",
						},
						{
						...
						},
						{
							...
							a: "The money path on the flow chart breaks. If the account still holds a balance, the Money Relocation Wizard runs first so no funds are stranded — you choose a destination, verify with your PIN, and funds are moved safely.",
						},
						{
							q: "Can I get notifications for linked accounts?",
							a: "Yes. The Alert Routing card lets you toggle notifications per linked account — when money is received, when a balance drops below a threshold, and which channel (Push, SMS, Email, WhatsApp) to use.",
						},
						{
						...
						>
							...
							>
								<span>{f.q}</span>
								<i
									className={`bi bi-chevron-down ${supportFaq === i ? "bi-chevron-up" : ""}`}
	...
	const linkLimitsModal = (
		...
		>
			<div>
				...
				<InfoBox variant="warning">
					<i className="bi bi-arrow-up-circle" /> To request a limit increase
					you must provide a justification and supporting document. Processing
		...
		>
			<div>
				...
				<Toggle
					...
					description="Combine transactions from all linked accounts"
				/>
				<Toggle
					checked={false}
					onChange={() => {}}
				...
				<div style={{ marginTop: 12 }}>
					<InfoBox variant="info">
						<i className="bi bi-file-earmark-lock" /> Consent expiry: 11 July
						2026. You can renew or revoke any consent at any time from this
						panel.
					</InfoBox>
				</div>
			</div>
					...
					render: () => (
						<div>
							<SelectField
								...
								defaultValue="Transaction Hub"
							/>
							<SelectField
								label="Activation PIN length"
								options={["4 digits", "6 digits"]}
								defaultValue="4 digits"
							/>
							<Toggle
								checked
								onChange={() => {}}
								label="Require biometric for activation"
								description="Add fingerprint / Face ID to the activation PIN gate"
							/>
							<Toggle
								checked
								onChange={() => {}}
								label="Auto-start tour on first activation"
								description="Launch the tour guide after the first dashboard is activated"
							/>
						</div>
					),
				},
					...
					render: () => (
						<div>
							...
							<Toggle
								...
								description="Remind 30 / 14 / 7 days before consent expiry"
							/>
							<Toggle
								checked={false}
								onChange={() => {}}
					...
					render: () => (
						<div>
							<Toggle
								...
								description="Highlight key sections while exploring a dashboard"
							/>
							<Toggle
								checked={false}
								onChange={() => {}}
			...
			footer={
				<>
					<button
						type="button"
						className={s.button}
						onClick={() => close("preferencesModal")}
					>
						Cancel
					</button>
					<button
