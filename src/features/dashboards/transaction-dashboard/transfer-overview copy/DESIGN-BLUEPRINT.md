...
> Refined: August 28, 2026 — removed local chrome, migrated to shared modal primitives
...
| Modal / wizard | Uses shared `ModalShell`, `SimpleModal`, `FlowModal`, `TabbedModal` from `shared/components/modals.tsx` |
| Shell toast | `.toastContainer`, `.paymoToast` (from `shell.module.css`) |
| Drawer / context panel | `.leftDrawer`, `.rightAside` (from `shell.module.css`) |

---

## 20. SHARED MODAL ARCHITECTURE

All 22 modals use the shared transaction modal primitives from `shared/components/modals.tsx`:

| Modal | Component | Notes |
|-------|-----------|-------|
| Initiate Transfer | `FlowModal` | 4-step wizard with stepper, PIN and receipt |
| Bulk Transfer | `FlowModal` | 4-step upload → review → pay → receipt |
| Schedule Transfer | `FlowModal` | 3-step details → schedule → confirmation |
| Manage Beneficiaries | `TabbedModal` | 3 tabs: All, Favorites, Recent |
| Add Beneficiary | `SimpleModal` | Form with loading → success |
| Transfer Detail | `SimpleModal` | Read-only review, hidden footer |
| Edit Schedule | `SimpleModal` | Form with toggle switches |
| International Transfer | `FlowModal` | 4-step with compliance step |
| QR Pay | `SimpleModal` | QR preview + form |
| Transfer Limits | `SimpleModal` | 3 limit inputs + toggle switches |
| Retry Transfer | `SimpleModal` | Warning callout + funding source |
| Transfer Analytics | `TabbedModal` | 3 tabs: Volume, Success Rate, Recipients |
| Security Check | `SimpleModal` | 3 soft-box score cards |
| Transfer Notifications | `SimpleModal` | 3 status cards |
| Profile | `SimpleModal` | Avatar + stats |
| Attention Review Queue | `SimpleModal` | 3 action rows with cross-modal navigation |
| Dispute Transfer | `SimpleModal` | Issue type + description |
| Edit Beneficiary | `SimpleModal` | Name + account + favorite toggle |
| Fee Calculator | `SimpleModal` | Amount + method → fee |
| Full Transfer History | `TabbedModal` | Table view |
| Quick Send to Favorite | `SimpleModal` | Amount + note |
| Add to Favorites | `SimpleModal` | Nickname + quick-send toggle |

Cross-modal navigation (e.g. clicking Retry in attention queue → opens retry modal) uses `openModal` callback passed from the page component.

---

## 21. CODE-COMPLETE CHECKLIST (refined August 28, 2026)
...
- [x] Refinement: removed local sidebar/header chrome — shell provides those.
- [x] Refinement: 22 modals migrated from legacy MBox to shared `SimpleModal`/`FlowModal`/`TabbedModal`.
- [x] Refinement: TypeScript typecheck passes — zero diagnostics in transfer-overview files.
- [x] Refinement: CSS module cleaned — 1740 LOC → ~680 LOC (page-level styles only).
