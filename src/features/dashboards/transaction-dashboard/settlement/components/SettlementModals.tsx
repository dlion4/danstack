...
export default function SettlementModals({
	...
	const flowDoneMsg: Record<FlowKey, string> = {
		init: "Settlement initiated successfully.",
		recon: "Reconciliation completed.",
		disp: "Dispute submitted successfully.",
	};
						...
						>
							...
							>
								<div className={styles.stepN}>
									{n < current ? <i className="fa-solid fa-check" /> : n}
								</div>
		...
		<>
						...
						<div className="row g-3">
							<div className="col-md-6">
								...
								<select
									...
									onChange={(e) => setInitSourceType(e.target.value)}
								>
									{Object.keys(SOURCE_OPTIONS).map((t) => (
							...
							<div className="col-md-6">
								...
								<select className={styles.fc}>
									{SOURCE_OPTIONS[initSourceType]?.map((o) => (
					...
					<div className={styles.fstepActive}>
						...
						<div className="row g-3">
							<div className="col-md-12">
								...
								<div className={styles.pills}>
									{Object.keys(DEST_OPTIONS).map((type) => (
										<button
											key={type}
											className={`${styles.pill} ${initDestType === type ? styles.pillActive : ""}`}
											onClick={() => setInitDestType(type)}
										>
											{type}
										</button>
									))}
							...
							<div className="col-md-12">
								...
								<select className={styles.fc}>
									{DEST_OPTIONS[initDestType]?.map((o) => (
										<option key={o}>{o}</option>
									))}
								</select>
							</div>
							...
							<div className="col-md-6">
								<label className={styles.fl}>
									Developer / External Ref (Optional)
								</label>
								<input
									className={styles.fc}
									placeholder="e.g., ext-tx-9921"
									value={initRef}
					...
					<div className={styles.fstepActive}>
						...
						<div className="mb-3">
							...
							<div className={styles.pills}>
								{["RTGS", "PesaLink", "EFT", "Internal Ledger"].map((c) => (
									<button
										...
										disabled={
											initDestType === "Internal PayMo Account" &&
											c !== "Internal Ledger"
										}
									>
										{c}
							...
							{initDestType === "Internal PayMo Account" && (
								...
								>
									Internal accounts automatically default to Internal Ledger
									transfer.
								</small>
							)}
						</div>
				...
				{flows.init === 4 && (
					<div className={styles.fstepActive}>
						...
						<div className={`${styles.summaryBox} mb-3`}>
							{[
								["Source", initSourceType],
								["Destination", initDestType],
								["Amount", `KES ${Number(initAmount).toLocaleString()}`],
								[
									"Channel",
									initDestType === "Internal PayMo Account"
										? "Internal Ledger"
				...
				{flows.init >= 5 && (
					<div className={`${styles.receipt} ${styles.fstepActive}`}>
						...
						<p className={styles.receiptSub}>
							Reference: <strong>SET-88424</strong> • Expected completion: 14:47
							EAT
						</p>
					</div>
				)}
			...
			<MBox
				...
				footer={actionFooter(
					...
					"Retry Now",
					"btnPmP",
					"Retry initiated for 1 settlement. New reference: SET-88425",
					"SET-88425",
				)}
			>
				{actionBody(
					...
					<>
						<div className="mb-3">
							...
							{[
								...
								{
									label: "SET-88419 • KES 12.1M • Co-op → Equity",
									checked: false,
								},
							].map((c) => (
						...
						<div className={styles.summaryBoxWarn} style={{ fontSize: 12 }}>
							<i className="fa-solid fa-circle-info me-1" /> Retry will incur
							additional RTGS fee of KES 2,250 per transaction.
						</div>
					</>,
				)}
			</MBox>
			...
			>
				...
				{flows.recon >= 4 && (
					<div className={`${styles.receipt} ${styles.fstepActive}`}>
						...
						<p className={styles.receiptSub}>
							1,142 matched • 47 exceptions resolved • Report generated.
						</p>
					</div>
				)}
			</MBox>
			...
			>
				...
				{flows.disp >= 3 && (
					<div className={`${styles.receipt} ${styles.fstepActive}`}>
						...
						<p className={styles.receiptSub}>
							Case #SET-44901 created. Counterparty notified.
						</p>
					</div>
				)}
			</MBox>
						...
						<tbody>
							<tr>
								...
								<td>
									<button
										className={`${styles.btnPm} ${styles.btnSm}`}
										onClick={() => onOpen("initiateSettlementModal")}
									>
										Submit
									</button>
								</td>
							...
							<tr>
								...
								<td>
									<button
										className={`${styles.btnPm} ${styles.btnSm}`}
										onClick={() => onOpen("reconciliationWizardModal")}
									>
										Reconcile
									</button>
								</td>
			...
			<MBox
				...
				footer={actionFooter(
					...
					"Save Rules",
					"btnPmP",
					"Automation rules updated successfully.",
				)}
			>
				{actionBody(
					"autoRulesModal",
						...
						{rulesTab === "active" && (
							<div>
								{newRuleNote && (
									...
									>
										<i className="fa-solid fa-circle-check" /> {newRuleNote}
									</div>
								)}
								{rules.map((r) => (
									<div
									...
									>
										...
										<div className="row g-2">
											<div className="col-md-3">
												...
												<select className={styles.fc} defaultValue={r.type}>
													{RULE_TYPES.map((t) => (
														<option key={t}>{t}</option>
													))}
												</select>
											...
											<div className="col-md-3">
												...
												<select className={styles.fc} defaultValue={r.channel}>
													{RULE_CHANNELS.map((c) => (
							...
							>
								<div className="row g-3">
									...
									<div className="col-md-4">
										...
										<select
											className={styles.fc}
											defaultValue="Retry transaction"
										>
							...
							<button
								...
								onClick={() =>
									doAction(
										...
										"Report generated and submitted to CBK.",
										"CBK-20250627-001",
									)
								}
							>
			...
			>
				<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
					June 2025 — RTGS cut-off 15:00 daily • PesaLink 24/7 • Weekend
					deferral active for non-urgent batches.
				</p>
				<div className="table-responsive">
					<table className={styles.tbl}>
						<thead>
			...
			<MBox
				...
				title={
					<>
						<i className="fa-solid fa-money-bill-transfer me-2" />
						FX Settlement
					</>
				}
				footer={actionFooter(
					...
					"btnPmP",
					"FX settlement executed. KES 64,725,000 credited.",
					"FX-20250627-8841",
				)}
			>
				{actionBody(
					...
					<>
						<div className="mb-3">
							...
							<select
								className={styles.fc}
								defaultValue="USD Nostro (Citibank) — 8.42M"
							>
			...
			<MBox
				...
				title={
					<>
						<i
							className="fa-solid fa-heart-pulse me-2"
						...
						<div className="col-md-3 col-6" key={t.label}>
							<div className={styles.miniStat} style={{ background: t.bg }}>
								<div
									...
									style={{ color: t.color, fontSize: t.big ? 28 : 24 }}
								>
									{t.value}
								</div>
								<div
			...
			<MBox
				...
				title={
					<>
						<i className="fa-solid fa-scissors me-2" />
				...
				footer={actionFooter(
					...
					"Accept Partial",
					"btnPmP",
					"Partial settlement accepted. Remaining KES 2.7M scheduled for tomorrow.",
				)}
			>
				{actionBody(
					"partialSettlementModal",
			...
			<MBox
				...
				title={
					<>
						<i className="fa-solid fa-microchip me-2" />
				...
				footer={actionFooter(
					...
					"Save Config",
					"btnPmP",
					"Engine configuration saved. Changes effective immediately.",
				)}
			>
				{actionBody(
					"engineConfigModal",
					<>
						...
						{["Enable weekend deferral", "Enable smart channel routing"].map(
							(c) => (
						...
						<div className="mb-1 mt-3">
							...
							<button
								...
								onClick={() =>
									doAction(
										"pesaLinkModal",
										"PesaLink window forced closed. Remaining 28 transactions moved to next window.",
									)
								}
							>
								Force Close
			...
			<MBox
				...
				footer={actionFooter(
					...
					"Submit All Urgent",
					"btnPmD",
					"14 batches submitted with urgent flag. Additional fee: KES 31,500",
				)}
			>
				{actionBody(
					"rtgsUrgentModal",
			...
			<MBox
				...
				title={
					<>
						<i className="fa-solid fa-tower-broadcast me-2" />
			...
			>
				<div className="text-center">
					...
					<p style={{ fontSize: 13, color: "var(--pm-muted)" }}>
						james.kamau@paymo.co.ke · Treasury
					</p>
				</div>
			</MBox>
			...
			<MBox
				...
				footer={actionFooter(
					...
					"Upload & Validate",
					"btnPmP",
					"478 settlements uploaded and validated. Ready for submission.",
				)}
			>
				{actionBody(
					"batchUploadModal",
					<>
						...
						<div className={styles.summaryBoxInfo} style={{ fontSize: 12 }}>
							<i className="fa-solid fa-circle-info me-1" /> Template: Bank
							Code, Amount, Purpose, Reference. Max 500 rows per upload.
						</div>
					</>,
				)}
			</MBox>
			...
			<MBox
				...
				title={
					<>
						<i className="fa-solid fa-arrows-left-right me-2" />
				...
				footer={actionFooter(
					...
					"Execute Transfer",
					"btnPmP",
					"Internal transfer of USD 500,000 executed.",
					"NT-20250627-001",
				)}
			>
				{actionBody(
			...
			<MBox
				...
				title={
					<>
						<i
							className="fa-solid fa-plug me-2"
				...
				footer={actionFooter(
					...
					"Save & Connect",
					"btnPmP",
					"Paymo connected successfully. Webhook verified.",
					"PYM-KEY-8841",
				)}
			>
				{actionBody(
					...
					<>
						...
						<div className="mb-3">
							<label className={styles.fl}>Environment</label>
							<div className={styles.pills}>
								{["Sandbox", "Production"].map((e) => (
									<button
										key={e}
										className={`${styles.pill} ${apiEnv === e ? styles.pillActive : ""}`}
										onClick={() => setApiEnv(e)}
									>
										{e}
									</button>
						...
						<div className={styles.summaryBoxInfo} style={{ fontSize: 12 }}>
							<i className="fa-solid fa-shield-halved me-1" /> Connecting
							enables: collecting customer payments, automated payouts, refunds
							and float rebalancing.
						</div>
					</>,
				)}
			</MBox>
						...
						<>
							<div className={`${styles.pills} mb-3`}>
								{MODAL_BIZ.map((b) => (
									<button
										key={b.id}
										className={`${styles.pill} ${bizSel === b.id ? styles.pillActive : ""}`}
							...
							<div className={`${styles.pills} mb-3`}>
								{(["overview", "perms", "ledger"] as const).map((t) => (
									<button
										key={t}
										className={`${styles.pill} ${bizTab === t ? styles.pillActive : ""}`}
										onClick={() => setBizTab(t)}
									>
										{t === "overview"
											? "Overview"
							...
							{bizTab === "perms" && (
								<div>
									...
									<div
										...
										style={{ fontSize: 12 }}
									>
										<i className="fa-solid fa-circle-info me-1" /> {granted}/8
										granted — the business is notified by email when you request
										a permission.
									</div>
								</div>
							)}
			...
			<MBox
				...
				footer={actionFooter(
					...
					"Rebalance Now",
					"btnPmP",
					"Rebalance initiated — float credited for auto-settlement.",
					"RB-102",
				)}
			>
				{actionBody(
					...
					<>
						...
						<div className="mb-3">
							<label className={styles.fl}>Source Wallet</label>
							<select
								className={styles.fc}
								defaultValue="Business Wallet — KES 4,820,000"
							>
								<option>Business Wallet — KES 4,820,000</option>
								<option>Virtual Wallet — KES 1,240,000</option>
							</select>
						</div>
						<div className="mb-3">
							<label className={styles.fl}>Amount (KES)</label>
							<input className={styles.fc} defaultValue="3000000" />
						</div>
						<div className={styles.summaryBoxInfo} style={{ fontSize: 12 }}>
							<i className="fa-solid fa-bolt me-1" /> Auto-settle uses this
							float to pay your customers instantly. Min float for Land Buyers
							LTD: KES 3,000,000.
						</div>
					</>,
				)}
				...
				{actionBody(
					...
					<>
						...
						<div className="mb-3">
							...
							<select className={styles.fc} defaultValue="Virtual Wallet">
								<option>Virtual Wallet</option>
								<option>Business Float — Land Buyers LTD</option>
								<option>Business Float — Company 2</option>
								<option>Equity Bank • 01-2345678-0</option>
							</select>
						</div>
						<div className="row g-3 mb-3">
							<div className="col-md-7">
								<label className={styles.fl}>Amount (KES)</label>
								<input className={styles.fc} defaultValue="150000" />
							</div>
							<div className="col-md-5">
								<label className={styles.fl}>Reference</label>
								<input className={styles.fc} placeholder="e.g. float top-up" />
							</div>
						</div>
						<div className={styles.summaryBoxInfo} style={{ fontSize: 12 }}>
							<i className="fa-solid fa-circle-info me-1" /> Transfers between
							your own wallets are instant and free. Bank transfers settle in
							1–2 business days.
						</div>
					</>,
				)}
			...
			<MBox
				...
				footer={actionFooter(
					...
					"Top Up",
					"btnPmP",
					"Wallet topped up. Funds available instantly.",
					"TU-552",
				)}
			>
				{actionBody(
					...
					<>
						<div className="mb-3">
							...
							<select className={styles.fc} defaultValue="Business Wallet">
						...
						<div className="mb-3">
							<label className={styles.fl}>Funds From</label>
							<select className={styles.fc} defaultValue="Linked Bank Account">
								<option>Linked Bank Account</option>
								<option>Corporate Card •••• 4421</option>
							</select>
						</div>
						<div className={styles.summaryBoxAccent} style={{ fontSize: 13 }}>
							<i className="fa-solid fa-bolt me-1" /> Available immediately for
							payouts and float rebalancing.
						</div>
					</>,
				)}
			</MBox>
			...
			>
				{actionBody(
					...
					<>
						<div className="mb-3">
							<label className={styles.fl}>Business</label>
							<select className={styles.fc} defaultValue="Company 2">
								<option>Company 2</option>
								<option>Land Buyers LTD</option>
							</select>
						</div>
						<div className="row g-3 mb-3">
							<div className="col-md-6">
								...
								<input className={styles.fc} placeholder="e.g. J. Otieno" />
							</div>
							<div className="col-md-6">
								<label className={styles.fl}>Transaction Ref</label>
								<input className={styles.fc} placeholder="e.g. ORD-8890" />
							</div>
						</div>
						<div className="row g-3 mb-3">
							<div className="col-md-6">
								<label className={styles.fl}>Amount (KES)</label>
								<input className={styles.fc} defaultValue="12400" />
							</div>
							<div className="col-md-6">
								<label className={styles.fl}>Reason</label>
								<select
									className={styles.fc}
									defaultValue="Customer returned item"
								>
						...
						<div className={styles.summaryBoxWarn} style={{ fontSize: 12 }}>
							<i className="fa-solid fa-circle-info me-1" /> Refunds over KES
							5,000 require a second approver for this business.
						</div>
					</>,
				)}
			</MBox>
			...
			<MBox
				...
				footer={actionFooter(
					...
					"Schedule Payout",
					"btnPmP",
					"Payout scheduled. Receipt sent to the business.",
					"PO-9921",
				)}
			>
				{actionBody(
					...
					<>
						<div className="mb-3">
							<label className={styles.fl}>Business</label>
							<select className={styles.fc} defaultValue="Land Buyers LTD">
								<option>Land Buyers LTD — KES 5,200,000 pending</option>
								<option>Company 2 — KES 1,200,000 pending</option>
							</select>
						</div>
						<div className="mb-3">
							<label className={styles.fl}>When</label>
							<div className={styles.pills}>
								{["Now", "Today", "Weekly"].map((w) => (
									<button
										key={w}
										className={`${styles.pill} ${payoutWhen === w ? styles.pillActive : ""}`}
										onClick={() => setPayoutWhen(w)}
									>
										{w}
									</button>
								))}
						...
						<div className="row g-3 mb-3">
							<div className="col-md-6">
								<label className={styles.fl}>Amount (KES)</label>
								<input className={styles.fc} defaultValue="5200000" />
							</div>
							<div className="col-md-6">
								<label className={styles.fl}>Settlement Account</label>
								<select
									className={styles.fc}
									defaultValue="Equity Bank • 01-2345678-0"
								>
									<option>Equity Bank • 01-2345678-0</option>
									<option>PayMo Wallet • BIZ-88213</option>
								</select>
							</div>
						</div>
						<div className={styles.summaryBoxAccent} style={{ fontSize: 13 }}>
