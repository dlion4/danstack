...
const initialMockData: PaymentRailsContent = {
	...
	attention: [
		{
			icon: "bi-bank",
			tone: "danger",
			title: "Equity Bank API health degraded",
			sub: "Last sync: 47 min ago",
			action: "Investigate",
			modal: "bankHealthModal",
		},
		{
			icon: "bi-exclamation-triangle",
			tone: "warn",
			title: "RTGS cut-off in 42 minutes",
			sub: "KES 12.4M queued for today",
			action: "Manage",
		...
		},
		{
			icon: "bi-link-45deg",
			tone: "info",
			title: "SWIFT credential expires in 5 days",
			sub: "Certificate renewal required",
			action: "Renew",
			modal: "railConfigModal",
		},
	],
	suggestions: [
		{
			...
			tone: "warn",
			title: "Route KES 8.2M to PesaLink",
			sub: "Save KES 4,100 vs RTGS today",
			action: "Apply",
		...
		},
		{
			icon: "bi-graph-up",
			tone: "info",
			title: "A/B test new ACH rule",
			sub: "Projected 0.8% cost reduction",
			action: "Start Test",
			modal: "abTestModal",
		},
		{
			icon: "bi-currency-exchange",
			tone: "warn",
			title: "Rebalance USD nostro position",
			sub: "Current FX exposure: +$142K",
			action: "Rebalance",
			modal: "nostroModal",
		},
	],
	...
	audit: [
		...
		},
		{
			...
			target: "Equity Bank API",
		},
		{
			time: "26 Jun 16:05",
			actor: "a.kamau",
			action: "Updated routing rule",
			target: "High-Value Instant",
		},
		{
			time: "26 Jun 11:30",
			actor: "f.otieno",
			action: "Rebalanced nostro",
			target: "Nostro USD",
		},
	],
};
				...
				<div className={s.pageBar}>
					...
					<div className="d-flex flex-wrap" style={{ gap: 8 }}>
						...
						<button
							type="button"
							className={cx(s.btn, s.btnSm)}
							onClick={() => openModal("railConfigModal")}
						>
						...
						>
							...
							<div className="d-flex flex-wrap mt-3" style={{ gap: 8 }}>
								...
								<button
									type="button"
									className={cx(s.btn, s.btnSm, s.btnGlassOnAccent)}
									onClick={() => openModal("routingRulesModal")}
								>
									Rules
								</button>
								<button
					...
					<div className={s.sectionHead}>
						<div>
							...
							<p className={s.sectionSub}>
								Real-time API health, supported rails, settlement windows and
								limits.
							</p>
						</div>
						<div className="d-flex" style={{ gap: 8 }}>
							...
							<button
								type="button"
								className={cx(s.btn, s.btnSm)}
								onClick={() => openModal("bankHealthModal")}
							>
							...
							<tbody>
								{c.banks.map((b) => (
									<tr key={b.name}>
										...
										<td>
											<span
												className={cx(s.badge, toneBadge[healthTone[b.health]])}
											>
								...
								<div className={s.rowItem} key={r.name}>
									<div>
										<div className={s.rowTitle}>
											{r.name}{" "}
											<span className={cx(s.badge, s.badgeOutline)}>
												P{r.priority}
											</span>
										</div>
									...
									<span
										className={cx(
											s.badge,
											r.status === "Active" ? s.badgeSuccess : s.badgeWarn,
										)}
									>
										{r.status}
									</span>
								...
								<div className={s.rowItem} key={r.name}>
									...
									<div className="d-flex align-items-center" style={{ gap: 8 }}>
										<span
											className={cx(
												s.badge,
												r.enabled ? s.badgeSuccess : s.badgeOutline,
											)}
										>
				...
				<div className={s.card}>
					<div className={s.sectionHead}>
						<div>
							...
							<p className={s.sectionSub}>
								Volume, success rate, average settlement time and cost per rail.
							</p>
						</div>
						<div className="d-flex" style={{ gap: 8 }}>
							<button
								type="button"
								className={cx(s.btn, s.btnSm)}
								onClick={() => openModal("performanceModal")}
							>
							...
							<div className={s.sectionHead}>
								...
								<div className="d-flex" style={{ gap: 8 }}>
									...
									<button
										type="button"
										className={cx(s.btn, s.btnSm)}
										onClick={() => openModal("fxRebalanceModal")}
									>
								...
								<div className={s.rowItem} key={n.name}>
									...
									<div className="text-end">
										...
										<span
											className={cx(
												s.badge,
												n.status === "Matched" ? s.badgeSuccess : s.badgeWarn,
											)}
										>
											{n.status}
										</span>
