					...
					{data.heroStats.map((stat: any) => {
						...
						return (
							<div key={stat.id} className={colClass}>
								<div
									className={`${styles.card} ${stat.isAccent ? styles.cardAccent : ""}`}
									style={{
										minHeight: 170,
										...(stat.borderLeft
								...
								>
									...
									<div
										...
										style={{
											margin: "8px 0",
											color: stat.isAccent ? "#fff" : "var(--ink-900)",
										}}
									>
										{stat.value}
									</div>
									{stat.badge && (
										<span
											className={`${styles.badge} ${stat.badge.variant === "success" ? styles.badgeSuccess : stat.badge.variant === "warning" ? styles.badgeWarning : styles.badgeInfo}`}
										>
											<i
												className={`bi ${stat.badge.variant === "success" ? "bi-check-circle" : stat.badge.variant === "warning" ? "bi-clock" : "bi-lightning"}`}
											></i>{" "}
											{stat.badge.text}
										</span>
									)}
							...
							{activeType === "bulk" && (
								<div className={styles.quickGrid}>
									...
									<button
										...
										onClick={() => openModal("bulkUploadModal")}
									>
										<i className="bi bi-file-earmark-spreadsheet me-1"></i> ISO
										20022
									</button>
								</div>
							)}
