...
function Page({ onNavigate }: { onNavigate?: (p: string) => void }) {
  ...
  return (
    <>
      <Sidebar open={sideOpen} onClose={() => setSideOpen(false)} onNavigate={onNavigate} current="inventory" data={store} brandSub="Page 8 · Inventory &amp; Stock" />
      <div className="pm-main">
        <Topbar onMenu={() => setSideOpen(true)} current="inventory" data={store} searchId="inv-search" searchPlaceholder="Search SKU, batch, PO…" />
      ...
      <QuickBar actions={[
        { icon: "bi-clipboard-check", label: "Stock Count", primary: true, onClick: () => store.openModal("countWizard") },
        { icon: "bi-box-arrow-in-down", label: "Receive PO", onClick: () => store.openModal("receiveWizard") },
        { icon: "bi-arrow-left-right", label: "Transfer", onClick: () => store.openModal("transferWizard") },
        { icon: "bi-sliders", label: "Adjust", onClick: () => store.openModal("adjustmentWizard") },
        { icon: "bi-cart-plus", label: "Reorder", onClick: () => store.openModal("reorderWizard") },
        { icon: "bi-download", label: "Export", onClick: () => store.openModal("exportLedger") },
