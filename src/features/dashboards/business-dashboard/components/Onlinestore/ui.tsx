...
export const toneOf: Record<string, string> = {
  ...
  "Pending inspection": "amber", Approved: "blue", Quarantined: "violet", Restocked: "green", Refunded: "slate", Destroyed: "red",
...
export function StatusBadge({ status }: { status: string }) {
  const icon =
    status === "Active" || status === "Received" || status === "Restocked" || status === "Completed" || status === "Found" ? "bi-check-circle-fill" :
    status === "Draft" ? "bi-pencil-fill" :
    status === "Archived" || status === "Closed" ? "bi-archive-fill" :
    status === "Sent" || status === "Approved" ? "bi-send-check-fill" :
    status === "Partial" || status === "Counting" ? "bi-hourglass-split" :
    status === "Pending inspection" ? "bi-search" :
