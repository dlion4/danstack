...
const initialMockData: RegistrationConfig = {
	...
	brandFeatures: [
		{
			...
			sub: "256-bit encryption · PCI DSS L1 · SOC 2",
		},
		{
		...
		},
		{
			...
			title: "25+ markets, 40+ currencies",
			sub: "One account, borderless reach",
		},
	],
	...
	accountTypes: [
		...
		},
		{
			...
			badge: { label: "Free setup", kind: "badgeOk" },
		...
		},
		{
			...
			badge: { label: "Free API credits", kind: "badgeGold" },
		...
		personal: [
			{
				...
				s: "Fund via bank, card, or mobile money",
			},
			{
				ic: "bi-receipt",
				c: "#9ccafc",
				t: "Set up your first bill payment",
				s: "Electricity, airtime, subscriptions",
			},
			{
				ic: "bi-people",
				c: "#fcd34d",
				t: "Invite friends and earn",
				s: "Get rewards for every referral",
			},
		],
		business: [
			{
				ic: "bi-building-check",
				c: "#9ff5cd",
				t: "Complete your business profile",
				s: "Unlock higher limits",
			},
			{
				ic: "bi-person-plus",
				c: "#9ccafc",
				t: "Add your first team member",
				s: "Assign roles & permissions",
			},
			{
				ic: "bi-link-45deg",
				c: "#fcd34d",
				t: "Create a payment link",
				s: "Start accepting payments instantly",
			},
		],
		developer: [
			{
				ic: "bi-book",
				c: "#9ff5cd",
				t: "Explore the API docs",
				s: "REST, GraphQL & 10+ SDKs",
			},
			{
				ic: "bi-discord",
				c: "#9ccafc",
				t: "Join the Discord community",
				s: "2,400+ developers",
			},
			{
				ic: "bi-rocket",
				c: "#fcd34d",
				t: "Build your first integration",
				s: "Make an API call in 30 seconds",
			},
		],
	},
...
const PANEL_COPY: Record<AccountType, { title: string; sub: string }> = {
	...
	business: {
		...
		sub: "Set up your business profile.",
	},
	developer: {
		title: "Create your developer account",
		sub: "Set up your developer profile.",
	},
};
...
export default function Register() {
	...
	const [selfieState, setSelfieState] = useState<"idle" | "capturing" | "done">(
		"idle",
	);
	...
	const securityReady =
		pwdScore >= 2 && pwdMatch && pinComplete && agreeTerms && agreeAge;
	...
	const panelHeading = (() => {
		if (step === 0)
			return {
				...
				sub: "Choose the account type that fits you best.",
			};
		if (step === 1 && acctType) return PANEL_COPY[acctType];
		if (step === 2)
			return {
				title: "Verify your identity",
				sub: "Required for secure financial services.",
			};
		if (step === 3)
			return {
				title: "Secure your account",
				sub: "Set up how you'll sign in.",
			};
		...
		for (let i = 0; i < 60; i += 1) {
			...
			piece.style.background =
				content.confettiColors[
					Math.floor(Math.random() * content.confettiColors.length)
				];
	...
	const successSteps = acctType
		? content.nextSteps[acctType]
		: content.nextSteps.personal;
			...
			<div className={s.authWrap}>
				...
				<div className={s.authBrand}>
					...
					<div>
						...
						<span className={cx(s.pill, "mb-3")}>
							<span className={s.pillDot} /> {content.banner.pill}
						</span>
						<h1 className={cx(s.brandTitle, "mt-3")}>
							{content.banner.titleTop}
							<span className={s.textGradient}>
								{content.banner.titleAccent}
							</span>
						</h1>
						<p
							className={cx(s.mutedText, "mt-3")}
						...
						{content.brandFeatures.map((feat) => (
							<div className={s.brandFeat} key={feat.title}>
								...
								<div>
									...
									<div className={s.dimText} style={{ fontSize: "0.82rem" }}>
										{feat.sub}
									</div>
								</div>
							</div>
							...
							{content.brandStats.map((stat) => (
								<div className="col-4" key={stat.label}>
									<div className={s.statChip}>
										<div
											className={cx(s.textGradient, "fw-bold")}
											style={{ fontSize: "1.2rem" }}
										>
											{stat.value}
										</div>
										<div className={s.dimText} style={{ fontSize: "0.65rem" }}>
											{stat.label}
										</div>
									</div>
								</div>
					...
					>
						...
						<div className="d-flex justify-content-between align-items-center mb-4 gap-3">
							<div>
								...
								<p
									className={s.dimText}
									style={{ fontSize: "0.86rem", margin: 0 }}
								>
									{panelHeading.sub}
								</p>
							</div>
						...
						<div className={cx(s.stepPane, step === 0 && s.active)}>
							<div className="mb-3">
										...
										<div className="col-6" key={provider.id}>
											...
											>
												{socialConnecting === provider.id ? (
													<>
														<span
															className={s.spinnerP}
															style={{
																borderTopColor: "#c9ffe9",
																borderColor: "rgba(201,255,233,.3)",
															}}
														/>{" "}
														Connecting…
													</>
								...
								<div className={s.dividerRow}>
									<span>or pick an account type</span>
								</div>
							</div>
									...
									<div className="col-12" key={card.id}>
										...
										>
											...
											<div className="d-flex gap-3">
												<div className={cx(s.typeIcon, s[card.tone])}>
												...
												<div>
													<div className="d-flex align-items-center gap-2 flex-wrap">
														...
														{card.badge && (
															<span
																className={cx(s.badgeMini, s[card.badge.kind])}
															>
																{card.badge.label}
															</span>
														)}
													...
													<p
														...
														style={{ fontSize: "0.84rem", margin: 0 }}
													>
														{card.text}
													</p>
													<div className="mt-2 d-flex gap-1 flex-wrap">
														{card.tags.map((tag) => (
															<span
																key={tag}
																className={cx(s.badgeMini, s.badgeNative)}
															>
																{tag}
															</span>
														))}
							...
							>
								<div className="row g-3">
									<div className="col-md-6">
										<label className={s.formLabel} htmlFor="regFirst">
											First name
										</label>
										<input
											id="regFirst"
									...
									<div className="col-md-6">
										<label className={s.formLabel} htmlFor="regLast">
											Last name
										</label>
										<input
											id="regLast"
									...
									{isBusiness && (
										<div className="col-12">
											<label className={s.formLabel} htmlFor="regBizName">
												Business name
											</label>
											<input
												id="regBizName"
									...
									<div className="col-12">
										<label className={s.formLabel} htmlFor="regEmail">
											Email address
										</label>
										<input
											id="regEmail"
											...
											className={cx(
												"form-control",
												emailOk !== null &&
													(emailOk ? s.inputValid : s.inputInvalid),
											)}
											required
										...
										/>
										{emailOk === false && (
											<div className={s.fieldErr}>
												<i className="bi bi-exclamation-circle" /> Enter a valid
												email
											</div>
										)}
										{emailOk === true && (
											<div className={s.fieldOk}>
												<i className="bi bi-check-circle" /> Looks good
											</div>
										)}
									</div>

									<div className="col-12">
										<label className={s.formLabel} htmlFor="regPhone">
											Phone number
										</label>
									...
									{!isDeveloper && (
										<div className="col-md-6">
											<label className={s.formLabel} htmlFor="regDob">
												Date of birth
											</label>
											<input
												id="regDob"
											...
											{dobError && (
												<div className={s.fieldErr}>
													<i className="bi bi-exclamation-circle" /> Must be 18
													or older
												</div>
											)}
										</div>
									...
									<div className="col-md-6">
										<label className={s.formLabel} htmlFor="regCountry">
										...
										>
											{content.countries.map((cty) => (
												<option value={cty.value} key={cty.value}>
													{cty.label}
												</option>
											))}
										</select>
									...
									{isDeveloper && (
										<>
											<div className="col-12">
												<label className={s.formLabel} htmlFor="regUseCase">
													Primary use case
												</label>
												<select
													id="regUseCase"
												...
												>
													{content.devUseCases.map((uc) => (
														<option key={uc}>{uc}</option>
													))}
												</select>
											</div>
											<div className="col-12">
													...
													{content.devLanguages.map((lang) => {
														...
														return (
															<button
																...
																className={cx(
																	s.badgeMini,
																	selected ? s.badgeOk : s.badgeNative,
																)}
																style={{
																	cursor: "pointer",
									...
									{isBusiness && (
										<>
											<div className="col-md-6">
												<label className={s.formLabel} htmlFor="regBizType">
													Business type
												</label>
												<select
													id="regBizType"
												...
												>
													{content.businessTypes.map((bt) => (
														<option key={bt}>{bt}</option>
													))}
												</select>
											</div>
											<div className="col-md-6">
												<label className={s.formLabel} htmlFor="regVolume">
													Est. monthly volume
												</label>
												<select
													id="regVolume"
												...
												>
													{content.monthlyVolumes.map((vol) => (
														<option key={vol}>{vol}</option>
													))}
												</select>
											</div>
										</>
									...
									<div className="col-12">
										<label className={s.formLabel} htmlFor="regRef">
								...
								<div className="d-flex gap-2 mt-4">
									...
									<button
										type="submit"
										className={cx(s.btnPaymo, "flex-grow-1")}
										disabled={!basicReady}
									>
						...
						<div className={cx(s.stepPane, step === 2 && s.active)}>
							...
							>
								<div className="d-flex align-items-center gap-2">
									...
									<span className={s.mutedText} style={{ fontSize: "0.84rem" }}>
										Your documents are encrypted and verified securely.
							...
							<div
								...
								onKeyDown={(e) => {
									if (e.key === "Enter") docInputRef.current?.click();
								}}
								onDragOver={(e) => {
									e.preventDefault();
									setDocDrag(true);
							...
							>
								...
								>
									{docFile ? (
										...
										<>
											Drag &amp; drop or{" "}
											<span style={{ color: "#2ee6a0" }}>browse</span>
										</>
									)}
								</div>
								<div
									className={s.dimText}
									style={{ fontSize: "0.74rem", marginTop: "0.2rem" }}
								>
								...
								<>
									...
									<div
										className={cx(
											s.uploadZone,
											"mb-3",
											bizFileCount > 0 && s.hasFile,
										)}
										role="button"
										tabIndex={0}
										onClick={() => bizInputRef.current?.click()}
										onKeyDown={(e) => {
											if (e.key === "Enter") bizInputRef.current?.click();
										}}
									>
										<i
											className={cx("bi bi-files", s.uploadIconAlt)}
											style={{ fontSize: "1.6rem" }}
										/>
										<div
											className={s.mutedText}
											style={{ fontSize: "0.88rem", marginTop: "0.4rem" }}
										>
											{bizFileCount > 0 ? (
												<>
													<i
														...
														style={{ color: "#86efac" }}
													/>{" "}
													{bizFileCount} document(s) attached
												</>
											) : (
							...
							<div
								className={cx(
									s.uploadZone,
									"mb-3",
									selfieState === "done" && s.hasFile,
								)}
								role="button"
								tabIndex={0}
								onClick={handleSelfieCapture}
								onKeyDown={(e) => {
									if (e.key === "Enter") handleSelfieCapture();
								}}
							>
								<i
									className={cx("bi bi-person-bounding-box", s.uploadIconAlt)}
								/>
								<div
									className={s.mutedText}
									style={{ fontSize: "0.9rem", marginTop: "0.5rem" }}
								...
								<div
									...
									style={{ fontSize: "0.74rem", marginTop: "0.2rem" }}
								>
									Blink and turn your head when prompted
								</div>
							</div>
							...
							{verifying && (
								<div
									className={cx(s.glass, "p-4 text-center mb-3")}
									style={{ borderRadius: "14px" }}
								>
									<div className={cx(s.verifyAnim, "mb-3")} />
									<div style={{ fontWeight: 600, color: "#fff" }}>
										{content.verifyStages[verifyStage]}
									</div>
									<div
										className={s.dimText}
						...
						<div className={cx(s.stepPane, step === 3 && s.active)}>
							<form onSubmit={handleCreate} noValidate>
								<label className={s.formLabel} htmlFor="regPwd">
									Create password
								</label>
								...
								<div className={s.strengthBar}>
									{[0, 1, 2, 3].map((i) => (
										<div
											...
											style={
												pwd.length > 0 && i < pwdScore
													? { background: STRENGTH_COLORS[pwdScore - 1] }
													: undefined
											}
										/>
									))}
								...
								<div className="d-flex justify-content-between mt-1">
									<span className={s.dimText} style={{ fontSize: "0.74rem" }}>
									...
									<span
										style={{
											...
											fontWeight: 600,
											color: pwd.length
												? STRENGTH_COLORS[Math.max(0, pwdScore - 1)]
												: undefined,
										}}
									>
										{pwd.length
											? STRENGTH_LABELS[Math.max(0, pwdScore - 1)]
											: ""}
									</span>
								</div>

								<label className={s.formLabel} htmlFor="regPwd2">
									Confirm password
								</label>
								<input
									id="regPwd2"
									...
									className={cx(
										"form-control",
										pwd2Touched && (pwdMatch ? s.inputValid : s.inputInvalid),
									)}
									placeholder="Re-enter password"
									value={pwd2}
									onChange={(e) => setPwd2Raw(e.target.value)}
								/>
								{pwd2Touched && !pwdMatch && (
									<div className={s.fieldErr}>
										<i className="bi bi-exclamation-circle" /> Passwords
										don&apos;t match
									</div>
								)}
								...
								<div className="mt-4">
									<div className={cx(s.secCard, "mb-2", passkeyOn && s.on)}>
										...
										<div style={{ flex: 1 }}>
											...
											>
												Set up a Passkey{" "}
												<span className={cx(s.badgeMini, s.badgeOk, "ms-1")}>
													Recommended
												</span>
											</div>
											<div
											...
											>
												Faster, phishing-resistant login with Face ID / Touch ID
											</div>
										</div>
										<label className={s.toggleSwitch}>
											<input
												type="checkbox"
												checked={passkeyOn}
												onChange={(e) => setPasskeyOn(e.target.checked)}
											/>
											<span className={s.toggleSlider} />
										</label>
									</div>
									<div className={cx(s.secCard, bioOn && s.on)}>
										<div className={cx(s.secIcon, s.secIconAlt)}>
										...
										<div style={{ flex: 1 }}>
											...
											<div
												...
												style={{ fontSize: "0.78rem" }}
											>
												Use your device fingerprint or face unlock
											</div>
										</div>
										<label className={s.toggleSwitch}>
											<input
												type="checkbox"
												checked={bioOn}
												onChange={(e) => setBioOn(e.target.checked)}
											/>
											<span className={s.toggleSlider} />
										</label>
									</div>
								...
								<div className="d-flex gap-2 mt-3">
									...
									<button
										type="submit"
										className={cx(s.btnPaymo, "flex-grow-1")}
										disabled={!securityReady || creating}
									>
										{creating ? (
											<>
												<span className={s.spinnerP} /> Creating your account…
											</>
										) : (
											"Create account"
										)}
								...
								<p
									className={cx(s.dimText, "text-center mt-2")}
									style={{ fontSize: "0.72rem" }}
								>
									<i className="bi bi-shield-lock me-1" /> Protected by
									reCAPTCHA · Zero-knowledge encryption
								</p>
							</form>
						</div>
						...
						<div className={cx(s.stepPane, step === 4 && s.active)}>
							<div className="text-center">
								...
								<h2
									...
									className={s.headlineColor}
								>
									Welcome to Paymo,{" "}
									<span className={s.textGradient}>{first || "there"}</span>! 🎉
								</h2>
								<p
									className={s.mutedText}
									style={{ maxWidth: "400px", margin: "0.5rem auto 0" }}
								>
									Your account is ready. We&apos;ve sent a verification email —
									confirm it to unlock all features.
								</p>
							</div>
							...
							{isDeveloper && (
								...
								>
									<div className="d-flex justify-content-between align-items-center mb-2 flex-wrap gap-2">
										...
										<span className={cx(s.badgeMini, s.badgeGold)}>
											Copy now — shown once
										</span>
									</div>
									<div className="input-group">
							...
							<div className="mt-4">
								<div className={cx(s.formLabel, "mb-2")}>
								...
								<div className="d-flex flex-column gap-2">
									{successSteps.map((nextStep) => (
										<button
											type="button"
											className={s.nextStepCard}
											key={nextStep.t}
										>
											<div
												className={s.nextStepIcon}
												style={{ color: nextStep.c }}
											>
							...
							>
								{dashLoading ? (
									<>
										<span className={s.spinnerP} /> Loading…
									</>
								) : (
									<>
