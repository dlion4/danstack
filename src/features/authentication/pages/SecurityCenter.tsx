	...
	alerts: [
		{
			...
			desc: "Notify when your account is accessed from a new device.",
			channels: ["Email", "Push"],
			enabled: true,
		},
		{
			...
			desc: "Immediate alert when password or PIN changes.",
		...
		},
		{
			...
			desc: "Alert for transactions above your custom threshold.",
			channels: ["Push"],
			enabled: true,
			threshold: 10000,
		},
		{
			...
			desc: "Notify admins when API credentials change.",
			channels: ["Email"],
			enabled: true,
		},
		{
			...
			desc: "Warn after repeated failed attempts.",
			channels: ["Email"],
			enabled: false,
		},
		{
			...
			desc: "High priority fraud and risk engine alerts.",
			channels: ["SMS", "Push"],
			enabled: true,
		},
	],
	...
	recommendations: [
		{
			key: "twofa",
			title: "Enable 2FA",
			desc: "Require an additional factor for logins and sensitive actions.",
		...
		},
		{
			...
			desc: "Use phishing-resistant authentication on supported devices.",
		...
		},
		{
			...
			desc: "Confirm devices, alerts, apps, and recovery options.",
			link: "Start review",
			completes: "review",
		},
		{
			...
			desc: "Refresh recovery questions and answers.",
			link: "Update now",
		},
		{
			key: "recovery",
			title: "Generate new recovery codes",
			desc: "Keep fresh backup codes offline.",
			link: "Generate codes",
		},
		{
			key: "apps",
			title: "Review connected apps",
			desc: "Remove stale or excessive OAuth permissions.",
			link: "Review apps",
		},
	],
...
export default function SecurityCenter() {
	...
	const [autoLogout, setAutoLogout] = useState(
		content.autoLogoutOptions.find((o) => o.selected)?.value ??
			content.autoLogoutOptions[0].value,
	);
	...
	const historyRows = content.history.filter(
		(r) =>
			(historyStatus === "all" || r[6] === historyStatus) &&
			(historyLocation === "all" || r[3] === historyLocation),
	);
	...
	const toggleTrust = (id: number) => {
		setSessions((prev) =>
			prev.map((x) => (x.id === id ? { ...x, trusted: !x.trusted } : x)),
		);
	...
	const commitRename = () => {
		if (renamingId !== null && renameDraft.trim()) {
			setSessions((prev) =>
				prev.map((x) =>
					x.id === renamingId ? { ...x, device: renameDraft.trim() } : x,
				),
			);
	...
	const completeRecommendation = (rec: Recommendation) => {
		...
		if (rec.completes) {
			setComponents((prev) =>
				prev.map((c) => (c.key === rec.completes ? { ...c, done: true } : c)),
			);
		}
	...
	const applyEmergency = (action: "lock" | "freeze" | "panic") => {
		if (action === "lock")
			showEmergency(
				"Account locked for 24 hours. Identity verification required to unlock.",
			);
		if (action === "freeze")
			showEmergency("Money movement frozen. Fraud review started.");
		if (action === "panic") {
			setSessions((prev) => prev.filter((x) => x.current));
			...
			<div className={s.shell}>
				<div className="row g-4 align-items-start">
							...
							<div className={s.sideContent}>
								<div>
									...
									<span className={cx(s.pill, "mb-3")}>
										<span className={s.pillDot} /> {content.brand.pill}
									</span>
									<h1 className={cx(s.sideTitle, "mb-3")}>
										{content.brand.titlePre}
										<span className={s.textGradient}>
											{content.brand.titleAccent}
										</span>
									</h1>
									<p
										className={cx(s.mutedText, "mb-4")}
									...
									<div className={cx(s.flowFrame, "mb-4")}>
										<img
											src="/assets/flow-3d.jpg"
											alt="Security telemetry flow"
										/>
										<div className={s.flowCaption}>
											<span className={cx(s.badgeMini, s.badgeNative)}>
												Live risk engine
											</span>
											<div className="fw-bold text-white mt-1">
												Device, location, API and login telemetry
											</div>
										</div>
									</div>
								</div>
											...
											<div className={cx(s.glass, "p-3")}>
												<div
													className={cx(
														s.mono,
														"fw-bold",
														stat.gold ? s.textGradientGold : s.textGradient,
													)}
												>
													{stat.value}
												</div>
					...
					<section className="col-lg-8">
						...
						<div className={cx(s.glassStrong, "p-3 p-md-4 mb-4")}>
							<div className="d-flex flex-column flex-md-row justify-content-between align-items-md-start gap-3 mb-4">
								<div>
									...
									<p className={cx(s.mutedText, "mb-0")}>
										Monitor your account security and manage every active
										session.
									</p>
								</div>
								<div className="d-flex flex-wrap gap-2">
									...
									<button
										type="button"
										className={cx(s.btnDanger, "btn btn-sm")}
										onClick={() => goPanel("emergency")}
									>
										...
										{components.map((c) => (
											<div className="col-sm-6" key={c.key}>
												<div
													className={cx(
														s.glass,
														"p-3 h-100 d-flex align-items-center justify-content-between",
													)}
							...
							<div
								className={cx(s.notice, "mt-3", notice && s.show)}
								role="status"
							>
							...
							<div className={cx(s.glass, "p-3 p-md-4 mb-4")}>
								<div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-3">
									<div>
										...
										<p
											className={cx(s.mutedText, "mb-0")}
											style={{ fontSize: "0.92rem" }}
										>
											Review devices currently signed in and revoke anything
											unfamiliar.
										</p>
									</div>
									<div className="d-flex gap-2 flex-wrap">
										<select
											...
											onChange={(e) => {
												...
												showNotice(
													`Auto-logout updated to ${e.target.options[e.target.selectedIndex].text.replace("Auto logout: ", "")}.`,
												);
											}}
										...
										>
											<i className="bi bi-box-arrow-right me-1" /> Log out all
											other devices
										</button>
									</div>
								</div>
										...
										<div className="col-md-6" key={session.id}>
											<div className={cx(s.sessionCard, "h-100")}>
												...
												<div className="d-flex gap-2 flex-wrap">
													...
													>
														<i
															className={`bi ${session.trusted ? "bi-patch-check-fill" : "bi-patch-check"} me-1`}
														/>
								...
								<div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-3">
									...
									<div className="d-flex gap-2 flex-wrap">
										...
										>
											...
											{content.historyLocations.map((loc) => (
												...
												<tr key={`${r[0]}-${r[1]}`}>
													...
													<td>
														<span
															className={cx(
																s.badgeMini,
																s[STATUS_BADGE[r[6]] ?? "badgeNative"],
															)}
														>
															{r[6]}
														</span>
								...
								<div className="d-flex justify-content-between align-items-start gap-3 mb-3">
									<div>
										...
										<p
											...
											style={{ fontSize: "0.92rem" }}
										>
											Trusted devices can use longer sessions and lighter
											step-up prompts. Limit: {content.trustedLimit} devices.
										</p>
									</div>
									<span className={cx(s.badgeMini, s.badgeNative)}>
										{trustedDevices.length} / {content.trustedLimit}
									</span>
								</div>
								<div className="row g-3">
									{trustedDevices.length > 0 ? (
										...
										<div className="col-12">
											<div
												className={cx(s.glass, "p-4 text-center")}
												style={{
													borderRadius: "14px",
													color: "var(--sc-ink-2)",
												}}
								...
								<div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-3">
									<div>
										...
										<p
											...
											style={{ fontSize: "0.92rem" }}
										>
											Customize delivery methods and thresholds for high-risk
											events.
										</p>
									</div>
									<button
													...
													<label className={s.toggle}>
														<input
															...
															onChange={(e) => {
																...
																showNotice(
																	`${rule.label} ${e.target.checked ? "enabled" : "disabled"}.`,
																);
															}}
												...
												{rule.threshold !== undefined && (
													<div className="mt-3">
														...
														<input
															...
															onBlur={() =>
																showNotice("Alert threshold updated.")
															}
														/>
													</div>
												)}
						...
						<div
							className={cx(s.panel, panel === "recommendations" && s.active)}
						>
							<div className={cx(s.glass, "p-3 p-md-4 mb-4")}>
								<div className="d-flex justify-content-between align-items-start gap-3 mb-3">
									<div>
										...
										<p
											...
											style={{ fontSize: "0.92rem" }}
										>
											Complete these actions to improve your score.
										</p>
									</div>
									<span className={cx(s.badgeMini, s.badgeSoon)}>
										{openActions} open
									</span>
								</div>
								<div className="row g-3">
									{content.recommendations.map((rec) => {
										const done = rec.completes
											? (components.find((c) => c.key === rec.completes)
													?.done ?? false)
											: false;
										return (
						...
						<div className={cx(s.panel, panel === "emergency" && s.active)}>
							<div className={cx(s.glass, "p-3 p-md-4 mb-4")}>
								<div className="d-flex align-items-start gap-3 mb-4">
									...
									<div>
										<h3 className={cx(s.h3, "mb-1")}>
											Account Lock & Emergency Freeze
										</h3>
										<p
											className={cx(s.mutedText, "mb-0")}
											style={{ fontSize: "0.92rem" }}
										>
											Use these controls if you suspect unauthorized access or
											card fraud.
										</p>
									</div>
								</div>
								<div className="row g-3">
									<div className="col-md-4">
										<div className={cx(s.alertCard, "h-100")}>
											...
											>
												Blocks all new logins and sensitive account changes.
											</p>
											<button
												type="button"
												className={cx(s.btnOutline, "btn btn-sm w-100")}
												onClick={() => applyEmergency("lock")}
											>
									...
									<div className="col-md-4">
										<div className={cx(s.alertCard, "h-100")}>
											...
											<p
												...
												style={{ fontSize: "0.86rem" }}
											>
												Stops card spend, payouts, beneficiary additions, and
												API transfers.
											</p>
											<button
												type="button"
												className={cx(s.btnDanger, "btn btn-sm w-100")}
												onClick={() => applyEmergency("freeze")}
											>
												<i className="bi bi-credit-card-2-front me-1" /> Freeze
												movement
											</button>
										</div>
									</div>
									<div className="col-md-4">
										<div className={cx(s.alertCard, "h-100")}>
											...
											<p
												...
												style={{ fontSize: "0.86rem" }}
											>
												Revokes sessions, freezes money movement, and starts a
												fraud review.
											</p>
											<button
												type="button"
												className={cx(s.btnDanger, "btn btn-sm w-100")}
												onClick={() => applyEmergency("panic")}
											>
								...
								<div
									className={cx(s.notice, "mt-3", emergencyNotice && s.show)}
									role="status"
								>
									<i className="bi bi-check-circle-fill" />
								...
								<div className="d-flex justify-content-between align-items-start gap-3 mb-3">
									<div>
										...
										<p
											...
											style={{ fontSize: "0.92rem" }}
										>
											OAuth-connected apps and their granted permissions.
										</p>
									</div>
									<span className={cx(s.badgeMini, s.badgeAdv)}>
												...
												<div className={cx(s.appCard, "h-100")}>
													...
													<div className="d-flex flex-wrap gap-1 my-3">
														{app.perms.map((perm) => (
															<span
																key={perm}
																className={cx(s.badgeMini, s.badgeAdv)}
															>
																{perm}
															</span>
														))}
