...
interface FeatureCard {
	...
	title: string;
	sub: string;
	tone: "featTone1" | "featTone2" | "featTone3";
}
		...
		features: [
			{
				...
				sub: "Sign in with Face ID, Touch ID, or Windows Hello",
				tone: "featTone1",
			},
			{
				icon: "bi-shield-lock",
				title: "Zero-knowledge architecture",
				sub: "We never store your passwords in plain text",
				tone: "featTone2",
			},
			{
				icon: "bi-graph-up-arrow",
				title: "Risk-based authentication",
				sub: "Step-up challenges only when something looks unusual",
				tone: "featTone3",
			},
		] as FeatureCard[],
		compliance: [
...
export default function Login() {
	...
	const [passkeySupported] = useState<boolean>(
	...
	const magicTimerRef = useRef<ReturnType<typeof setInterval> | undefined>(
		undefined,
	);
	...
	const switchTab = useCallback((tab: AuthTabId) => {
		setActiveTab(tab);
		try {
			localStorage.setItem("paymo_last_auth", tab);
		} catch {
			/* private mode */
		}
	}, []);
	...
	useEffect(() => {
       ...
       contextual banner via #redirect hash. */
		...
		try {
			last = localStorage.getItem("paymo_last_auth");
		} catch {
			/* noop */
		}
		try {
			name = localStorage.getItem("paymo_user_name");
		} catch {
			/* noop */
		}
		if (name) setWelcomeName(name);
		const validTabs: AuthTabId[] = [
			"passkey",
			"password",
			"pin",
			"magic",
	...
	const handlePasskey = useCallback(() => {
		...
		later(() => {
			...
			try {
				localStorage.setItem("paymo_user_name", "Amara");
			} catch {
				/* noop */
			}
	 ...
	 * ======================================================================= */
	const magicOk =
		magicEmail.length > 0 ? EMAIL_RE.test(magicEmail.trim()) : null;
	...
	const handleSwitchAccount = () => {
		...
		setWelcomeName(null);
		showToast(
			"Remembered account cleared. Sign in with a different account.",
			"ok",
		);
	};
	...
	const renderPwHint = () => {
		...
		return emailOk ? (
			<div className={cx(s.fieldHint, s.ok)}>
				<i className="bi bi-check-circle" /> Looks good
			</div>
		) : (
			<div className={cx(s.fieldHint, s.err)}>
				<i className="bi bi-exclamation-circle" /> Enter a valid email or phone
			</div>
		);
	};
	...
	const renderMagicHint = () => {
		...
		return magicOk ? (
			<div className={cx(s.fieldHint, s.ok)}>
				<i className="bi bi-check-circle" /> Valid email
			</div>
		) : (
			<div className={cx(s.fieldHint, s.err)}>
				<i className="bi bi-exclamation-circle" /> Enter a valid email
			</div>
		);
	};
	...
	const passkeyButtonContent: Record<PasskeyStatus, ReactNode> = {
		idle: (
			<>
				<i className="bi bi-fingerprint me-1" /> Sign in with Passkey
			</>
		),
		scanning: (
			<>
				<span
					className={cx(s.spinIc, "spinner-border spinner-border-sm me-2")}
				/>{" "}
	...
	const pwSubmitContent: Record<SubmitStatus, ReactNode> = {
		...
		mfa: (
			<>
				<i className="bi bi-shield-lock me-1" /> 2FA required
			</>
		),
	};
	...
	return (
		<div className={s.authPage}>
			<div className={s.authWrap}>
				...
				<div className={s.authLeft}>
					...
					<div>
						<span className={cx(s.pill, "mb-3")}>
							<span className={s.pillDot} /> {content.leftPanel.pill}
						</span>
						<h1 className={cx(s.heroTitle, "mb-3")}>
							{content.leftPanel.titleTop}
							<br />
							<span className={s.textGradient}>
								{content.leftPanel.titleAccent}
							</span>
						</h1>
						<p
						...
						>
							{content.leftPanel.features.map((feat) => (
								<div
									className={cx(s.featCard, "d-flex align-items-center gap-3")}
									key={feat.title}
								>
									...
									<div>
										...
										<div className={s.dimText} style={{ fontSize: "0.78rem" }}>
											{feat.sub}
										</div>
									</div>
								</div>
				...
				<div className={cx(s.authRight, s.fadeOnLoad)}>
					<div className="w-100" style={{ maxWidth: "440px" }}>
						{/* Mobile logo */}
						<div
							className="d-flex d-lg-none align-items-center mb-4 justify-content-center"
						...
						<div className="text-center mb-1">
							...
							<p className={s.mutedText} style={{ fontSize: "0.92rem" }}>
								Sign in to your Paymo account securely.
							</p>
						</div>
						...
						{showContextBanner && (
							<div
								className={cx(s.contextBanner, "mb-3 p-2 px-3")}
								role="status"
							>
								<i className="bi bi-info-circle me-1" /> Please sign in to
								access your <strong>Treasury Dashboard</strong>.
							</div>
						)}

						{/* Device fingerprint */}
						<div className="text-center mb-3">
							<span className={s.deviceChip}>
								<i className="bi bi-laptop" /> {deviceChip}
							</span>
						</div>
						...
						<div className={cx(s.glassStrong, s.authCard)}>
							{/* ===== PASSKEY PANEL ===== */}
							<div
								className={cx(s.authPanel, activeTab === "passkey" && s.active)}
								role="tabpanel"
							>
								<div className="text-center">
									<div
										className={cx(
											s.passkeyVisual,
											passkeyStatus === "scanning" && s.scanning,
										)}
										style={
									...
									>
										Use Face ID, Touch ID, or your security key — no password
										needed.
									</p>
									<button
									...
									{!passkeySupported && (
										...
										>
											<i className="bi bi-exclamation-triangle" /> This device
											doesn&apos;t support passkeys. Try another method.
										</div>
									)}
									<p
										className={cx(s.dimText, "mt-3 mb-0")}
							...
							>
								<form onSubmit={handlePasswordSubmit} noValidate>
									<div className="mb-3">
										<label className={s.formLabel} htmlFor="pwEmail">
											Email or Phone
										</label>
										<input
											type="text"
									...
									<div className="mb-2">
										<label className={s.formLabel} htmlFor="pwPass">
											Password
										</label>
							...
							{/* ===== PIN PANEL ===== */}
							<div
								className={cx(s.authPanel, activeTab === "pin" && s.active)}
								role="tabpanel"
							>
								<div className="text-center">
									<h5 className={s.panelTitle}>Enter your 6-digit PIN</h5>
									<p className={s.mutedText} style={{ fontSize: "0.84rem" }}>
										Quick access for trusted devices.
									</p>
									<div
										className={cx(s.pinWrap, pinError && s.error)}
										aria-live="polite"
									>
							...
							{/* ===== MAGIC LINK PANEL ===== */}
							<div
								className={cx(s.authPanel, activeTab === "magic" && s.active)}
								role="tabpanel"
							>
								{magicSentTo === null ? (
									<div>
										<h5 className={cx(s.panelTitle, "text-center")}>
											Passwordless sign in
										</h5>
										<p
											className={cx(s.mutedText, "text-center mb-3")}
											style={{ fontSize: "0.84rem" }}
										>
											Enter your email and we&apos;ll send you a secure login
											link.
										</p>
										<div className="mb-3">
											<label className={s.formLabel} htmlFor="magicEmail">
												Email address
											</label>
											<input
												type="email"
									...
									<div className={s.magicSent}>
										...
										<p
											...
											style={{ fontSize: "0.84rem", marginBottom: "1rem" }}
										>
											Check your inbox at{" "}
											<strong className={s.headlineColor}>{magicSentTo}</strong>
										...
										>
											{magicCountdown > 0 ? (
												`Resend in ${magicCountdown}s`
											) : (
												<>
													<i className="bi bi-arrow-repeat me-1" /> Resend link
												</>
											)}
										</button>
										<div className="mt-3">
							...
							{/* ===== SOCIAL PANEL ===== */}
							<div
								className={cx(s.authPanel, activeTab === "social" && s.active)}
								role="tabpanel"
							>
								<h5 className={cx(s.panelTitle, "text-center")}>
									Continue with
								...
								<p
									className={cx(s.dimText, "mt-3 mb-0 text-center")}
									style={{ fontSize: "0.74rem" }}
								>
									<i className="bi bi-shield-check me-1" /> We only access your
									name, email, and photo. We never post on your behalf.
								</p>
							</div>
						</div>
						...
						<div className={s.authFoot}>
							...
							<select
								...
								onChange={(e) =>
									showToast(
										`Language switched to ${e.target.value.trim()}`,
										"ok",
									)
								}
							>
								{content.languages.map((lang) => (
									<option key={lang} style={{ background: "#04150e" }}>
										{lang}
									</option>
								))}
							</select>
						...
						<p
							className={cx(s.dimText, "text-center")}
			...
			{/* ===== Toast (legacy #toast) ===== */}
			<div
				className={cx(s.toastPaymo, s.glassStrong, toast.visible && s.show)}
				role="status"
			>
				<i
					className={`bi ${content.toastIcons[toast.type]}`}
					style={{ color: content.toastColors[toast.type] }}
				/>
				<span>{toast.msg}</span>
			</div>
		</div>
