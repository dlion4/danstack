	...
	compareRows: [
		{
			...
			securityBadge: "badgeSoon",
			friction: "Typing + memory",
			bestUse: "Legacy backup",
		},
		{
			method: "Passkey",
			securityLabel: "Very high",
			securityBadge: "badgeOk",
			friction: "Touch or glance",
			bestUse: "Primary login",
		},
		{
			method: "Biometric",
			securityLabel: "High",
			securityBadge: "badgeNative",
			friction: "Instant unlock",
			bestUse: "Mobile app, approvals",
		},
	],
	benefitCards: [
		{
			...
			text: "Won't work on fake Paymo domains.",
		},
		{
			icon: "bi-lightning-charge",
			title: "Faster login",
			text: "No typing. Just Face ID, Touch ID, or Windows Hello.",
		},
		{
			icon: "bi-cloud-check",
			title: "Cross-device",
			text: "Sync via iCloud, Google, or password managers.",
		},
		{
			icon: "bi-eye-slash",
			title: "Private by design",
			text: "Biometrics stay on your device.",
		},
	],
	wizardSteps: [
		{
			title: "Check compatibility",
			desc: "Detect secure context, platform authenticator, and WebAuthn support.",
			icon: "bi-device-ssd",
		},
		{
			title: "Create passkey",
			desc: "Use your device security prompt or a password manager.",
			icon: "bi-fingerprint",
		},
		{
			title: "Name your passkey",
			desc: "Choose a clear label for future management.",
			icon: "bi-pencil-square",
		},
		{
			title: "Test passkey",
			desc: "Authenticate once to confirm it works.",
			icon: "bi-check2-circle",
		},
		{
			title: "Success",
			desc: "Passwordless sign-in is ready.",
			icon: "bi-stars",
		},
	],
	iosSteps: [
	...
	recoveryCards: [
		{
			...
			title: "Email + password + 2FA",
			text: "Standard fallback for low-risk recovery.",
		},
		{
			icon: "bi-shield-question",
			title: "Security questions",
			text: "Use enrolled questions plus step-up verification.",
		},
		{
			icon: "bi-person-vcard",
			title: "Identity verification",
			text: "Support-assisted review for high assurance.",
		},
	],
	policies: [
		{
			...
			text: "All admin accounts.",
			defaultOn: true,
		},
		{
			id: "noPasswordOnly",
			title: "Disable password-only",
			text: "Sensitive roles.",
			defaultOn: true,
		},
		{
			id: "bulk",
...
export default function Passkeys() {
	...
	const [wizardBusy, setWizardBusy] = useState<null | "creating" | "testing">(
		null,
	);

	const nextWizard = () =>
		setWizardIndex((i) => Math.min(i + 1, config.wizardSteps.length - 1));
	...
	const commitRename = () => {
		...
		if (name) {
			setPasskeys((prev) =>
				prev.map((p) => (p.id === modal.id ? { ...p, name } : p)),
			);
		}
		setModal(null);
	};
	...
	const qrCells = useMemo(() => {
		...
		for (let i = 0; i < 81; i++) {
			const finder =
				(i < 21 && i % 9 < 3) || (i < 27 && i % 9 > 5) || (i > 53 && i % 9 < 3);
	...
	const qrSeconds = String(Math.floor((qrLeftMs % 60_000) / 1000)).padStart(
		2,
		"0",
	);
	...
	const pairProgress =
		(Math.min(pairStep, config.pairSteps.length) / config.pairSteps.length) *
		100;

	/* ---------- enterprise policies ---------- */
	const [policyState, setPolicyState] = useState<Record<string, boolean>>(() =>
		Object.fromEntries(
			initialMockData.policies.map((p) => [p.id, p.defaultOn]),
		),
	);
		...
		download(
			...
			"name,device,created,lastUsed,status,sync\n" +
				passkeys
					.map(
						(p) =>
							`${p.name},${p.device},${p.created},${p.lastUsed},${p.status},${p.sync}`,
					)
					.join("\n"),
		);
		...
		download(
			...
			"policy,status\n" +
				config.policies
					.map(
						(p) =>
							`${p.title},${(policyState[p.id] ?? p.defaultOn) ? "on" : "off"}`,
					)
					.join("\n"),
		);
	...
	return (
		<div className={styles.passkeysPage} ref={rootRef}>
			...
			<div className={`container ${styles.authPanel}`}>
				{/* ================= HERO ================= */}
				<section
					className={`row g-4 align-items-stretch mb-4 ${styles.reveal}`}
					data-reveal
				>
					<div className="col-lg-6">
						<div className={`${styles.glassStrong} p-4 p-lg-5 h-100`}>
							...
							>
								{config.heroTitleA}{" "}
								<span className={styles.textGradient}>{config.heroTitleB}</span>
							</h1>
							<p
								className="mb-4"
								style={{
								...
								{config.heroStats.map((s) => (
									<div className="col-6 col-md-3" key={s.label}>
										<div className={`${styles.glass} p-3 h-100`}>
											<div className={`${styles.textGradient} fw-bold fs-4`}>
												{s.value}
											</div>
					...
					<div className="col-lg-6">
						<div
							className={`${styles.heroVisual} h-100 d-flex align-items-center justify-content-center p-4`}
						>
							<div className={`${styles.heroInner} text-center`}>
								...
								<div
									className={`${styles.glass} p-3 mx-auto ${styles.compatCard}`}
								>
									<div className="d-flex align-items-center justify-content-between mb-2">
										<span className={`${styles.mono} ${styles.mutedSmall}`}>
											DEVICE CHECK
										</span>
										<span
									...
									<div
										...
										style={{ fontSize: "0.88rem", color: "var(--pk-ink2)" }}
									>
										{compat === "pending" &&
				...
				{/* ================= 58.1 OVERVIEW ================= */}
				<section
					className={`${styles.glass} p-3 p-lg-4 mb-4 ${styles.reveal}`}
					data-reveal
				>
					<div className={styles.sectionLabel}>
						58.1 · Passkey &amp; Biometric Overview
					</div>
									...
									<tbody>
										{config.compareRows.map((row) => (
											<tr key={row.method}>
												...
												<td>
													<span
														className={`${styles.badgeMini} ${styles[row.securityBadge]}`}
													>
														{row.securityLabel}
													</span>
												</td>
						...
						{config.benefitCards.map((card) => (
							<div className="col-md-3" key={card.title}>
								<div className={`${styles.deviceCard} h-100`}>
									<i
										className={`bi ${card.icon} ${styles.textGradient} fs-3`}
									/>
				...
				{/* ================= 58.2 WIZARD ================= */}
				<section
					className={`row g-4 mb-4 ${styles.reveal}`}
					data-reveal
									...
									<div className="row g-2 mb-3">
										<div className="col-6">
											<div className={`${styles.glass} p-3`}>
												<div className={`${styles.mono} ${styles.mutedSmall}`}>
													WEB AUTHN
												</div>
												<div
													className={`fw-bold ${hasWebAuthn ? "text-success" : "text-warning"}`}
												>
													{hasWebAuthn === null
														? "Not checked"
														: hasWebAuthn
										...
										<div className="col-6">
											<div className={`${styles.glass} p-3`}>
												...
												<div
													className={`fw-bold ${isSecure ? "text-success" : "text-warning"}`}
												>
													{isSecure === null
														? "Not checked"
														: isSecure
								...
								<div>
									...
									<div className={`${styles.flowFrame} mb-3`}>
										<div className={styles.flowLabel}>
											...
											<div className="fw-bold text-white">
												Face ID, Touch ID, Windows Hello, or Android
												fingerprint.
											</div>
										</div>
									</div>
									<div className="d-flex flex-wrap gap-2">
										...
										>
											{wizardBusy === "creating" ? (
												...
												<>
													<i className="bi bi-fingerprint me-1" /> Create
													passkey
												</>
											)}
										</button>
							...
							{wizardIndex === 3 && (
								<div>
									...
									<div className={`${styles.glass} p-4 text-center mb-3`}>
										<i
											className={`bi bi-fingerprint ${styles.textGradient} ${styles.bigFingerprint}`}
										/>
										<h5 className="mt-2">Touch or glance to test</h5>
										<p className={styles.mutedSmall}>
											This confirms Paymo can use the new passkey.
										</p>
									</div>
									<button
										className={`btn ${styles.btnPrimaryPaymo}`}
				...
				>
					...
					<div className="row g-4 align-items-center">
						<div className="col-lg-6">
							<div
								className={`${styles.tabPanel} ${bioTab === "ios" ? styles.tabPanelActive : ""}`}
							>
							...
							<div
								className={`${styles.tabPanel} ${bioTab === "android" ? styles.tabPanelActive : ""}`}
							>
								<div className="row g-3">
						...
						<div className="col-lg-6">
							<div className={styles.flowFrame}>
								<div className={styles.flowLabel}>
									<span
										className={`${styles.badgeMini} ${styles.badgeNative} mb-1`}
									>
										Local unlock
									</span>
									<div className="fw-bold text-white d-block">
										Biometric signal never leaves your device.
									</div>
								</div>
							</div>
						</div>
				...
				>
											...
											<div className="d-flex align-items-center gap-3">
												...
												<div>
													<div className="fw-bold text-white">
														...
														{pk.status === "primary" && (
															<span
																className={`${styles.badgeMini} ${styles.badgeNative} ms-1`}
															>
																Primary
															</span>
														)}
					...
					<div className="col-lg-4">
						<div className={`${styles.glass} p-3 p-lg-4 h-100`}>
							...
							<div className="d-flex flex-column gap-3">
								{config.managementRules.map((rule) => (
									<div className="d-flex gap-2" key={rule}>
										<i
											className={`bi bi-check-circle-fill ${styles.checkRule}`}
										/>
										<span
											style={{ color: "var(--pk-ink2)", fontSize: "0.9rem" }}
										>
											{rule}
										</span>
									</div>
							...
							<button
								className={`btn ${styles.btnPrimaryPaymo} w-100`}
				...
				<section className={`row g-4 mb-4 ${styles.reveal}`} data-reveal>
					<div className="col-lg-5">
						<div
							className={`${styles.glass} p-3 p-lg-4 h-100 position-relative overflow-hidden`}
						>
							<div className={styles.sectionLabel}>
								58.5 · Cross-Device Passkeys
							</div>
							<h2 className="mb-3">
								Sign in with your phone on another device.
							</h2>
							<p style={{ color: "var(--pk-ink2)" }}>
								Scan the pairing QR from your phone, approve with Face ID or
								fingerprint, then register a new passkey for the device you're
								using.
							</p>
							<div className="d-flex flex-column gap-2 mt-3">
								<div className={styles.deviceCard}>
									<span
										className={`${styles.badgeMini} ${styles.badgeNative} me-2`}
									>
										1
									</span>
									Select "another device" on desktop.
								</div>
								<div className={styles.deviceCard}>
									<span
										className={`${styles.badgeMini} ${styles.badgeNative} me-2`}
								...
								<div className={styles.deviceCard}>
									...
									Authenticate locally and approve.
								</div>
							</div>
						</div>
					</div>
					<div className="col-lg-7">
						<div
							className={`${styles.glassStrong} p-4 h-100 text-center position-relative`}
						>
							<div className="position-relative d-inline-block mb-3">
								<div className={styles.qrBox}>
									{qrCells.map((on, i) => (
										<span
											key={i}
											className={`${styles.qrCell} ${on ? "" : styles.qrCellOff}`}
										/>
									))}
								</div>
							...
							<button
								...
								disabled={pairing}
							>
								<i className="bi bi-qr-code-scan me-1" /> Simulate phone
								approval
							</button>
							<div className={`${styles.progressThin} mt-3`}>
								<span
									className={styles.progressThinFill}
									style={{ width: `${pairProgress}%` }}
								/>
							</div>
							<div
								className="mt-2"
								style={{ fontSize: "0.85rem", color: "var(--pk-ink3)" }}
							>
								{pairStatusText}
							</div>
						</div>
				...
				>
					<div className={styles.sectionLabel}>
						58.6 · Recovery Without Passkey
					</div>
					<div className="row g-4 align-items-center">
						<div className="col-lg-5">
							<h2 className="mb-3">Keep a safe backup path.</h2>
							<p style={{ color: "var(--pk-ink2)" }}>
								If you lose all passkeys, you can still regain access with
								traditional recovery. Paymo recommends keeping at least two
				...
				{/* ================= 58.7 ENTERPRISE ================= */}
				<section
					className={`${styles.glassStrong} p-3 p-lg-4 ${styles.reveal}`}
					data-reveal
				>
					<div className="d-flex flex-wrap align-items-end justify-content-between gap-3 mb-3">
						<div>
							<div className={styles.sectionLabel}>
								58.7 · Enterprise Passkey Management
							</div>
							<h2 className="mb-0">Admin-grade enforcement.</h2>
						</div>
						<button
							className={`btn ${styles.btnOutlinePaymo}`}
							onClick={exportEnterprise}
						>
							<i className="bi bi-file-earmark-spreadsheet me-1" /> Export audit
							CSV
						</button>
					</div>
					<div className="row g-3">
						{config.policies.map((policy) => (
							<div className="col-lg-3 col-md-6" key={policy.id}>
								<div
									className={`${styles.enterpriseCard} h-100 d-flex justify-content-between gap-3`}
								>
									<div>
										<h6>{policy.title}</h6>
										<p
					...
					<div className="row g-3 mt-2">
						{config.enterpriseStats.map((s) => (
							<div className="col-md-4" key={s.label}>
								<div className={`${styles.glass} p-3`}>
									<div
										className={`${s.gold ? styles.textGradientGold : styles.textGradient} fw-bold fs-3`}
									>
										{s.value}
									</div>
									<div style={{ color: "var(--pk-ink3)", fontSize: "0.78rem" }}>
										{s.label}
									</div>
								</div>
							</div>
						...
						<div className={styles.modalBox}>
							...
							<div className="px-3 pb-3" style={{ color: "var(--pk-ink2)" }}>
								{modal.mode === "remove" ? (
									<p className="mb-0">
										Remove{" "}
										<strong>
											{passkeys.find((p) => p.id === modal.id)?.name}
										</strong>
										? You will need a password or another passkey on this
										device.
									</p>
								) : (
									<>
							...
							<div className="d-flex justify-content-end gap-2 p-3 border-0">
								...
								<button
									...
									onClick={
										modal.mode === "remove" ? commitRemove : commitRename
									}
								>
