...
interface MfaConfig {
	brand: {
		...
		title: string;
		copy: string;
		securityStrip: Array<{ icon: string; color: string; label: string }>;
	};
...
const initialMockData: MfaConfig = {
	...
	methods: [
		...
		},
		{
			...
			icon: "bi-phone",
			label: "SMS / WhatsApp",
			sub: "Send code to +254 •••••4321",
		},
		{
			id: "push",
			icon: "bi-bell",
			label: "Push approval",
			sub: "Approve from trusted phone",
		},
		{
			id: "passkey",
			icon: "bi-fingerprint",
		...
		},
		{
			...
			icon: "bi-usb-symbol",
			label: "Security key",
			sub: "YubiKey or FIDO2 hardware key",
		},
		{
			id: "recovery",
			icon: "bi-key",
		...
		items: [
			{
				...
				color: "#2ee6a0",
				text: "Use recovery codes if you lost your phone or authenticator app.",
			},
			{
				icon: "bi-phone",
				color: "#7cf5c8",
				text: "SMS and WhatsApp codes expire after 5 minutes.",
			},
			{
				icon: "bi-headset",
				color: "#fbbf24",
				text: "Support-assisted recovery requires identity verification and may take 24-48 hours.",
			},
		],
	},
...
export default function Mfa() {
	...
	const [pushStatus, setPushStatus] = useState<"idle" | "sending" | "visible">(
		"idle",
	);
				...
				<div className="container">
					<div className="row g-4 align-items-stretch">
									...
									<div>
										<div className="d-flex align-items-center gap-2 mb-4">
											...
											<div>
												...
												>
													Borderless African finance infrastructure
												</div>
											</div>
										</div>
						...
						<div className="col-lg-7">
							<section
								className={cx(s.glassStrong, "p-4 p-md-5 h-100")}
							...
							>
								{!success ? (
									<div>
										<div className="d-flex flex-wrap align-items-start justify-content-between gap-3 mb-4">
											<div>
												...
												<span className={s.pill}>
													<span className={s.pillDot} /> {content.header.pill}
												</span>
												<h2
													className="mt-3 mb-2"
													style={{
											...
											<div className="text-lg-end">
												...
												>
													Session expires in{" "}
													<span className={s.mono} style={{ color: "#fcd34d" }}>
														{mm}:{ss}
													</span>
												</div>
											</div>
												...
												<div className="col-md-6">
													<div className={s.riskRow}>
														<i
															...
															style={{ color: "var(--mf-accent-2)" }}
														/>
														<span>{content.risk.location}</span>
													</div>
												</div>
												<div className="col-md-6">
													<div className={s.riskRow}>
														<i
															...
															style={{ color: "var(--mf-accent-3)" }}
														/>
														<span>{content.risk.ipReputation}</span>
													</div>
												</div>
												<div className="col-md-6">
													<div className={s.riskRow}>
														<i
															...
															style={{ color: "var(--mf-accent-4)" }}
														/>
														<span>{content.risk.deviceFingerprint}</span>
													</div>
												</div>
											</div>
										</div>
										...
										<hr
											style={{
												borderColor: "var(--mf-glass-border)",
												margin: "1.5rem 0",
											}}
										/>
										...
										>
											{method === "totp" && (
												<div className="row g-4 align-items-center">
													<div className="col-md-5 text-center">
														<div
															className={s.totpRing}
															style={
																{ "--progress": `${totpPct}%` } as CSSProperties
															}
														>
															<span className={s.totpTime}>
																{String(totpSeconds).padStart(2, "0")}
															</span>
														</div>
													...
													<div className="col-md-7">
														...
														>
															Open Google Authenticator, Authy, Microsoft
															Authenticator, or your configured TOTP app.
														</p>
														{otpBoxes}
														<button
															type="button"
														...
														>
															Verify code{" "}
															<i className="bi bi-check-circle ms-1" />
														</button>
														<button
															type="button"
														...
														>
															<i className="bi bi-key me-1" /> Cannot access
															authenticator?
														</button>
													</div>
												</div>
												...
												<div>
													<div className="d-flex align-items-center gap-3 mb-3">
														...
														<div>
															...
															>
																Destination: {content.sms.destination}
															</div>
														</div>
													</div>
													<div
																		...
																		style={
																			channel === ch
																				? {
																						...
																						color: "#bdf5d8",
																						background: "rgba(46,230,160,0.09)",
																						minHeight: 0,
																					}
																				: { minHeight: 0 }
																		}
													...
													<button
														type="button"
														className={cx(s.btnPaymo, "w-100 mb-3")}
														onClick={sendSmsCode}
														disabled={smsSent && smsLeft > 0}
													>
												...
												<div className="row g-4 align-items-center">
														...
														<div className={s.pushDevice}>
															...
															<div className={s.pushScreen}>
																...
																>
																	...
																	>
																		{browserLabel} is requesting access from{" "}
																		{content.push.requester}.
																	</p>
																	<div className="d-grid gap-2 mt-3">
																		...
																		<button
																			...
																			onClick={() =>
																				showError(
																					"Approval denied on trusted device.",
																				)
																			}
																		>
																			Deny
													...
													<div className="col-md-7">
														...
														<p
															...
															style={{ fontSize: "0.92rem" }}
														>
															We sent a push request to{" "}
															<strong className="text-white">
																{content.push.deviceName}
															</strong>
															, last used 2 days ago in {content.push.requester}
															.
														</p>
														<div className={cx(s.inlineAlert, "mb-3")}>
															<i className="bi bi-phone-vibrate me-1" /> Open
															your Paymo mobile app and tap Approve.
														</div>
														<button
															type="button"
															className={cx(s.btnPaymo, "w-100")}
														...
														>
															{pushStatus === "idle" ? (
																<>
																	<i className="bi bi-bell me-1" /> Send push
																	request
																</>
															) : (
																<>
														...
														>
															{pushStatus === "idle" &&
																"Waiting to send approval request."}
															{pushStatus === "sending" && (
															...
															{pushStatus === "visible" && (
																<span style={{ color: "#86efac" }}>
																	Request visible on trusted phone. Use the
																	phone preview to approve.
																</span>
															)}
														</div>
													</div>
											...
											{method === "passkey" && (
												...
												>
													<div
														className={cx(s.methodIcon, "mx-auto")}
														style={{
															width: "72px",
															height: "72px",
															borderRadius: "24px",
													...
													>
														...
														Android biometric, or a synced passkey.
													</p>
													<div
														className={cx(s.glass, "p-3 my-3")}
														style={{ borderRadius: "16px", textAlign: "left" }}
													>
														<div
															className="d-flex justify-content-between"
															style={{ fontSize: "0.85rem" }}
														>
															<span style={{ color: "var(--mf-ink-3)" }}>
																WebAuthn support
															</span>
															<span
																className={cx(
																	s.badgeMini,
																	webAuthnSupported ? s.badgeOk : s.badgeWarn,
																)}
															>
																{webAuthnSupported
																	? "Detected"
														...
														>
															...
															<span className={cx(s.textGradient, "fw-bold")}>
																{content.passkey.registeredCount}
															</span>
														</div>
													</div>
													<button
													...
													>
														{passkeyBusy ? (
															<>
																<span className="spinner-border spinner-border-sm me-2" />{" "}
																Waiting for platform prompt...
															</>
														) : (
															<>
																<i className="bi bi-fingerprint me-1" /> Verify
																with passkey
															</>
														)}
													</button>
												...
												<div className="row g-4 align-items-center">
													...
													<div className="col-md-7">
														...
														<p
															...
															style={{ fontSize: "0.92rem" }}
														>
															Insert your YubiKey, Titan key, or compatible
															FIDO2 device. Touch it when it blinks.
														</p>
														<button
															type="button"
															className={cx(s.btnPaymo, "w-100")}
															onClick={simulateHardwareKey}
															disabled={keyBusy}
														>
															{keyBusy ? (
																<>
																...
																<>
																	<i className="bi bi-usb-drive me-1" /> Start
																	security key check
																</>
															)}
														</button>
												...
												<div style={{ maxWidth: "540px", margin: "0 auto" }}>
													<div className="d-flex align-items-center gap-3 mb-3">
														...
														<div>
															...
															>
																Enter one of your saved 10-character backup
																codes.
															</div>
														</div>
													</div>
													<label
														className={s.labelPaymo}
													...
													<input
														...
														value={recoveryCode}
														onChange={(e) =>
															handleRecoveryInput(e.target.value)
														}
														onKeyDown={(e) => {
															if (
																e.key === "Enter" &&
													...
													<div className={cx(s.inlineAlert, "my-3")}>
														...
														Recovery codes are single-use. We will ask you to
														generate new codes after login.
													</div>
													<button
														type="button"
														className={cx(s.btnPaymo, "w-100")}
													...
													>
														<i className="bi bi-download me-1" /> Download new
														code template
													</button>
												</div>
											)}
										...
										<div className="mt-4 d-flex flex-wrap gap-2 justify-content-between align-items-center">
											...
											<button
												...
												disabled={accountCleared}
											>
												{accountCleared ? (
													<>
														<i className="bi bi-check-lg me-1" /> Remembered
														account cleared
													</>
												) : (
													<>
														<i className="bi bi-arrow-left me-1" /> Use
														different account
													</>
												)}
											</button>
									...
									<div className={s.successState}>
										...
										<p
											className={s.mutedText}
											style={{ maxWidth: "420px", margin: "0 auto 1.4rem" }}
										>
											Your MFA challenge was completed successfully. Paymo
											Shield is redirecting you to account selection.
										</p>
										<div
											className={cx(s.glass, "p-3 mb-3")}
											style={{
										...
										<button
											type="button"
											className={s.btnPaymo}
											onClick={continueToAccountType}
										>
											{routeReady ? (
												<>
					...
					<div className={cx("modal-content", s.modalContent)}>
						...
						<div className="modal-footer border-0">
							...
							<button
								...
								className={cx(s.btnPaymo, "btn btn-sm")}
								data-bs-dismiss="modal"
								onClick={startSupportRecovery}
							>
								Start support recovery
							</button>
						</div>
					</div>
				</div>
