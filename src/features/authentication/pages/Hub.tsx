...
const initialMockData: HubConfig = {
	...
	dashboards: [
		...
		},
		{
			...
			features: [
				"Role permissions and approval rules",
				"Login and device audit trail",
...
export default function Hub() {
	...
	const [currentFilter, setCurrentFilter] = useState<DashCategory | "all">(
		"all",
	);
	const [currentAccount, setCurrentAccount] = useState<AccountKind | "all">(
		"all",
	);
	...
	useEffect(() => {
		...
		const io = new IntersectionObserver(
			(entries) =>
				entries.forEach((e) => {
					if (e.isIntersecting) e.target.classList.add(styles.in as string);
				}),
			{ threshold: 0.12 },
		);
	...
	const filtered = useMemo(() => {
		...
		return content.dashboards.filter((d) => {
			...
			const accountOk =
				currentAccount === "all" || d.account === currentAccount;
			const queryOk =
				!q ||
				[d.title, d.desc, d.type, ...d.features, ...d.actions]
					.join(" ")
					.toLowerCase()
					.includes(q);
	...
	const selected =
		content.dashboards.find((d) => d.id === selectedId) ??
		content.dashboards[0];
	const riskBadge =
		selected.cat === "risk"
	...
	const handleRemember = () => {
		...
		showToast(
			...
			`${selected.title} will open first next time.`,
		);
	};
	...
	const handleTour = () => {
		showToast(
			"Preview tour started",
			`Highlighting the most important actions in ${selected.title}.`,
		);
	};
	...
	const handleLock = () => {
		showToast(
			"Session locked",
			"Your dashboard session is locked. Re-authentication would be required in production.",
		);
	};
	...
	const handleCreateCustom = () => {
		...
		showToast(
			...
			`${name} includes: ${activeWidgets.join(", ") || "no widgets selected"}.`,
		);
	};
	...
	return (
		<div className={s.hubPage} ref={revealRef}>
			...
			<div className="container position-relative" style={{ zIndex: 2 }}>
					...
					<div className="d-flex align-items-center gap-3">
						...
						<div>
							...
							<div className={s.brandSub}>
								Dashboard selection hub · Zero-trust routed session
							</div>
						</div>
					</div>
					<div className="d-flex flex-wrap gap-2">
						{content.accountTabs.map((tab) => (
							<button
								...
								className={cx(
									s.quickTab,
									currentAccount === tab.id && s.active,
								)}
				...
				<div className={cx(s.glassStrong, s.layoutFrame, s.reveal)}>
					...
					<section className={cx(s.heroStrip, "mb-4")}>
						<img
							src="/assets/emerald-network.jpg"
							alt="Paymo dashboard command center"
						/>
						<div className={s.heroContent}>
							<div className="row g-4 align-items-center">
								<div className="col-lg-7">
									<span className={cx(s.pill, "mb-3")}>
										<span className={s.pillDot} /> {content.hero.pill}
									</span>
					...
					<div className="row g-4">
						...
						<div className="col-xl-8">
							{/* command palette */}
							<section
								className={cx(s.glass, "p-3 p-md-4 mb-4", s.reveal)}
							...
							>
								<div className="d-flex align-items-center justify-content-between mb-3 gap-3 flex-wrap">
									...
									<button
										...
										data-bs-target="#customDashModal"
									>
										<i className="bi bi-plus-square me-1" /> Create Custom
										Dashboard
									</button>
								</div>
													...
													<div className={s.cardInner}>
														<div className="d-flex justify-content-between align-items-start mb-3">
															<span className={s.cardIcon}>
																<i className={`bi ${dash.icon}`} />
															</span>
															<span
																className={cx(s.badgeMini, s[dash.badgeClass])}
															>
																{dash.badge}
															</span>
														</div>
														...
														<div className={s.quickActions}>
															{dash.actions.map((action) => (
																<button
																	...
																	key={action}
																	onClick={(e) =>
																		handleQuickAction(e, dash, action)
																	}
																>
																	{action}
																</button>
						...
						<aside className="col-xl-4">
							<div className={s.sidePanel}>
								{/* selected route */}
								<section
									className={cx(s.selectedPanel, s.glass, "mb-3", s.reveal)}
								...
								>
									<div className="d-flex align-items-center justify-content-between mb-3 gap-2">
										...
										<span className={cx(s.badgeMini, riskBadge.cls)}>
											{riskBadge.text}
										</span>
									</div>
									<div className={cx(s.selectedPreview, "mb-3")}>
										<span className={s.previewGlyph} aria-hidden="true">
											{selected.emoji}
										</span>
									...
									>
										<i className="bi bi-box-arrow-in-right me-1" /> Go to{" "}
										{selected.title}
									</button>
									<div className="d-flex gap-2 mt-2">
										<button
											type="button"
											className={cx(s.btnOutline, "btn btn-sm flex-fill")}
								...
								{/* notifications */}
								<section
									className={cx(s.glass, "p-3 mb-3", s.reveal)}
								...
								>
									...
									{alertsCleared ? (
										<div
											className={cx(s.dimText, "text-center py-3")}
											style={{ fontSize: "0.86rem" }}
										>
											<i className="bi bi-check2-circle me-1" /> All
										...
										content.notifications.map((n) => (
											<div className={s.notificationItem} key={n.dash}>
												<span
													className={s.notificationDot}
													style={{ color: n.color, background: n.color }}
												/>
												<div>
													<strong
														style={{ color: "#fff", fontSize: "0.86rem" }}
													>
														{n.dash}
													</strong>
													<div
								...
								{/* session context */}
								<section
									className={cx(s.glass, "p-3", s.reveal)}
									aria-label="Session context"
								>
									<div className="d-flex align-items-center gap-2 mb-3">
										<i
											className="bi bi-fingerprint"
											style={{ color: "var(--hb-accent)", fontSize: "1.2rem" }}
										/>
										<h2
											className="h6 mb-0"
											style={{ color: "var(--hb-ink-0)", fontWeight: 800 }}
									...
									<div
										...
										style={{ fontSize: "0.84rem", color: "var(--hb-ink-2)" }}
									>
										<div className="d-flex justify-content-between">
											<span>Device</span>
											<strong className="text-white">{deviceText}</strong>
										</div>
										<div className="d-flex justify-content-between">
											<span>Location</span>
									...
									<div className={cx(s.loadingBar, "mt-3")}>
										<span />
									</div>
								</section>
							</div>
					...
					<div className={cx("modal-content", s.modalContent)}>
						...
						<div className="modal-body pt-0">
							<p style={{ color: "var(--hb-ink-2)", fontSize: "0.9rem" }}>
								Build a tailored workspace with widgets, scheduled exports, team
								sharing and specific KPIs.
							</p>
							<div className="row g-3">
								<div className="col-md-5">
									<label
										className={s.sectionKicker}
										htmlFor="customDashName"
									...
									<div className={s.customGrid}>
										{content.widgets.map((widget) => (
											<button
												...
												className={cx(
													s.widgetOption,
													activeWidgets.includes(widget.name) && s.active,
												)}
												onClick={() => toggleWidget(widget.name)}
											>
												<i
													className={`bi ${widget.icon} me-1`}
			...
			<div
				className={cx(s.toastish, s.glass, "p-3", toast && s.show)}
				role="status"
				aria-live="polite"
			>
				<div className="d-flex align-items-start gap-2">
					<i
						...
						style={{ color: "#86efac", fontSize: "1.15rem" }}
					/>
					<div>
						<strong className="text-white">
							{toast?.title ?? "Action completed"}
						</strong>
						<div style={{ fontSize: "0.82rem", color: "var(--hb-ink-2)" }}>
							{toast?.body ?? "Your preference was saved."}
						</div>
					</div>
				</div>
			</div>
