		...
		return (
			<>
				<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
					Describe the problem and the assistant will suggest the most likely fix before
					you open a ticket.
				</p>
				{options.map((o) => (
					<button
...
function ChatPanel() {
	const [msgs, setMsgs] = useState([
		{ who: "PayMo Support", text: "Hi John, how can we help today?", me: false },
	]);
	...
	return (
		<>
			<div
				...
				style={{ background: "var(--pm-surface-2)", height: 260, overflowY: "auto" }}
			>
				{msgs.map((mm, i) => (
					<div
					...
					>
						<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>{mm.who}</div>
						<div
							style={{
			...
			<div className="d-flex gap-2">
				...
				/>
				<button type="button" className={`${s.btnPm} ${s.btnPmP}`} onClick={send}>
					Send
				</button>
			</div>
		</>
...
export default function SupportSlasModals({ active, onClose, onOpen, data }: Props) {
	...
	return (
		<>
			...
			<MBox
				...
				title={
					<>
						<i className="bi bi-ticket-detailed" style={{ color: "var(--pm-primary)" }} />
					...
					<>
						...
						{ticketStep === 3 && (
							<>
								...
								<Chk label="Include my recent API logs automatically" defaultChecked />
							</>
						)}
						{ticketStep >= 4 && (
							<div className={s.receipt}>
								...
								<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
									Reference TIC-8901. Premium SLA applies — first response within 4 hours.
								</p>
							</div>
						)}
					</>
			...
			>
				{m.busy === "declareIncidentModal" ? (
					...
					<>
						<Stepper s={s} labels={["Severity", "Details", "Declare"]} current={incStep} />
						{incStep === 1 && (
							<div className="mb-3">
						...
						{incStep >= 3 && (
							<div className={s.receipt}>
								...
								<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
									INC-2312 raised. The on-call engineer has been paged and a war room is
									open.
								</p>
							</div>
						)}
			...
			<MBox
				...
				title={
					<>
						<i className="bi bi-arrow-up-circle" style={{ color: "var(--pm-primary)" }} />
							...
							<div className="col-md-4" key={t.key}>
								<button
									...
									className={`${s.tierCard} ${
										m.isPicked("tier", t.key) || (!m.isPicked("tier", "") && t.current)
											? s.tierCardActive
											: ""
									}`}
								...
								>
									...
									<div
										className={s.sv}
										style={{ fontSize: 18, margin: "8px 0", color: "var(--pm-primary)" }}
									>
										{t.price}
									</div>
									<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>{t.sla}</div>
									<ul
										style={{
											fontSize: 11,
			...
			<MBox
				...
				title={
					<>
						<i className="bi bi-file-earmark-bar-graph" style={{ color: "var(--pm-accent)" }} />
						Monthly SLA Report
					</>
				}
				footer={m.footer(
					...
					m.cancelAnd(
						<button
							...
							className={`${s.btnPm} ${s.btnPmP}`}
							onClick={() =>
								m.doAction("viewSlaReportModal", "Report generated and downloading...")
							}
						>
						...
						{data.slaRows.map((r) => (
							<div key={r.metric} className={s.statusRow}>
								<div style={{ minWidth: 0 }}>
									...
									<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>{r.target}</div>
								</div>
			...
			<MBox
				...
				title={
					<>
						<i className="bi bi-person-badge" style={{ color: "var(--pm-primary)" }} />
			...
			<MBox
				...
				title={
					<>
						<i className="bi bi-ladder" style={{ color: "var(--pm-warning)" }} />
						Escalation Matrix
					</>
				}
				footer={m.closeOnly()}
							...
							{data.escalation.map((e) => (
								<tr key={e.level}>
									...
									<td data-label="Response">
										<span className={`${s.badge} ${s.badgeNeutral}`}>{e.responseTime}</span>
									</td>
								</tr>
							))}
						</tbody>
			...
			<MBox
				...
				title={
					<>
						<i className="bi bi-exclamation-circle" style={{ color: "var(--pm-muted)" }} />
						SLA Exclusions
					</>
				}
				footer={m.closeOnly()}
				...
				footer={m.footer(
					...
					<>
						...
						<button
							...
							className={`${s.btnPm} ${s.btnPmP}`}
							onClick={() =>
								m.doAction("maintenanceCalendarModal", "Subscribed to maintenance alerts.")
							}
						>
							Subscribe to Alerts
						</button>
						...
						{data.maintenance.map((w) => (
							<div key={w.window} className={s.statusRow}>
								<div style={{ minWidth: 0 }}>
									...
									<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>{w.window}</div>
								</div>
				...
				footer={m.footer(
					...
					<>
						<button
							...
							onClick={() =>
								m.doAction("ticketDetailModal", "Ticket escalated to L3 Platform Engineering.")
							}
						>
							Escalate
						</button>
						...
						<button
							type="button"
							className={`${s.btnPm} ${s.btnPmP}`}
							onClick={() => m.doAction("ticketDetailModal", "Reply sent successfully.")}
						>
							Send Reply
						</button>
					</>,
			...
			>
				{m.body(
					...
					<>
						...
						<div
							className="p-3 rounded mb-3"
							style={{ background: "var(--pm-surface-2)", maxHeight: 220, overflowY: "auto" }}
						>
							{[
								["You", "About 2% of payment.success events never reach our endpoint."],
								[
									"Sarah N. (PayMo)",
									"Thanks — I can see 14 deliveries timed out at 10s. Can you confirm your endpoint's p99 response time?",
								],
								["You", "Our p99 is around 4s, so that shouldn't be the trigger."],
							].map(([who, text]) => (
						...
						<div className="mb-3">
							<Lbl s={s}>Reply</Lbl>
							<Fld s={s} as="textarea" rows={3} placeholder="Type your reply…" />
						</div>
					</>,
				)}
			</MBox>
							...
							{data.incidents.map((i) => (
								<tr key={i.id}>
									...
									<td data-label="Severity">
										<span className={`${s.badge} ${s[i.tone]}`}>{i.severity}</span>
									</td>
				...
				footer={m.footer(
					...
					<>
						...
						<button
							...
							className={`${s.btnPm} ${s.btnPmP}`}
							onClick={() =>
								m.doAction("communityForumModal", "Your question has been posted to the forum.")
							}
						>
							Ask a Question
						</button>
				...
				{m.body(
					...
					<>
						<div className={`${s.note} ${s.noteWarn} mb-3`}>
							<i className="bi bi-lock me-1" /> Shared Slack Connect channels are an
							Enterprise tier benefit.
						</div>
						<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
							Enterprise customers get a private Slack Connect channel staffed by PayMo
							engineers 24/7, with a 15-minute acknowledgement target for SEV1 issues.
						</p>
						<div className="mb-3">
			...
			<MBox
				...
				title={
					<>
						<i className="bi bi-diagram-3" style={{ color: "var(--pm-info)" }} />
				...
				{m.body(
					...
					<>
						...
						<div className="mb-3">
							...
							<Fld s={s} type="datetime-local" defaultValue="2026-07-10T10:00" />
						</div>
						<div className="mb-3">
							<Lbl s={s}>Attendees (Emails)</Lbl>
							<Fld s={s} as="textarea" rows={2} placeholder="one email per line" />
						</div>
					</>,
				)}
			</MBox>
			...
			<MBox
				...
				title={
					<>
						<i className="bi bi-calendar-check" style={{ color: "var(--pm-warning)" }} />
						Schedule QBR
					</>
				}
				footer={m.footer(
				...
				{m.body(
					...
					<>
						...
						<div className="mb-3 mt-3">
							...
							<Fld s={s} type="url" mono placeholder="https://hooks.slack.com/services/…" />
						</div>
					</>,
				)}
			</MBox>
			...
			>
				{m.body(
					...
					<>
						<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
							Our solutions engineers can build or review a bespoke integration — legacy
							core banking bridges, custom reconciliation, or high-volume batch flows.
						</p>
						<div className="mb-3">
			...
			>
				<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
					Monthly uptime is measured per API family from synthetic probes fired every 30
					seconds from three independent regions.
				</p>
				<div className={`${s.note} ${s.noteMuted} mb-3`}>
					<code>Uptime % = (Total minutes − Downtime minutes) / Total minutes × 100</code>
				</div>
			...
			<MBox
				...
				title={
					<>
						<i className="bi bi-geo-alt" style={{ color: "var(--pm-danger)" }} />
						On-Site Support
					</>
				}
				footer={m.footer(
			...
			>
				{m.body(
					...
					<>
						<div className={`${s.note} ${s.noteWarn} mb-3`}>
							<i className="bi bi-lock me-1" /> On-site engineering visits are an Enterprise
							tier benefit.
						</div>
						<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
							Enterprise customers can request an engineer on-site in Nairobi, Mombasa or
							Kisumu for go-live support, migration weekends, or incident command.
						</p>
				...
				footer={m.footer(
					...
					<>
						...
						<button
							...
							className={`${s.btnPm} ${s.btnPmP}`}
							onClick={() =>
								m.doAction("manageSupportContactsModal", "Support contacts updated.")
							}
						>
							Save Contacts
						</button>
						...
						{data.contacts.map((ct) => (
							<div key={ct.email} className={s.statusRow}>
								<div style={{ minWidth: 0 }}>
									...
									{ct.primary && (
										<span className={`${s.badge} ${s.badgeP} ms-2`}>Primary</span>
									)}
									<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
										{ct.email} · {ct.role}
									</div>
			...
			<MBox
				...
				title={
					<>
						<i className="bi bi-chat-dots" style={{ color: "var(--pm-primary)" }} />
						Live Chat Support
					</>
				}
				footer={
