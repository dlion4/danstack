...
const initialMockData: CardSecurityFraudPreventionConfig = {
  ...
  headerSub: 'Advanced AI-powered threat detection, risk assessment, and real-time fraud monitoring across all payment channels',
  searchPlaceholder: 'Search threats, incidents, compliance, security actions...',
  ...
  pageSub: 'Configure comprehensive AI threat detection and prevention systems for all card types. Manage real-time fraud alerts, configure fraud rules, and monitor transaction security across all payment networks.',
  hero: {
    score: '91/100',
  ...
  openCases: [
    ...
    { caseId: 'RPL-22019', type: 'Replacement', card: 'Visa ****4521', status: 'Awaiting dispatch', statusTone: 'badgeP', nextStep: 'Card personalized, courier pending' },
  ],
}

async function fetchCardSecurityConfig(): Promise<CardSecurityFraudPreventionConfig> {
  try {
    const res = await fetch('/api/card-security', { headers: { Accept: 'application/json' } })
...
export default function CardSecurityFraudPrevention() {
  ...
  return (
    <div className={styles.cardSecurityPage}>
      <div className={styles.main}>
        ...
        {error && !errorDismissed && (
          <div className="alert alert-danger m-3 d-flex align-items-center justify-content-between" role="alert">
            <span><i className="bi bi-exclamation-triangle me-2" />Failed to load security data. Showing cached data.</span>
          ...
          <div className="row g-3">
            <div className="col-lg-4">
              <div className={`${styles.card} ${styles.cardAccent}`} style={{ minHeight: 170 }}>
                <p style={{ margin: '0', fontSize: 12, color: 'rgba(255,255,255,.78)', textTransform: 'uppercase', fontWeight: 600 }}>Threat detection engine is live <span style={{ color: '#86efac' }}>●</span></p>
                <div className={styles.sv} style={{ margin: '8px 0', color: '#fff' }}>91/100 Protection</div>
                <p style={{ margin: '0', fontSize: 12, color: 'rgba(255,255,255,.78)' }}>Advanced AI-powered threat detection across all card portfolios with 3D Secure and behavioral analysis.</p>
                <div className="mt-3">
      ...
      <CardSecurityModals active={activeModal} onClose={() => setActiveModal(null)} onOpen={setActiveModal} config={config} />
    </div>
  )
}
