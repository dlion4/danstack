...
const CARDS_FOR_STATEMENT = [
	"All Cards",
	"Global Shopping **3841",
	"Sub Master **9021",
];
const STATEMENT_MONTHS = ["June 2025", "May 2025", "Custom Range"];
const WITHDRAW_DESTINATIONS = ["Main PayMo Wallet", "M-Pesa"];
const RENEW_ACTIONS = [
	"Extend expiry date (keep PAN)",
	"Issue new PAN (migrate subs)",
];
const DISPUTE_REASONS = [
	"I did not authorize this charge",
	"Goods/services not received",
	"Charged incorrect amount",
	"Subscription was cancelled prior",
];
const SUBS_FOR_DISPUTE = [
	"AliExpress · KES 6,240 · 25 Jun",
	"Apple Services · KES 400 · 22 Jun",
];
...
export default function VirtualDebitCardsModals({
	active,
	onClose,
	onOpen,
}: ModalsProps) {
						...
						<button
							...
							onClick={() => {
								if (step === 3) {
									doAction(
										"createCardModal",
										"Virtual Card Issued! Your new card is ready for immediate online use.",
									);
									return;
								}
			...
			>
				...
				{step === 1 && (
					<div className={styles.fstepActive}>
						...
						<div className="row g-3">
							{CARD_TYPES.map((type) => (
								<div key={type} className="col-md-6">
									...
									>
										<i
											className={`bi ${type === "Global Shopping" ? "bi-globe" : type === "Subscription Manager" ? "bi-arrow-repeat" : type === "Single-Use Burner" ? "bi-lightning-charge" : "bi-briefcase"} d-block mb-2`}
											style={{
												fontSize: 24,
												color:
										...
										<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
											{type === "Global Shopping"
												? "Multi-use, reloadable, online purchases"
												: type === "Subscription Manager"
													? "Dedicated for recurring payments"
													: type === "Single-Use Burner"
														? "Auto-deletes after one transaction"
														: "Strict limits and MCC restrictions"}
										</div>
									</div>
								</div>
							))}
						...
						<div className="mt-3">
							<label className={styles.formLabel}>Card Nickname (Alias)</label>
							<input
								className={styles.formControl}
								placeholder="e.g. Flight Bookings, Online Grocery"
							/>
						</div>
					</div>
				)}
				{step === 2 && (
					<div className={styles.fstepActive}>
						...
						>
							Funds are not locked; they are pulled from the funding source at
							the exact moment of transaction up to the specified limits.
						</div>
					</div>
				)}
				{step === 3 && (
					<div className={styles.fstepActive}>
						...
						<div className="d-flex gap-2 mb-3">
							{["vCardBg1", "vCardBg2", "vCardBg3", "vCardBg4", "vCardBg5"].map(
								(color) => (
									<div
										key={color}
										...
										style={{
											...
											...(selectedCardColor === color
												? {
														...
														boxShadow: "0 0 0 2px var(--pm-primary)",
													}
												: {}),
										}}
						...
						<div className="p-3 border rounded mb-3" style={{ fontSize: 13 }}>
							<div className="d-flex justify-content-between mb-1">
								...
								<strong>{selectedCardType || "Global Shopping"}</strong>
							</div>
							<div className="d-flex justify-content-between mb-1">
								<span className="text-muted">Funding</span>
								<strong>PayMo Main Wallet</strong>
							</div>
							<div className="d-flex justify-content-between">
								<span className="text-muted">Limit</span>
								<strong>KES 20,000 / month</strong>
							</div>
						</div>
						<label className={styles.formLabel}>
					...
					<div className={styles.fstepActive}>
						<div className={styles.receipt}>
							...
							<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
								Your new card is ready for immediate online use.
							</p>
							<div
								className={`${styles.vCard} ${styles.vCardBg1} mx-auto mt-3`}
								style={{ maxWidth: 320, textAlign: "left", cursor: "default" }}
							>
							...
							>
								<button
									className={`${styles.btnPm} ${styles.btnSm}`}
									onClick={() => onOpen("viewCardDetailsModal")}
								>
									Reveal CVV
								</button>
								<button
									className={`${styles.btnPm} ${styles.btnSm}`}
									onClick={() => onOpen("merchantLockModal")}
								>
									Lock to Merchant
								</button>
							</div>
			...
			{!cardRevealed ? (
				<>
					<p style={{ fontSize: 13, color: "var(--pm-muted)" }}>
						Enter your PayMo PIN to reveal the full card number and CVV for
				...
				<div className="text-center">
					<div
						className={`${styles.vCard} ${styles.vCardBg1} mx-auto`}
						style={{ maxWidth: 320, textAlign: "left", cursor: "default" }}
					>
						<div className={styles.vCardBadge}>ACTIVE</div>
						<div className={styles.vCardLogo}>PayMo</div>
					...
					<div className="mt-3 d-flex gap-2 justify-content-center">
						...
						<button className={`${styles.btnPm} ${styles.btnSm}`}>
							<i className="bi bi-clipboard" /> Copy CVV
						</button>
						<button
							className={`${styles.btnPm} ${styles.btnSm}`}
					...
					<p style={{ fontSize: 11, color: "var(--pm-danger)", marginTop: 16 }}>
						Details will auto-hide in 60 seconds.
					</p>
				</div>
			)}
		</MBox>
		...
		<MBox
			...
			footer={
				<>
					...
					<button
						className={`${styles.btnPm} ${styles.btnPmW}`}
						onClick={() =>
							doAction(
								"singleUseCardModal",
								"Single-Use Card Generated! The card is active for 24 hours.",
								"BRN-9921",
							)
						}
					>
		...
		>
			{renderActionBody(
				...
				<>
					...
					>
						Single-use cards automatically destroy themselves after one approved
						transaction. Perfect for sketchy websites or free trials.
					</div>
					<div className="mb-3">
						<label className={styles.formLabel}>
							Exact Purchase Amount (KES)
						</label>
						<input
							type="number"
							className={styles.formControl}
					...
					<div className="mb-3">
						<label className={styles.formLabel}>Validity Period</label>
						<select className={styles.formControl}>
		...
		<MBox
			...
			footer={
				<>
					...
					<button
						className={`${styles.btnPm} ${styles.btnPmS}`}
						onClick={() =>
							doAction(
								"topUpCardModal",
								"Card funded successfully! AWS charge should clear on next retry.",
								"",
							)
						}
					>
						Top Up KES {topUpAmount}
					</button>
				</>
			}
		>
			{renderActionBody(
				...
				<>
					...
					>
						<i className="bi bi-exclamation-triangle me-1" /> AWS is trying to
						charge KES 4,500. This card has insufficient funds.
					</div>
					<div className="mb-3">
						...
						<div className={styles.amountChips}>
							{["1000", "3000", "5000", "10000"].map((val) => (
								<span
									key={val}
									className={`${styles.amountChip} ${topUpAmount === val ? styles.active : ""}`}
									onClick={() => pickChip(val)}
								>
	...
	const renderFreezeCard = () => (
		<MBox
			...
			title={
				<>
					<i className="bi bi-snow text-info me-2" />
					Freeze Virtual Card
				</>
			}
			footer={
				<>
					...
					<button
						...
						style={{
							...
							borderColor: "var(--pm-info)",
						}}
						onClick={() =>
							doAction(
								"freezeCardModal",
								"Card frozen successfully. All new charges will be blocked.",
								"",
							)
						}
					>
		...
		>
			{renderActionBody(
				...
				<>
					<div className="mb-3">
						<label className={styles.formLabel}>Card to Freeze</label>
						<select className={styles.formControl}>
							{CARDS_FOR_FREEZE.map((c) => (
								<option key={c}>{c}</option>
							))}
						</select>
					</div>
					...
					>
						Freezing is temporary. All new transactions will be declined.
						Existing pending authorizations may still clear. You can unfreeze at
						any time.
					</div>
				</>,
			)}
				...
				<>
					...
					<button
						...
						onClick={() =>
							doAction(
								"deleteCardModal",
								"Card terminated and securely destroyed. KES 0.00 returned to wallet.",
								"",
							)
						}
					>
				...
				<>
					<div className="text-center">
						...
						<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
							...
							<strong>Global Web Shopping (**3841)</strong>.
						</p>
						<ul
						...
						>
							...
							<li>
								Any remaining dedicated balance will return to Main Wallet.
							</li>
						</ul>
			...
			footer={
				<>
					...
					<button
						className={`${styles.btnPm} ${styles.btnPmP}`}
						onClick={() =>
							doAction("editLimitsModal", "Limits updated successfully!", "")
						}
					>
						Save Limits
					</button>
	...
	const renderMerchantLock = () => (
				...
				<>
					...
					<button
						...
						onClick={() =>
							doAction(
								"merchantLockModal",
								"Card locked to NETFLIX.COM successfully.",
								"",
							)
						}
					>
		...
		>
			{renderActionBody(
				...
				<>
					<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
						Locking a card ensures it can <strong>only</strong> be charged by
						the specified merchant. Any other charge attempts will be instantly
						declined.
					</p>
					<div className="mb-3">
						<label className={styles.formLabel}>Card</label>
						<select className={styles.formControl}>
							{CARDS_FOR_MERCHANT_LOCK.map((c) => (
								<option key={c}>{c}</option>
							))}
						</select>
					</div>
					<div className="form-check form-switch mb-3">
						...
						<label className="form-check-label" style={{ fontWeight: 700 }}>
							Enable Merchant Lock
						</label>
					</div>
					<div className="mb-3">
						<label className={styles.formLabel}>Allowed Merchant Name</label>
						<input
							type="text"
							className={styles.formControl}
					...
					<div
						...
						style={{ background: "var(--pm-warning-soft)", fontSize: 12 }}
					>
						If the merchant uses multiple payment gateways with different
						billing names, locking may cause legitimate renewals to fail.
					</div>
				</>,
			)}
		</MBox>
	...
	const renderRotateCvv = () => (
			...
			footer={
				<>
					...
					<button
						className={`${styles.btnPm} ${styles.btnPmP}`}
						onClick={() =>
							doAction("rotateCvvModal", "Dynamic CVV settings saved.", "")
						}
					>
						Save Settings
					</button>
		...
		>
			{renderActionBody(
				...
				<>
					<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
						Dynamic CVV automatically changes your card's 3-digit security code
						periodically, rendering stolen card details useless.
					</p>
					<div className="mb-3">
						<label className={styles.formLabel}>Card</label>
						<select className={styles.formControl}>
					...
					<div className="mb-3">
						<label className={styles.formLabel}>Rotation Frequency</label>
						<select className={styles.formControl}>
					...
					>
						...
						<div style={{ fontSize: 10, color: "var(--pm-danger)" }}>
							Expires in 14h 22m
						</div>
					</div>
					<button className={`${styles.btnPmOutline} w-100`}>
			...
			footer={
				<>
					...
					<button
						className={`${styles.btnPm} ${styles.btnPmP}`}
						onClick={() =>
							doAction("editAliasModal", "Card alias updated.", "")
						}
					>
						Save
					</button>
		...
		<MBox
			...
			footer={
				<>
					...
					<button
						className={`${styles.btnPm} ${styles.btnPmP}`}
						onClick={() => onOpen("addSubscriptionModal")}
					>
						Add Known Subscription
					</button>
				</>
			}
		>
					...
					<tbody>
						<tr>
							...
							<td>
								<button
									className={`${styles.btnPm} ${styles.btnSm}`}
									onClick={() => onOpen("manageSingleSubModal")}
								>
									Edit
								</button>
							</td>
		...
		>
			{renderActionBody(
				...
				<>
					...
					>
						<div className="d-flex justify-content-between mb-1">
							<span className="text-muted">Avg Amount</span>
							<strong>KES 1,400</strong>
						</div>
						<div className="d-flex justify-content-between mb-1">
							<span className="text-muted">Billed On</span>
							<strong>26th of Month</strong>
						</div>
						<div className="d-flex justify-content-between">
							<span className="text-muted">Linked Card</span>
							<strong>Sub Master (**9021)</strong>
						</div>
					</div>
					<div className="mb-3">
						<label className={styles.formLabel}>Change Linked Card</label>
						<select className={styles.formControl}>
							<option>Sub Master (**9021)</option>
							<option>Global Shopping (**3841)</option>
						</select>
						<div
							style={{ fontSize: 11, color: "var(--pm-muted)", marginTop: 4 }}
						>
							Requires updating the card on the Netflix website. We'll generate
					...
					<div className="form-check form-switch mb-3">
						...
						/>
						<label className="form-check-label">
							Allow auto-pay from this card
						</label>
					</div>
					<button className={`${styles.btnPm} ${styles.btnPmDSoft} w-100`}>
						<i className="bi bi-slash-circle" /> Block Future Charges (Cancel
						Sub)
					</button>
				</>,
			)}
				...
				<>
					...
					<button
						...
						onClick={() =>
							doAction(
								"cancelSubModal",
								"Spotify has been added to the blocklist. Future charges will be declined.",
								"",
							)
						}
					>
			...
			{renderActionBody(
				...
				<>
					<div className="text-center">
						...
						<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
							PayMo will automatically decline any future charge attempts from{" "}
							<strong>Spotify</strong> on the card **3841.
						</p>
						<div
							className="p-3 rounded text-start"
							style={{ background: "var(--pm-warning-soft)", fontSize: 12 }}
						>
							Note: Blocking charges at the card level does not cancel your
							contract with the merchant. You may still owe them money. We
							recommend also cancelling on their website.
						</div>
					</div>
				</>,
			)}
	...
	const renderAddSubscription = () => (
		...
		>
			{renderActionBody(
				...
				<>
					<p style={{ fontSize: 13, color: "var(--pm-muted)" }}>
						Add a subscription manually if PayMo hasn't auto-detected it yet to
						include it in your forecasting.
					</p>
					<div className="mb-3">
						<label className={styles.formLabel}>Service Name</label>
						<input
							className={styles.formControl}
							placeholder="e.g. Gym Membership, Notion"
						/>
					</div>
					<div className="mb-3">
					...
					<div className="mb-3">
						<label className={styles.formLabel}>Linked Card</label>
						<select className={styles.formControl}>
							{CARDS_FOR_ALIAS.map((c) => (
								<option key={c}>{c}</option>
							))}
						</select>
					</div>
	...
	const renderBulkCreate = () => (
				...
				<>
					...
					<button
						...
						onClick={() =>
							doAction(
								"bulkCreateModal",
								"10 cards generated successfully. Download CSV for PAN details.",
								"",
							)
						}
					>
		...
		>
			{renderActionBody(
				...
				<>
					<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
						Issue multiple cards at once for ad-campaigns, employee allowances,
						or batch purchasing.
					</p>
					<div className="mb-3">
						<label className={styles.formLabel}>
							Number of Cards to Issue (Max 50)
						</label>
					...
					<div className="mb-3">
						<label className={styles.formLabel}>Naming Pattern</label>
						<input
							type="text"
							className={styles.formControl}
							placeholder="e.g. FB-Ads-Campaign-[Seq]"
						/>
					</div>
					<div className="mb-3">
						<label className={styles.formLabel}>Funding Source</label>
	...
	const renderDisputeTx = () => (
				...
				<>
					...
					<button
						...
						onClick={() =>
							doAction(
								"disputeTxModal",
								"Dispute submitted to network. Card frozen. Case ID: CB-99102",
								"CB-99102",
							)
						}
					>
		...
		>
			{renderActionBody(
				...
				<>
					<div className="mb-3">
						<label className={styles.formLabel}>Transaction</label>
						<select className={styles.formControl}>
							{SUBS_FOR_DISPUTE.map((s) => (
					...
					<div
						...
						style={{ background: "var(--pm-warning-soft)", fontSize: 12 }}
					>
						Filing a false chargeback may result in account termination.
						Chargebacks take 7-14 days to resolve via VISA/Mastercard networks.
					</div>
				</>,
			)}
		</MBox>
				...
				<>
					...
					<button
						...
						onClick={() =>
							doAction(
								"reportFraudModal",
								"ALL CARDS FROZEN. Our security team will contact you shortly.",
								"",
							)
						}
					>
			...
			{renderActionBody(
				...
				<>
					<div className="text-center">
						...
						<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
							This will instantly freeze <strong>ALL</strong> active virtual
							cards and physical cards linked to your account.
						</p>
						<div className="form-check text-start mt-4 mb-3">
							<input className="form-check-input" type="checkbox" />
							<label className="form-check-label font-weight-bold">
								I confirm my account is compromised
							</label>
						</div>
					</div>
				</>,
			...
			footer={
				<>
					...
					<button
						className={`${styles.btnPm} ${styles.btnPmP}`}
						onClick={() =>
							doAction("cardStatementModal", "Statement PDF downloaded.", "")
						}
					>
	...
	const renderWithdrawCard = () => (
		<MBox
			id="withdrawCardModal"
			active={active}
			onClose={onClose}
			title={
				<>
					<i className="bi bi-arrow-down-circle text-danger me-2" />
					Unload Card Funds
				</>
			}
			footer={
				<>
					...
					<button
						className={`${styles.btnPm} ${styles.btnPmD}`}
						onClick={() =>
							doAction("withdrawCardModal", "Funds moved successfully.", "")
						}
					>
						Withdraw KES 12,000
					</button>
		...
		>
			{renderActionBody(
				...
				<>
					<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
						Move locked funds from a prepaid virtual card back to your main
						wallet or M-Pesa.
					</p>
					<div className="mb-3">
						<label className={styles.formLabel}>From Card</label>
						<select className={styles.formControl}>
							<option>Prepaid Travel Card (**8812) — KES 12,000 avail</option>
						</select>
					</div>
					<div className="mb-3">
				...
				<>
					...
					<button
						...
						onClick={() =>
							doAction(
								"renewCardModal",
								"Card expiry extended successfully.",
								"",
							)
						}
					>
			...
			{renderActionBody(
				...
				<>
					...
					<div className="form-check mb-3">
						...
						/>
						<label className="form-check-label">
							Notify Visa Token Service to auto-update merchants (Apple,
							Netflix, etc.)
						</label>
					</div>
				</>,
	...
	const renderAuth3DS = () => (
				...
				<>
					...
					<button
						...
						onClick={() =>
							doAction(
								"auth3DSModal",
								"Purchase authorized! You can return to the merchant site.",
								"",
							)
						}
					>
		...
		>
			{renderActionBody(
				...
				<>
					<div
						className="p-3 border rounded text-start mb-3"
						style={{ fontSize: 13 }}
					>
						<div className="d-flex justify-content-between mb-2">
							<span className="text-muted">Merchant</span>
							<strong>AliExpress.com</strong>
						</div>
						<div className="d-flex justify-content-between mb-2">
							<span className="text-muted">Amount</span>
							<strong>KES 2,100</strong>
						</div>
						<div className="d-flex justify-content-between">
					...
					<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
						Enter your PayMo PIN to authorize this online purchase.
					</p>
				...
				<>
					...
					<button
						...
						onClick={() =>
							doAction(
								"cardSecurityModal",
								"Global security settings updated.",
								"",
							)
						}
					>
		...
		>
			{renderActionBody(
				...
				<>
					<div className="row g-3 mb-3">
						...
						<div className="col-md-8">
							<div
								...
								style={{ background: "var(--pm-surface-2)", fontSize: 12 }}
							>
								<strong>Strong Posture:</strong> You use dynamic CVVs, have
								tight MCC locks on subscription cards, and 3D secure is
								mandatory.
								<br />
								<span className="text-danger">Recommendation:</span> Consider
								freezing the 'Spotify' card since it hasn't been used in 60
								days.
							</div>
						</div>
					</div>
					<h6 style={{ fontWeight: 700, marginTop: 20 }}>
						Global Overrides (Applies to all cards)
					</h6>
					<div className={styles.statusRow}>
						<div>
							<strong>Block Magstripe/Fallback</strong>
							<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
								EMV Chip & Contactless only
							</div>
						</div>
						<div className="form-check form-switch">
							<input
					...
					<div className={styles.statusRow}>
						<div>
							<strong>Block High-Risk MCCs</strong>
							<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
					...
					<div className={styles.statusRow}>
						<div>
							<strong>Mandatory 3D Secure</strong>
							<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
								Decline CNP transactions if merchant lacks 3DS
							</div>
						</div>
						<div className="form-check form-switch">
							<input className="form-check-input" type="checkbox" />
					...
					<div className="row g-3">
						<div className="col-md-6">
							<div
								...
								style={{ background: "var(--pm-surface-2)" }}
							>
								<h6 style={{ fontWeight: 700, fontSize: 13 }}>
								...
								>
									...
									<strong>KES 12,240</strong>
								</div>
								<div className={`${styles.progress} mb-3`}>
									<div
										className={styles.progressBar}
										style={{ width: "30%", background: "var(--pm-warning)" }}
									/>
								</div>
								<div
									className="d-flex justify-content-between mb-1"
								...
								<div className={`${styles.progress} mb-3`}>
									<div
										className={styles.progressBar}
										style={{ width: "15%", background: "var(--pm-accent)" }}
									/>
								</div>
								<div
									className="d-flex justify-content-between mb-1"
								...
								>
									<span>Transport</span>
									<strong>KES 2,400</strong>
								</div>
								<div className={styles.progress}>
									<div
										className={styles.progressBar}
										style={{ width: "10%", background: "var(--pm-info)" }}
									/>
								</div>
							</div>
						</div>
				...
				<>
					...
					>
						...
						<div style={{ fontSize: 11, color: "#7F1D1D" }}>
							AWS Web Services tried to charge KES 4,500 on card **4418.
						</div>
						<div className="mt-2">
							<button
					...
					>
						...
						<div style={{ fontSize: 11, color: "#1E40AF" }}>
							AliExpress purchase of KES 2,100 requires your approval.
						</div>
						<div className="mt-2">
							<button
								className={`${styles.btnPm} ${styles.btnSm}`}
					...
					>
						...
						<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
							Netflix charged KES 1,400 on Sub Master (**9021).
						</div>
					</div>
					<div
						className="p-3 rounded mb-2"
			...
			footer={
				<>
					...
					<button
						className={`${styles.btnPm} ${styles.btnPmP}`}
						onClick={() =>
							doAction("notifPrefsModal", "Notification settings saved.", "")
						}
					>
						Save Settings
					</button>
				...
				<>
					<div className={styles.statusRow}>
						<div>
							...
							<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
								Push notification for every swipe/charge
							</div>
						</div>
						<div className="form-check form-switch">
							<input
					...
					<div className={styles.statusRow}>
						<div>
							<strong>Large Purchases</strong>
							<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
								Alert for amounts &gt; KES 10,000
							</div>
						</div>
						<div className="form-check form-switch">
							<input
					...
					<div className={styles.statusRow}>
						<div>
							...
							<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
								3 days before recurring charge
							</div>
						</div>
						<div className="form-check form-switch">
							<input
			...
			<div className="text-center">
				<div
					...
					style={{
						...
						height: 64,
						fontSize: 24,
						background: "var(--pm-gradient-sunset)",
					}}
				>
					JK
				</div>
				...
				<div className="row g-2 text-start mt-3" style={{ fontSize: 13 }}>
					...
					<div className="col-6">
						...
						>
							...
							<br />
							<strong>KES 48,200 this mo</strong>
						</div>
					</div>
				</div>
			</div>
		...
		>
			{renderActionBody(
				...
				<>
					<div className={styles.statusRow}>
						<div>
							<strong>Declined: Insufficient Funds (AWS)</strong>
							<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
								Card **4418 requires KES 4,500
							</div>
						</div>
						<button
							className={`${styles.btnPm} ${styles.btnSm} ${styles.btnPmD}`}
					...
					<div className={styles.statusRow}>
						<div>
							<strong>Card Expiring Soon</strong>
							<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
								Global Shopping Card expires in 15 days
							</div>
						</div>
						<button
							className={styles.btnPm}
							onClick={() => {
								onOpen("renewCardModal");
								onClose();
							}}
						>
							Renew
					...
					<div className={styles.statusRow}>
						<div>
							<strong>Netflix Sub Increased</strong>
							<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
								+KES 300 detected this cycle
							</div>
						</div>
						<button
							className={styles.btnPm}
							onClick={() => {
								onOpen("subManagerModal");
								onClose();
							}}
						>
							Review
					...
					<div className={styles.statusRow}>
						<div>
							<strong>Pending 3D Secure</strong>
							<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
								AliExpress order KES 2,100
							</div>
						</div>
						<button
							className={styles.btnPm}
