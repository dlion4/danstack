...
import {
	...
	findModule,
} from "@/features/Layouts/dashboard-cards-layout/data/cardsLayoutData";
...
const initialMockData: VirtualDebitCardsConfig = {
	statCards: [
		{
			...
			value: "KES 48,200",
			badge: { icon: "bi-graph-up", text: "+12% this month", tone: "badgeI" },
			lines: [],
			miniBars: [
		...
		},
		{
			...
			lines: [],
			progress: { width: "40%", color: "var(--pm-warning)" },
		},
		{
			key: "secured",
	...
	suggestions: [
		...
		},
		{
			...
			sub: "Freelance Services card rarely uses >KES 10k",
			actionLabel: "Adjust",
			modal: "editLimitsModal",
		},
	],
	quickActions: [
		{
	...
	virtualCards: [
		{
			alias: "Global Web Shopping",
			pan: "4532 8821 0092 3841",
			exp: "12/28",
			cvv: "882",
			net: "VISA",
			bg: "vCardBg1",
			bal: "KES 12,500 avail",
			status: "active",
		},
		{
			alias: "Subscription Master",
			pan: "5123 9901 2244 9021",
			exp: "08/29",
			cvv: "***",
			net: "MASTERCARD",
			bg: "vCardBg2",
			bal: "KES 50,000 limit",
			status: "active",
		},
		{
			alias: "AWS & Hosting",
			pan: "4532 1100 8872 4418",
			exp: "01/27",
			cvv: "***",
			net: "VISA",
			bg: "vCardBg3",
		...
		{
			alias: "Single-Use Burner",
			pan: "5123 0000 0000 1192",
			exp: "06/25",
			cvv: "***",
			net: "MASTERCARD",
			bg: "vCardBg5",
		...
		{
			alias: "Travel & Booking",
			pan: "4532 6610 9921 5504",
			exp: "11/26",
			cvv: "***",
			net: "VISA",
			bg: "vCardBgFrozen",
	...
	securityControls: [
		{
			...
			items: [
				{
					label: "Card Status",
					sub: "Active and accepting charges",
					actionLabel: "Freeze",
					modal: "freezeCardModal",
				},
				{
					...
					actionLabel: "Delete",
					actionClass: "pm-btn-danger-soft",
					modal: "deleteCardModal",
				},
				{
					label: "Card Alias",
					sub: "Global Web Shopping",
					actionLabel: "Edit",
					modal: "editAliasModal",
				},
			],
		},
		{
			...
			items: [
				{
					label: "Merchant Lock",
					sub: "Lock to Amazon only",
					actionLabel: "Manage",
					modal: "merchantLockModal",
				},
				{
					label: "Dynamic CVV",
					sub: "Auto-rotates every 24h",
					actionLabel: "Enable",
					modal: "rotateCvvModal",
				},
				{
					label: "Spending Limit",
					sub: "KES 50,000 / month",
					actionLabel: "Adjust",
					modal: "editLimitsModal",
				},
			],
	...
	subscriptions: [
		{
			...
			statusTone: "badgeS",
			actionLabel: "Manage",
			actionModal: "manageSingleSubModal",
		},
		{
			service: "Apple Music",
			icon: "bi-apple",
		...
		},
		{
			...
			statusTone: "badgeD",
			actionLabel: "Fund Card",
			actionClass: "pm-btn-primary",
			actionModal: "topUpCardModal",
		},
	],
};
			...
			<div className={styles.pageBar}>
				<div>
					<div className={styles.breadcrumb}>
						...
						<strong>Virtual Debit Cards</strong>
					</div>
					<h2 className={styles.pageH2}>
						PAGE 5.3 — Virtual Debit Card Center
					</h2>
					<p className={styles.pageSub}>
						Create and manage multiple virtual debit cards for safer online
						shopping, compartmentalized subscriptions, and single-use e-commerce
						transactions.
					</p>
				</div>
				<div className="d-flex flex-wrap" style={{ gap: 8 }}>
					...
					>
						<i className="bi bi-arrow-repeat" /> Subscriptions
					</button>
					<button
			...
			<div className="row g-3">
				...
				{config.statCards.map((s) => (
					<div key={s.key} className={s.col}>
						...
						>
							<p className={styles.statLabel} style={{ color: s.labelColor }}>
								{s.label}
							</p>
			...
			<div className={styles.card}>
				...
				<div className={styles.vCardGrid}>
					{config.virtualCards.map((card) => (
						<div
							key={card.alias}
							className={`${styles.vCard} ${styles[card.bg]}`}
							onClick={() => setActiveModal("viewCardDetailsModal")}
						>
							<div
								className={styles.vCardBadge}
								style={
									card.status === "low"
							...
							>
								{getCardBadge(card.status)}
							</div>
							<div className={styles.vCardLogo}>PayMo</div>
							<div className={styles.vCardChip} />
							<div className={styles.vCardNumber}>
								{getPanDisplay(card.pan)}
							</div>
						...
						{config.recentActivity.map((item) => (
							<div key={item.title} className={styles.feedItem}>
								...
								<div style={{ flex: 1 }}>
									...
									<div style={{ fontSize: 11, color: item.statusColor }}>
										{item.status}
									</div>
								...
								<tr key={sub.service}>
									<td data-label="Service">
										<div className="d-flex align-items-center gap-2">
											<div className={`${styles.subIcon} ${sub.iconColor}`}>
									...
									<td data-label="Actions">
										<button
											className={`${styles.btnPm} ${styles.btnSm} ${sub.actionClass ? styles[sub.actionClass] : ""}`}
											onClick={() => setActiveModal(sub.actionModal)}
										>
											{sub.actionLabel}
										</button>
									</td>
