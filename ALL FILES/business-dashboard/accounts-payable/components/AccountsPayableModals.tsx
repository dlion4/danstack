...
export default function AccountsPayableModals({
	active,
	onClose,
}: ModalsProps) {
	const s = styles as Record<string, string>;
				...
				<>
					...
					{suppStep < 4 && (
						<button
							...
							onClick={() => nextFlow("supplier", 4)}
						>
							{suppStep === 3 ? "Submit Profile" : "Next"}{" "}
							<i
								className={`bi ${suppStep === 3 ? "bi-check2" : "bi-arrow-right"}`}
							/>
						</button>
					)}
				</>
		...
		>
				...
				<div className={cx(s.fstep, s.fstepActive)}>
					...
					<div className="mb-3">
						...
						<input
							type="email"
							className={s.formControl}
							defaultValue="billing@supplier.com"
						/>
					</div>
					<div className="mb-3">
			...
			{suppStep === 2 && (
				<div className={cx(s.fstep, s.fstepActive)}>
					...
					<div className="mb-3">
						<label className={s.formLabel}>Preferred Payment Method</label>
						<select className={s.formControl}>
			...
			{suppStep === 3 && (
				<div className={cx(s.fstep, s.fstepActive)}>
					...
					<div className="mb-3">
						<label className={s.formLabel}>Early Payment Discount</label>
						<input
							type="text"
							className={s.formControl}
							defaultValue="2% if paid within 10 days"
						/>
					</div>
					<div className="form-check">
						...
						<label className="form-check-label">
							Enable auto-reminders for upcoming payments
						</label>
					</div>
				</div>
			)}
				...
				renderActionBody(
					...
					<div className={s.receipt}>
						...
						<p style={{ fontSize: 12, color: "var(--pm-muted)" }}>
							Supplier ID: SUP-2025-019 · Awaiting KYC verification
						</p>
					</div>,
				)}
		</MBox>
				...
				<>
					...
					<button
						...
						onClick={() =>
							doAction(
								"processInvoiceModal",
								"Invoice processed and sent for approval!",
								"INV-2025-5512",
							)
						}
					>
				...
				<>
					<div className="mb-3">
						...
						<select className={s.formControl}>
							...
							<option>CloudServe</option>
							<option>Global Logistics</option>
							<option>DesignWorks</option>
						</select>
					</div>
					<div className="mb-3">
					...
					<div className="mb-3">
						...
						<textarea
							className={s.formControl}
							rows={2}
							defaultValue="Monthly office supplies order"
						/>
					</div>
					<div className="mb-3">
					...
					<div className="mb-3">
						<label className={s.formLabel}>Upload Invoice PDF</label>
						<div
	...
	const renderBulkPaySuppliers = () => (
		<MBox
			id="bulkPaySuppliersModal"
			active={active}
			onClose={onClose}
			title={
				<>
					<i className="bi bi-people text-accent me-2" />
					Bulk Pay Suppliers
				</>
			}
			size="lg"
			footer={
				<>
					...
					{bulkStep < 3 && (
						<button
							className={cx(s.btnPm, s.btnPmP)}
							onClick={() => nextFlow("bulkPay", 3)}
						>
							{bulkStep === 2 ? "Authorize Batch" : "Next"}{" "}
							<i
								className={`bi ${bulkStep === 2 ? "bi-shield-lock" : "bi-arrow-right"}`}
							/>
						</button>
					)}
				</>
		...
		>
			...
			{bulkStep === 1 && (
				<div className={cx(s.fstep, s.fstepActive)}>
					...
					>
						<strong>3 approved invoices selected</strong> · Total: KES 165,500
						<br />
						OfficeMart KES 12,500 · CloudServe KES 128,000 · DesignWorks KES
						28,000
					</div>
					<div className="mb-3">
					...
					<div className="form-check">
						...
						<label className="form-check-label">
							Capture available early-payment discounts
						</label>
					</div>
				</div>
			)}
			{bulkStep === 2 && (
				<div className={cx(s.fstep, s.fstepActive)}>
					...
					>
						...
						<strong>Maker-Checker required</strong> · You are the maker. A
						checker (CFO or Director) must approve before execution.
					</div>
					<div className="mb-3">
						<label className={s.formLabel}>Authorization PIN</label>
			...
			{bulkStep === 3 &&
				renderActionBody(
					"bulkPaySuppliersModal",
					<div className={s.receipt}>
						<div className={s.receiptIcon}>
							<i className="bi bi-check-lg" />
						</div>
						<h5 style={{ fontWeight: 700, color: "var(--pm-accent)" }}>
							Batch authorized and processing!
						</h5>
						<p style={{ fontSize: 12, color: "var(--pm-muted)" }}>
							Ref: BAP-2025-06 · 3 payments · KES 165,500
						</p>
					</div>,
				)}
		</MBox>
	...
	const renderApprovalQueue = () => (
		<MBox
			...
			title={
				<>
					<i className="bi bi-check2-square text-primary me-2" />
			...
			{renderActionBody(
				...
				<>
					<div className="mb-3">
						...
						<select className={s.formControl}>
							<option>INV-4419 — OfficeMart KES 12,500</option>
							<option>INV-3410 — CloudServe KES 128,000</option>
						</select>
					</div>
					<div className="mb-3">
			...
			footer={
				<>
					...
					<button
						className={cx(s.btnPm, s.btnPmP)}
						onClick={() =>
							doAction("reconciliationModal", "Reconciliation started!")
						}
					>
			...
			{renderActionBody(
				...
				<>
					<div className="mb-3">
						...
						<select className={s.formControl}>
							<option>Equity Bank — Main Operations</option>
							<option>KCB — Business Account</option>
						</select>
					</div>
					<div className="mb-3">
					...
					<div
						...
						style={{ background: "var(--pm-warning-soft)", fontSize: 13 }}
					>
						1 unreconciled statement found for Equity Bank main account
					</div>
				</>,
			)}
		</MBox>
	...
	const renderSupplierDetail = () => (
		...
		>
			...
			>
				...
				<div className="d-flex justify-content-between">
					<span>Terms</span>
					<strong>Net 30 / 2% early pay</strong>
				</div>
				<div className="d-flex justify-content-between">
	...
	const renderInvoiceDetail = () => (
		...
		>
			...
			>
				...
				<div className="d-flex justify-content-between">
					<span>Due Date</span>
					<strong>28 Jun 2025</strong>
				</div>
				<div className="d-flex justify-content-between">
					<span>Status</span>
					<span className={cx(s.badge, s.badgeW)}>Pending Approval</span>
				</div>
			</div>
		</MBox>
	);
	...
	const renderDiscountTracking = () => (
		...
		>
			<div className={s.statusRow}>
				<div>
					<strong>OfficeMart — 2% Early Pay</strong>
					<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
						Expires tomorrow · Save KES 250
					</div>
				</div>
				<button
					className={cx(s.btnPm, s.btnSm, s.btnPmP)}
					onClick={() =>
						doAction(
							"discountTrackingModal",
							"Discount captured! KES 250 saved.",
						)
					}
				>
					Capture
			...
			<div className={s.statusRow}>
				<div>
					<strong>CloudServe — 1.5% Net 15</strong>
					<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
						3 days left · Save KES 1,920
					</div>
				</div>
				<button
					className={cx(s.btnPm, s.btnSm)}
					onClick={() =>
						doAction("discountTrackingModal", "Discount captured!")
					}
				>
					Capture
				</button>
			...
			footer={
				<>
					...
					<button
						...
						onClick={() =>
							doAction(
								"earlyPaymentCalcModal",
								"Early payment scheduled! Saving KES 250.",
							)
						}
					>
						Pay Now
			...
			{renderActionBody(
				...
				<>
					<div className="mb-3">
						<label className={s.formLabel}>Invoice</label>
						<select className={s.formControl}>
							<option>INV-4419 — OfficeMart KES 12,500 (2% discount)</option>
						</select>
					</div>
					<div
						className="p-3 rounded"
					...
					>
						...
						<div className="d-flex justify-content-between">
							...
							<strong style={{ color: "var(--pm-accent)" }}>KES 250</strong>
						</div>
						<div className="d-flex justify-content-between">
							<span>Net Payment</span>
							<strong style={{ color: "var(--pm-accent)" }}>KES 12,250</strong>
						</div>
					</div>
				</>,
			)}
	...
	const renderOnboardSupplierInvite = () => (
		<MBox
			id="onboardSupplierInviteModal"
			active={active}
			onClose={onClose}
			title={
				<>
					<i className="bi bi-envelope me-2" />
					Invite Supplier to Self-Onboard
				</>
			}
			footer={
				<>
					...
					<button
						...
						onClick={() =>
							doAction(
								"onboardSupplierInviteModal",
								"Invitation sent! Supplier will appear as Pending.",
							)
						}
					>
		...
		>
			{renderActionBody(
				...
				<>
					...
					>
						Invite suppliers to self-onboard. They upload KRA PIN/CR12 and
						submit invoices directly into your queue.
					</div>
					<div className="mb-3">
						<label className={s.formLabel}>Supplier Company Name</label>
						<input
							type="text"
							className={s.formControl}
							placeholder="e.g. DesignWorks Agency"
						/>
					</div>
					<div className="mb-3">
						<label className={s.formLabel}>Contact Email</label>
						<input
							type="email"
							className={s.formControl}
							placeholder="billing@designworks.com"
						/>
					</div>
					<div className="mb-3">
						<label className={s.formLabel}>Custom Message</label>
						<textarea
							className={s.formControl}
							rows={3}
							defaultValue="Hello, we are moving our AP process to PayMo. Please click the link to submit your business details and bank information."
						/>
					</div>
				</>,
			)}
		...
		<MBox
			...
			footer={
				<>
					...
					<button
						className={cx(s.btnPm, s.btnPmP)}
						onClick={() => doAction("taxSettingsModal", "Tax settings saved!")}
					>
						Save
					</button>
				</>
		...
		>
			{renderActionBody(
				...
				<>
					...
					<div className="mb-3">
						<label className={s.formLabel}>VAT Handling</label>
						<select className={s.formControl}>
							<option>Auto-calculate 16% VAT</option>
	...
	const renderExportAP = () => (
			...
			footer={
				<>
					...
					<button
						className={cx(s.btnPm, s.btnPmP)}
						onClick={() => {
							downloadFile("ap-export.csv", "Supplier,Invoice,Amount,Status");
							onClose();
						}}
					>
						Export
		...
		>
			...
			<div className="form-check">
				<input className="form-check-input" type="checkbox" />
				<label className="form-check-label">
					Include discount tracking data
				</label>
			</div>
		</MBox>
	);
	...
	const renderNotifications = () => (
		...
		>
			...
			>
				<strong>Failed PesaLink Transfer</strong>
				<div style={{ fontSize: 11 }}>
					Inv-0092 to Global Logistics failed. Invalid account length.
				</div>
			</div>
			<div
				className="p-3 rounded mb-2"
				style={{ background: "var(--pm-warning-soft)", fontSize: 13 }}
			>
				<strong>Discount expiring soon</strong>
				<div style={{ fontSize: 11 }}>
					OfficeMart 2% discount expires tomorrow.
				</div>
			</div>
			<div
				className="p-3 rounded mb-2"
				style={{ background: "var(--pm-info-soft)", fontSize: 13 }}
			>
				<strong>New Supplier Onboarded</strong>
				<div style={{ fontSize: 11 }}>
					DesignWorks Agency completed self-onboarding.
				</div>
			</div>
		</MBox>
	);
				...
				<div className="row g-2 text-start mt-3" style={{ fontSize: 13 }}>
					...
					<div className="col-6">
						<div
							...
							style={{ background: "var(--pm-surface-2)" }}
						>
							<span style={{ color: "var(--pm-muted)" }}>Pending Tasks</span>
							<br />
							<strong>8 Approvals</strong>
						</div>
					</div>
				</div>
			</div>
	...
	return (
		<>
			{renderAddSupplier()}
			{renderProcessInvoice()}
			{renderBulkPaySuppliers()}
			{renderApprovalQueue()}
			...
			{renderInvoiceDetail()}
			{renderPaymentDetail()}
			{renderDiscountTracking()}
			{renderEarlyPaymentCalc()}
			{renderOnboardSupplierInvite()}
			{renderTaxSettings()}
			{renderExportAP()}
			{renderNotifications()}
			{renderProfile()}
