...
function Stepper({ flowKey, current }: { flowKey: string; current: number }) {
	...
	if (!def) return null;
	const pct = def.labels.length > 1
		? Math.round(((current - 1) / (def.labels.length - 1)) * 100)
		: 100;
	return (
		<div className={s.stepBar}>
			<div className={s.stepBarFill} style={{ width: `${pct}%` }} />
		</div>
	);
}

export default function FinancialReportingModals({
	active,
	onClose,
	onOpen,
}: ModalsProps) {
	...
	const renderReceipt = (r: Result) => (
		<div className={s.receipt}>
			...
			<div className="d-flex justify-content-center mt-3" style={{ gap: 8 }}>
				...
				<button className={cx(s.btnPm, s.btnSm)} onClick={onClose}>
					Done
				</button>
			</div>
		</div>
	...
	const renderGenerateCustomReport = () => (
				...
				<>
					...
					{gcrStep < 3 && (
						<button
							...
							onClick={() => nextFlow("gcr", 3)}
						>
							{gcrStep === 2 ? "Generate" : "Continue"}{" "}
							<i
								className={`bi ${gcrStep === 2 ? "bi-check2" : "bi-arrow-right"}`}
							/>
						</button>
					)}
				</>
		...
		>
			...
			{gcrStep === 1 && (
				<div className={cx(s.fstep, s.fstepActive)}>
							...
							<div key={t.key} className="col-md-4">
								...
								>
									<i
										...
										style={{ fontSize: 24, color: t.iconColor }}
									/>
				...
				<div className={cx(s.fstep, s.fstepActive)}>
					...
					<div className="row g-3">
						<div className="col-md-6">
							<label className={s.formLabel}>Specific Report</label>
							<select className={s.formControl}>
								<option>Income Statement (P&L)</option>
								<option>Trial Balance</option>
								<option>General Ledger Extract</option>
							</select>
						</div>
						<div className="col-md-6">
							<label className={s.formLabel}>Branch/Location</label>
							<select className={s.formControl}>
								<option>All Consolidated</option>
				...
				renderActionBody(
					...
					<div className={s.receipt}>
						...
						<h5 style={{ fontWeight: 700, color: "var(--pm-accent)" }}>
							Report generated and scheduled for delivery!
						</h5>
						<p style={{ fontSize: 12, color: "var(--pm-muted)" }}>
							Reference: REP-19028
						</p>
	...
	const renderRunMonthEnd = () => (
		<MBox
			...
			title={
				<>
					<i className="bi bi-calendar2-check text-success me-2" />
			...
			footer={
				<>
					...
					{mecStep < 3 && (
						<button
							className={cx(s.btnPm, s.btnPmP)}
							onClick={() => nextFlow("mec", 3)}
						>
							{mecStep === 2 ? "Lock Ledgers" : "Continue"}{" "}
							<i
								className={`bi ${mecStep === 2 ? "bi-lock" : "bi-arrow-right"}`}
							/>
						</button>
					)}
				</>
		...
		>
				...
				<div className={cx(s.fstep, s.fstepActive)}>
					...
					<div className={s.statusRow}>
						<div>
							...
							<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
								Automated journal entries ready
							</div>
						</div>
			...
			{mecStep === 2 && (
				<div className={cx(s.fstep, s.fstepActive)}>
					...
					<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
						You are about to soft-lock the period <strong>June 2025</strong>. No
						further operational transactions can be backdated into this month.
					</p>
					<div className="mb-3">
						<label className={s.formLabel}>Closing Period</label>
						<input className={s.formControl} value="June 2025" disabled />
					</div>
					<div className="form-check mb-3">
						<input className="form-check-input" type="checkbox" />
						<label className="form-check-label" style={{ fontSize: 13 }}>
							I confirm all material adjustments have been posted.
						</label>
					</div>
				</div>
			)}
			{mecStep === 3 && (
				<div className={s.receipt}>
					...
					<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
						June 2025 is now closed. Standard financial statements are being
					...
					<div className="d-flex justify-content-center mt-3">
						<button
							...
							onClick={() => {
								onClose();
								onOpen("plSnapshotModal");
							}}
						>
							View June Financials
						</button>
					</div>
				</div>
			)}
	...
	const renderExportKRAVAT = () => (
		<MBox
			id="exportKRAVATModal"
			active={active}
			onClose={onClose}
			title={
				<>
					<i className="bi bi-bank text-warning me-2" />
					KRA VAT Return Extract
				</>
			}
			footer={
				<>
					...
					<button
						...
						onClick={() =>
							doAction(
								"exportKRAVATModal",
								"VAT Extract Generated for iTax upload.",
							)
						}
					>
		...
		>
			{renderActionBody(
				...
				<>
					...
					>
						<i className="bi bi-info-circle me-1" /> This tool generates a CSV
						file formatted specifically for the KRA iTax portal (VAT3 template).
					</div>
					<div className="row g-3">
						<div className="col-md-6">
							<label className={s.formLabel}>Tax Period</label>
							<select className={s.formControl}>
						...
						<div className="col-md-6">
							...
							<input className={s.formControl} value="P051234567M" disabled />
						</div>
						<div className="col-12">
							<label className={s.formLabel}>Extract Sections</label>
							<div className="form-check">
								<input
									className="form-check-input"
									type="checkbox"
									defaultChecked
								/>
								<label className="form-check-label">Output VAT (Sales)</label>
							</div>
							<div className="form-check">
								<input
									className="form-check-input"
	...
	const renderScheduleReport = () => (
		...
		>
			{renderActionBody(
				...
				<>
					<div className="mb-3">
						<label className={s.formLabel}>Select Report Template</label>
						<select className={s.formControl}>
							<option>Weekly Cash Flow Summary</option>
							<option>Monthly Income Statement</option>
							<option>Daily Collections & Reconciliations</option>
						</select>
					</div>
					<div className="row g-3">
						<div className="col-md-6">
						...
						<div className="col-12">
							...
							<input
								className={s.formControl}
								defaultValue="titus@company.com, audit@kpmg.co.ke"
							/>
						</div>
					</div>
				</>,
	...
	const renderViewAuditLog = () => (
		<MBox
			...
			title={
				<>
					<i className="bi bi-shield-lock text-purple me-2" />
		...
		>
			<div className="d-flex flex-wrap mb-3" style={{ gap: 8 }}>
				...
				/>
				<select className={s.formControl} style={{ width: 150 }}>
					<option>All Users</option>
					<option>Titus G.</option>
					<option>System API</option>
				</select>
				<select className={s.formControl} style={{ width: 180 }}>
	...
	const renderInviteAuditor = () => (
				...
				<>
					...
					<button
						...
						onClick={() =>
							doAction(
								"inviteAuditorModal",
								"Auditor invitation sent securely.",
							)
						}
					>
						Send Invitation
		...
		>
			{renderActionBody(
				...
				<>
					<div className="mb-3">
						<label className={s.formLabel}>Auditor Email Address</label>
						<input
							type="email"
							className={s.formControl}
					...
					<div className="mb-3">
						<label className={s.formLabel}>Access Duration</label>
						<select className={s.formControl}>
					...
					<div className="mb-3">
						<label className={s.formLabel}>Data Scope</label>
						<div className="form-check">
							<input
								className="form-check-input"
								type="checkbox"
						...
						<div className="form-check">
							...
							/>
							<label className="form-check-label">
								View Transaction Level Data & Receipts
							</label>
						</div>
						<div className="form-check">
							<input
						...
						<div className="form-check">
							<input className="form-check-input" type="checkbox" />
							<label className="form-check-label">
								View Employee Payroll Data (PII)
							</label>
						</div>
					</div>
				</>,
	...
	const renderCashFlowForecast = () => (
		...
		>
			<div className={cx(s.pills, "mb-3")}>
				{["7day", "30day", "90day"].map((t) => (
					<button
						key={t}
						className={`${s.pill} ${cfTab === t ? s.pillActive : ""}`}
						onClick={() => setTabState((prev) => ({ ...prev, cfTab: t }))}
					>
						{t === "7day" ? "7 Days" : t === "30day" ? "30 Days" : "90 Days"}
					</button>
				))}
			</div>
			{cfTab === "7day" && (
					...
					<div className="row g-3 mb-4">
						<div className="col-4">
							...
							>
								...
								<strong style={{ fontSize: 18 }}>KES 12.45M</strong>
							</div>
						</div>
						<div className="col-4">
							<div
							...
							>
								...
								<strong style={{ fontSize: 18, color: "var(--pm-accent)" }}>
									+ KES 3.2M
								</strong>
							</div>
						</div>
			...
			{cfTab === "30day" && (
				<p className="text-center p-4" style={{ color: "var(--pm-muted)" }}>
					30-day chart visualization goes here...
				</p>
			)}
			{cfTab === "90day" && (
				<p className="text-center p-4" style={{ color: "var(--pm-muted)" }}>
					90-day chart visualization goes here...
				</p>
			)}
		</MBox>
	);
	...
	const renderPLSnapshot = () => (
		...
		>
			<div className="d-flex justify-content-between mb-3">
				<span style={{ fontWeight: 700, fontSize: 16 }}>June 2025</span>
				<span className={cx(s.badge, s.badgeI)}>Consolidated View</span>
			</div>
			<div className="p-3 border rounded mb-3">
				<div className="d-flex justify-content-between mb-2">
					<span style={{ color: "var(--pm-muted)" }}>Total Gross Revenue</span>
					<strong>KES 8,240,000</strong>
				</div>
				<div className="d-flex justify-content-between mb-2">
					<span style={{ color: "var(--pm-muted)" }}>
						Cost of Goods Sold (COGS)
					</span>
					<strong>− KES 2,100,000</strong>
				</div>
			...
			<div className="p-3 border rounded mb-3">
				<div className="d-flex justify-content-between mb-2">
					...
					<strong>− KES 3,250,000</strong>
				</div>
				<div className="d-flex justify-content-between mb-2">
					<span style={{ color: "var(--pm-muted)" }}>Marketing & Sales</span>
				...
				<div className="d-flex justify-content-between mb-2">
					...
					<strong>− KES 280,000</strong>
				</div>
				<div className="d-flex justify-content-between mb-2">
					<span style={{ color: "var(--pm-muted)" }}>Rent & Utilities</span>
					<strong>− KES 320,000</strong>
				</div>
				<hr className={s.divider} />
				<div className="d-flex justify-content-between">
					<span style={{ fontWeight: 700 }}>Total Opex</span>
					<strong>KES 4,300,000</strong>
				</div>
			</div>
			<div
				className="p-3 rounded"
	...
	const renderTaxReadiness = () => (
		...
		>
			...
			<div className={s.statusRow}>
				<div>
					<strong>KRA VAT (June)</strong>
					<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
						Extract generated, ready for iTax
					</div>
				</div>
				<span className={cx(s.badge, s.badgeS)}>Ready</span>
			</div>
			<div className={s.statusRow}>
				<div>
					<strong>e-TIMS Integration</strong>
					<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
						API connection healthy, 100% matched
					</div>
				</div>
			...
			<div className={s.statusRow}>
				<div>
					<strong>PAYE / NSSF / SHIF</strong>
					<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
						Pending June payroll execution
					</div>
				</div>
			...
			<div className={s.statusRow}>
				<div>
					<strong>CR12 Annual Return</strong>
					<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
						Due August 2025
					</div>
				</div>
				<span className={cx(s.badge, s.badgeI)}>On Track</span>
			</div>
		</MBox>
	);
	...
	const renderETimsReconciliation = () => (
		<MBox
			id="eTimsReconciliationModal"
			active={active}
			onClose={onClose}
			title={
				<>
					<i className="bi bi-receipt me-2" />
					e-TIMS Invoice Reconciliation
				</>
			}
			size="lg"
			footer={
				<>
					...
					<button
						...
						onClick={() =>
							doAction(
								"eTimsReconciliationModal",
								"Manual sync triggered. All invoices up to date.",
							)
						}
					>
						Force Sync Now
				...
				<>
					<div className="row g-3 mb-3">
						...
						<div className="col-md-4">
							...
							>
								<div
									style={{ fontSize: 20, fontWeight: 700, color: "#047857" }}
								>
									142
								</div>
	...
	const renderBranchPerformance = () => (
		...
		>
			<div className="table-responsive">
				<table className={s.tbl}>
					<thead>
						<tr>
							<th>Metric</th>
							<th style={{ background: "var(--pm-info-soft)" }}>Nairobi HQ</th>
							<th style={{ background: "var(--pm-warning-soft)" }}>
								Mombasa Branch
							</th>
							<th>Consolidated</th>
						</tr>
			...
			>
				...
				<strong>Insight:</strong> Mombasa branch has slower collection times
				(DSO 34 vs 22). Consider implementing automated payment reminders for
				coastal clients.
			</div>
		</MBox>
	);
	...
	const renderCustomerSpendAnalytics = () => (
		<MBox
			id="customerSpendAnalyticsModal"
			active={active}
			onClose={onClose}
			title={
				<>
	...
	const renderVendorExpenseAnalytics = () => (
		<MBox
			id="vendorExpenseAnalyticsModal"
			active={active}
			onClose={onClose}
			title={
				<>
					<i className="bi bi-cart me-2" />
					Supplier & Expense Analytics
				</>
			}
			size="lg"
			footer={
		...
		>
			<div className={cx(s.pills, "mb-3")}>
				<button
					className={`${s.pill} ${veTab === "category" ? s.pillActive : ""}`}
					onClick={() =>
						setTabState((prev) => ({ ...prev, veTab: "category" }))
					}
				>
					By Category
				</button>
				<button
					className={`${s.pill} ${veTab === "supplier" ? s.pillActive : ""}`}
					onClick={() =>
						setTabState((prev) => ({ ...prev, veTab: "supplier" }))
					}
				>
					By Supplier
				</button>
			...
			{veTab === "supplier" && (
				<p className="text-center p-4" style={{ color: "var(--pm-muted)" }}>
					Supplier breakdown table visualization goes here...
				</p>
			)}
		</MBox>
	);
	...
	const renderReconciliationExceptions = () => (
		<MBox
			id="reconciliationExceptionsModal"
			active={active}
			onClose={onClose}
			title={
				<>
		...
		>
			<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
				The following incoming bank deposits could not be automatically matched
				to an open invoice.
			</p>
			<div className="table-responsive">
				<table className={s.tbl}>
						...
						<tr>
							...
							<td data-label="Action">
								<button
									...
									onClick={() =>
										doAction(
											"reconciliationExceptionsModal",
											"Matched to INV-2045 successfully.",
										)
									}
								>
									Accept
							...
							<td data-label="Action">
								<button
									...
									onClick={() =>
										doAction(
											"reconciliationExceptionsModal",
											"Matched to INV-2043 successfully.",
										)
									}
								>
									Accept
	...
	const renderDownloadStatement = () => (
					...
					<button
						...
						onClick={() =>
							doAction(
								...
								"File generated and download started.",
							)
						}
					>
						Download Now
		...
		>
			{renderActionBody(
				...
				<>
					<div className="mb-3">
						<label className={s.formLabel}>Data Source</label>
						<select className={s.formControl}>
							<option>PayMo Wallet Statement</option>
							<option>M-Pesa Collections</option>
							<option>Consolidated Financial Report</option>
						</select>
					</div>
					<div className="row g-3 mb-3">
						<div className="col-6">
	...
	const renderConfigureDashboards = () => (
		...
		>
			{renderActionBody(
				...
				<>
					<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
						Select which metrics appear on your main Financial Reporting view.
					</p>
					<div className="form-check form-switch mb-3">
						<input
							...
							defaultChecked
						/>
						<label className="form-check-label">Liquidity Position</label>
					</div>
					<div className="form-check form-switch mb-3">
						<input
							className="form-check-input"
							...
							defaultChecked
						/>
						<label className="form-check-label">YTD Revenue</label>
					</div>
					<div className="form-check form-switch mb-3">
						<input
							className="form-check-input"
							...
							defaultChecked
						/>
						<label className="form-check-label">Pending Reconciliations</label>
					</div>
					<div className="form-check form-switch mb-3">
						<input
							className="form-check-input"
							...
							defaultChecked
						/>
						<label className="form-check-label">Compliance & Audit Score</label>
					</div>
					<div className="form-check form-switch mb-3">
						<input className="form-check-input" type="checkbox" />
						<label className="form-check-label">
							Top Expense Category Tracker
						</label>
					</div>
					<div className="form-check form-switch">
						<input className="form-check-input" type="checkbox" />
						<label className="form-check-label">Live FX Rates (USD/KES)</label>
					</div>
				</>,
			)}
		</MBox>
	...
	const renderUserActivityLog = () => (
		...
		>
			...
			>
				<div className="d-flex justify-content-between mb-2">
					<span style={{ color: "var(--pm-muted)" }}>User</span>
					<strong>Titus G. (Admin)</strong>
				</div>
				<div className="d-flex justify-content-between mb-2">
					<span style={{ color: "var(--pm-muted)" }}>Timestamp</span>
					<strong>28 Jun 2025, 14:32:01 EAT</strong>
				</div>
				<div className="d-flex justify-content-between mb-2">
					<span style={{ color: "var(--pm-muted)" }}>IP Address</span>
					<strong>197.232.14.8 (Kenya)</strong>
				</div>
				<div className="d-flex justify-content-between mb-2">
					<span style={{ color: "var(--pm-muted)" }}>Device</span>
					<strong>Mac OS X, Chrome 114</strong>
				</div>
				<div className="d-flex justify-content-between">
  ...
  "hash": "a4b1c2d3e4f5..."
	...
	const renderStatutoryDeductions = () => (
		<MBox
			id="statutoryDeductionsModal"
			active={active}
			onClose={onClose}
			title={
				<>
					<i className="bi bi-shield-check text-warning me-2" />
					Statutory Deductions (June 2025)
				</>
			}
			size="lg"
			footer={
			...
			<div className="row g-3 mb-3">
				<div className="col-md-4">
					...
					>
						...
						<strong style={{ fontSize: 20, color: "var(--pm-warning)" }}>
							KES 450,200
						</strong>
					</div>
				</div>
				<div className="col-md-4">
					...
					>
						...
						<strong style={{ fontSize: 20, color: "var(--pm-info)" }}>
							KES 84,000
						</strong>
					</div>
				</div>
				<div className="col-md-4">
					...
					>
						...
						<strong style={{ fontSize: 20, color: "var(--pm-accent)" }}>
							KES 72,500
						</strong>
					</div>
				</div>
						...
						<tr>
							...
							<td data-label="Action">
								<button
									className={cx(s.btnPm, s.btnSm)}
									onClick={() =>
										doAction("statutoryDeductionsModal", "CSV Downloaded.")
									}
								>
									CSV Extract
								</button>
						...
						<tr>
							...
							<td data-label="Action">
								<button
									className={cx(s.btnPm, s.btnSm)}
									onClick={() =>
										doAction("statutoryDeductionsModal", "Excel Downloaded.")
									}
								>
									Excel Extract
								</button>
	...
	const renderExportTrialBalance = () => (
		<MBox
			id="exportTrialBalanceModal"
			active={active}
			onClose={onClose}
			title={
				<>
					<i className="bi bi-file-spreadsheet text-danger me-2" />
			...
			footer={
				<>
					...
					<button
						...
						onClick={() =>
							doAction(
								"exportTrialBalanceModal",
								"Trial Balance downloaded successfully.",
							)
						}
					>
		...
		>
			{renderActionBody(
				...
				<>
					<div className="d-flex justify-content-between mb-3">
						<span style={{ fontWeight: 700 }}>As of: 28 Jun 2025</span>
						<span className={cx(s.badge, s.badgeI)}>Consolidated</span>
					</div>
					<div
						className="table-responsive"
	...
	const renderReportDeliverySettings = () => (
		<MBox
			id="reportDeliverySettingsModal"
			active={active}
			onClose={onClose}
			title={
				<>
			...
			footer={
				<>
					...
					<button
						...
						onClick={() =>
							doAction(
								"reportDeliverySettingsModal",
								"Delivery configuration saved.",
							)
						}
					>
						Save Settings
		...
		>
			{renderActionBody(
				...
				<>
					<div className="mb-3">
						<label className={s.formLabel}>Delivery Channel</label>
						<select className={s.formControl}>
							<option>Email (Secure Attachment)</option>
							<option>SFTP Server</option>
							<option>Webhook (JSON Payload)</option>
						</select>
					</div>
					<div className="mb-3">
						<label className={s.formLabel}>SFTP Host (If applicable)</label>
						<input className={s.formControl} placeholder="sftp.company.com" />
					</div>
					<div className="mb-3">
						<label className={s.formLabel}>SFTP Port</label>
						<input className={s.formControl} placeholder="22" />
					</div>
					<div className="mb-3">
						<label className={s.formLabel}>Security</label>
						<div className="form-check">
							<input
								className="form-check-input"
								type="checkbox"
							...
							/>
							<label className="form-check-label">
								Password protect PDFs (using KRA PIN)
							</label>
						</div>
						<div className="form-check">
							<input
	...
	const renderDisputeAudit = () => (
		...
		>
			...
			>
				<div className="d-flex justify-content-between mb-2">
					<span style={{ color: "var(--pm-muted)" }}>User</span>
					<strong>James K. (Sales)</strong>
				</div>
				<div className="d-flex justify-content-between mb-2">
					<span style={{ color: "var(--pm-muted)" }}>Target</span>
					<strong>Invoice #INV-2041</strong>
				</div>
				<div className="d-flex justify-content-between mb-2">
					<span style={{ color: "var(--pm-muted)" }}>Action</span>
					<strong>Status changed to VOID</strong>
				</div>
				<div className="d-flex justify-content-between">
					<span style={{ color: "var(--pm-muted)" }}>Reason Provided</span>
					<strong>"Duplicate generated by error"</strong>
				</div>
			</div>
			<div
				className="p-3 rounded"
				style={{ background: "var(--pm-surface-2)", fontSize: 12 }}
			>
				No financial impact recorded as payment had not been reconciled against
				this invoice. e-TIMS credit note auto-filed.
			</div>
		</MBox>
	);
	...
	const renderFinancialHealthScore = () => (
		...
		>
			...
			<div
				...
				style={{ background: "var(--pm-surface-2)", fontSize: 12 }}
			>
				<i className="bi bi-stars me-1" style={{ color: "var(--pm-purple)" }} />{" "}
				AI Suggestion: Your liquidity is strong enough to place KES 5M in a
				30-day Money Market Fund to generate yield without impacting operational
				cash flow.
			</div>
		</MBox>
	);
		...
		>
			<div className="text-center">
				...
				<p style={{ fontSize: 13, color: "var(--pm-muted)" }}>
					Finance Director · Financial Reporting Admin
				</p>
				<div className="row g-2 text-start mt-3" style={{ fontSize: 13 }}>
					<div className="col-6">
						<div
							...
							style={{ background: "var(--pm-surface-2)" }}
						>
							<span style={{ color: "var(--pm-muted)" }}>Approval Limit</span>
							<br />
	...
	const renderNotifications = () => (
		<MBox
			...
			footer={
				<>
					<button className={cx(s.btnPm, s.btnSm)}>Mark all read</button>
					<button className={s.btnPm} onClick={onClose}>
						Close
					</button>
				</>
		...
		>
			...
			>
				<strong>PAYE Return Due in 3 days</strong>
				<div style={{ fontSize: 11 }}>KES 450,200 pending extract</div>
			</div>
			<div
				className="p-3 rounded mb-2"
				style={{ background: "var(--pm-warning-soft)", fontSize: 13 }}
			>
				<strong>4 Unmatched Bank Deposits</strong>
				<div style={{ fontSize: 11 }}>Require manual allocation</div>
			</div>
			<div
				className="p-3 rounded mb-2"
				style={{ background: "var(--pm-purple-soft)", fontSize: 13 }}
			>
				<strong>Auditor access expiring</strong>
				<div style={{ fontSize: 11 }}>KPMG Team · Expires in 48 hrs</div>
			</div>
		</MBox>
	);
	...
	return (
		<>
			...
			{renderPLSnapshot()}
			{renderTaxReadiness()}
			{renderETimsReconciliation()}
			{renderBranchPerformance()}
			{renderCustomerSpendAnalytics()}
			{renderVendorExpenseAnalytics()}
			{renderReconciliationExceptions()}
			{renderDownloadStatement()}
			{renderConfigureDashboards()}
			{renderUserActivityLog()}
			{renderStatutoryDeductions()}
			{renderExportTrialBalance()}
			{renderReportDeliverySettings()}
			{renderDisputeAudit()}
