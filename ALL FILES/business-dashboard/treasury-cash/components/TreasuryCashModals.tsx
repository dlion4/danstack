...
function downloadFile(n: string, c: string, t = "text/plain") {
...
export default function TreasuryCashModals({
	active,
	onClose,
	onOpen,
}: ModalsProps) {
			...
			footer={
				<>
					...
					{transferStep < 3 && (
						...
						>
							{transferStep === 2 ? "Execute" : "Next"}{" "}
							<i
								className={`bi ${transferStep === 2 ? "bi-lock" : "bi-arrow-right"}`}
							/>
						</button>
					)}
				</>
				...
				<div className={cx(s.fstep, s.fstepActive)}>
					<div className="mb-3">
						...
						<select className={s.formControl}>
							<option>Equity Bank — Main Operations (KES 4.2M)</option>
							<option>KCB — Business Savings (KES 2.8M)</option>
						</select>
					</div>
					<div className="mb-3">
						<label className={s.formLabel}>To Account</label>
						<select className={s.formControl}>
							<option>KCB — Business Savings</option>
							<option>NCBA — Payroll Disbursement</option>
						</select>
					</div>
					<div className="mb-3">
					...
					<div className="mb-3">
						...
						<select className={s.formControl}>
							<option>Savings sweep</option>
							<option>Payroll funding</option>
			...
			{transferStep === 2 && (
				<div className={cx(s.fstep, s.fstepActive)}>
					...
					>
						...
						<strong>Maker-Checker required</strong> · Enter authorization PIN
					</div>
	...
	const renderAddAccount = () => (
		<MBox
			...
			title={
				<>
					<i className="bi bi-plus-circle text-accent me-2" />
					Add Bank Account
				</>
			}
			footer={
				<>
					...
					{addBankStep < 3 && (
						<button
							className={cx(s.btnPm, s.btnPmP)}
							onClick={() => nextFlow("addBank", 3)}
						>
							{addBankStep === 2 ? "Connect" : "Next"}{" "}
		...
		>
			...
			{addBankStep === 1 && (
				<div className={cx(s.fstep, s.fstepActive)}>
					<div className="mb-3">
						<label className={s.formLabel}>Bank</label>
						<select className={s.formControl}>
					...
					<div className="mb-3">
						...
						<input
							type="text"
							className={s.formControl}
							defaultValue="New Treasury Account"
						/>
					</div>
				</div>
			)}
			{addBankStep === 2 && (
				<div className={cx(s.fstep, s.fstepActive)}>
					...
					>
						<i className="bi bi-link" /> <strong>Bank API connection</strong> ·
						We'll verify account ownership via bank API
					</div>
					<div className="mb-3">
					...
					<div className="mb-3">
						<label className={s.formLabel}>Swift/Branch Code</label>
						<input
							type="text"
							className={s.formControl}
							defaultValue="EQBLKENA"
						/>
					</div>
				</div>
			)}
			{addBankStep === 3 &&
				renderActionBody(
					...
					<div className={s.receipt}>
						...
						<p style={{ fontSize: 12 }}>Bank API verification successful</p>
					</div>,
				)}
		</MBox>
	);
	...
	const renderBookFX = () => (
		<MBox
			...
			title={
				<>
					<i className="bi bi-currency-exchange text-warning me-2" />
					Book FX Trade
				</>
			}
			size="lg"
			footer={
				<>
					...
					{fxStep < 3 && (
						<button
							className={cx(s.btnPm, s.btnPmP)}
							onClick={() => nextFlow("fx", 3)}
						>
							{fxStep === 2 ? "Execute" : "Next"}{" "}
							<i
								className={`bi ${fxStep === 2 ? "bi-lock" : "bi-arrow-right"}`}
							/>
						</button>
					)}
				</>
			...
			{fxStep === 1 && (
				<div className={cx(s.fstep, s.fstepActive)}>
					<div className="mb-3">
						...
						<select className={s.formControl}>
							<option>USD/KES — 129.10</option>
							<option>EUR/KES — 140.25</option>
							<option>GBP/KES — 162.80</option>
						</select>
					</div>
					<div className="mb-3">
				...
				<div className={cx(s.fstep, s.fstepActive)}>
					...
					>
						<div className="d-flex justify-content-between">
							<span>Trade</span>
							<strong>Buy USD 10,000 @ 129.10</strong>
						</div>
						<div className="d-flex justify-content-between">
							<span>KES Cost</span>
							<strong>KES 1,291,000</strong>
						</div>
						<div className="d-flex justify-content-between">
							<span>Source Account</span>
							<strong>Co-op Bank FX Reserve</strong>
						</div>
					</div>
				</div>
			)}
			{fxStep === 3 &&
				renderActionBody(
					...
					<div className={s.receipt}>
						...
						<p style={{ fontSize: 12 }}>
							Ref: FX-2025-0010 · USD 10,000 @ 129.10
						</p>
					</div>,
				)}
		</MBox>
	...
	const renderCrossBorder = () => (
		<MBox
			...
			title={
				<>
					<i className="bi bi-globe2 text-info me-2" />
			...
			footer={
				<>
					...
					{cbStep < 3 && (
						<button
							className={cx(s.btnPm, s.btnPmP)}
							onClick={() => nextFlow("cb", 3)}
						>
							{cbStep === 2 ? "Execute" : "Next"}{" "}
							<i
								className={`bi ${cbStep === 2 ? "bi-lock" : "bi-arrow-right"}`}
							/>
						</button>
					)}
				</>
		...
		>
			...
			{cbStep === 1 && (
				<div className={cx(s.fstep, s.fstepActive)}>
					<div className="mb-3">
						<label className={s.formLabel}>Destination Country</label>
						<select className={s.formControl}>
					...
					<div className="mb-3">
						...
						<input
							type="text"
							className={s.formControl}
							defaultValue="Global Vendor Inc."
						/>
					</div>
					<div className="mb-3">
						<label className={s.formLabel}>Amount</label>
			...
			{cbStep === 2 && (
				<div className={cx(s.fstep, s.fstepActive)}>
					...
					>
						<div className="d-flex justify-content-between">
							<span>FX Rate</span>
							<strong>USD/KES 129.10</strong>
						</div>
						<div className="d-flex justify-content-between">
							<span>KES Amount</span>
							<strong>KES 645,500</strong>
						</div>
						<div className="d-flex justify-content-between">
							<span>Fee</span>
			...
			{cbStep === 3 &&
				renderActionBody(
					...
					<div className={s.receipt}>
						...
						<h5 style={{ fontWeight: 700, color: "var(--pm-accent)" }}>
							Cross-border payment sent!
						</h5>
						<p style={{ fontSize: 12 }}>Ref: CB-2025-005</p>
					</div>,
				)}
		</MBox>
	);
	...
	const renderInvestCash = () => (
		<MBox
			...
			title={
				<>
					<i className="bi bi-graph-up-arrow text-purple me-2" />
					Invest Cash
				</>
			}
			footer={
				<>
					...
					{investStep < 3 && (
						<button
							className={cx(s.btnPm, s.btnPmP)}
							onClick={() => nextFlow("invest", 3)}
						>
							{investStep === 2 ? "Invest" : "Next"}{" "}
							<i className="bi bi-arrow-right" />
						</button>
					)}
				</>
		...
		>
			...
			{investStep === 1 && (
				<div className={cx(s.fstep, s.fstepActive)}>
					<div className="mb-3">
						<label className={s.formLabel}>Investment Type</label>
						<select className={s.formControl}>
							<option>MMF (CIC Money Market)</option>
							<option>T-Bill 91-day</option>
							<option>T-Bill 182-day</option>
							<option>Fixed Deposit 30-day</option>
						</select>
					</div>
					<div className="mb-3">
						<label className={s.formLabel}>Amount (KES)</label>
				...
				<div className={cx(s.fstep, s.fstepActive)}>
					...
					>
						...
						<div className="d-flex justify-content-between">
							...
							<strong style={{ color: "var(--pm-accent)" }}>~11% p.a.</strong>
						</div>
						<div className="d-flex justify-content-between">
							<span>Maturity</span>
							<strong>Open (withdraw anytime)</strong>
						</div>
					</div>
				</div>
			)}
			{investStep === 3 &&
				renderActionBody(
					...
					<div className={s.receipt}>
						...
						<p style={{ fontSize: 12 }}>Ref: INV-MMF-2025-005</p>
					</div>,
				)}
		</MBox>
	);
		...
		<MBox
			...
			footer={
				<>
					...
					<button
						className={cx(s.btnPm, s.btnPmP)}
						onClick={() => doAction("sweepSetupModal", "Sweep configured!")}
					>
						Save
					</button>
				</>
				...
				<>
					...
					<div className="form-check">
						<input
							...
							defaultChecked
						/>
						<label className="form-check-label">Daily sweep at 6 PM</label>
					</div>
				</>,
			)}
		</MBox>
		...
		>
			...
			>
				...
				<div className="d-flex justify-content-between">
					...
					<strong>22 Jun 2025 · KES 500K → KCB</strong>
				</div>
				<div className="d-flex justify-content-between">
					<span>Threshold</span>
					<strong>KES 2.0M</strong>
				</div>
			</div>
		</MBox>
	);
	...
	const renderReconciliation = () => (
		...
		>
			{renderActionBody(
				...
				<>
					<div className="mb-3">
						<label className={s.formLabel}>Account</label>
						<select className={s.formControl}>
							<option>Equity Bank — Main Operations</option>
						</select>
					</div>
					...
					<div
						...
						style={{ background: "var(--pm-warning-soft)", fontSize: 13 }}
					>
						1 unreconciled statement · 3 unmatched transactions
					</div>
				</>,
			)}
		</MBox>
		...
		>
			<div className={s.statusRow}>
				...
				<button
					...
					onClick={() => doAction("approvalQueueModal", "All sweeps approved!")}
				>
					Review
				</button>
			</div>
		...
		>
			...
			>
				<div className="d-flex justify-content-between">
					...
					<strong>KES 10.2M</strong>
				</div>
				<div className="d-flex justify-content-between">
					<span>MMF</span>
					<strong>KES 3.2M (11%)</strong>
				</div>
				<div className="d-flex justify-content-between">
					<span>T-Bills</span>
					<strong>KES 5.0M (12.5%)</strong>
				</div>
				<div className="d-flex justify-content-between">
					<span>Fixed Deposit</span>
					<strong>KES 2.0M (9%)</strong>
				</div>
			</div>
			<button
				className={cx(s.btnPm, s.btnSm)}
			...
			>
				<i className="bi bi-plus" /> New Investment
			</button>
		</MBox>
	);
		...
		<MBox
			...
			footer={
				<>
					...
					<button
						className={cx(s.btnPm, s.btnPmP)}
						onClick={() => doAction("treasurySettingsModal", "Settings saved.")}
					>
						Save
					</button>
				</>
				...
				<>
					<div className="mb-3">
						<label className={s.formLabel}>
							Global Daily Transfer Limit (KES)
						</label>
						<input
							type="number"
							className={s.formControl}
					...
					<div className="mb-3">
						<label className={s.formLabel}>Approval Policy</label>
						<select className={s.formControl}>
							<option>Maker-Checker required for all</option>
							<option>Required only {">"} 1,000,000</option>
					...
					<div className="form-check form-switch mb-2">
						...
						<label className="form-check-label">
							Auto-reconciliation daily at 6 AM
						</label>
					</div>
		...
		>
			...
			>
				...
				<div style={{ fontSize: 11 }}>
					Sweep of KES 1.4M pending your approval
				</div>
			</div>
			<div
				className="p-3 rounded mb-2"
				style={{ background: "var(--pm-warning-soft)", fontSize: 13 }}
			>
				<strong>FX Alert Hit</strong>
				<div style={{ fontSize: 11 }}>USD/KES fell below 129.50</div>
			</div>
			<div
				className="p-3 rounded mb-2"
				style={{ background: "var(--pm-accent-soft)", fontSize: 13 }}
			>
				<strong>Sweep Successful</strong>
				<div style={{ fontSize: 11 }}>
					End of day sweep completed to Equity Bank
				</div>
			</div>
		</MBox>
	);
		...
		>
			<div className="text-center">
				...
				<p style={{ fontSize: 13, color: "var(--pm-muted)" }}>
					esther.a@corporate.com · CFO / Treasury
				</p>
				<div className="row g-2 text-start mt-3" style={{ fontSize: 13 }}>
					<div className="col-6">
						...
						>
							<span style={{ color: "var(--pm-muted)" }}>Auth Limit</span>
							<br />
					...
					<div className="col-6">
						<div
							...
							style={{ background: "var(--pm-surface-2)" }}
						>
							<span style={{ color: "var(--pm-muted)" }}>Accounts</span>
							<br />
							<strong>6 managed</strong>
						</div>
					</div>
				</div>
			</div>
