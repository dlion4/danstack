...
const initialMockData: SettlementConfig = {
	...
	headerSub:
		"Real-time settlements, clearing house integration, reconciliation, and compliance",
	...
	hero: {
		...
		detail:
			"1,284 transactions • 99.7% success rate • 14 batches processed • 3 disputes under review.",
	},
	statCards: [
		{
			key: "settled",
	...
	quickActions: [
		{
			...
			modal: "initiateSettlementModal",
		},
		{
			icon: "bi-list-check",
			label: "Reconcile",
			color: "var(--pm-accent)",
			modal: "reconciliationWizardModal",
		},
		{
			icon: "bi-upload",
			label: "Bulk Upload",
	...
	exceptionQueue: [
		{
			...
			sub: "14 items • KES 18.4M",
			actionLabel: "Retry All",
			modal: "retrySettlementModal",
		},
		{
			title: "Partial settlements",
			sub: "8 items • KES 6.2M",
			actionLabel: "Resolve",
			modal: "partialSettlementModal",
		},
		{
	...
	autoRules: [
		...
		{
			title: "Auto-escalate disputes > KES 5M",
			sub: "Route to Treasury Manager",
			status: "Active",
			tone: "badgeS",
		},
		{
			title: "Weekend batch deferral",
			sub: "Hold non-urgent batches until Monday 8 AM",
			status: "Paused",
			tone: "badgeW",
		},
	],
	...
	regReports: [
		{
			label: "CBK Daily Settlement Return",
			status: "Submitted",
			tone: "badgeS",
		},
		{ label: "KRA Withholding Tax", status: "Due in 2 days", tone: "badgeW" },
		{
			label: "AML Large Transaction Report",
			status: "Submitted",
			tone: "badgeS",
		},
	],
...
const ACTIVE_SETTLEMENTS = {
	...
	rows: [
		[
			...
			"STR:KES 45.0M",
			"RTGS",
			{ badge: "In Progress", tone: "badgeI" as BadgeTone },
			"14m",
			{ action: "Track", modal: "settlementDetailModal" },
		],
		[
			...
			"STR:KES 12.8M",
			"PesaLink",
			{ badge: "Settled", tone: "badgeS" as BadgeTone },
			"—",
			{ action: "Receipt", modal: "settlementDetailModal" },
		],
		[
			...
			"STR:KES 8.4M",
			"RTGS",
			{ badge: "Retry #2", tone: "badgeW" as BadgeTone },
			"32m",
			{ action: "Force", modal: "retrySettlementModal", tone: "btnPmD" },
		],
	],
};
		...
		<div className={styles.settlementPage}>
			...
			{error && !errorDismissed && (
				<div
					...
					role="alert"
				>
					<strong>Could not load settlement data.</strong> Showing the built-in
					defaults.{" "}
				...
				<div className={styles.content}>
					...
					<div className="row g-3">
						...
						{config.statCards.map((card) => (
							<div
								className={`${card.key === "settled" ? "col-lg-2" : "col-lg-3"} col-md-4 col-6`}
								key={card.key}
							>
								<div
								...
								>
									...
									<div
										...
										style={{
											margin: "6px 0",
											fontSize: card.key === "disputes" ? 24 : undefined,
										}}
									>
										{card.value}
									</div>
									...
									{card.kind === "progress" && card.progress && (
										<div className="mt-2">
											{card.progress.map((p, i) => (
												<div key={p.label}>
													<div
														className={`d-flex justify-content-between ${i > 0 ? "mt-1" : ""}`}
						...
						>
							<div>
								<h3 className={styles.st}>
									...
									/>{" "}
									1.7.1 — Settlement Overview Dashboard
								</h3>
								<p className={styles.ss}>
									Real-time view of all settlement channels, success rates,
									pending volumes, and exception queues.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
									className={`${styles.btnPm} ${styles.btnSm}`}
									onClick={() => openM("settlementCalendarModal")}
								>
							...
							<div className="col-lg-8">
								<div className={styles.ub}>
									<h4 className={styles.ubTitle}>
										Settlement Channels Performance
									</h4>
						...
						>
							<div>
								<h3 className={styles.st}>
									...
									/>{" "}
									1.7.2 — Real-time Settlement Engine
								</h3>
								<p className={styles.ss}>
									Instant settlement initiation, status tracking, and automated
									retry logic for high-value and time-critical transactions.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
							...
							<div className="col-lg-5">
								<div className={styles.ub}>
									...
									<button
										...
										onClick={() => openM("engineConfigModal")}
									>
										Configure Engine
									</button>
								</div>
						...
						>
							<div>
								<h3 className={styles.st}>
									...
									/>{" "}
									1.7.3 — Clearing House Integration
								</h3>
								<p className={styles.ss}>
									PesaLink, RTGS, ACH, and SWIFT clearing status, cut-off
									timers, and direct integration controls.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
									className={`${styles.btnPm} ${styles.btnSm}`}
									onClick={() => openM("clearingStatusModal")}
								>
									Live Status
								</button>
							</div>
								...
								<div className="col-lg-6" key={ch.title}>
									<div className={styles.ub}>
										...
										<div className={styles.sr}>
											...
											<span
												className={`${styles.badge} ${styles[ch.windowStatus.tone]}`}
											>
												{ch.windowStatus.text}
											</span>
										</div>
										<div className={styles.sr}>
											<div>
						...
						>
							<div>
								<h3 className={styles.st}>
									...
									/>{" "}
									1.7.4 — Reconciliation & Dispute Resolution
								</h3>
								<p className={styles.ss}>
									Automated and manual reconciliation, exception handling, and
							...
							<div>
								<h3 className={styles.st}>
									<i
										...
										style={{ color: "var(--pm-info)" }}
									/>{" "}
									1.7.5 — Settlement Reports & Analytics
								</h3>
								<p className={styles.ss}>
									Daily, weekly, and monthly settlement performance, fee
									analysis, and regulatory reports.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
						...
						>
							<div>
								<h3 className={styles.st}>
									...
									/>{" "}
									1.7.6 — Automated Settlement Rules
								</h3>
								<p className={styles.ss}>
									Configure intelligent auto-settlement, retry logic, cut-off
							...
							<div>
								<h3 className={styles.st}>
									<i
										...
										style={{ color: "var(--pm-purple)" }}
									/>{" "}
									1.7.7 — Nostro/Vostro & Regulatory Compliance
								</h3>
								<p className={styles.ss}>
									Cross-border account management, FX settlement, and automated
