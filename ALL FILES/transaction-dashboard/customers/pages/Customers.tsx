...
const initialMockData: CustomersConfig = {
	...
	headerSub:
		"Full lifecycle customer onboarding, KYC, accounts, permissions and support",
	...
	hero: {
		...
		detail:
			"3,241 active accounts • 1,284 pending KYC • 347 corporate entities • 98% verification rate",
		actions: [
	...
	statCards: [
		...
		},
		{
			...
			lines: [
				"3 KYC disputes • 4 account issues",
				"2 card requests • 5 general",
			],
			attention: true,
		},
	],
	...
	accounts: [
		...
		{
			num: "ACC-20240302-3344",
			cust: "Grace Wanjiku Ltd",
			type: "Savings",
			bal: "KES 2,847,200",
			status: "Active",
			opened: "02 Mar 2024",
		...
		{
			num: "ACC-20240511-5566",
			cust: "Samuel Kipchoge",
			type: "Current",
			bal: "KES 8,200",
			status: "Suspended",
			opened: "11 May 2024",
	...
	kycApprovedNote:
		"Last 30 days: 312 customers approved automatically via eKYC.",
	kycRejectedNote:
		"24 rejections this month. Common reasons: blurry ID, mismatched address, expired documents.",
	riskDist: [
	...
	linkedBanks: [
		{
			name: "Equity Bank (Open Banking)",
			sub: "2 accounts linked • Last sync 2h ago",
			status: "Active",
			tone: "badgeS",
		},
		{
			name: "KCB Bank",
			sub: "1 account linked • Consent expires 12 Jul",
			status: "Expiring",
			tone: "badgeW",
		},
		{
			name: "Co-op Bank",
			sub: "Consent revoked by customer",
			status: "Revoked",
			tone: "badgeD",
		},
	],
		...
		<div className={styles.customersPage}>
			...
			{error && !errorDismissed && (
				<div
					...
					role="alert"
				>
					<strong>Could not load customer data.</strong> Showing the built-in
					defaults.{" "}
			...
			<div className={styles.main}>
				...
				<div className={styles.pageBar}>
					...
					<div className="d-flex flex-wrap" style={{ gap: 8 }}>
						...
						<button
							className={styles.btnPm}
							onClick={() => openM("supportTicketsModal")}
						>
					...
					<div className="row g-3">
						...
						{config.statCards.map((card) => (
							<div className={card.colClass} key={card.key}>
								<div
									className={`${styles.card} ${card.attention ? styles.attentionCard : ""}`}
									style={{ minHeight: 170 }}
								>
							...
							<div className={`${styles.card} h-100`}>
								<div className="mb-3">
									...
									<p className={styles.ss}>
										Frequent customer &amp; account workflows
									</p>
								</div>
									...
									{config.quickActions.map((qa) => (
										...
										>
											<i
												...
												style={{ color: ACTIVE_COLOR[qa.icon] ?? qa.color }}
											/>{" "}
											{qa.label}
										</button>
									))}
						...
						>
							<div>
								<h3 className={styles.st}>
									...
									/>{" "}
									1.14.1 — Customer Directory &amp; Onboarding
								</h3>
								<p className={styles.ss}>
									Complete customer list with search, filters, onboarding
									status, KYC progress and quick actions.
								</p>
							</div>
												...
												{config.customers.map((c) => {
													...
													return (
														<tr key={c.name}>
															...
															<td>
																<span
																	className={`${styles.badge} ${styles[kb.tone]}`}
																>
																	{kb.text}
																</span>
															</td>
															...
															<td>
																<span
																	className={`${styles.badge} ${c.status === "Active" ? styles.badgeS : styles.badgeD}`}
																>
																	{c.status === "Active"
																		? "Active"
																		: "Suspended"}
					...
					<div className={styles.card}>
						...
						>
							<div>
								...
								<p className={styles.ss}>
									Open, close, suspend, reactivate and manage all account types
											...
											<tbody>
												{config.accounts.map((a) => (
													<tr key={a.num}>
														...
														<td>
															<span
																className={`${styles.badge} ${a.status === "Active" ? styles.badgeS : styles.badgeD}`}
															>
						...
						>
							<div>
								<h3 className={styles.st}>
									...
									/>{" "}
									1.14.3 — KYC / AML &amp; Compliance Center
								</h3>
								<p className={styles.ss}>
									End-to-end KYC workflow, document management, risk scoring,
									PEP screening and audit logs.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
								...
								<div className={styles.ub}>
												...
												<tbody>
													{config.kycQueue.map((r) => (
														<tr key={r.customer}>
															...
															<td>
																<span
																	className={`${styles.badge} ${styles[r.riskTone]}`}
																>
																	{r.risk}
																</span>
															</td>
									...
									{kycTab === "approved" && (
										<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
											{config.kycApprovedNote}
										</p>
									)}
									{kycTab === "rejected" && (
										<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
											{config.kycRejectedNote}
										</p>
									)}
								</div>
							</div>
						...
						>
							<div>
								<h3 className={styles.st}>
									...
									/>{" "}
									1.14.4 — Permissions, Roles &amp; Access Control
								</h3>
								<p className={styles.ss}>
									Role-based access, user permissions, API keys, linked users
									and audit logs.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
					...
					<div className={styles.card}>
						...
						>
							<div>
								...
								<p className={styles.ss}>
									Generate statements, compliance reports, send notifications
									and manage customer communication preferences.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
												...
												{config.commPrefs.map((s) => (
													<tr key={s.segment}>
														...
														<td>
															<input
																type="checkbox"
																defaultChecked={s.email}
																aria-label={`${s.segment} email`}
															/>
														</td>
														<td>
															<input
																...
																aria-label={`${s.segment} sms`}
															/>
														</td>
														<td>
															<input
																type="checkbox"
																defaultChecked={s.push}
																aria-label={`${s.segment} push`}
															/>
														</td>
														<td>
															<input
																type="checkbox"
																defaultChecked={s.whatsapp}
																aria-label={`${s.segment} whatsapp`}
															/>
														</td>
														<td>
															<button
						...
						>
							<div>
								<h3 className={styles.st}>
									...
									/>{" "}
									1.14.6 — Support Tickets &amp; Customer Service
								</h3>
								<p className={styles.ss}>
									Full ticketing system with SLA tracking, knowledge base and
											...
											<tbody>
												{config.tickets.map((t) => (
													<tr key={t.t}>
														...
														<td>
															<span
																className={`${styles.badge} ${styles[priorityBadge(t.p)]}`}
															>
																{t.p}
															</span>
														</td>
														<td>
															<span
																className={`${styles.badge} ${styles[ticketStatusBadge(t.st).tone]}`}
															>
							...
							<div>
								<h3 className={styles.st}>
									<i
										...
										style={{ color: "var(--pm-primary-light)" }}
									/>{" "}
									1.14.7 — Linked Services &amp; External Connections
								</h3>
								<p className={styles.ss}>
									Manage linked external bank accounts, open banking consents,
								...
								<div className={styles.ub}>
									...
									{config.linkedBanks.map((b) => (
										<div className={styles.sr} key={b.name}>
											...
											<span className={`${styles.badge} ${styles[b.tone]}`}>
												{b.status}
											</span>
										</div>
									))}
								...
								<div className={styles.ub}>
									...
									{config.integrations.map((i) => (
										<div className={styles.sr} key={i.name}>
											...
											<span className={`${styles.badge} ${styles[i.tone]}`}>
												Active
											</span>
										</div>
									))}
