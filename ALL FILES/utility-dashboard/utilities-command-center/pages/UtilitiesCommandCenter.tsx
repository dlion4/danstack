...
import { fetchCommandCenter, initialMockData } from "../data/commandCenterData";
...
export default function UtilitiesCommandCenter() {
	const { data } = useQuery({
		queryKey: ["utilities-command-center"],
	...
	const connected =
		filter === "all"
			? config.connectedServices
			: config.connectedServices.filter((c) => c.status === filter);
	const filterCounts = {
		all: config.connectedServices.length,
		active: config.connectedServices.filter((c) => c.status === "active")
			.length,
		pending: config.connectedServices.filter((c) => c.status === "pending")
			.length,
		paused: config.connectedServices.filter((c) => c.status === "paused")
			.length,
	};
			...
			<div className={s.main}>
				...
				<div className={s.pageBar}>
					...
					<div className="d-flex flex-wrap" style={{ gap: 8 }}>
						<button
							className={s.btnPm}
							onClick={() => open("healthCheckModal")}
						>
							<i className="bi bi-heart-pulse" /> Health Check
						</button>
						<button className={s.btnPm} onClick={() => open("billInboxModal")}>
							<i className="bi bi-inbox" /> Bill Inbox
						</button>
						<button
							className={s.btnPm}
							onClick={() => open("payBillQuickModal")}
						>
							<i className="bi bi-credit-card" /> Pay Bill
						</button>
						<button
							className={`${s.btnPm} ${s.btnPmP}`}
							onClick={() => open("addUtilityModal")}
						>
							<i className="bi bi-plus-lg" /> Add Utility
						</button>
					</div>
				</div>
				...
				<div className={s.content}>
					...
					<div className="row g-3">
						<div className="col-lg-4">
							...
							>
								...
								>
									{config.hero.live} <span style={{ color: "#34D399" }}>●</span>
								</p>
								<div
									className={s.sv}
							...
							<div className={s.card}>
								<div className="d-flex justify-content-between align-items-center mb-3">
									...
									<button
										...
										onClick={() => open("attentionDetailModal")}
									>
										View all
									</button>
								</div>
							...
							<div className={s.card}>
								<div className="mb-3">
									...
									<p className={s.sectionSub}>
										Frequent utility payment workflows
									</p>
								</div>
						...
						>
							<div>
								<h3 className={s.sectionTitle}>
									...
									/>{" "}
									3.1.1 — Utilities Overview Dashboard
								</h3>
								<p className={s.sectionSub}>
									All your connected services with real-time status and
									quick-pay options.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
									className={`${s.btnPm} ${s.btnSm}`}
									onClick={() => open("manageUtilitiesModal")}
								>
						...
						<div className={s.serviceGrid}>
							{config.services.map((svc) => (
								<div
									...
									onClick={() => open(svc.modal)}
									role="button"
									tabIndex={0}
								>
									<span
										className={s.statusDot}
										style={{ background: STATUS_COLOR[svc.status] }}
										title={svc.status}
									/>
									<div
										className={`${s.iconCircle} mx-auto mb-2`}
										style={{
											background: svc.bg,
											color: svc.color,
											width: 44,
						...
						>
							<div>
								<h3 className={s.sectionTitle}>
									...
									/>{" "}
									3.1.2 — Connected Services & Status Monitor
								</h3>
								<p className={s.sectionSub}>
									Monitor, onboard, edit, pause or manage all your connected
									utility accounts.
								</p>
							</div>
							<div className={s.tabPills}>
								{(
									...
									<button
										key={key}
										className={`${s.tabPill} ${filter === key ? s.tabPillActive : ""}`}
									...
									{connected.map((c) => {
										...
										return (
											<tr key={`${c.provider}-${c.account}`}>
												...
												<td data-label="Account / Meter">
													<code style={{ fontSize: 12 }}>{c.account}</code>
												</td>
												<td data-label="Status">
													<span className={`${s.badge} ${sb.cls}`}>
														{sb.label}
													</span>
												</td>
												<td data-label="Last Payment" style={{ fontSize: 12 }}>
													{c.lastPay}
												</td>
												<td data-label="Next Due" style={{ fontSize: 12 }}>
													{c.nextDue}
												</td>
						...
						>
							<div>
								<h3 className={s.sectionTitle}>
									...
									/>{" "}
									3.1.3 — Utility Spending Analytics & Budget Tracker
								</h3>
								<p className={s.sectionSub}>
									Track spending trends, set budgets, and discover cost-saving
									opportunities.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
									className={`${s.btnPm} ${s.btnSm}`}
									onClick={() => open("budgetSettingsModal")}
								>
									<i className="bi bi-sliders" /> Budget Settings
								</button>
								<button
									className={`${s.btnPm} ${s.btnSm}`}
									onClick={() => open("exportReportModal")}
								>
						...
						<div className="row g-3">
									...
									>
										{config.spendByCategory.map((c) => (
											<div
												...
												style={{ height: c.height, background: c.color }}
												onClick={() => open("spendDetailModal")}
												role="button"
												tabIndex={0}
											>
							...
							<div className="col-lg-4">
								<div className={s.utilityBlock}>
									...
									<button
										...
										onClick={() => open("budgetSettingsModal")}
									>
										<i className="bi bi-pencil" /> Edit Budgets
									</button>
								</div>
							</div>
							<div className="col-lg-3">
								<div className={s.utilityBlock}>
									...
									<h4
										style={{ fontSize: 13, fontWeight: 600, margin: "0 0 8px" }}
									>
										<i className="bi bi-lightbulb text-warning" /> Tips
									</h4>
									<ul
										style={{
											fontSize: 12,
						...
						>
							<div>
								<h3 className={s.sectionTitle}>
									...
									/>{" "}
									3.1.4 — Smart Automation & Household Management
								</h3>
								<p className={s.sectionSub}>
									Auto-pay configurations, scheduled payments, household
									members, and notification preferences.
								</p>
							</div>
						</div>
						<div className="row g-3">
								...
								<div className={s.utilityBlock}>
									<div className="d-flex justify-content-between align-items-center mb-3">
										<h4 style={{ fontSize: 14, fontWeight: 600, margin: 0 }}>
											<i className="bi bi-arrow-repeat text-primary me-1" />{" "}
											Auto-Pay Rules
										</h4>
										<button
											className={`${s.btnPm} ${s.btnSm}`}
											onClick={() => open("autoPaySetupModal")}
										>
									...
									{config.autoPayRules.map((r) => (
										...
										>
											<div>
												...
												<div style={{ color: "var(--pm-muted)", fontSize: 11 }}>
													{r.rule}
												</div>
											</div>
											<div className="form-check form-switch m-0">
												<input
													...
													aria-label={`Toggle auto-pay ${r.name}`}
												/>
											</div>
										</div>
									))}
							...
							<div className="col-lg-4">
								<div className={s.utilityBlock}>
									<div className="d-flex justify-content-between align-items-center mb-3">
										...
										<button
											className={`${s.btnPm} ${s.btnSm}`}
											onClick={() => open("schedulePaymentModal")}
										>
											<i className="bi bi-plus" /> Schedule
										</button>
									</div>
					...
					<div className={s.card}>
						<div className="d-flex justify-content-between align-items-center mb-3">
							...
							<button
								className={`${s.btnPm} ${s.btnSm}`}
								onClick={() => open("transactionHistoryModal")}
							>
								View All
							</button>
						</div>
