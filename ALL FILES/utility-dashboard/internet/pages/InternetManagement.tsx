...
import { fetchInternetManagement, initialMockData } from "../data/internetData";
...
export default function InternetManagement() {
	const { data } = useQuery({
		queryKey: ["internet-management"],
		queryFn: fetchInternetManagement,
		staleTime: 60_000,
		retry: 1,
	});
			...
			<div className={s.main}>
				<div className={s.pageBar}>
					...
					<div className="d-flex flex-wrap" style={{ gap: 8 }}>
						<button
							className={s.btnPm}
							onClick={() => open("connectivityHealthModal")}
						>
							<i className="bi bi-activity" /> Net Health
						</button>
						<button
							className={s.btnPm}
							onClick={() => open("dataUsageAnalyticsModal")}
						>
							<i className="bi bi-pie-chart" /> Usage
						</button>
						<button className={s.btnPm} onClick={() => open("bulkTopupModal")}>
							<i className="bi bi-collection" /> Bulk Pay
						</button>
						<button
							className={`${s.btnPm} ${s.btnPmP}`}
							onClick={() => open("addConnectionModal")}
						>
							<i className="bi bi-plus-lg" /> Add Connection
						</button>
					</div>
				</div>
				...
				<div className={s.content}>
							...
							<div className={s.card}>
								<div className="mb-3">
									...
									<p className={s.sectionSub}>
										Frequent connectivity workflows
									</p>
								</div>
						...
						>
							<div>
								<h3 className={s.sectionTitle}>
									...
									/>{" "}
									3.4.1 — Broadband & Fiber Connections
								</h3>
								<p className={s.sectionSub}>
									Manage your fixed home and office internet connections.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
									className={`${s.btnPm} ${s.btnSm}`}
									onClick={() => open("reportFaultModal")}
								>
					...
					<div className={s.card}>
						...
						>
							<div>
								<h3 className={s.sectionTitle}>
									<i className="bi bi-sim" style={{ color: "#10B981" }} /> 3.4.2
									— Mobile Data & Airtime Management
								</h3>
								<p className={s.sectionSub}>
									Manage multiple SIM cards, track data balances, and automate
									bundle renewals.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
									className={`${s.btnPm} ${s.btnSm}`}
									onClick={() => open("bulkTopupModal")}
								>
									<i className="bi bi-collection" /> Bulk Pay
								</button>
								<button
									className={`${s.btnPm} ${s.btnSm} ${s.btnPmP}`}
									onClick={() => open("buyDataModal")}
								>
									<i className="bi bi-phone" /> Buy Bundle
								</button>
							</div>
						</div>
						<div className="row g-3">
								...
								<div key={sim.number} className="col-md-4">
									...
									>
										<div className="d-flex align-items-center gap-3 mb-3">
											<div
												className={s.iconCircle}
												style={{ background: sim.bg, color: sim.color }}
											>
										...
										<div className="row g-2 mb-3">
											<div className="col-6">
												...
												<strong style={{ fontSize: 16 }}>{sim.bal}</strong>
											</div>
											<div className="col-6">
												<div style={{ fontSize: 10, color: "var(--pm-muted)" }}>
													EXPIRY
						...
						>
							<div>
								<h3 className={s.sectionTitle}>
									...
									/>{" "}
									3.4.3 — Analytics, Subscriptions & Automation
								</h3>
								<p className={s.sectionSub}>
									Analyze bandwidth usage, manage auto-renew schedules, and
									track network outages.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
									...
									onClick={() => open("dataUsageAnalyticsModal")}
								>
									<i className="bi bi-graph-up" /> Full Report
								</button>
							</div>
						</div>
						<div className="row g-3">
							<div className="col-lg-5">
								<div className={s.utilityBlock}>
									...
									>
										<span>
											<strong>Total:</strong> 1.2 TB
										</span>
										<span>
											<strong>Peak:</strong> Streaming (68%)
										</span>
										<span>
											<strong>Cost:</strong> KES 14,200
										</span>
									</div>
								</div>
							</div>
							<div className="col-lg-3">
								<div className={s.utilityBlock}>
									...
									<button
										...
										onClick={() => open("autoRenewSetupModal")}
									>
										Manage Rules
									</button>
								</div>
							...
							<div className="col-lg-4">
								<div className={s.utilityBlock}>
									...
									{config.networkStatus.map((n) => {
										const c = NET_BG[n.tone];
										return (
											<div
												key={n.name}
											...
											>
												<div style={{ fontWeight: 600, color: c.color }}>
													<i className={`bi ${n.icon} me-1`} /> {n.name}
												</div>
												<div
													style={{
														fontSize: 11,
														color:
															n.tone === "muted" ? "var(--pm-muted)" : c.color,
													}}
												>
													{n.detail}
												</div>
									...
									<button
										...
										onClick={() => open("networkOutagesModal")}
									>
										View Outage Map
									</button>
								</div>
