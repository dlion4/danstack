						...
						{relStep === 1 && (
							<>
								...
								<div className={`${s.note} ${s.noteInfo}`}>
									<i className="bi bi-info-circle me-1" /> Semantic versioning
									is enforced. A major bump requires a 6-month deprecation
									notice for the prior version.
								</div>
							</>
						)}
						{relStep === 2 && (
						...
						{relStep === 3 && (
							<>
								...
								<div className={`${s.note} ${s.noteWarn}`}>
									<i className="bi bi-exclamation-triangle me-1" /> Publishing
									is irreversible. The spec will be frozen and served from the
									gateway immediately.
								</div>
							</>
						)}
						{relStep >= 4 && (
							<div className={s.receipt}>
								...
								<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
									v2.5.0 is live on the gateway and developers have been
									notified.
								</p>
							</div>
						)}
					...
					<>
							...
							<>
								...
								<div className="mb-3">
									...
									<Fld
										s={s}
										as="select"
										options={["v2.4.1 (Current)", "v3.0.0-beta"]}
									/>
								</div>
							</>
						)}
						{depStep === 3 && (
							<div className="mb-3">
								...
								<Chk
									label="Emit Sunset + Deprecation response headers"
									defaultChecked
								/>
								<Chk label="Email affected consumers monthly" defaultChecked />
								<Chk
									label="Show a banner in the developer portal"
									defaultChecked
								/>
						...
						{depStep >= 4 && (
							<div className={s.receipt}>
								...
								<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
									Sunset headers are now active. Consumers will be notified
									monthly.
								</p>
							</div>
						)}
				...
				{m.tab("migTab", "v1v2") === "v1v2" ? (
					<>
						...
						>
							...
							<li>
								<code className={s.codeInline}>bank_code</code> is now required
								on bank disbursements
							</li>
...
+ POST /v2/collections   { "transaction_ref": "..." }`}
					...
					<>
						...
						>
							<li>
								GraphQL endpoint at{" "}
								<code className={s.codeInline}>/v3/graphql</code>
							</li>
					...
					<>
						...
						<button
							...
							onClick={() =>
								m.doAction(
									"runLinterModal",
									"Lint run complete. Report sent to CI.",
								)
							}
						>
							Re-run Linter
					...
					m.cancelAnd(
						<button
							...
							onClick={() =>
								m.doAction(
									"submitFeatureModal",
									"Feature request submitted for review!",
								)
							}
						>
							Submit Request
			...
			>
				{m.body(
					...
					<>
						...
						<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
							Mobile clients over-fetch on the REST balance and transaction
							endpoints. A GraphQL layer would let each screen request exactly
							the fields it needs, cutting payload size and battery use on
							low-end Android devices.
						</p>
						<div className={`${s.note} ${s.noteMuted} mb-3`}>
							<strong>PayMo response:</strong> On the v3 roadmap. A closed beta
							is planned for Q4 2026 alongside the gRPC ledger stream.
						</div>
			...
			>
				{m.body(
					...
					<>
						...
						>
							Follow this item to receive an email whenever the status, scope or
					...
					<>
						...
						<div className="mb-3">
							...
							<Fld
								s={s}
								mono
								defaultValue="https://docs.paymo.com/migrate/v1-to-v2"
							/>
						</div>
					...
					m.cancelAnd(
						<button
							...
							onClick={() =>
								m.doAction(
									"testAutomationConfigModal",
									"CI/CD hooks updated successfully.",
								)
							}
						>
							Save Config
					...
					<>
						<div className="mb-3">
							...
							<Fld
								s={s}
								as="select"
								options={["Pact", "Dredd", "Schemathesis", "Custom"]}
							/>
						</div>
						<div className="mb-3">
						...
						<div className="mb-3">
							...
							<Fld
								...
								type="url"
								mono
								placeholder="https://ci.company.com/hooks/paymo"
							/>
						</div>
					</>,
				)}
			...
			>
				<div className="d-flex flex-column gap-2">
					{[
						...
						[
							"Configure sunset headers",
							"bi-sliders",
							"configureDeprecationModal",
						],
						[
							"Broadcast breaking change",
							"bi-broadcast",
							"broadcastChangeModal",
						],
