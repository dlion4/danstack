...
function WarRoomChat() {
	const [msgs, setMsgs] = useState([
		{
			who: "John D.",
			text: "Confirmed elevated STK timeouts from Daraja.",
			t: "10:42",
		},
		{
			who: "Sarah W.",
			text: "Failing over to secondary Daraja pool.",
			t: "10:45",
		},
							...
							<div key={ch.name} className={s.statusRow}>
								...
								>
									<i
										...
										style={{ color: ch.color, fontSize: 18 }}
									/>
									<div>
					...
					<>
						<button
							...
							onClick={() =>
								m.doAction(
									"pagerDutySetupModal",
									"Test alert sent to PagerDuty. Check your phone!",
								)
							}
						>
							Send Test
				...
				{m.body(
					...
					<>
						...
						<div className={`${s.note} ${s.noteInfo}`}>
							<i className="bi bi-info-circle me-1" /> Events are sent to the
				...
				footer={m.footer(
					...
					m.cancelAnd(
						<button
							...
							className={`${s.btnPm} ${s.btnPmP}`}
							onClick={() =>
								m.doAction("escalationPolicyModal", "Escalation policy saved.")
							}
						>
							Save Policy
						</button>,
					...
					m.cancelAnd(
						<button
							...
							onClick={() =>
								m.doAction(
									"postIncidentReviewModal",
									"Incident resolved and RCA published!",
								)
							}
						>
							Publish RCA
					...
					m.cancelAnd(
						<button
							...
							onClick={() =>
								m.doAction(
									"logRetentionModal",
									"Log retention policies updated successfully.",
								)
							}
						>
							Save Policies
					...
					<>
						...
						<div className="mb-3">
							...
							<Fld
								s={s}
								as="select"
								options={["1 year", "3 years", "7 years"]}
							/>
						</div>
						<div className={`${s.note} ${s.noteWarn}`}>
							<i className="bi bi-exclamation-triangle me-1" /> CBK regulations
							require a minimum 7-year archive for financial transaction logs.
						</div>
					</>,
				)}
			</MBox>
			...
			>
				...
				{data.spans.map((sp) => (
					<div key={sp.span} className="mb-2">
						...
						>
							<span style={{ fontFamily: "var(--pm-font-mono)" }}>
								{sp.span}
							</span>
							...
							{data.spans.map((sp) => (
								<tr key={sp.span}>
									...
									<td data-label="Impact">
										<span className={`${s.badge} ${s[sp.tone]}`}>
											{sp.impact}
										</span>
									</td>
								</tr>
					...
					<>
						...
						<div className="mb-3">
							...
							<Fld
								s={s}
								as="select"
								options={["gRPC (recommended)", "HTTP/protobuf"]}
							/>
						</div>
						<div className="mb-3">
					...
					<>
						...
						<button
							...
							onClick={() =>
								m.doAction(
									"prometheusExportModal",
									"Scrape endpoint enabled and secured.",
								)
							}
						>
    ...
    static_configs:
      - targets: ['metrics.paymo.co.ke']`}
						...
						<div className="mb-3">
							...
							<Chk
								label="paymo_webhook_delivery_failures_total"
								defaultChecked
							/>
						</div>
					</>,
					...
					m.cancelAnd(
						<button
							...
							onClick={() =>
								m.doAction(
									"businessMetricModal",
									"Business metric preferences updated.",
								)
							}
						>
							Save
