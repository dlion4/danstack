...
interface CardItem {
	...
	expiry: string;
	brand: string;
	statusBadge?: { text: string; tone: BadgeTone };
	actions: { label: string; modal: string; actionClass?: string }[];
	borderDanger?: boolean;
}
interface AutoReloadRule {
	card: string;
	desc: string;
	badge: string;
	tone: BadgeTone;
...
const initialMockData: PrepaidCardManagementConfig = {
	...
	statCards: [
		{
			...
			lines: [
				"4 Physical, 8 Virtual. Used for fleet fuel, online software, and staff expenses.",
			],
		},
		{
			key: "balance",
	...
	attention: [
		{
			...
			sub: "Fleet Card #2 · Insufficient Funds",
			actionLabel: "Fund",
			actionClass: "btnDanger",
		...
		},
		{
			...
			sub: "Virtual Ads Card · Exp: 07/25",
			actionLabel: "Renew",
		...
		},
		{
			...
			iconText: "FS",
			title: "Unusual Online Spend",
			sub: "Staff Card · KES 15,000 at Amazon",
			actionLabel: "Review",
			modal: "disputeTxModal",
		},
	],
	suggestions: [
		{
			...
			sub: "Avoid declines on Marketing Ads Card",
			actionLabel: "Setup",
			modal: "autoReloadModal",
		},
		{
			icon: "bi-geo-alt",
			...
			sub: "Lock Fleet cards to Kenya only",
			actionLabel: "Apply",
			modal: "geoBlockModal",
		},
		{
			...
			iconBg: "var(--pm-info-soft)",
			iconColor: "var(--pm-info)",
			title: "Freeze Unused Card",
			sub: "Travel Card inactive for 90 days",
			actionLabel: "Freeze",
	...
	cards: [
		{
			...
			number: "4812",
			title: "Marketing Ads",
			sub: "Active · Virtual · VISA",
			balance: "KES 14,500",
			expiry: "08/26",
			brand: "VISA",
			actions: [
				{ label: "Fund", modal: "topupCardModal" },
				{ label: "Controls", modal: "cardControlsModal" },
			],
		},
		{
			id: "p1",
			type: "physical",
			bg: "bgGradientEmerald",
			number: "9091",
			title: "Fleet #1 - John",
			sub: "Active · Physical · VISA",
			balance: "KES 22,100",
			expiry: "12/27",
			brand: "VISA",
			actions: [
		...
		},
		{
			...
			number: "3321",
			title: "Software Subs",
			sub: "Low Balance · Virtual · Mastercard",
			balance: "KES 150",
			expiry: "05/25",
			brand: "Mastercard",
			statusBadge: { text: "LOW BAL", tone: "badgeD" },
			borderDanger: true,
			actions: [
				{
					label: "Fund Now",
				...
				{ label: "Auto-load", modal: "autoReloadModal" },
			],
		},
		{
			id: "f1",
			...
			number: "7762",
			title: "Travel Expo",
			sub: "Frozen · Physical · VISA",
			balance: "KES 4,500",
			expiry: "09/25",
			brand: "VISA",
			statusBadge: { text: "FROZEN", tone: "badgeD" },
			actions: [
				{ label: "Unfreeze", modal: "freezeCardModal" },
				{ label: "Unload", modal: "unloadCardModal" },
			],
		},
	],
	autoReloadRules: [
		{
			card: "Marketing Ads",
			desc: "Load KES 10K when balance < KES 1K",
			badge: "Active",
			tone: "badgeS",
		},
		{
			card: "Software Subs",
			desc: "Load KES 5K on 1st of month",
			badge: "Active",
			tone: "badgeS",
		},
	],
	restrictions: [
		{
			label: "Geo-Blocking",
			desc: "Physical cards locked to Kenya & EA",
			actionLabel: "Edit",
			modal: "geoBlockModal",
		},
		{
			label: "MCC Blocks",
			desc: "Gambling, Crypto, Adult blocked on all",
			actionLabel: "Edit",
			modal: "mccBlockModal",
		},
	],
...
async function fetchPrepaidCardManagement(): Promise<PrepaidCardManagementConfig> {
	...
	try {
		const res = await fetch("/api/prepaid-card-management", {
...
export default function PrepaidCardManagement() {
	...
	const filteredCards =
		cardFilter === "all"
			? config.cards
			: config.cards.filter((c) => c.type === cardFilter);
				...
				<div className={styles.pageBar}>
					...
					<div className="d-flex flex-wrap" style={{ gap: 8 }}>
						<button
							...
							onClick={() => setActiveModal("bulkIssueModal")}
						>
							<i className="bi bi-layers" /> Bulk Issue
						</button>
						<button
							className={styles.btnPm}
							onClick={() => setActiveModal("bulkTopupModal")}
						>
							<i className="bi bi-cash-stack" /> Bulk Top-up
						</button>
						<button
				...
				<div className={styles.content}>
					...
					<div className="row g-3">
						<div className="col-lg-4">
							...
							>
								...
								>
									Prepaid Card Program is Active{" "}
								...
								>
									4 Physical, 8 Virtual. Used for fleet fuel, online software,
									and staff expenses.
								</p>
						...
						{config.statCards.slice(1).map((s) => (
							<div key={s.key} className={s.col}>
								...
								>
									...
									{s.key === "lowbal" && (
										<button
											className={`${styles.btnPm} ${styles.btnSm} mt-2 w-100`}
											onClick={() => setActiveModal("bulkTopupModal")}
										>
											Top-up Now
										</button>
					...
					<div className={styles.card}>
						...
						>
							<div>
								<h3 className={styles.sectionTitle}>
									...
									/>{" "}
									5.5.1 — Prepaid Card Directory & Portfolio
								</h3>
								<p className={styles.sectionSub}>
									View and manage all issued physical and virtual prepaid cards.
								</p>
							</div>
						...
						<div className={styles.tabPills + " mb-3"}>
							<button
								className={`${styles.tabPill} ${cardFilter === "all" ? styles.tabPillActive : ""}`}
								onClick={() => setCardFilter("all")}
							>
								All Cards (12)
							</button>
							<button
								className={`${styles.tabPill} ${cardFilter === "virtual" ? styles.tabPillActive : ""}`}
								onClick={() => setCardFilter("virtual")}
							>
								Virtual (8)
							</button>
							<button
								className={`${styles.tabPill} ${cardFilter === "physical" ? styles.tabPillActive : ""}`}
								onClick={() => setCardFilter("physical")}
							>
								Physical (4)
							</button>
							<button
								className={`${styles.tabPill} ${cardFilter === "frozen" ? styles.tabPillActive : ""}`}
								onClick={() => setCardFilter("frozen")}
							>
								Frozen (1)
							</button>
						</div>
						...
						<div className="row g-3">
							{filteredCards.map((card) => (
								<div key={card.id} className="col-lg-3 col-md-4 col-sm-6">
									<div
										className={`${styles.creditCardUi} ${styles[card.bg]}`}
										onClick={() => setActiveModal("cardDetailModal")}
										style={
											card.borderDanger
										...
										<div className="d-flex justify-content-between align-items-start">
											...
											<div>
												{card.type === "virtual" ? (
													...
													<i className="bi bi-snow text-info fs-5" />
												) : (
													<i className="bi bi-contactless text-white opacity-75 fs-5" />
												)}
											</div>
										</div>
										<div>
											{card.type === "physical" && (
												<div className={styles.ccChip + " mb-2"} />
											)}
									...
									<div className="d-flex justify-content-between mt-2 px-1">
										{card.actions.map((a) => (
											<button
												key={a.label}
												className={`${styles.btnPm} ${styles.btnSm} ${a.actionClass ? styles[a.actionClass] : styles.btnOutline} w-100 ${card.actions.indexOf(a) === 0 ? "me-1" : "ms-1"}`}
												onClick={() => setActiveModal(a.modal)}
											>
												{a.label}
											</button>
						...
						<div className="text-center mt-3">
							<button
								className={styles.btnPm}
								onClick={() => setActiveModal("cardListModal")}
							>
								View All Cards
							</button>
						</div>
					...
					<div className="row g-3 mb-4">
						<div className="col-lg-6">
							<div className={`${styles.card} h-100`}>
								<div className="d-flex justify-content-between align-items-center mb-3">
									<h3 className={styles.sectionTitle}>
										<i className="bi bi-cash-stack text-success" /> 5.5.2 —
										Funding & Top-ups
									</h3>
									<button
										className={styles.btnPm}
								...
								<div className={styles.utilityBlock + " mb-3"}>
									...
									<button
										className={`${styles.btnPm} ${styles.btnSm} mt-2 w-100`}
										onClick={() => setActiveModal("autoReloadModal")}
									>
								...
								<div className="d-flex gap-2">
									<button
										className={`${styles.btnPm} flex-fill justify-content-center`}
									...
									<button
										className={`${styles.btnPm} flex-fill justify-content-center`}
										onClick={() => setActiveModal("unloadCardModal")}
									>
										Unload to Wallet
									</button>
								</div>
						...
						<div className="col-lg-6">
							<div className={`${styles.card} h-100`}>
								...
								<div className="d-flex gap-2">
									...
									<button
										className={`${styles.btnPm} flex-fill justify-content-center`}
										onClick={() => setActiveModal("viewPinModal")}
									>
										PIN Mgmt
									</button>
									<button
					...
					<div className={styles.card}>
						...
						>
							<div>
								<h3 className={styles.sectionTitle}>
									<i className="bi bi-bar-chart-line text-info" /> 5.5.4 —
									Spending Analytics & Ledger
								</h3>
								<p className={styles.sectionSub}>
									Track real-time spend across your card program, analyze
									merchants, and export reconciliations.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
									className={styles.btnPm}
									onClick={() => setActiveModal("spendAnalyticsModal")}
								>
									<i className="bi bi-pie-chart" /> Analytics
								</button>
								<button
									className={styles.btnPm}
									onClick={() => setActiveModal("exportReportModal")}
								>
									...
									{config.transactions.map((t) => (
										<tr key={t.date + t.merchant}>
											...
											<td data-label="Status">
												<span
													className={`${styles.badge} ${styles[t.statusTone]}`}
												>
													{t.status}
												</span>
											</td>
											<td data-label="Receipt">
												<button
													className={`${styles.btnPm} ${styles.btnSm}`}
													onClick={() => setActiveModal(t.actionModal)}
												>
													<i
														className={`bi ${t.actionLabel === "Receipt" ? "bi-receipt" : "bi-info-circle"}`}
													/>
												</button>
											</td>
										</tr>
						...
						<div className="text-center mt-3">
							<button
								className={styles.btnPm}
								onClick={() => setActiveModal("txLedgerModal")}
							>
								View Full Ledger
							</button>
						</div>
