...
const initialMockData: CardAnalyticsReportingConfig = {
	...
	statCards: [
		...
		},
		{
			...
			lines: [
				"Corp. Policy Violations: 42 Flags",
				"Chargebacks (Rate): 18 (0.01%)",
				"Unactivated Cards: 890",
			],
			warnBorder: true,
		},
	],
	issuanceFunnel: [
		{
			...
			value: "5,200",
			progress: { width: "100%", color: "var(--pm-border-2)" },
		},
		{
		...
		},
		{
			label: "Activated",
			value: "3,690 (82%)",
			progress: { width: "71%", color: "var(--pm-primary)" },
		},
		{
			label: "First Transaction Made",
			value: "3,100 (84%)",
			progress: { width: "60%", color: "var(--pm-accent)" },
		},
	],
	revenueOps: [
		{
	...
	topMerchants: [
		...
		{
			name: "Safaricom Postpay/Airtime",
			sub: "85k txns · Recurring",
			rank: "#2",
		},
	],
	corporateTable: [
		{
			...
			spend: "KES 3.4M",
			budgetPct: "78%",
			budgetColor: "var(--pm-info)",
			violations: "12 Flagged",
			violationTone: "badgeW",
		},
		{
			company: "TechStart Inc",
			dept: "Engineering & SaaS",
			cards: "42",
			spend: "KES 1.8M",
			budgetPct: "92%",
			budgetColor: "var(--pm-warning)",
			violations: "2 Critical",
			violationTone: "badgeD",
		},
		{
			...
			spend: "KES 8.5M",
			budgetPct: "45%",
			budgetColor: "var(--pm-accent)",
			violations: "0 Flags",
			violationTone: "badgeS",
		},
	],
};
...
async function fetchCardAnalyticsReporting(): Promise<CardAnalyticsReportingConfig> {
	...
	try {
		const res = await fetch("/api/card-analytics-reporting", {
				...
				{error && !errorDismissed && (
					...
					>
						<span>
							...
							Failed to load analytics data. Showing cached data.
						</span>
						<button
							className="btn-close"
				...
				<div className={styles.pageBar}>
					...
					<div className="d-flex flex-wrap" style={{ gap: 8 }}>
						<button
							...
							onClick={() => setActiveModal("predictiveEngineModal")}
						>
							<i className="bi bi-stars" /> AI Predict
						</button>
						<button
							className={styles.btnPm}
							onClick={() => setActiveModal("scheduleReportModal")}
						>
							<i className="bi bi-calendar3" /> Schedule
						</button>
						<button
							className={styles.btnPm}
							onClick={() => setActiveModal("buildReportModal")}
						>
							<i className="bi bi-hammer" /> Build Custom
						</button>
						<button
					...
					<div className="row g-3">
						<div className="col-lg-3 col-md-6">
							...
							>
								...
								>
									<i className="bi bi-arrow-up" /> +1,200 this week
								</span>
								<div
									className="mt-3"
						...
						>
							<div>
								<h3 className={styles.sectionTitle}>
									...
									/>{" "}
									5.8.1 — Card Portfolio Analytics
								</h3>
								<p className={styles.sectionSub}>
									Analyze issuance pipelines, activation rates, replacements,
									and revenue drivers across the portfolio.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
									className={styles.btnPm}
									onClick={() => setActiveModal("issuanceAnalyticsModal")}
								>
									Issuance Data
								</button>
								<button
									className={styles.btnPm}
									onClick={() => setActiveModal("usageTrendsModal")}
								>
									Usage Trends
								</button>
							</div>
								...
								<div className={styles.utilityBlock}>
									<div className="d-flex justify-content-between align-items-center mb-3">
										<h4 style={{ fontSize: 14, fontWeight: 700, margin: 0 }}>
											Issuance & Activation Funnel
										</h4>
										<button
											className={styles.btnPm}
											onClick={() => setActiveModal("activationDetailsModal")}
										>
											Details
										</button>
									</div>
										...
										<div key={f.label} className="mb-3">
											...
											<div className={styles.progress}>
												<div
													...
													style={{
														width: f.progress.width,
														background: f.progress.color,
													}}
												/>
											</div>
										</div>
									...
									>
										...
										<div
											style={{ fontWeight: 700, color: "var(--pm-primary)" }}
										>
											1.2 Days
										</div>
									</div>
							...
							<div className="col-lg-3">
								<div className={styles.utilityBlock}>
									<div className="d-flex justify-content-between align-items-center mb-3">
										...
										<button
											className={styles.btnPm}
											onClick={() => setActiveModal("revenueAnalyticsModal")}
										>
									...
									<div className={styles.statusRow}>
										<div>
											<strong>Replacement Rate</strong>
										...
										<div className="text-end">
											...
											<a
												...
												onClick={(e) => {
													e.preventDefault();
													setActiveModal("replacementLogModal");
												}}
												style={{
													fontSize: 11,
							...
							<div className="col-lg-4">
								<div className={styles.utilityBlock}>
									<div className="d-flex justify-content-between align-items-center mb-3">
										...
										<button
											className={styles.btnPm}
											onClick={() => setActiveModal("channelMixModal")}
										>
											Drill Down
										</button>
									</div>
									<div className={styles.chartBars}>
										{config.channelMix.map((ch) => (
											<div
												...
												style={{ height: ch.height, background: ch.color }}
												title={`${ch.label}: ${ch.height}`}
											>
									...
									<div
										...
										style={{ gap: 10, fontSize: 12, justifyContent: "center" }}
									>
										{config.channelMix.map((ch) => (
											<span key={ch.label}>
												<span
						...
						>
							<div>
								<h3 className={styles.sectionTitle}>
									...
									/>{" "}
									5.8.2 — Spend Analytics & Insights
								</h3>
								<p className={styles.sectionSub}>
									Understand cardholder spending behavior, merchant
									concentration, recurring flows, and predictive risk factors.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
							...
							<div className="col-lg-4">
								<div className={styles.utilityBlock}>
									<div className="d-flex justify-content-between align-items-center mb-3">
										...
										<button
											className={styles.btnPm}
											onClick={() => setActiveModal("topCategoriesModal")}
										>
											Categories
										</button>
									</div>
									...
									<div className="d-flex justify-content-between mt-3">
										<button
											className={styles.btnPm}
											onClick={() => setActiveModal("geoSpendModal")}
										>
											<i className="bi bi-globe" /> Geo / Intl Spend
										</button>
										<button
											className={styles.btnPm}
											onClick={() => setActiveModal("subscriptionTrackerModal")}
										>
								...
								<div className={styles.utilityBlock}>
									<div className="d-flex justify-content-between align-items-center mb-3">
										...
										<button
											className={styles.btnPm}
											onClick={() =>
												setActiveModal("merchantConcentrationModal")
											}
										>
											View Top
										</button>
									...
									<button
										...
										onClick={() => setActiveModal("merchantCrossSellModal")}
									>
										<i className="bi bi-link-45deg" /> Partner / Cross-Sell
										Analysis
									</button>
								</div>
							</div>
							<div className="col-lg-4">
								<div className={styles.utilityBlock}>
									<div className="d-flex justify-content-between align-items-center mb-3">
										...
										<button
											className={styles.btnPm}
											onClick={() => setActiveModal("ltvAnalyticsModal")}
										>
											LTV Model
										</button>
									</div>
									...
									>
										<div className="d-flex justify-content-between align-items-center">
											...
											<button
												className={`${styles.btnPm} ${styles.btnSm} ${styles.btnDanger}`}
												onClick={() => setActiveModal("churnRiskModal")}
											>
												Action
											</button>
										</div>
										<div
											style={{ fontSize: 11, color: "#7F1D1D", marginTop: 4 }}
										>
											Cards with &gt;30% volume drop last 60 days
										</div>
									</div>
									<div
									...
									>
										<div className="d-flex justify-content-between align-items-center">
											...
											<button
												...
												onClick={() => setActiveModal("upsellOpportunityModal")}
											>
												View
											</button>
										</div>
										<div
											style={{ fontSize: 11, color: "#065F46", marginTop: 4 }}
										>
											Debit users eligible for Credit Limits
										</div>
									</div>
								</div>
							</div>
						...
						>
							<div>
								<h3 className={styles.sectionTitle}>
									...
									/>{" "}
									5.8.3 — Corporate Card Reporting
								</h3>
								<p className={styles.sectionSub}>
									Monitor aggregate business spend, department budget
									utilization, compliance tracking, and policy violations.
								</p>
							</div>
							<div className="d-flex" style={{ gap: 8 }}>
								<button
									className={styles.btnPm}
									onClick={() => setActiveModal("complianceTaxModal")}
								>
									<i className="bi bi-file-earmark-ruled" /> Compliance & Tax
								</button>
								<button
									className={styles.btnPm}
									onClick={() => setActiveModal("corpSpendSummaryModal")}
								>
									<i className="bi bi-building" /> Corp Summary
								</button>
							</div>
						</div>
								...
								<tbody>
									{config.corporateTable.map((row) => (
										<tr key={row.company}>
											...
											<td>
												<span
													className={`${styles.badge} ${styles[row.violationTone]}`}
												>
													{row.violations}
												</span>
											</td>
											<td>
												<div className="d-flex gap-1">
													<button
														className={styles.btnPm}
														onClick={() => setActiveModal("deptBudgetModal")}
													>
														Budgets
													</button>
													<button
														className={styles.btnPm}
														onClick={() =>
															setActiveModal("policyViolationModal")
														}
													>
														Audit
													</button>
