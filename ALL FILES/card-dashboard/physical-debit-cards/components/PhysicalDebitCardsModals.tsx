...
function showLoading(target: string | HTMLElement, cb: () => void) {
	...
	ov.innerHTML =
		'<div class="' +
		styles.spinner +
		'"></div><p style="margin-top:12px;font-size:13px;font-weight:600;color:var(--pm-primary)">Processing...</p>';
...
const OrderCardModal = () => {
	...
	return (
		<div>
			<div
				className={`${styles.stepper} ${styles.isotope ? "isotope" : ""}`}
				id="orderStepper"
			>
				{labels.map((l, i) => (
					<div
			...
			<div id="orderStep1">
				...
				<div className="row g-3 mt-1">
					{[
						"debitStandard|Standard Debit|Free issuance",
						"debitPremium|Premium Travel|KES 1,000 fee",
						"debitBusiness|SME Business|Linked to Biz Wallet",
					].map((t) => {
						...
						return (
							<div key={label} className="col-md-4">
								<div
									...
									onClick={(e) => selectCardTier(e.currentTarget)}
								>
									<div
										className={`${styles.debitCard} ${styles[bg as keyof typeof styles]} mx-auto mb-2`}
										style={{ width: 100, padding: 8, minHeight: 60 }}
									>
										<div className={styles.cardLogo} style={{ fontSize: 8 }}>
											PayMo
										</div>
									</div>
									<strong>{label}</strong>
			...
			<div className="mt-4">
				...
				<div className="mb-3">
					...
					<input
						...
						defaultValue="JAMES KAMAU"
						maxLength={21}
					/>
				</div>
				<div className="mb-3">
					<label className={styles.formLabel}>Linked Account</label>
					<select className={styles.formControl}>
						<option>Primary PayMo Wallet (KES 24,500)</option>
						<option>Business Wallet (KES 105,000)</option>
					</select>
				</div>
				<div className="mb-3">
					<label className={styles.formLabel}>Delivery Method</label>
					<select className={styles.formControl}>
						<option>Express Courier (1-2 days) - KES 300</option>
				...
				<div className="mb-3">
					...
					<textarea className={styles.formControl} rows={2}>
						Apt 4B, Kilimani Heights, Argwings Kodhek Rd, Nairobi
					</textarea>
				</div>
				<div
				...
				<button
					...
					onClick={() =>
						processAction(
							"orderCardModal",
							"Order received. Your Premium Travel Debit card is in production.",
							"",
						)
					}
				>
	...
	<div>
		...
		>
			<div className="d-flex justify-content-between mb-2">
				...
				<strong style={{ color: "var(--pm-primary)" }}>TRK-9921448</strong>
			</div>
			<div className="d-flex justify-content-between mb-2">
				<span>Courier</span>
				<strong>Fargo Courier Express</strong>
			</div>
			<div className="d-flex justify-content-between">
		...
		<div className={`${styles.stepper} ${styles.stepperColumn}`}>
			{[
				...
				{
					icon: "bi-check",
					title: "Card Personalized & Printed",
					sub: "26 Jun, 09:15 AM",
					done: true,
				},
				{
				...
				<div
					...
					style={s.done || s.active ? {} : { opacity: 0.5 }}
				>
					<div
					...
					>
						{s.done ? <i className="bi bi-check" /> : s.icon}
					</div>
					<div>
						<div
							style={{
								fontWeight: 600,
								color: s.active ? "var(--pm-primary)" : "",
							}}
						>
							{s.title}
						</div>
		...
		>
			<i className="bi bi-shield-lock me-1" /> You must provide the 4-digit
			Delivery OTP <strong>(8812)</strong> to the rider to receive your card.
		</div>
	</div>
);
...
const ActivateCardModal = () => (
	<div>
		<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
			Scan the QR code on the card carrier document or enter the details
			manually.
		</p>
		<div
			className="text-center p-4 border rounded mb-3 border-dashed"
			style={{ background: "var(--pm-surface-2)", cursor: "pointer" }}
		>
			<i
		...
		<div className="text-center mb-3 text-muted" style={{ fontSize: 12 }}>
			OR ENTER MANUALLY
		</div>
		<div className="mb-3">
			<label className={styles.formLabel}>Last 4 digits of Card</label>
			<input
				className={styles.formControl}
		...
		<div className="mb-3">
			<label className={styles.formLabel}>CVV (3 digits on back)</label>
			<input className={styles.formControl} type="password" maxLength={3} />
		</div>
	</div>
);
...
const PinManagementModal = () => (
	<div>
		<div className="mb-3">
			...
			<select className={styles.formControl}>
				<option>Standard Debit (**** 8422)</option>
				<option>Business Debit (**** 1102)</option>
			</select>
		</div>
		<div className="mb-3">
			<label className={styles.formLabel}>
				Current PIN (Leave blank if new activation)
			</label>
		...
		<div className="mb-3">
			<label className={styles.formLabel}>New 4-Digit PIN</label>
			<div className={styles.pinInput}>
				<input
					type="password"
					maxLength={1}
		...
		>
			<i className="bi bi-exclamation-circle me-1" /> Do not use sequential
			numbers (1234) or repeated digits (1111).
		</div>
	</div>
);
...
const ResetPinModal = () => {
	...
	return (
		<div>
			<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
				To reset your PIN securely, we will send an OTP to your registered phone
				number, and you must verify your identity.
			</p>
			<div className="mb-3">
			...
			<button
				className={`${styles.btnPm} ${styles.btnPmP} w-100 mb-3`}
...
const CardLimitsModal = () => (
	<div>
		...
		<div className="mb-4">
			<label className={styles.formLabel + " d-flex justify-content-between"}>
				<span>Contactless Tap Limit (No PIN)</span>
				<span>KES 5,000</span>
			</label>
			<input
				type="range"
		...
		<div className="form-check form-switch">
			...
			>
				Require App Approval for transactions &gt; KES 50,000
			</label>
		</div>
	</div>
);
...
const GeoFencingModal = () => (
	<div>
		...
		>
			Block transactions that occur outside your designated secure regions.
			Reduces skimming fraud risk by 80%.
		</div>
		<div className="mb-3">
			<label className={styles.formLabel}>Geographic Scope</label>
			<select className={styles.formControl + " mb-2"}>
				<option selected>Kenya Only (Highest Security)</option>
				<option>East Africa (EAC)</option>
				<option>Africa Region</option>
				<option>Global (All enabled countries)</option>
			</select>
		</div>
		<div className="mb-3">
			<label className={styles.formLabel}>Blocked Countries / Regions</label>
			<div className="p-2 border rounded d-flex flex-wrap gap-2 mb-2">
				{["Nigeria", "South Africa", "High-Risk Zone A"].map((c) => (
					...
					>
						{c} <i className="bi bi-x ms-1" style={{ cursor: "pointer" }} />
					</span>
				))}
			</div>
			<select className={styles.formControl}>
				<option>+ Add country to blocklist</option>
		...
		<div className="form-check form-switch">
			...
			<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
				Block physical POS transactions if your phone's GPS is more than 50km
...
const TravelModeModal = () => (
	<div>
		<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
			Pre-declare your travel to prevent legitimate international transactions
			from being blocked by our fraud systems.
		</p>
		<div className="mb-3">
			<label className={styles.formLabel}>Destination Country/Countries</label>
			<input
				className={styles.formControl}
		...
		<div className="form-check">
			...
			<label className="form-check-label" style={{ fontSize: 13 }}>
				Temporarily increase daily POS limit to KES 200k during travel
			</label>
		</div>
		<div
			...
			style={{ background: "var(--pm-warning-soft)", fontSize: 12 }}
		>
			<i className="bi bi-info-circle me-1" /> Card will automatically revert to
			domestic Geo-Fencing rules on the return date.
		</div>
	</div>
);
	...
	return (
		<div>
			<div className={styles.tabPills + " mb-3"}>
				<button
					className={`${styles.tabPill} ${tab === "categories" ? styles.tabPillActive : ""}`}
					onClick={() => setTab("categories")}
				>
					Categories
				</button>
				<button
					className={`${styles.tabPill} ${tab === "specific" ? styles.tabPillActive : ""}`}
			...
			{tab === "categories" ? (
				<div>
					<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
						Toggle which merchant categories (MCCs) are allowed to process
						payments.
					</p>
				...
				<div>
					<div className="mb-3">
						<label className={styles.formLabel}>Block specific merchant</label>
						<div className="d-flex gap-2">
							<input
								className={styles.formControl}
								placeholder="Search recent merchants..."
							/>
							<button className={`${styles.btnPm} ${styles.btnDanger}`}>
								Block
							</button>
						</div>
					</div>
					...
					<div className="p-2 border rounded d-flex justify-content-between align-items-center mb-2">
						<span style={{ fontSize: 13 }}>Spotify Premium</span>
					...
					<div className="p-2 border rounded d-flex justify-content-between align-items-center">
						<span style={{ fontSize: 13 }}>Betika.com</span>
						<button className={`${styles.btnPm} ${styles.btnSm}`}>
							Unblock
						</button>
					</div>
...
const FreezeCardModal = () => (
	<div className="text-center p-2">
		<div
			className={styles.iconCircle + " mx-auto mb-3"}
			style={{
				width: 64,
				height: 64,
				fontSize: 28,
		...
		<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
			This will block all new purchases and ATM withdrawals. Recurring
			subscriptions may also fail. You can unfreeze it instantly at any time.
		</p>
		<div className="text-start mt-4 mb-3">
			<label className={styles.formLabel}>Select Card to Freeze</label>
			<select className={styles.formControl}>
				<option>Standard Debit (**** 8422)</option>
				<option>Business Debit (**** 1102)</option>
			</select>
...
const ReportLostModal = () => (
	<div>
		<div
			className="p-3 rounded mb-3 text-danger"
			style={{
				background: "var(--pm-danger-soft)",
				fontSize: 13,
		...
		>
			<strong>WARNING:</strong> This action is permanent. Your current card will
			be irrevocably cancelled to prevent fraud, and a new card number will be
			issued.
		</div>
		<div className="mb-3">
		...
		<div className="mb-3">
			...
			{[
				"Lost or misplaced permanently",
				"Stolen (suspect foul play)",
				"Card details compromised online",
			].map((opt, i) => (
				<div
					key={opt}
					className="p-2 border rounded mb-2 d-flex align-items-center gap-2"
					onClick={(e) => selectRadioCard(e.currentTarget)}
				>
					<input type="radio" name="lostR" defaultChecked={i === 0} />{" "}
					<span>{opt}</span>
				</div>
			))}
		</div>
		<div className="mb-3">
			<label className={styles.formLabel}>Date/Time Noticed</label>
		...
		<div className="form-check mt-3">
			...
			<label
				...
				style={{ fontSize: 13, fontWeight: 600 }}
			>
				Order replacement card immediately (KES 500 fee)
			</label>
		</div>
	</div>
);
...
const ReplaceCardModal = () => (
	<div>
		<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
			Use this if your physical card is cracked, the chip is unreadable, or
			contactless doesn't work. Your old card will remain active until you
			activate the new one.
		</p>
		<div className="mb-3">
			<label className={styles.formLabel}>Card to Replace</label>
			<select className={styles.formControl}>
				<option>Legacy Debit (**** 9421) - Frozen</option>
				<option>Standard Debit (**** 8422)</option>
			</select>
		</div>
		<div className="mb-3">
			<label className={styles.formLabel}>Reason for replacement</label>
			<select className={styles.formControl}>
				<option>Chip damaged/unreadable</option>
				<option>Magnetic stripe damaged</option>
				<option>Contactless not working</option>
				<option>Card physically broken</option>
				<option>Name change</option>
			</select>
		</div>
		<div className="mb-3">
			<label className={styles.formLabel}>
				Upload Photo of Damaged Card (Optional)
			</label>
			<input type="file" className={styles.formControl} />
			<div style={{ fontSize: 10, color: "var(--pm-muted)", marginTop: 4 }}>
				May waive replacement fee if manufacturing defect.
			</div>
		</div>
		<div
			className="p-3 rounded mb-3"
...
const CancelCardModal = () => (
	<div>
		...
		>
			<strong>PERMANENT CLOSURE:</strong> This will delete the card from your
			profile. It cannot be undone or reactivated. Any pending refunds will be
			routed to your main wallet.
		</div>
		<div className="mb-3">
			<label className={styles.formLabel}>Select Card to Cancel</label>
			<select className={styles.formControl}>
				<option>Legacy Debit (**** 9421) - Frozen</option>
			</select>
		</div>
		...
		<>
			<label className={styles.formLabel}>Type "CANCEL" to confirm</label>
			<input type="text" className={styles.formControl} placeholder="CANCEL" />
		</>
	</div>
);
	...
	return (
		<div className="text-center">
			{!revealed ? (
				<div>
					<i
						className="bi bi-fingerprint d-block mb-3"
						style={{ fontSize: 48, color: "var(--pm-primary)" }}
					/>
					<h6 style={{ fontWeight: 700 }}>Authentication Required</h6>
					<p style={{ fontSize: 13, color: "var(--pm-muted)" }}>
						Enter your PayMo PIN or use biometrics to reveal full card PAN and
						CVV.
					</p>
				...
				<div>
					<div
						className={`${styles.debitCard} ${styles.debitStandard} mx-auto mb-4`}
					...
					>
						...
						<div className="d-flex justify-content-between align-items-end mt-2">
							<div>
								<div style={{ fontSize: 10, opacity: 0.8 }}>VALID THRU</div>
							...
							<div>
								...
								<div style={{ fontFamily: "monospace", fontSize: 16 }}>481</div>
							</div>
						</div>
					...
					<div className="d-flex justify-content-center gap-2">
						<button className={`${styles.btnPm} ${styles.btnSm}`}>
							<i className="bi bi-clipboard" /> Copy Number
						</button>
						<button
					...
					<p style={{ fontSize: 11, color: "var(--pm-muted)", marginTop: 16 }}>
						Details will auto-hide in 30 seconds.
					</p>
				</div>
			)}
		</div>
...
const ToggleConfirmModal = () => (
	<div className="text-center p-2">
		<i
			className="bi bi-shield-check mb-3 d-block"
			style={{ fontSize: 36, color: "var(--pm-accent)" }}
		/>
		<h6 style={{ fontWeight: 700 }}>Security Control Updated</h6>
		<p style={{ fontSize: 13, color: "var(--pm-muted)", marginBottom: 0 }}>
			The card setting has been instantly applied to the processing network.
		</p>
	</div>
);
	...
	<div>
		<div className="row g-3">
			<div className="col-md-6">
				<label className={styles.formLabel}>Corporate Program</label>
				<select className={styles.formControl}>
					<option>Employee Expense Cards</option>
					<option>Department Purchasing</option>
				</select>
			</div>
			<div className="col-md-6">
				<label className={styles.formLabel}>Card Branding</label>
				<select className={styles.formControl}>
					<option>Company Logo + PayMo</option>
					<option>Standard PayMo Biz</option>
				</select>
			</div>
			<div className="col-12">
				<label className={styles.formLabel}>Upload Employee List (CSV)</label>
				<div
				...
				>
					...
					<div style={{ fontSize: 11, color: "var(--pm-muted)" }}>
						Format: Name, ID, Department, Limit
					</div>
				</div>
			</div>
			<div className="col-md-6">
				<label className={styles.formLabel}>Delivery Strategy</label>
				<select className={styles.formControl}>
					<option>Centralized (Ship to HR HQ)</option>
					<option>Direct to Employee Home</option>
				</select>
			</div>
			<div className="col-md-6">
						...
						<tr key={c.num}>
							<td>
								<div className="d-flex align-items-center gap-2">
									<div
										className={`${styles.debitCard} ${styles[c.bg as keyof typeof styles]} ${styles.debitCardMini}`}
										style={{
											filter: c.status === "Frozen" ? "grayscale(1)" : "none",
										}}
									/>{" "}
									**** {c.num}
								</div>
							...
							<td>
								<span
									className={`${styles.badge} ${styles[c.tone as keyof typeof styles]}`}
								>
									{c.status}
								</span>
							</td>
	...
	<div>
				...
				{[
					...
					return (
						...
						>
							<span>
								<i
									className={`bi ${i < 4 ? "bi-check-circle-fill text-success" : "bi-exclamation-circle-fill text-warning"} me-2`}
								></i>{" "}
								{label}
							</span>
							<span>{val}</span>
		...
		<div className="p-3 rounded" style={{ background: "var(--pm-surface-2)" }}>
			<h6 style={{ fontWeight: 700, fontSize: 13 }}>
				<i className="bi bi-lightbulb text-warning me-1" /> Recommendations to
				reach 100/100
			</h6>
			<ul
				style={{
			...
			>
				<li>
					Lower contactless threshold to KES 2,000 to reduce tap-fraud risk.
				</li>
		...
		{[
			{
				...
				title: "Declined: Naivas Supermarket",
				sub: "KES 6,200 · Daily tap limit exceeded.",
				meta: "1 hr ago",
				link: true,
			},
			{
				...
				sub: "Premium card dispatched to courier. OTP 8812.",
				meta: "3 hrs ago",
				link: false,
			},
			{
				...
				sub: "Card **** 9421 expires in 45 days.",
				meta: "1 day ago",
				link: false,
			},
			{
			...
			>
				...
				<div
					style={{
						fontSize: 11,
						color: n.link ? "#7F1D1D" : "var(--pm-ink-soft)",
					}}
				>
					{n.sub}
				</div>
...
const RenewCardModal = () => (
	<div>
		...
		>
			<div style={{ fontWeight: 700, color: "var(--pm-purple)" }}>
				Legacy Debit (**** 9421)
			</div>
			<div style={{ fontSize: 12, color: "#6D28D9" }}>
				Expires: 08/25 (in 45 days)
			</div>
		</div>
		<p style={{ fontSize: 13, color: "var(--pm-ink-soft)" }}>
			Your new card will inherit the same PIN, settings, and linked accounts.
			The old card will deactivate automatically when the new one is activated.
		</p>
		<div className="mb-3">
			<label className={styles.formLabel}>Delivery Address Confirmation</label>
			<textarea className={styles.formControl} rows={2}>
				Apt 4B, Kilimani Heights, Argwings Kodhek Rd, Nairobi
			</textarea>
		</div>
		<div className="form-check">
			<input className="form-check-input" type="checkbox" defaultChecked />
			<label className="form-check-label" style={{ fontSize: 13 }}>
				Auto-update my subscriptions (Netflix, Spotify) with the new expiry date
				using Visa Account Updater.
			</label>
		</div>
	</div>
);
...
const AlertPrefsModal = () => (
	<div>
		<div className="mb-3">
			<label className={styles.formLabel}>Transaction Alerts</label>
			<select className={styles.formControl}>
				<option>All transactions</option>
				<option selected>Transactions over KES 1,000</option>
				<option>International & Online only</option>
			</select>
		</div>
		<div className="table-responsive">
			<table className={styles.table}>
				...
				<tbody>
					{[
						...
						<tr key={row[0] as string}>
							<td>{row[0]}</td>
							<td>
								<input type="checkbox" defaultChecked={row[1] as boolean} />
							</td>
							<td>
								<input type="checkbox" defaultChecked={row[2] as boolean} />
							</td>
							<td>
								<input type="checkbox" defaultChecked={row[3] as boolean} />
							</td>
						</tr>
					))}
				</tbody>
		...
		<div className="row g-2 text-start mt-3" style={{ fontSize: 13 }}>
			...
			<div className="col-6">
				<div
					...
					style={{ background: "var(--pm-surface-2)" }}
				>
					<span className="text-muted">Global Limit</span>
					<br />
					<strong>KES 500k / day</strong>
				</div>
			</div>
			<div className="col-6">
				<div
...
function PhysicalDebitCardsModals({
	active,
	onClose,
	onOpen,
}: PhysicalDebitCardsModalsProps) {
