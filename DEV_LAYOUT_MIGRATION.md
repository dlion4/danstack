...
3. **Use the `@/*` alias for the shared shell CSS** (`@/features/Layouts/shell/styles/dashboard.module.css`) — `vite.config.ts` has `resolve.tsconfigPaths: true`, so it works in both `tsc` and the build, and avoids relative-path depth mistakes (the cards home's `../../shell/...` is actually one `../` short; it only "works" because that file is currently unused/uncompiled).
